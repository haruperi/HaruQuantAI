...
This module defines all parameter-space, request, response, and scenario
simulation models used by the Optimization Service. All primary models
inherit from Contract to ensure deterministic serialization and trace
propagation.
"""
...
class Contract(BaseModel):
    """Local base model for optimization service contracts."""
...
class ParameterRange(BaseModel):
    ...
    Args:
        ...
        type: The parameter type classification.
            ``"int"``         — discrete integer values.
            ``"float"``       — continuous floating-point values.
            ``"categorical"`` — enumerated choices from ``options``.
            ``"bool"``        — boolean True/False toggle.
            ``"fixed"``       — single immutable value from ``fixed_value``.
            ``"constrained"`` — numeric range further governed by
                space-level constraint expressions.
        min_value: Minimum numeric boundary (``int``/``float``/
            ``constrained`` only).
        max_value: Maximum numeric boundary (``int``/``float``/
            ``constrained`` only).
        step: Optional discretization step size.
        options: Allowed choices for ``categorical`` type.
        fixed_value: Single immutable value for ``fixed`` type.
        conditional_on: Parent parameter name for conditional activation.
        conditional_values: Parent values that activate this parameter.
    """

    name: str = Field(..., description="Unique parameter identifier.")
    type: Literal["int", "float", "categorical", "bool", "fixed", "constrained"] = (
        Field(..., description="Parameter type classification.")
    )
    ...
    fixed_value: Any | None = Field(
        default=None,
        description="Immutable value used when type is 'fixed'.",
    )
    conditional_on: str | None = Field(
        default=None,
        description="Parent parameter name for conditional execution.",
    )
    conditional_values: list[Any] | None = Field(
        default=None,
        description="Parent values that activate this parameter.",
    )

    @model_validator(mode="after")
    def validate_range_boundaries(self) -> ParameterRange:
        """Validate numeric boundaries, options, and fixed values.

        Returns:
            ParameterRange: The validated instance (self).

        Raises:
            ValueError: If boundaries, options, or fixed_value are invalid
                for the declared type.
        """
        if self.type in ("int", "float", "constrained"):
            if self.min_value is None or self.max_value is None:
                param_type = self.type
                msg = f"min_value and max_value are required for type '{param_type}'."
                raise ValueError(msg)
            ...
            if not self.options:
                raise ValueError(
                    "options must be provided and non-empty for categorical type."
                )
        elif self.type == "fixed":
            if self.fixed_value is None:
                raise ValueError("fixed_value must be provided for fixed type.")
        return self
...
class ParameterSpace(BaseModel):
    ...
    Args:
        ...
        constraints: Optional Python expressions validating parameter
            relationships.  Evaluated before candidate execution.
    """
...
class ParameterCandidate(BaseModel):
    ...
    Args:
        ...
        candidate_hash: Unique SHA-256 hash identifying the candidate
            parameter combination.
    """

    parameters: dict[str, Any] = Field(..., description="Parameter values mapping.")
    candidate_hash: str = Field(..., description="SHA-256 deduplication identifier.")
...
class OptimizationResult(BaseModel):
    ...
    Args:
        ...
        metrics: Detailed metrics mapping (e.g. sharpe, drawdown,
            trade_count).
...
class OptimizationSummary(BaseModel):
    ...
    Args:
        ...
        candidates: Chronological or ranked list of all evaluated
            candidate results.
    """
    ...
    candidates: list[OptimizationResult] = Field(
        default_factory=list,
        description="List of all evaluated candidates.",
    )
    ...
    def to_dataframe(self) -> Any:  # noqa: ANN401
        ...
        """
        import pandas as pd  # optional heavy dep, deferred intentionally

        records = [
            {
                "score": c.score,
                **{f"param_{k}": v for k, v in c.parameters.items()},
                **c.metrics,
            }
...
class UnsupervisedConfigRequest(BaseModel):
    """Configuration request options for unsupervised classification.
...
class UnsupervisedRunSummary(BaseModel):
    ...
    Args:
        ...
        cluster_labels: Mapping of candidate hashes to cluster IDs.
...
class UnsupervisedAnalysisRequest(BaseModel):
    """Request for running unsupervised clustering on optimization results.
...
class OptimizationRequest(Contract):
    ...
    Args:
        ...
        search_method: Sweep algorithm selection.
        ...
        dry_run: Dry-run execution flag; defaults to ``True``.
    """
    ...
    dry_run: bool = Field(
        default=True,
        description="Lightweight dry-run execution flag.",
    )
...
class OptimizationResponse(Contract):
    ...
    status: OptimizationStatus = Field(
        ..., description="Standard workflow status outcome."
    )
...
class WalkForwardRequest(Contract):
    ...
    fold_mode: Literal["rolling", "anchored", "expanding"] = Field(
        default="rolling",
        description="Walk-forward window split mode.",
    )
    train_fraction: float = Field(
        default=0.7,
        description="Fraction of window allocated to training.",
    )
    ...
    bar_duration: timedelta = Field(
        default=timedelta(days=1),
        description=(
            "Duration of one bar; used to convert purging/embargo bars "
            "into calendar time."
        ),
    )
...
class WalkForwardResponse(Contract):
    ...
    walk_forward_efficiency: float = Field(
        ..., description="Walk-Forward Efficiency (WFE) percentage."
    )
    status: OptimizationStatus = Field(
        ...,
        description=("Canonical walk-forward status (e.g. 'ready_for_risk_review')."),
    )
...
class MonteCarloResult(BaseModel):
    ...
    losing_streak_distribution: list[int] = Field(
        ..., description="Max losing streak counts per path."
    )
...
class MonteCarloRequest(Contract):
    ...
    ruin_threshold: float = Field(
        default=0.5,
        description="Drawdown ruin fraction threshold (0-1).",
    )
...
class ProfitTargetResult(BaseModel):
    ...
    mean_trades_to_reach: float | None = Field(
        default=None,
        description=("Average trade steps to reach target balance."),
    )
