...
|---|---|---|---|---|---|---|
| Partial | `WF-TRD-001` | Internal | Validate and package a route-aware action | Canonical `TradingRequest` | Validated package or structured rejection | `FR-TRD-024 → FR-TRD-027 → FR-TRD-028` |
...
| Partial | `WF-TRD-003` | Cross-domain | Start and enable a live session | Approved config and Data session/channel | Package-only or mutation-enabled session | `FR-TRD-033 → FR-TRD-034` |
...
| Partial | `WF-TRD-005` | Cross-domain | Resolve an unknown route outcome | Timeout/malformed authority response | Retry locked until authority resolution | `FR-TRD-030 → FR-TRD-044` |
| Partial | `WF-TRD-006` | Cross-domain | Read route facts and aggregate readiness | Data/Simulation read evidence | Fresh structured readiness assessment | `FR-TRD-026 → FR-TRD-027` |
| Partial | `WF-TRD-007` | Cross-domain | Enforce kill switch and emergency controls | Risk state and approved control request | New actions blocked; gated cancel/close result | `FR-TRD-021 → FR-TRD-023`, `FR-TRD-050` |
| Partial | `WF-TRD-008` | Cross-domain | Persist evidence and recover state | Trading events and injected stores | Reconstructed projections and unresolved attempts | `FR-TRD-037 → FR-TRD-042`, `FR-TRD-051 → FR-TRD-055` |
| Partial | `WF-TRD-009` | Cross-domain | Perform safe live shutdown | Operator/runtime stop request | Admission stopped and unresolved-work report | `FR-TRD-035` |
...
| Partial | `WF-TRD-011` | Cross-domain | Build execution/reconciliation evidence | Receipts, readiness, incidents | Immutable report to Analytics/Portfolio/UI/API | `FR-TRD-049` |
...
|---|---|---|---|---|
| Partial | `models.py` | Define routes, requests, intents, receipts, records, and envelopes | `TradingRoute`, `TradingRequest`, `StandardTradingEnvelope`, `OrderIntent`, `ExecutionReceipt`, `TradeRecord` | **Standard library:** `datetime`, `decimal`, `enum`, `typing`<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** Utils public time/serialization contracts |
| Partial | `errors.py` | Define/map finite errors and redact boundary payloads | `TradingError`, `map_trading_error`, `redact_trading_payload` | **Standard library:** `typing`<br>**Required third-party:** None<br>**Local:** `models.py → StandardTradingEnvelope`; Utils public error/redaction APIs |
| Partial | `registry.py` | Expose the exact typed Python public API | `get_public_contracts` | **Standard library:** `collections.abc`<br>**Required third-party:** None<br>**Local:** `models.py`; `errors.py` |
| Partial | `__init__.py` | Expose the approved contract API | All exports above | **Standard library:** None<br>**Required third-party:** None<br>**Local:** files above |
...
|---|---|---|---|---|---|---|
| Partial | `FR-TRD-001` | The system shall expose only `sim`, `paper`, and `live` action routes; package-only is a side-effect mode, not a route. | `TradingRoute: type[StrEnum]` | None | `TradingError`: unknown route | **Usage:** `tests/trading/usage/test_usage_contracts.py::test_usage_models_trading_route()`<br>**Unit:** `tests/trading/unit/contracts/test_models.py::test_trading_route_rejects_unknown()` |
| Partial | `FR-TRD-002` | The system shall validate one immutable canonical request with route, action, trace, authority, approval, Risk, idempotency, and UTC evidence. | `TradingRequest` | None | `TradingError`: invalid/missing field or unsafe numeric/time value | **Usage:** `tests/trading/usage/test_usage_contracts.py::test_usage_models_trading_request()`<br>**Unit:** `tests/trading/unit/contracts/test_models.py::test_trading_request_requires_governed_evidence()` |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `events.py` | Define versioned attempt/receipt/fill/reconciliation/incident events | `TradingEvent` | **Standard library:** `datetime`, `enum`<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** `contracts.models` |
| Partial | `stores.py` | Define minimal injected state operations | `TradingStateStore` and its five public operations | **Standard library:** `datetime`, `typing`<br>**Required third-party:** None<br>**Local:** `events.py`; `contracts.models` |
| Partial | `idempotency.py` | Reserve caller keys and detect material conflicts | `IdempotencyReservation`, `reserve_idempotency` | **Standard library:** `hashlib`<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** `stores.py`; Utils canonical JSON |
| Partial | `projections.py` | Apply ordered events with optimistic versions | `TradingProjection`, `apply_execution_event` | **Standard library:** None<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** `events.py`; `stores.py`; `contracts.models` |
...
| Partial | `__init__.py` | Expose state API | All exports above | **Standard library:** None<br>**Required third-party:** None<br>**Local:** files above |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `orders.py` | Aggregate Decimal/order/operation validation | `validate_order_request` | **Standard library:** `decimal`<br>**Required third-party:** None<br>**Local:** `contracts.models`; Data/Risk public evidence |
...
| Partial | `readiness.py` | Aggregate required freshness, capability, permission, and policy checks | `ReadinessAssessment`, `assess_execution_readiness` | **Standard library:** `typing`<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** `orders.py`; `snapshots.py`; `contracts.models` |
...
| Partial | `__init__.py` | Expose validation API | All exports above | **Standard library:** None<br>**Required third-party:** None<br>**Local:** files above |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `capabilities.py` | Validate provider/schema/security/action/timeout/retry contracts | `validate_adapter_capability` | **Standard library:** `collections.abc`<br>**Required third-party:** None<br>**Local:** `contracts.models`; Brokers `BrokerFeatureFlags`; Simulation public contracts |
| Partial | `responses.py` | Normalize and classify authority responses | `classify_authority_response` | **Standard library:** `collections.abc`, `decimal`<br>**Required third-party:** None<br>**Local:** `contracts.models`; `state.events` |
| Partial | `dispatcher.py` | Select sim or broker authority and make the sole dispatch | `dispatch_order_intent` | **Standard library:** `collections.abc`<br>**Required third-party:** None<br>**Local:** `capabilities.py`; `responses.py`; `state`; Brokers `BrokerAdapter` (mutation traits); Simulation public intent consumer |
| Partial | `__init__.py` | Expose routing API | All exports above | **Standard library:** None<br>**Required third-party:** None<br>**Local:** files above |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `snapshots.py` | Normalize route authority facts | `AuthoritySnapshot` | **Standard library:** `datetime`, `decimal`<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** `contracts.models`; Data/Simulation public reads |
| Partial | `compare.py` | Detect missing, extra, mismatched, and stale records | `ReconciliationReport`, `compare_authority_state` | **Standard library:** None<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** `snapshots.py`; `state` |
| Partial | `authority.py` | Persist incidents/transitions and control retry lock | `AuthorityResolution`, `resolve_unknown_outcome` | **Standard library:** `typing`<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** `compare.py`; `state`; `routing.responses` |
| Partial | `__init__.py` | Expose reconciliation API | All exports above | **Standard library:** None<br>**Required third-party:** None<br>**Local:** files above |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `events.py` | Define and emit health/staleness/timeout/latency/cost/incident evidence | `OperationalEvent`, `emit_runtime_event` | **Standard library:** `collections.abc`, `datetime`, `enum`<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** `contracts.errors`; `state.events`; Utils audit/redaction contracts |
...
| Partial | `__init__.py` | Expose monitoring API | All exports above | **Standard library:** None<br>**Required third-party:** None<br>**Local:** files above |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `config.py` | Validate immutable enablement/security settings and secret references | No package-level export | **Standard library:** None<br>**Required third-party:** `pydantic>=2.13.4`, `pydantic-settings>=2.14.2`<br>**Local:** Utils settings; Data session provider |
| Partial | `session.py` | Own live/paper lifecycle and injected dependencies | `LiveSession`, `LiveSession.start`, `LiveSession.status`, `LiveSession.stop` | **Standard library:** `collections.abc`<br>**Required third-party:** None<br>**Local:** contracts, state, validation, routing, reconciliation, monitoring |
...
| Partial | `__init__.py` | Expose live lifecycle/gate API | Exports above | **Standard library:** None<br>**Required third-party:** None<br>**Local:** files above |
...
| Partial | `FR-TRD-034` | The system shall return the actual session mode, admission, authority, health, reconciliation, and unresolved-work state. | `LiveSession.status() -> StandardTradingEnvelope` | Read-only | None | **Usage:** `tests/trading/usage/test_usage_live.py::test_usage_session_status()`<br>**Unit:** `tests/trading/unit/live/test_session.py::test_status_never_overstates_readiness()` |
| Partial | `FR-TRD-035` | The system shall stop admission, drain/mark work, flush evidence, reconcile, and report every incomplete shutdown step. | `LiveSession.stop() -> StandardTradingEnvelope` | Local state mutation; persistence write | `TradingError`: shutdown dependency failure | **Usage:** `tests/trading/usage/test_usage_live.py::test_usage_session_stop()`<br>**Unit:** `tests/trading/unit/live/test_session.py::test_stop_reports_flush_and_reconciliation_failure()` |
...
| Partial | `orders.py` | Submit, modify, and cancel route-aware orders | `submit_order`, `modify_order`, `cancel_order` | **Standard library:** None<br>**Required third-party:** None<br>**Local:** contracts, validation, state, routing, live |
| Partial | `positions.py` | Close/modify positions and reduce approved exposure | `close_position`, `modify_position`, `reduce_exposure` | **Standard library:** `decimal`<br>**Required third-party:** None<br>**Local:** contracts, validation, state, routing, live |
| Partial | `controls.py` | Pause/resume, synchronize, and trigger/clear scoped switches | `pause_strategy`, `resume_strategy`, `sync_positions`, `trigger_kill_switch`, `clear_kill_switch` | **Standard library:** None<br>**Required third-party:** None<br>**Local:** contracts, state, reconciliation, live; Risk public contracts |
| Partial | `emergency.py` | Execute explicit gated mass cancellation/closure | `cancel_all_orders`, `close_all_positions` | **Standard library:** None<br>**Required third-party:** None<br>**Local:** contracts, state, routing, live |
| Partial | `__init__.py` | Expose canonical action API | All exports above | **Standard library:** None<br>**Required third-party:** None<br>**Local:** files above |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `evidence.py` | Build immutable execution/reconciliation evidence | `build_trading_report` | **Standard library:** None<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** contracts, state, reconciliation, monitoring |
| Partial | `__init__.py` | Expose reporting API | `build_trading_report` | **Standard library:** None<br>**Required third-party:** None<br>**Local:** `evidence.py` |
...
| Status | Requirement ID | Responsibility | Class / Function / Method | Side Effects | Raises | Usage / Test |
|---|---|---|---|---|---|---|
| Partial | `FR-TRD-049` | The system shall package receipts, factual costs, readiness, reconciliation, incidents, warnings, and unresolved actions without calculating performance/TCA. | `build_trading_report(request: TradingRequest, store: TradingStateStore) -> StandardTradingEnvelope` | Read-only | `TradingError`: missing/inconsistent evidence | **Usage:** `tests/trading/usage/test_usage_reporting.py::test_usage_evidence_build_trading_report()`<br>**Unit:** `tests/trading/unit/reporting/test_evidence.py::test_report_does_not_compute_analytics_metrics()` |
...
| Partial | `NFR-TRD-002` | Determinism | Canonical JSON, Decimal material, IDs, projections, and comparisons shall be deterministic. | Replay/hash tests |
...
| Partial | `NFR-TRD-004` | Reliability | Unknown outcomes shall freeze the conflict scope until reconciliation; blind retries are forbidden. | Timeout/reconciliation tests |
| Partial | `NFR-TRD-005` | API boundary | Consumers shall use documented public exports; package import shall have no runtime side effect. | Import/catalog tests |
