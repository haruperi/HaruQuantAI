...
def test_prepared_dataset_rejects_provider_object() -> None:
    ...
    with pytest.raises(ValueError, match=r"."):
        create_research_value(
            "PreparedDataset", object(), "v1", _quality(), _HASH, _HASH, ("source",)
...
def test_quality_report_distinguishes_fatal_warning() -> None:
    ...
    warning = create_research_value(
        "ResearchWarning", "SPARSE", "Sparse sample", "warning"
    )
    report = create_research_value(
        "DataQualityReport", ({"code": "BAD_OHLC"},), (warning,), (), ()
    )
...
def test_edge_result_is_advisory() -> None:
    ...
    result = create_research_value(
        "EdgeResult", "v1", "mean_reversion", {}, {}, "inconclusive", 1, (), True
    )
...
def test_quality_report_records_windows() -> None:
    ...
    report = create_research_value(
        "MarketStructureQualityReport", "v1", {"windows": [20, 40]}, {}, {}, 1.0, ()
    )
...
def test_warning_details_are_bounded() -> None:
    ...
    with pytest.raises(ValueError, match=r"."):
        create_research_value(
            ...
            "Too many",
            "warning",
            details={str(i): i for i in range(33)},
        )
