...
def test_api_and_ui_have_independent_feature_registries() -> None:
    ...
    assert set(re.findall(r"FEAT-UI-\d{2}", ui_rows)) == {
        f"FEAT-UI-{number:02d}" for number in range(1, 25)
    }
...
def test_repository_feature_inventory_is_reconciled() -> None:
    ...
    assert statuses.count("Completed") == 213
    assert statuses.count("Pending") == 16
    ...
    assert "(92.61%)" in project
    assert "| Completed | 213 |" in project
    assert "| Pending | 16 |" in project
