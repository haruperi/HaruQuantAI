...
async def fr_brokers_076_to_083_canonical_ticks(adapter: object) -> None:
    """FR-BRK-076..083: Stage 3 — Web Tick Retrieval & Canonical Output."""
    _header("Stage 3: Web Tick Retrieval & Canonical Output (FR-BRK-076..083)")
    for op in (
        "permissions",
        "accounts",
        "select_account",
    ...
    bars = await get_broker_historical_bars(
        adapter,
        "EURUSD",
        "M1",
    ...
    require_success("Dukascopy bars", bars)
    print(_format_result(bars))
    print(f"Data -> bars='{get_broker_value_field(bars, 'data')}'")

    tick_hour = (end - timedelta(days=7)).replace(
        minute=0,
        second=0,
        microsecond=0,
    )
    ticks = await get_broker_ticks(
        adapter,
        "EURUSD",
        tick_hour,
        tick_hour + timedelta(hours=1),
        limit=5,
    )
    require_success("Dukascopy ticks", ticks)
    print(_format_result(ticks))
    print(f"Data -> ticks='{get_broker_value_field(ticks, 'data')}'")
...
async def _run() -> None:
    ...
    _feature_header(
        ...
        "-> bounded chart/json3 tick retrieval\n"
        "-> canonical ticks"
    )
    ...
    try:
        async with real_session("dukascopy") as adapter:
            ...
            # Stage 3: web-chart tick retrieval and canonical tick output
