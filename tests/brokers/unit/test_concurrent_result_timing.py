...
def test_concurrent_calls_do_not_exchange_provider_latency() -> None:
    ...
    async def run() -> tuple[StandardResponse[None], StandardResponse[None]]:
    ...
    first_provider = first.metadata.extensions["provider_latency_ms"]
    second_provider = second.metadata.extensions["provider_latency_ms"]
    assert isinstance(first_provider, float)
    assert isinstance(second_provider, float)
    assert first_provider <= first.metadata.execution_ms
    assert second_provider <= second.metadata.execution_ms
