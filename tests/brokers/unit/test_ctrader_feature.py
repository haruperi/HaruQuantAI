"""Unit and lifecycle tests for FEAT-BRK-CONNECT_CTRADER."""
...
def _make_context(
    ...
    scope = FeatureScope(owner_id="FEAT-BRK-CONNECT_CTRADER")
...
class _FakeCTraderTransport:
    ...
    async def connect(self) -> bool:
        if self.fails:
            raise (self.error or ConnectionError("cTrader connect failed"))
        self._connected = True
        return True

    async def send(self, request: Any, response_type: Any) -> Any:
        self.calls.append({"request": request, "response_type": response_type})
        if self.fails:
            raise (self.error or ConnectionError("cTrader send failed"))
        return {}
...
def test_manifest_spec() -> None:
    assert SPEC.feature_id == "FEAT-BRK-CONNECT_CTRADER"
    assert SPEC.domain == "brokers"
    assert PROVIDER_CTRADER_CAPABILITY in SPEC.provides
    ...
    assert "client_id" in SPEC.config_keys
    assert "access_token" in SPEC.config_keys
...
def test_feature_factory() -> None:
    feat = feature()
    assert isinstance(feat, CTraderFeature)
...
async def test_feature_mount_and_unmount() -> None:
    ...
    assert PROVIDER_CTRADER_CAPABILITY in registry
    assert isinstance(registry[PROVIDER_CTRADER_CAPABILITY], ProviderBackend)
...
async def test_manage_sessions_open_success() -> None:
    svc = CTraderProviderService(config=CTraderConfig(probe_symbol=None))
    req_id = _gen_id()
...
async def test_manage_sessions_environment_mismatch() -> None:
    ...
    sess = BrokerSessionRef(
        ...
        profile_version=1,
        account_ref="test_account",
        environment="SIMULATION",  # not in DEMO/LIVE/SANDBOX/TESTNET
        generation=1,
    )
    req = ManageSessionsRequest(
        request_id=_gen_id(),
...
async def test_manage_sessions_close() -> None:
    fake_transport = _FakeCTraderTransport()
    fake_transport._connected = True
    svc = CTraderProviderService(transport=fake_transport)  # type: ignore[arg-type]
    req_id = _gen_id()
    sess = BrokerSessionRef(
        session_id=_gen_id(),
        profile_id=_gen_id(),
...
# ==============================================================================
# 6. Order Transport Tests (Unreleased Denial)
# ==============================================================================


@pytest.mark.asyncio
async def test_transport_orders_unreleased_rejection() -> None:
    svc = CTraderProviderService()
    req_id = _gen_id()
    sess = BrokerSessionRef(
        session_id=_gen_id(),
...
@pytest.mark.asyncio
async def test_gateway_dispatch_to_ctrader() -> None:
    context, _scope, registry = _make_context()
    ctrader_feat = feature()
    await ctrader_feat.mount(context, {"probe_symbol": "EURUSD", "environment": "DEMO"})
    ...
    gw_feat.service.bind_profile(sess.profile_id, PROVIDER_CTRADER_CAPABILITY)
    req = ManageSessionsRequest(
        request_id=_gen_id(),
        capability_snapshot_id=_gen_id(),
        operation="ASSESS_READINESS",
    ...
    await ctrader_feat.unmount(context)
...
async def test_open_probe_failure() -> None:
    fake_transport = _FakeCTraderTransport(
        fails=True, error=ConnectionError("TCP connection refused")
    )
    svc = CTraderProviderService(
        config=CTraderConfig(probe_symbol="EURUSD"),
