...
def test_v2_excludes_empirical_drift_from_operational_invariants() -> None:
    """V2 ignores empirical observations while v1 retains its exact behavior."""
    left, right = paired_evidence()
    right["orders"] = (
        {
            **right["orders"][0],  # type: ignore[index]
            "quantity": "0.2",
            "filled": "0.2",
            "placed_at": "2026-08-20T11:00:03+00:00",
        },
    )
    right["deals"] = (
        {
            **right["deals"][0],  # type: ignore[index]
            "quantity": "0.2",
            "price": "1.2050",
            "executed_at": "2026-08-20T11:00:04+00:00",
        },
    )
    right["positions"] = (
        {
            **right["positions"][0],  # type: ignore[index]
            "quantity": "0.2",
            "profit": "7",
            "opened_at": "2026-08-20T11:00:04+00:00",
        },
    )
    right["receipts"] = (
        {
            **right["receipts"][0],  # type: ignore[index]
            "filled_quantity": "0.2",
            "average_price": "1.2050",
            "authority_timestamp": "2026-08-20T11:00:04+00:00",
            "received_at": "2026-08-20T11:00:08+00:00",
        },
    )
    right["events"] = tuple(
        {
            **event,
            "occurred_at": f"2026-08-20T11:00:0{index + 4}+00:00",
        }
        for index, event in enumerate(right["events"])  # type: ignore[union-attr]
    )

    v2_result = compare_parity_evidence(left, right, get_parity_envelope("v2"))
    v1_result = compare_parity_evidence(left, right, _ENVELOPE)

    assert v2_result["passed"] is True, v2_result["failures"]
    assert v1_result["passed"] is False


def test_v2_rejects_response_classification_mutation() -> None:
    """V2 retains exact response classification semantics."""
    left, right = paired_evidence()
    right["receipts"] = (
        {
            **right["receipts"][0],  # type: ignore[index]
            "response_classification": "unknown_outcome",
        },
    )
    result = compare_parity_evidence(left, right, get_parity_envelope("v2"))
    assert result["passed"] is False
    assert any("receipt.status_classification" in f for f in result["failures"])


def test_v2_rejects_lifecycle_mutation() -> None:
    """V2 retains exact order lifecycle semantics."""
    left, right = paired_evidence()
    right["orders"] = (
        {
            **right["orders"][0],  # type: ignore[index]
    ...
    result = compare_parity_evidence(left, right, get_parity_envelope("v2"))
    assert result["passed"] is False
    assert any("order.lifecycle_state" in f for f in result["failures"])
