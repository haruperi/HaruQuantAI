...
def _data(response: Any) -> object:
    ...
    assert response.metadata.domain == "optimization"
...
def test_run_robustness_analysis_supports_both_request_forms() -> None:
    ...
    monte_carlo = _data(
        run_robustness_analysis(monte_carlo_request(), max_simulations=5)
    )
    stress = _data(
        run_robustness_analysis(
            ExecutionStressAnalysisRequest(
...
def test_compare_optimization_runs_preserves_existing_decisions() -> None:
    ...
    comparison = _data(compare_optimization_runs((first, second)))
...
def test_rank_parameter_sets_delegates_canonical_ranking() -> None:
    ...
    ranked = _data(
        rank_parameter_sets((_score("a" * 64, 1.0, 1), _score("b" * 64, 2.0, 1)))
    )
...
def test_calculate_robustness_score_counts_applicable_checks() -> None:
    ...
    score = _data(calculate_robustness_score((True, False, True)))
...
def test_build_optimization_handoff_delegates_canonical_assembly() -> None:
    ...
    response = build_optimization_handoff(request)
    assert _data(response) == build_optimization_evidence(request)
...
def test_public_boundary_returns_catalogued_error_response() -> None:
    ...
    assert response.error.code == "OPT_INVALID_REQUEST"
    assert response.metadata.domain == "optimization"
