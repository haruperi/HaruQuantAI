...
@pytest.mark.asyncio
async def test_distributed_worker_pool_helper_errors() -> None:
    from unittest.mock import AsyncMock

    from app.contracts.common.models import ProblemDetails
    from app.services.workspace.distributed_worker_pool.distributed_worker_pool import (
        _async_run_scenarios,
    )

    mock_service = AsyncMock()
    mock_service.distribute_workers.return_value = WorkspaceFailure(
        request_id=str(uuid.uuid7()),
        code="WORKER_UNKNOWN",
        problem=ProblemDetails(
            type="urn:problem:worker-unknown",
    ...
    with pytest.raises(RuntimeError):
        await fr_ws_register_worker_capabilities(
            mock_service, desc, "https://test:9000"
        )

    with pytest.raises(RuntimeError):
        await fr_ws_secure_remote_workers(
            mock_service, str(uuid.uuid7()), str(uuid.uuid7()), 1
        )

    with pytest.raises(RuntimeError):
        await fr_ws_schedule_data_locality(
            mock_service,
            job_id=str(uuid.uuid7()),
            attempt_no=1,
            task_run_id=str(uuid.uuid7()),
    ...
    with pytest.raises(RuntimeError):
        await fr_ws_verify_artifact_transfer(
            mock_service,
            artifact=staged,
            job_id=str(uuid.uuid7()),
            attempt_no=1,
    ...
    # Async scenarios execution
    await _async_run_scenarios()


@pytest.mark.asyncio
async def test_distributed_worker_pool_edge_cases() -> None:
    service = DistributedWorkerPoolService()

    # 1. Prepare transfer on non-staged artifact
    ...
    req = DistributeWorkersRequest(
        request_id=str(uuid.uuid7()),
        capability_snapshot_id=str(uuid.uuid7()),
        operation="PREPARE_TRANSFER",
        artifact=art,
    )
    object.__setattr__(art, "state", "COMMITTED")
    res_not_staged = await service.distribute_workers(req)
    assert isinstance(res_not_staged, WorkspaceFailure)
    assert res_not_staged.code == "TRANSFER_INVALID"

    # 2. Release lease on non-existent lease
    res_no_lease = await service.distribute_workers(
        DistributeWorkersRequest(
            request_id=str(uuid.uuid7()),
            capability_snapshot_id=str(uuid.uuid7()),
            operation="RELEASE_LEASE",
    ...
    assert isinstance(res_no_lease, WorkspaceFailure)
    assert res_no_lease.code == "LEASE_UNAVAILABLE"
