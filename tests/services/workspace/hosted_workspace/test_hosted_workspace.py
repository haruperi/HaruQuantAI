...
@pytest.mark.asyncio
async def test_helper_edge_cases() -> None:
    ws_id = str(uuid.uuid7())
    p_id = str(uuid.uuid7())

    # Mock no context in success response
    mock_service = AsyncMock()
    mock_service.host_workspaces.return_value = HostWorkspacesSuccess(
        request_id=str(uuid.uuid7()),
        context=None,
    )
    with pytest.raises(RuntimeError, match="no context in success response"):
        await fr_ws_isolate_hosted_workspaces(mock_service, ws_id)

    # Mock WorkspaceFailure on authorize
    mock_service.host_workspaces.return_value = WorkspaceFailure(
        request_id=str(uuid.uuid7()),
        code="WORKSPACE_NOT_FOUND",
        problem=ProblemDetails(
            type="urn:problem:not-found", title="Not Found", status=404, detail="denied"
        ),
    )
    with pytest.raises(RuntimeError, match=r"\[WORKSPACE_NOT_FOUND\] denied"):
        await fr_ws_authorize_hosted_workspaces(
            mock_service,
            ws_id,
            PrincipalRef(principal_id=p_id, auth_provider="jwt"),
            "action",
        )

    # Mock no decision in success response for authorize
    mock_service.host_workspaces.return_value = HostWorkspacesSuccess(
        request_id=str(uuid.uuid7()),
        decision=None,
    )
    with pytest.raises(RuntimeError, match="no decision in success response"):
        await fr_ws_authorize_hosted_workspaces(
            mock_service,
            ws_id,
            PrincipalRef(principal_id=p_id, auth_provider="jwt"),
...
@pytest.mark.asyncio
async def test_async_run_scenarios_execution() -> None:
