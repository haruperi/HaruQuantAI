"""Feature mount and lifecycle tests for External Series Alignment."""
...
from app.services.data.external_series_alignment.feature import (
    ExternalSeriesAlignmentFeature,
    feature,
)
...
def _context(
    feature_instance: ExternalSeriesAlignmentFeature,
...
async def test_feature_mount_with_dict_config() -> None:
    ...
    config_dict = {
        ...
        "default_missing_policy": "NULL",
    }
    await feat.mount(ctx, config_dict)

    assert feat.service is not None
    assert feat.service.config.max_series_points_per_request == 50_000
    assert feat.service.config.default_max_age_seconds == 3600
    provided = registry.resolve(ALIGN_SERIES_CAPABILITY)
    assert provided is feat.service


@pytest.mark.asyncio
async def test_feature_mount_with_object_config() -> None:
    """Verify feature mounting with ExternalSeriesAlignmentConfig object."""
    feat = ExternalSeriesAlignmentFeature()
    ctx, registry, _ = _context(feat)
    cfg = ExternalSeriesAlignmentConfig(max_series_points_per_request=25_000)
    await feat.mount(ctx, cfg)

    assert feat.service is not None
    assert feat.service.config.max_series_points_per_request == 25_000
    provided = registry.resolve(ALIGN_SERIES_CAPABILITY)
    assert provided is feat.service
...
async def test_feature_mount_invalid_config_type() -> None:
    ...
    with pytest.raises(
        TypeError, match="max_series_points_per_request must be an integer"
    ):
        await feat.mount(ctx, {"max_series_points_per_request": "not_an_int"})

    with pytest.raises(TypeError, match="default_timezone must be a string"):
        await feat.mount(ctx, {"default_timezone": 123})

    with pytest.raises(TypeError, match="default_max_age_seconds must be an integer"):
        await feat.mount(ctx, {"default_max_age_seconds": "3600"})

    with pytest.raises(TypeError, match="default_missing_policy must be a string"):
        await feat.mount(ctx, {"default_missing_policy": 456})
