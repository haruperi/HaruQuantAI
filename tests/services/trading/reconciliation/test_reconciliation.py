...
def test_compare_snapshots_order_cases() -> None:
    ...
    local_orders = [
        ...
        {"order_id": "2", "symbol": "EURUSD", "state": "NEW", "remaining_volume": "1.0", "vwap": "1.10"},
        {"order_id": "3", "symbol": "EURUSD", "state": "NEW", "remaining_volume": "1.0"}, # None price case
    ]
    broker_orders = [
        {"ticket": "2", "symbol": "EURUSD", "state": "NEW", "volume_current": "1.0001", "price": "1.10"},
        {"ticket": "3", "symbol": "EURUSD", "state": "NEW", "volume_current": "1.0", "price": "1.10"}, # None price locally
    ]
...
def test_compare_snapshots_position_cases() -> None:
    ...
    local_positions = [
        ...
        {"position_id": "pos-3", "symbol": "EURUSD", "volume": "1.0", "vwap": "1.10"}, # Missing at broker case
    ]
    broker_positions = [
        {"position_id": "pos-1", "symbol": "EURUSD", "volume": "1.002", "price_open": "1.10"},
        {"position_id": "pos-2", "symbol": "EURUSD", "volume": "1.0", "price_open": "1.10"}, # None VWAP locally
    ]
    ...
    assert len(discs) == 3
    assert {d.discrepancy_type for d in discs} == {"volume_mismatch", "missing_at_broker", "margin_mismatch"}
...
def test_reconciliation_service_success(
    ...
    def mock_broker_call(name: str, *args: Any, **kwargs: Any) -> Any:
        if name == "get_order_info":
            return [DummyBrokerRecord(ticket=123, symbol="EURUSD", volume_current=1.0, price=1.10, state=0)]
        elif name == "get_position_info":
            return [DummyBrokerRecord(position_id="pos-1", symbol="EURUSD", volume=1.5, price_open=1.10)]
        else:
    ...
    trade_store.orders = [
        {"order_id": "123", "symbol": "EURUSD", "state": "NEW", "remaining_volume": "1.0", "vwap": "1.10"}
    ]
...
def test_reconciliation_service_orphan_block(
    ...
    def mock_broker_call(name: str, *args: Any, **kwargs: Any) -> Any:
        if name == "get_order_info":
            ...
            return [DummyBrokerRecord(position_id="pos-orphan", symbol="EURUSD", volume=1.0, price_open=1.10)]
        else:
            return DummyBrokerRecord(balance=1000.0, margin=200.0)
...
def test_reconciliation_service_orphan_order_block(
    ...
    def mock_broker_call(name: str, *args: Any, **kwargs: Any) -> Any:
        if name == "get_order_info":
            return [DummyBrokerRecord(ticket=999, symbol="GBPUSD", volume_current=1.0, price=1.20, state=0)]
        elif name == "get_position_info":
            return []
        else:
            return DummyBrokerRecord(balance=1000.0, margin=200.0)
