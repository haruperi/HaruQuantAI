...
def test_backtest_config_validation() -> None:
    ...
    with pytest.raises(ValueError, match="spread_points and slippage_points cannot be negative"):
        BacktestConfig(spread_points=-1.0)

    with pytest.raises(ValueError, match="spread_points and slippage_points cannot be negative"):
        BacktestConfig(slippage_points=-1.0)

    with pytest.raises(ValueError, match="contract_size and default_quantity must be positive"):
...
def test_gross_pnl_calc() -> None:
    ...
    assert _gross_pnl(Direction.SHORT, 1.1, 1.0, 1.0, 100000.0) == pytest.approx(10000.0)
...
def test_engine_missing_symbol_timeframe() -> None:
    ...
    config = StrategyConfig({
        ...
        "strategy_manifest": {
            "identity": {"strategy_id": "test_strat", "strategy_type": "trend_following"},
            "version": "1.0.0",
            "chart_requirements": {"main": {"symbol": "", "timeframe": ""}, "other": {}},
...
def test_time_exit_trigger() -> None:
    ...
    config_dict = {
        "schema_version": "1.0.0",
        "strategy_manifest": {
            "identity": {"strategy_id": "time_exit_strat", "strategy_type": "trend_following"},
            "version": "1.0.0",
        ...
        "action_rules": {
            "long_entry_action": {"enabled": True, "action": "OPEN", "entry_type": "MARKET"},
            "short_entry_action": {"enabled": True, "action": "OPEN", "entry_type": "MARKET"},
        ...
        def evaluate(self, context):
            ...
            if decision.intents:
                ...
                if intent.action == IntentAction.OPEN:
                    ...
                    decision = replace(decision, intents=(replace(intent, protection=mocker_req),))
            return decision
    ...
    now = datetime(2026, 6, 23, 12, 0, tzinfo=UTC)
    bars = [
        Bar(open_time=now, open=1.1000, high=1.1050, low=1.0950, close=1.1000, volume=100.0),
        Bar(open_time=now + timedelta(minutes=5), open=1.1000, high=1.1050, low=1.0950, close=1.1000, volume=100.0),
        Bar(open_time=now + timedelta(minutes=10), open=1.1000, high=1.1050, low=1.0950, close=1.1000, volume=100.0),
        Bar(open_time=now + timedelta(minutes=15), open=1.1000, high=1.1050, low=1.0950, close=1.1000, volume=100.0),
    ]
...
def test_pending_order_activation() -> None:
    ...
    config_dict = {
        "schema_version": "1.0.0",
        "strategy_manifest": {
            "identity": {"strategy_id": "pending_strat", "strategy_type": "trend_following"},
            "version": "1.0.0",
            "chart_requirements": {"main": {"symbol": "EURUSD", "timeframe": "M5"}, "other": {}},
            "supported_runtime_modes": ["SIMULATOR"],
            "strategy_capabilities": ["market_entry"],
        ...
        "action_rules": {
            "long_entry_action": {"enabled": True, "action": "OPEN", "entry_type": "LIMIT"},
            "short_entry_action": {"enabled": True, "action": "OPEN", "entry_type": "LIMIT"},
            "long_exit_action": {"enabled": True, "action": "CLOSE"},
            "short_exit_action": {"enabled": True, "action": "CLOSE"},
            "direction_conflict_policy": "REJECT_BOTH",
            "entry_exit_conflict_policy": "EXIT_PRIORITY",
    ...
    class PendingStrategy(BaseStrategy):
        ...
        def evaluate(self, context):
            ...
            if decision.intents:
                intent = decision.intents[0]
                limit_intent = replace(intent, entry_type=EntryType.LIMIT, limit_price=1.0900)
    ...
    bars = [
        Bar(open_time=now, open=1.1000, high=1.1050, low=1.0950, close=1.1000, volume=100.0),
        Bar(open_time=now + timedelta(minutes=5), open=1.1000, high=1.1050, low=1.0850, close=1.1000, volume=100.0),
    ]
...
def test_pending_stop_order() -> None:
    ...
    config_dict = {
        "schema_version": "1.0.0",
        "strategy_manifest": {
            "identity": {"strategy_id": "stop_strat", "strategy_type": "trend_following"},
            "version": "1.0.0",
            "chart_requirements": {"main": {"symbol": "EURUSD", "timeframe": "M5"}, "other": {}},
            "supported_runtime_modes": ["SIMULATOR"],
            "strategy_capabilities": ["market_entry"],
        ...
        "action_rules": {
            "long_entry_action": {"enabled": True, "action": "OPEN", "entry_type": "STOP"},
            "short_entry_action": {"enabled": True, "action": "OPEN", "entry_type": "STOP"},
            "long_exit_action": {"enabled": True, "action": "CLOSE"},
            "short_exit_action": {"enabled": True, "action": "CLOSE"},
            "direction_conflict_policy": "REJECT_BOTH",
            "entry_exit_conflict_policy": "EXIT_PRIORITY",
    ...
    class StopStrategy(BaseStrategy):
        ...
        def evaluate(self, context):
            ...
            if decision.intents:
                intent = decision.intents[0]
                stop_intent = replace(intent, entry_type=EntryType.STOP, stop_price=1.1100)
    ...
    bars = [
        Bar(open_time=now, open=1.1000, high=1.1050, low=1.0950, close=1.1000, volume=100.0),
        Bar(open_time=now + timedelta(minutes=5), open=1.1000, high=1.1200, low=1.0950, close=1.1000, volume=100.0),
    ]
