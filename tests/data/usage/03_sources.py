...
def example_fr_data_022_bounded_fetch() -> RawSourceBatch:
    ...
    logger.info("FR-DATA-022: fetching through a registered source")
    ...
    logger.info(
        "Fetched %d raw records at revision %s", len(batch.records), batch.revision
    )
    return batch
...
def example_fr_data_023_symbol_discovery() -> SymbolPage:
    ...
    logger.info("FR-DATA-023: listing a bounded source symbol page")
...
def example_fr_data_024_symbol_metadata() -> SymbolMetadata:
    ...
    logger.info("FR-DATA-024: reading normalized symbol metadata")
    ...
    logger.info(
        "Metadata timezone=%s missing=%s", metadata.timezone, metadata.missing_fields
    )
    return metadata
...
def example_fr_data_025_lazy_registration() -> MarketDataSource:
    ...
    logger.info("FR-DATA-025: resolving a lazily registered source")
    source = resolve_source(_SOURCE_ID)
    logger.info("Resolved source type=%s", type(source).__name__)
    return source


def example_fr_data_026_source_policy() -> None:
    """Evaluate caller-selected source order against explicit policy."""
    logger.info("FR-DATA-026: evaluating source readiness and license policy")
    ...
    logger.info("Approved source order=%s", plan.ordered_sources)
...
def example_fr_data_027_source_promotion() -> SourceDescriptor:
    ...
    logger.info("FR-DATA-027: promoting a source with complete evidence")
    auth = AuthContext(
        contract_version="v1",
    ...
    logger.info("Promoted source readiness=%s", promoted.readiness)
    return promoted
...
def example_fr_data_028_read_only_account_snapshot() -> AccountStateSnapshot:
    """Normalize account evidence from a caller-owned read-only adapter."""
    logger.info("FR-DATA-028: normalizing caller-owned broker account reads")
    ...
    logger.info(
        "Account=%s connected=%s trading_allowed=%s",
        snapshot.account_id,
        snapshot.connected,
