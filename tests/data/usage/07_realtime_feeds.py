...
def example_fr_data_046_start_internal_feed() -> None:
    ...
    _header("FR-DATA-046: starting an internal live-capable feed")
    status = start_internal_feed(_feed_config())
    print(f"Feed={status.feed_id} state={status.state} capacity={status.buffer_capacity}")


def example_fr_data_047_ingest_event() -> None:
    """Ingest a sequenced event and update durable heartbeat evidence."""
    _header("FR-DATA-047: ingesting one sequenced feed event")
    event = RawFeedEvent(
        feed_id=_FEED_ID,
        sequence=1,
    ...
    print(f"Accepted sequence={event.sequence} depth={result.buffer_depth}")


def example_fr_data_048_read_status() -> None:
    """Read feed depth, drift, gaps, drops, and heartbeat state."""
    _header("FR-DATA-048: reading status backed by real feed runtime state")
    status = get_feed_status(
        FeedStatusRequest(feed_id=_FEED_ID, request_id=generate_id("req"))
    )
    print(
        f"Feed state={status.state} depth={status.buffer_depth} drift_ms={status.drift_ms} dropped={status.dropped_count} gaps={status.gap_count}"
    )
