...
def example_fr_data_014_transaction() -> None:
    ...
    logger.info("FR-DATA-014: executing one bounded SQLite transaction")
    ...
    logger.info("Transaction committed and returned %d rows", len(result.rows))


def example_fr_data_015_migration() -> None:
    """Apply one checksummed domain-owned migration exactly once."""
    logger.info("FR-DATA-015: applying an idempotent usage migration")
    step = MigrationStep(
        domain="usage",
...
def _save_and_load_dataset(root: Path) -> MarketDataset:
    ...
    logger.info("Verified %s with sha256=%s", root / path, manifest.content_hash)
    return loaded
...
def example_fr_data_017_load_dataset(root: Path) -> MarketDataset:
    ...
    logger.info("FR-DATA-017: loading a governed CSV artifact")
...
def example_fr_data_018_save_dataset(root: Path) -> MarketDataset:
    ...
    logger.info("FR-DATA-018: saving a governed CSV artifact")
    return _save_and_load_dataset(root)


def _write_and_read_cache() -> None:
    """Write and read one identity-bound, TTL-limited cache entry."""
    logger.info("FR-DATA-019/020: writing and reading a versioned cache entry")
