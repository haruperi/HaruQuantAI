...
def example_32_tick_model_trading_bar() -> None:
    """Derive four deterministic ticks per trading bar."""
    try:
        response = generate_tick_series(
            _synthetic_bars(), model="trading_bar", trading_timeframe="H1"
        )
        if response.data is not None:
            data = to_tick_dataframe(response.data)
...
def example_34_tick_model_ohlc_m1() -> None:
    ...
    try:
        response = generate_tick_series(
            _synthetic_bars(),
            model="ohlc_m1",
            m1_dataset=_synthetic_bars(timeframe="M1", count=300),
            trading_timeframe="H1",
        )
        if response.data is not None:
            data = to_tick_dataframe(response.data)
...
def example_35_tick_model_real() -> None:
    """Normalize supplied genuine-tick-shaped evidence."""
    bars = _synthetic_bars()
    source_ticks = generate_tick_series(
        bars, model="trading_bar", trading_timeframe="H1"
    ).data
    if source_ticks is None:
        print("Unavailable -> source tick derivation returned no data")
        print("Data -> no invented ticks substituted")
        return
    try:
        response = generate_tick_series(
            bars,
            model="real",
            real_tick_dataset=source_ticks,
            trading_timeframe="H1",
        )
        if response.data is not None:
            data = to_tick_dataframe(response.data)
