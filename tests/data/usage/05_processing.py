...
def example_fr_data_036_resample_ohlcv() -> MarketDataset:
    ...
    _header("FR-DATA-036: resampling M1 OHLCV to M5")
    result = resample_ohlcv(_minute_bars(), "M5")
    print(
        f"M5 open={result.records[0].open} high={result.records[0].high} low={result.records[0].low} close={result.records[0].close} volume={result.records[0].volume}"  # type: ignore[union-attr]
    )
    return result
...
def example_fr_data_037_no_lookahead_alignment() -> None:
    ...
    _header("FR-DATA-037: aligning datasets without lookahead")
...
def example_fr_data_039_synthetic_bars() -> MarketDataset:
    ...
    print(f"Generated {result.record_count} explicitly synthetic H1 bars")
    return result
...
def example_fr_data_080_align_private_tabular_copy() -> pd.DataFrame:
    ...
    _header("FR-DATA-080: aligning a private tabular copy")
    ...
    print(f"Aligned private frame rows={len(aligned)}")
    return aligned


def example_fr_data_081_json_safe_records() -> None:
    """Serialize canonical bars and a private frame deterministically."""
    _header("FR-DATA-081: serializing deterministic JSON-safe records")
    dataset = _minute_bars()
    ...
    print(f"Serialized canonical={len(canonical)} tabular={len(tabular)}")
