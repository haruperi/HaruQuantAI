...
def example_fr_data_030_historical_market_read() -> MarketDataset:
    ...
    _header("FR-DATA-030: reading governed historical bars")
    ...
    print(f"Retrieved {dataset.record_count} normalized bars")
    return dataset


def example_fr_data_031_symbol_discovery() -> None:
    """Discover a deterministic bounded page of configured symbols."""
    _header("FR-DATA-031: discovering configured symbols")
...
def example_fr_data_032_symbol_metadata() -> None:
    ...
    _header("FR-DATA-032: reading normalized symbol metadata")
    ...
    print(f"Metadata digits={metadata.digits} timezone={metadata.timezone}")


def example_fr_data_033_availability() -> None:
    """Inspect measured range, gap, overlap, and completeness evidence."""
    _header("FR-DATA-033: inspecting local historical availability")
    ...
    print(
        f"Availability records={availability.record_count} completeness={availability.completeness}"
    )
...
def example_fr_data_034_current_schedule() -> None:
    ...
    _header("FR-DATA-034: reading the current configured trading session")
    ...
    print(
        f"Current sessions={len(schedule.sessions)} timezone={schedule.timezone}"
    )


def example_fr_data_035_historical_volume() -> None:
    """Summarize source-native historical volume with declared units."""
    _header("FR-DATA-035: summarizing historical volume")
    ...
    print(f"Historical volume total={result.summary.total}")
...
def example_fr_data_076_market_context_evidence() -> None:
    ...
    _header("FR-DATA-076: acquiring policy-neutral market context")
    ...
    print(f"Context session={evidence.session_state} spread={evidence.spread}")
...
def example_fr_data_079_fx_evidence() -> None:
    ...
    _header("FR-DATA-079: acquiring exact FX conversion evidence")
    ...
    print(f"Composite FX rate={evidence.composite_rate}")
