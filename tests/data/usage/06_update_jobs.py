...
def example_fr_data_041_backfill_key() -> str:
    ...
    _header("FR-DATA-041: deriving a canonical backfill idempotency key")
    key = derive_backfill_key(_chunk_request())
    print(f"Backfill key={key}")
    return key


def example_fr_data_044_create_recurrent_job() -> None:
    """Persist a disabled recurrent job definition before starting it."""
    _header("FR-DATA-044: creating a recurrent update-job definition")
    ...
    print(f"Created job={status.job_id} enabled={status.enabled}")


def example_fr_data_042_execute_chunk() -> None:
    """Retrieve, normalize, publish, and checkpoint one bounded chunk."""
    _header("FR-DATA-042: executing one bounded backfill chunk")
    result = execute_backfill_chunk(_chunk_request())
    if not result.committed:
        raise AssertionError("backfill chunk did not commit")
    print(
        f"Committed chunk key={result.idempotency_key} records={result.record_count} checkpoint={result.checkpoint}"
    )
...
def example_fr_data_084_private_chunking_boundary() -> None:
    ...
    _header("FR-DATA-084: verifying the private bounded chunking boundary")
    forbidden_exports = (
        "chunk_data",
        "chunk_sequence",
        "execute_backfill_chunk",
...
def example_fr_data_044_start_stop_worker() -> None:
    ...
    async def exercise() -> None:
        ...
        await asyncio.sleep(0)
        print(f"Worker lifecycle completed with state={stopped.state}")
