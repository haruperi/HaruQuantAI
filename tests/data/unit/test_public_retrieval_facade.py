...
from app.services.data.contracts import DataError, MarketDataRequest
from app.services.data.public_api import operations
...
def isolated_facade(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from app.services.data.access import historical, reference, sessions

    monkeypatch.setattr(operations, "ensure_source", lambda *_args: None)
    monkeypatch.setattr(operations, "ensure_source_access", lambda *_args: None)
    monkeypatch.setattr(operations, "ensure_storage", lambda *_args: None)
    monkeypatch.setattr(operations, "ensure_identity", lambda *_args: None)
    monkeypatch.setattr(operations, "resolve_calendar", lambda *_args: object())
    monkeypatch.setattr(historical, "fetch_market_dataset", lambda request: request)
    monkeypatch.setattr(reference, "fetch_symbol_metadata", lambda request: request)
    monkeypatch.setattr(reference, "discover_symbols", lambda request: request)
    monkeypatch.setattr(reference, "inspect_availability", lambda request: request)
    monkeypatch.setattr(
        sessions,
...
def test_get_market_data_supports_direct_keywords_and_defaults(
    ...
    assert result.quality_failure_behavior == "fail"
