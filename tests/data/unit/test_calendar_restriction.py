...
def test_empty_successful_event_result_is_open() -> None:
    ...
    assert evaluate_calendar_state([], _AT) == CALENDAR_STATE_OPEN
...
def test_at_on_event_release_is_event_state() -> None:
    ...
    assert evaluate_calendar_state(events, _AT) == CALENDAR_STATE_EVENT
...
def test_minutes_before_excludes_at_outside_window() -> None:
    ...
    events = [_event(delta=timedelta(minutes=5))]
    assert evaluate_calendar_state(events, _AT, before_minutes=0) == CALENDAR_STATE_OPEN
...
def test_minimum_impact_filters_low_events() -> None:
    ...
    assert (
        evaluate_calendar_state(events, _AT, minimum_impact=EventImpact.HIGH)
        == CALENDAR_STATE_OPEN
    )
    assert (
        evaluate_calendar_state(events, _AT, minimum_impact=EventImpact.LOW)
        == CALENDAR_STATE_BLACKOUT_BEFORE
    )
...
def test_event_release_wins_over_blackout_of_a_near_neighbour() -> None:
    ...
    assert evaluate_calendar_state(events, _AT, before_minutes=10) == (
        CALENDAR_STATE_EVENT
    )
...
def test_naive_at_raises() -> None:
    ...
    with pytest.raises(ValueError, match="timezone-aware"):
        evaluate_calendar_state(events, _AT.replace(tzinfo=None))
