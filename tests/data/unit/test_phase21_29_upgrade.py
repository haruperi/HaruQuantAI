...
def test_data_error_codes_are_approved_and_mapping_is_redacted() -> None:
    """Verify DATA_ERROR_CODES properties and error mapping."""
    assert isinstance(DATA_ERROR_CODES, frozenset)
    assert "UNSUPPORTED_TIMEFRAME" in DATA_ERROR_CODES
    assert "DATA_SERIALIZATION_FAILED" in DATA_ERROR_CODES
...
def test_set_cached_data_distinguishes_insert_update_no_op() -> None:
    ...
    symbol = f"PERSISTTEST_{uuid.uuid4().hex[:8]}"
    key = generate_cache_key("csv", symbol, "M1", "2026-01-01", "2026-01-02")
    ...
    first = set_cached_data(
        key, "csv", symbol, "M1", "2026-01-01", "2026-01-02", records_a, 60
    )
    ...
    changed = set_cached_data(
        key, "csv", symbol, "M1", "2026-01-01", "2026-01-02", records_b, 60
    )
