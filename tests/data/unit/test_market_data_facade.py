...
def isolated_facade(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # The migrated facades resolve their owned request and then delegate to the
    # private raw cores, so the request-capturing fakes patch the raw cores.
    monkeypatch.setattr(pipeline, "_fetch_market_dataset_raw", lambda request: request)
    monkeypatch.setattr(
        symbol_discovery, "_fetch_symbol_metadata_raw", lambda request: request
    )
    monkeypatch.setattr(
        symbol_discovery, "_discover_symbols_raw", lambda request: request
    )
    monkeypatch.setattr(
        symbol_discovery, "_inspect_availability_raw", lambda request: request
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda request, _calendar, **_kwargs: MarketSchedule(
...
def test_get_market_data_preserves_typed_request_form(
    ...
    assert _unwrap(get_market_data(request)) is request
...
def test_all_retrieval_reference_exports_accept_direct_keywords(
    ...
    tick = _unwrap(get_tick_data(source_id="mt5", symbol="EURUSD"))
    spread = _unwrap(get_spread_data(source_id="mt5", symbol="EURUSD"))
    metadata = _unwrap(get_symbol_metadata(source_id="mt5", symbol="EURUSD"))
    symbols = _unwrap(list_symbols(source_id="mt5"))
    ...
    hours = _unwrap(get_market_hours(source_id="mt5", symbol="EURUSD"))
    sessions = _unwrap(get_trading_sessions(source_id="mt5", symbol="EURUSD"))
    volume = _unwrap(
        get_historical_volume(
            source_id="mt5",
            symbol="EURUSD",
...
def test_typed_request_cannot_be_mixed_with_direct_keywords(
    ...
    response = get_market_data(request, symbol="GBPUSD")
