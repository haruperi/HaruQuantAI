"""Unit evidence for Strategy Trading Cockpit contracts."""
...
def test_trade_plan_lifecycle_is_immutable_and_sim_only() -> None:
    """Verify lifecycle transitions, amendments, and cockpit route discipline."""
    draft = _plan()
...
def test_operating_exit_manual_expectancy_and_governance_fail_closed() -> None:
    """Verify remaining cockpit policies and fail-closed fallbacks."""
...
def test_cockpit_contracts_reject_invalid_relationships() -> None:
    """Cover strict validation branches for cockpit contracts."""
    with pytest.raises(ValueError, match="non-empty"):
