"""Feature specification and mount tests for Provider and Broker Mapping."""
...
from app.services.catalogue.provider_mapping.feature import (
    ProviderMappingFeature,
    feature,
)
...
def _context(
    feature_instance: ProviderMappingFeature,
...
def test_spec_declares_exact_provider_and_retained_state() -> None:
    ...
    assert SPEC.provides == frozenset({MAP_PROVIDERS_CAPABILITY})
    assert SPEC.requires == frozenset()
    assert SPEC.config_keys == frozenset({"database_path"})
    assert SPEC.state is not None
    assert SPEC.state.namespace == "catalogue.provider_mappings"
...
async def test_mount_stages_provider_and_scope_withdraws_it(tmp_path: Any) -> None:
    """Verify mounting stages MAP_PROVIDERS_CAPABILITY and scope close withdraws it."""
    feature_instance = feature()
    assert isinstance(feature_instance, ProviderMappingFeature)
    context, registry, scope = _context(feature_instance)

    await feature_instance.mount(
        context,
        {"database_path": str(tmp_path / "mappings.db")},
    )
    assert feature_instance.service is not None
    assert registry.resolve(MAP_PROVIDERS_CAPABILITY) is feature_instance.service

    await scope.close()
    assert registry.resolve(MAP_PROVIDERS_CAPABILITY) is None
