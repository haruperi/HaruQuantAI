"""Unit and parity tests for indicator.rsi.default provider."""
...
def test_manifest_exact() -> None:
    """Verify RSI manifest matches canonical declaration."""
...
def test_factory_exact() -> None:
    """Verify create_provider returns RsiCapabilityV1 and rejects non-empty deps/config."""
    scope = EffectScope()
    cap = create_provider(
        dependencies={},
        config={},
    ...
    assert isinstance(cap, RsiCapabilityV1)
    assert callable(cap.calculate)

    # Reject non-empty dependencies
    with pytest.raises(
        ValueError, match="RSI provider accepts no dependencies or config"
    ):
        create_provider(
...
def test_contract_call() -> None:
    ...
    resp = cap.calculate(cast("Any", dataset), period=3, source="close")
    result: Any = unwrap_response(resp)
    assert result.output_columns == ("rsi_3",)
    values_df = get_indicator_result_values(result)
    assert "rsi_3" in values_df.columns
    assert len(values_df) == 7


def test_phase0_parity() -> None:
    """Verify RSI output matches phase 0 golden fixture numbers exactly."""
    scope = EffectScope()
    cap = create_provider(dependencies={}, config={}, scope=scope)
    dataset, golden = _load_golden_dataset()

    resp = cap.calculate(
        cast("Any", dataset), period=int(golden["rsi"]["period"]), source="close"
    )
    result: Any = unwrap_response(resp)
    values_df = get_indicator_result_values(result)
    values: list[float] = values_df[str(golden["rsi"]["output_column"])].tolist()
    expected = cast("list[float | None]", golden["rsi"]["expected"])
...
def test_import_no_io() -> None:
    ...
    script = """
import sys
import app.services.indicators.momentum.rsi_default.plugin as plugin
...
def test_composition_activation() -> None:
    ...
    dataset, _ = _load_golden_dataset()
    resp = rsi_cap.calculate(cast("Any", dataset), period=3)
    result: Any = unwrap_response(resp)
    assert result.output_columns == ("rsi_3",)


def test_facade_unbound_raises_capability_unavailable() -> None:
    """Verify calling rsi without an active provider raises CapabilityUnavailableError."""
    dataset, _ = _load_golden_dataset()
    ...
    with (
        ...
        rsi(cast("Any", dataset), period=3)

    detail = exc_info.value.detail
    assert detail.reason_code == CapabilityReasonCode.NOT_INSTALLED
    assert detail.capability == "indicator.rsi.v1"
    assert detail.consumer == "compatibility_facade"
    assert detail.dependency_chain == ("compatibility_facade", "indicator.rsi.v1")


def test_facade_bound_success_and_nested_restoration() -> None:
    """Verify rsi succeeds inside bind_runtime and restores prior runtime on exit."""
    manifest = load_manifest(_MANIFEST_PATH)
    report = resolve_providers(
        (manifest,),
        enabled_provider_ids=frozenset({manifest.provider_id}),
    ...
    with bind_runtime(runtime):
        resp = rsi(cast("Any", dataset), period=3)
        result: Any = unwrap_response(resp)
        assert result.output_columns == ("rsi_3",)
        ...
        resp_outer = rsi(cast("Any", dataset), period=3)
        assert unwrap_response(resp_outer).output_columns == ("rsi_3",)
