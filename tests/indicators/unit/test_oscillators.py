...
import pytest
from app.services.indicators import build_indicator_config, rsi, williams_r
...
def test_rsi_flat_prices_yield_flat_fifty() -> None:
    ...
    assert valid.any()
    assert (values.loc[valid, "rsi_2"] == 50.0).all()
...
def test_rsi_non_default_source_uses_qualified_output_name() -> None:
    ...
    assert "rsi_high_2" in result_metadata(result)["output_columns"]
