...
class LifecycleEventPayload:
    """Local payload used to verify scoped event subscriptions."""
...
async def test_background_task_and_listener_cleanup_on_scope_close() -> None:
    ...
    async def handler(_event: LifecycleEventPayload) -> None:
        return None

    context.subscribe(LifecycleEventPayload, handler, mode=EventMode.PARALLEL)
    assert event_bus.listener_count(LifecycleEventPayload) == 1
    await scope.close()
    assert task.done()
    assert task_cancelled
    assert event_bus.listener_count(LifecycleEventPayload) == 0
