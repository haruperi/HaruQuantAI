"""Runnable usage evidence for the public Trading validation API."""
...
def test_usage_orders_validate_order_request() -> None:
    """Validate all order and account evidence before route selection."""
    request = _request()
    assert validate_order_request(request, _account(), _symbol_capability()) is request


def test_usage_snapshots_get_route_snapshot() -> None:
    """Read explicit fresh route facts through an injected source."""

    def source(_route: object, _provider: object) -> dict[str, object]:
        """Return explicit route evidence for the usage example."""
        return _snapshot().model_dump(mode="python")

    assert get_route_snapshot(_request(), source).available  # type: ignore[arg-type]


def test_usage_readiness_assess() -> None:
    """Aggregate mandatory route and Risk evidence."""
...
def test_usage_plans_build_execution_plan() -> None:
    """Build deterministic executable material without side effects."""
    ...
    assert build_execution_plan(_request(), readiness).route.value == "sim"


def test_usage_snapshots_route_snapshot() -> None:
    """Inspect immutable provenance and freshness evidence."""
    assert _snapshot().source_id == "usage-data-source-001"


def test_usage_readiness_assessment() -> None:
    """Inspect a bounded passed readiness result."""
    assessment = assess_execution_readiness(
        _request(),
        _snapshot(),
        _risk(),
    ...
    assert assessment.failed_check_codes == ()
