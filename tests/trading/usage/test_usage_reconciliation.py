"""Runnable usage evidence for the public Trading reconciliation API."""
...
def test_usage_snapshots_authority_snapshot() -> None:
    """Consume normalized authority evidence without provider objects."""
    assert _snapshot().source_id == "usage-sim-read-001"


def test_usage_compare_authority_state() -> None:
    """Compare authority truth with one exact Trading projection."""
    assert not compare_authority_state(_snapshot(), _projection()).unresolved


def test_usage_compare_reconciliation_report() -> None:
    """Inspect deterministic discrepancy and evidence fields."""
    report: ReconciliationReport = compare_authority_state(
        _snapshot(),
        _projection(),
    )
...
def test_usage_authority_resolve_unknown() -> None:
    """Persist a retry lock and explicit no-retry resolution transition."""
    resolution = resolve_unknown_outcome(  # type: ignore[arg-type]
        _receipt(),
        _Store(),
        lambda _route: _snapshot(),
    )
    assert resolution.transition == "resolved_no_retry"


def test_usage_authority_resolution() -> None:
    """Inspect immutable retry and incident transition evidence."""
    ...
    assert resolution.incident_reference.startswith("trd-event-")
