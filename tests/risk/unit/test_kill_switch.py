...
class _CollectingAuditBus:
    """Minimal audit bus for kill-switch dispatch tests."""

    def __init__(self) -> None:
        self._handlers: dict[str, list[Any]] = {}

    def subscribe(self, event_type: str, handler: Any) -> None:
        """Register a test handler for an event type."""
        self._handlers.setdefault(event_type, []).append(handler)

    def publish(self, event: dict[str, object]) -> object:
        """Publish an event to matching test handlers."""
...
def test_event_dispatching(manager: KillSwitchManager) -> None:
    """Verify that triggering and resuming switches dispatches events to audit bus."""
    bus = _CollectingAuditBus()
    ...
    # Trigger switch
    manager.trigger("strategy", "strat1", "Test trigger", audit_bus=bus)
    ...
    # Resume switch
    manager.resume("strategy", "strat1", operator_role="admin", audit_bus=bus)
