...
from app.utils import logger as global_logger
...
def test_get_logger_configures_no_handlers() -> None:
    assert get_logger("unit.no_handlers").handlers == []
...
def test_json_formatter_preserves_colliding_context() -> None:
...
def test_sink_failure_uses_safe_fallback(
    ...
    with pytest.raises(ConfigurationError):
        configure_logging(LoggingSettings(file_path=tmp_path / "missing" / "app.log"))
...
def test_exception_logging_includes_traceback(tmp_path: Path) -> None:
    ...
    assert (
        "ZeroDivisionError"
        in json.loads(log_path.read_text(encoding="utf-8"))["exception"]
    )
...
def test_flush_logging_synchronizes_delivery_without_shutdown(tmp_path: Path) -> None:
    log_path = tmp_path / "app.log"
    configure_logging(
        LoggingSettings(file_path=log_path, log_directory=None, colorize=False)
    )
