"""WF-UTL-001: execute structured logging and redaction end to end."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="wf-utl-001-") as directory:
        ...
        try:
            ...
            bound = logger.bind(**redacted.value)
            bound.info("workflow record token=synthetic-secret")
            bound.bind(log_type="access").info("access workflow record")
            bound.debug("debug workflow record")
            bound.error("error workflow record")
