...
- [ ] Confirm the latest `main` commit and record it in the implementation PR.
- [ ] Create the implementation branch from that exact commit.
- [ ] Run the existing quality gates without modifications.
- [ ] Save the complete baseline output in the PR description or an attached artifact.
- [ ] Do not begin corrective code changes if the baseline already fails.
...
- [ ] Create `docs/architecture/audit/composability_remediation_baseline.md`.
- [ ] Record the baseline commit SHA.
- [ ] Record Python and `uv` versions.
- [ ] Record the existing test count and coverage generated locally.
- [ ] Record results for Ruff, mypy, Import Linter, AST checks, and pytest.
- [ ] State clearly that GitHub Actions results, not commit-message claims, are the authoritative remote evidence.
...
- [ ] `[profile] name = "live"` is rejected rather than silently becoming Research.
- [ ] Unknown profiles are rejected.
- [ ] Live readiness is false when any one required safety capability is missing.
- [ ] Two enabled providers for one capability without selection are rejected.
- [ ] Reconfiguring a provider remounts a consumer and replaces its captured provider object.
- [ ] Replacing a provider remounts all transitive consumers.
- [ ] A transactional replacement containing a task, listener, callback, and async context manager retains all replacement effects after commit.
- [ ] A pre-commit replacement failure preserves the exact old provider instance.
- [ ] A cleanup failure after commit is reported as a post-commit cleanup error, not a successful rollback.
- [ ] Mixed event modes on one event type invoke only matching handlers.
- [ ] Disposing one duplicate handler registration leaves the other registration active.
- [ ] Unexpected background-task failure changes feature state to `FAILED_RUNTIME`.
- [ ] Registering a new effect on a closed scope raises a lifecycle error.

## Task 0.3 — Quality and documentation

- [ ] Confirm the new tests fail for the expected reasons only.
- [ ] Do not weaken assertions to make the current implementation pass.
- [ ] Add a short failure-to-phase mapping to the baseline document.
...
- [ ] New characterization tests fail and identify the audited defects precisely.
