...
If any unresolved placeholder remains, stop and request the missing value instead of creating a branch,
or dry run.
...
3. Derive and validate the branch using the naming and safety rules in `AGENTS.md`. Create exactly one task branch, then perform all remaining planning inside it.
...
5. Append the next numbered, complete eight-part dry run to the task branch's `docs/dev/task/planner.md`.
   Record the task ID, main path and baseline commit, task branch, expected
...
7. Return a concise handoff containing the task ID, branch path, dry-run number, blockers, and
