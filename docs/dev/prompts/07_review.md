# Full Domain Completion Review and Correction Prompt
...
## Domain variable bindings

Set these three values before using this prompt:
...
- `[DOMAIN_NAME]`: Human-readable domain name, such as `Data`.
- `[DOMAIN]`: Lowercase filesystem and Python package identifier, such as `data`.
- `[DOM]`: Uppercase identifier used in feature, requirement, workflow, contract, and finding IDs, such as `DATA`.

These are intentional template variables resolved by the bindings above. They must not be reported as unresolved specification placeholders.

Any other unresolved placeholder found in an authoritative repository source remains a review finding.

Perform a completion review of the `[DOMAIN_NAME]` domain.
...
- `docs/CHANGELOG.md` Feature Registry requirements.
...
- Feature definitions in the Feature Registry section of `docs/CHANGELOG.md`.
...
4. It is registered in the Feature Registry section of `docs/CHANGELOG.md`.
...
A README or changelog declaration cannot make a submodule path public.
...
Verify that all implemented features and public exports are registered and current in the Feature Registry section of `docs/CHANGELOG.md`.

### Feature Registry definition

The Feature Registry consists of domain feature sections formatted as:

`## FEAT-[DOM]-NN: [Feature Name] ([module_name])`

Each feature section must contain a public-export status table:

`| Public export | Purpose | Status | Notes |`

Confirm:

- Every public export is registered.
- Every public export has its exact public signature or declaration.
- Every public export has an accurate purpose.
...
The Feature Registry section of `docs/CHANGELOG.md` is a mutable current-state registry.

It must be updated to match the final implementation.

Dated historical changelog entries are append-only and must never be rewritten.

Corrections must add a new dated historical entry describing the completed changes.
...
15. **Feature Registry section in `docs/CHANGELOG.md` accuracy**
