...
|---:|---|---|---:|---:|---:|
| 0 | Project Scaffold and CI Foundation | Repository scaffold / CI / docs | 41 | 0 | 41 |
| 1 | Utils Foundation | `app/utils/` | 1,168 | 0 | 1,168 |
| 1.5 | Core Domain Contracts | `app/contracts/` | 62 | 0 | 62 |
...
| 5 | Risk Governance | `app/services/risk/` | 684 | 0 | 684 |
| 6 | Analytics Service | `app/services/analytics/` | 465 | 0 | 465 |
| 7 | Trading Service | `app/services/trader/` + broker integrations | 91 | 0 | 91 |
| 8 | Simulation Engine | `app/services/simulation/` | 1,668 | 0 | 1,668 |
| 9 | Optimization Service | `app/services/optimization/` | 285 | 0 | 285 |
| 10 | Live Runtime | `app/services/live/` | 243 | 0 | 243 |
| 11 | UI and API Gateway | `api/ and ui/` | 373 | 0 | 373 |
| 12 | Research Edge Lab | `app/services/research/` | 297 | 0 | 297 |
| 13 | Conversation AI Layer | `app/services/conversation/` | 255 | 0 | 255 |
| **Total** |  |  | **7,549** | **0** | **7,549** |

**Hardening amendment delta:** 191 additional unchecked tasks were added to the original 7,358 unchecked tasks, bringing the total to 7,549 unchecked tasks.
...
#### Canonical risk decisions and state inputs

Requirements:

- [ ] Adopt the Phase 1.5 `RiskDecision`, `RiskRejection`, `PositionSizingResult`, `PortfolioRiskSnapshot`, `KillSwitchState`, and `RiskAuditEvent` contracts.
- [ ] Require Risk Governance to consume current exposure, pending orders, open positions, account state, correlation state, and freshness metadata before approving execution.
- [ ] Ensure every approved risk decision produces a bounded, auditable `OrderIntent` and every rejection produces a deterministic `RiskRejection`.
- [ ] Ensure stale portfolio state, stale account state, stale market data, missing exposure, or unknown pending orders fail closed.
- [ ] Add tests proving Risk rejects signals when portfolio/trading state is unavailable, stale, inconsistent, or not reconciled.
- [ ] Add tests proving kill-switch state blocks approvals regardless of signal quality or optimization evidence.
...
feat(risk-governance): implement pre-trade risk engine and position calculators



- Implement stateless limit checks (drawdown, leverage, concentration limits)

- Build position sizing calculators (fixed fractional, Kelly, volatility, milestone)

- Implement stress scenario evaluator and asset correlation stress testing

- Design unified global Kill Switch and role-based lifecycle permissions
...
- Done criterion: All 7,549 source checkbox tasks are implemented, tested, or explicitly deferred with owner-approved rationale.
