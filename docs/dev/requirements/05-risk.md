...
**Confirmed assumptions from source**
...
Prior source-confirmed decisions have been cross-applied to the functional, non-functional, security, testing, documentation, and assumption sections above. Newly identified audit refinements remain pending or recommended where owner/architecture approval is still required.

| ID | Pending decision | Target section | Production-readiness impact | Safe fallback if unresolved |
|---|---|---|---|---|
| RISK-PEND-001 | Confirm whether double-spend prevention is implemented inside Risk through a pending-approvals cache or enforced externally by Execution/Governance serialization. | Pre-trade risk review and governor decisions | Blocks live pre-trade approval semantics and concurrency tests. | Live-sensitive pre-trade approval fails closed with `PENDING_APPROVAL_DOUBLE_SPEND_BLOCKED`. |
| RISK-PEND-002 | Confirm the exact default fractional Kelly multiplier value. | Position sizing and allocation | Blocks production Kelly defaults and sizing fixtures. | Production profiles must define `kelly_fraction_multiplier`; missing value returns `INVALID_RISK_CONFIG`. |
| RISK-PEND-003 | Confirm the exact stressed-lookback policy for crisis correlation, VaR, and CVaR calculations. | Regime, VaR, CVaR, and correlation | Blocks crisis-regime calculation behavior and benchmark fixtures. | Live-sensitive crisis/high-volatility decisions return `needs_more_evidence` or `block` when stressed policy is required but unavailable. |
| RISK-PEND-004 | Confirm whether crisis-period references are configured profiles, evidence-pack inputs, or implementation fixtures. | Regime and evidence contracts | Blocks crisis evidence schema and repeatable tests. | Crisis-reference-dependent live decisions fail closed as missing evidence. |
| RISK-PEND-005 | Confirm which heavy-tailed parametric VaR distribution is supported first. | VaR/CVaR statistical risk | Blocks non-historical production-live parametric VaR implementation. | Historical VaR remains the only production-live default. |
| RISK-PEND-006 | Confirm whether Gaussian parametric VaR is allowed in production-live workflows after warning or requires explicit waiver. | VaR/CVaR statistical risk and approvals | Blocks live production waiver semantics and acceptance tests. | Gaussian parametric VaR in production-live returns `needs_approval` or `block` according to profile until waiver policy is approved. |

**Safe fallback requirements for pending decisions**
...
The risk module must distinguish official agent-callable tools from public Python contracts, deterministic services, internal helpers, and legacy compatibility exports.
...
**Transition and compatibility appendix**

The items in this section document current implementation traceability and transitional compatibility. They shall not expand the clean target Risk architecture or become agent-callable official tools unless explicitly promoted through a public contract, registry review, and mapped tests.
...
The primary public contracts are:
...
Every official tool shall return the standard HaruQuant tool response envelope with `status`, `message`, `data`, `error`, and `metadata`.

Each primary public contract shall define required fields, optional fields, validation rules, canonical enums, error behavior, schema version, serialization rules, and example payloads before implementation begins.
...
**Canonical enums and numeric policy**
...
**Time and deterministic replay policy**
...
**Exposure, concentration, and conversion**
...
**Drawdown, loss, and prop-firm rules**
...
**Margin, leverage, VaR, CVaR, and statistical risk**
...
**Market context, spread, slippage, session, and news**
...
**Pre-trade risk review and governor decisions**
...
**Approval tokens and exception workflow**
...
**Scenario, replay, reporting, and regimes**
...
| ID | Dataset size | Portfolio shape | Strategy count | History/scenarios | Cache state | Expected p95 | Variance |
|---|---:|---|---:|---|---|---:|---:|
| PERF-001 | <=100 KiB | 1 account, 1 position, 0 pending orders | 1 | 250 returns | cold | 25 ms sizing | 20% |
| PERF-002 | <=250 KiB | 1 account, 10 positions, 2 pending orders | 3 | 500 returns | warm | 50 ms pre-trade | 20% |
| PERF-003 | <=500 KiB | 1 account, 50 positions, 10 pending orders | 10 | 1,000 returns | warm | 100 ms pre-trade | 20% |
| PERF-004 | <=1 MiB | 1 account, 500 positions, 100 pending orders | 100 | 5,000 returns | warm | 250 ms snapshot | 25% |
| PERF-005 | <=1 MiB | 500 positions with 100-symbol correlation context | 100 | 5,000 returns | warm | 50 ms correlation-adjusted sizing | 25% |
| PERF-006 | <=1 MiB | 500 positions | 100 | 100 stress scenarios | warm | 5 s scenario analysis | 25% |
| PERF-007 | <=1 MiB | 500 positions | 100 | 10,000 audit records | warm | 2 s audit-chain verification | 25% |
| PERF-008 | <=500 KiB | 100 positions, high-volatility regime | 25 | 5,000 stressed returns | cold | 200 ms stressed VaR/CVaR | 25% |
| PERF-009 | <=500 KiB | 100 positions, crisis regime | 25 | 5,000 stressed returns | cold | 250 ms stressed correlation | 25% |
| PERF-010 | <=250 KiB | approval-required live-sensitive request | 5 | token-state backend mocked | warm | 75 ms token validation | 25% |
| PERF-011 | <=1 MiB | completed decision package with scenario outputs | 100 | 100 scenario results | warm | 1 s Markdown report | 25% |
| PERF-012 | >1 MiB | oversized public tool payload | any | any | not applicable | reject before expensive calculation | not applicable |
...
* [ ] Exceptions shall be mapped to deterministic error codes.
...
**Dependency, interface, maintainability, and concurrency**
...
**Confirmed edge cases and required handling**
...
`request_id` values below are illustrative only. Production callers should use the approved generated request ID or trace ID helper, preferably UUIDv7/time-sortable if adopted by the shared identity contract.
...
Before Builder handoff, usage examples shall include standard response-envelope examples for success and failure paths.
...
{
  "status": "success",
  "message": "Risk review approved.",
  "data": {
    "decision": "approve",
    "risk_decision_id": "risk_decision_example",
    "approved_volume": "0.10",
    "primary_failure_limit": null,
    "warnings": []
  },
  "error": null,
  "metadata": {
    "tool_name": "review_trade_risk",
    "tool_version": "v1-draft",
    "tool_category": "risk",
    "tool_risk_level": "governed",
    "request_id": "req-risk-approve",
    "execution_ms": 42.0,
    "read_only": true,
    "writes_file": false,
...
{
  "status": "success",
  "message": "Risk review rejected by configured limit.",
  "data": {
    "decision": "reject",
    "risk_decision_id": "risk_decision_reject",
    "primary_failure_limit": "max_daily_loss",
    "composite_breach_flags": ["daily_loss_limit_failed"]
  },
  "error": null,
  "metadata": {
    "tool_name": "review_trade_risk",
    "request_id": "req-risk-reject",
    "places_trade": false
  }
}
```
...
{
  "status": "success",
  "message": "Risk review blocked by active kill switch.",
  "data": {
    "decision": "block",
    "risk_decision_id": "risk_decision_block",
    "primary_failure_limit": "kill_switch",
    "kill_switch_state": "active"
  },
  "error": null,
  "metadata": {
    "tool_name": "check_risk_kill_switch",
    "request_id": "req-risk-block",
    "places_trade": false
  }
}
```
...
{
  "status": "success",
  "message": "Risk review needs more evidence.",
  "data": {
    "decision": "needs_more_evidence",
    "missing_evidence": ["broker_symbol_metadata", "fx_conversion_rate"],
    "retryable": true
  },
  "error": null,
  "metadata": {
    "tool_name": "review_trade_risk",
    "request_id": "req-risk-evidence",
    "places_trade": false
  }
}
```
...
{
  "status": "success",
  "message": "Risk review requires authorized approval.",
  "data": {
...
{
  "status": "error",
  "message": "Risk configuration is invalid.",
  "data": null,
  "error": {
    "code": "INVALID_RISK_CONFIG",
    "details": {
      "field": "kelly_fraction_multiplier",
      "reason": "required for production profile"
    }
  },
  "metadata": {
    "tool_name": "calculate_position_size",
    "request_id": "req-risk-error",
    "places_trade": false
  }
}
```
...
Status: BLOCKED: NOT READY FOR HANDOFF.

The requirements are a strong production baseline, but the Risk module is not yet safe for implementation handoff until export classifications, official tool contracts, requirement IDs, portfolio-under-risk ownership boundaries, and the pending production decisions in this file are resolved or explicitly deferred by the owner.
...
| Version | Date | Author | Pending items resolved | Notes |
|---|---|---|---|---|
| v8.0-draft | Pending | Owner/Architect | Pending | Source production requirements baseline. |
| v8.1-draft | 2026-06-08 | Codex documentation refinement | None | Marked Builder handoff blocked and integrated third architectural audit requirements. |

#### Notes / Future Improvements

**Portfolio-under-risk follow-up notes**
...
**Confirmed Decisions and Production Requirements**

The items in this section are confirmed production requirements unless explicitly labeled `Pending:` or `Recommendation:`.

**Canonical requirement structure**
...
**Canonical lifecycle states and glossary**
...
**Production configuration and runtime decisions**
...
**Production hardening and default profile policy**
...
**Storage, governance, and benchmark ownership**
...
**Approval-token compatibility policy**
...
**Explicitly prohibited role behavior**
