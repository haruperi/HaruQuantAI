...
- Official order execution, tick-accurate matching, fills, deals, account state, position state, margin state, equity state, simulation journals, reporting, and production-realism classification.
- Final official order volume, margin acceptance, execution price, fill status, risk approval, portfolio allocation, or live activation approval.
- Indicator calculation internals, data normalization, source readiness rules, broker account mutation, terminal setting mutation, or direct broker operations.
- Arbitrary user-provided Python execution, unless a future approved sandbox policy, vetting artifact, and approval workflow explicitly permits it.
- Regulatory reporting systems, venue execution adapters, official risk engines, official portfolio engines, or disaster-recovery orchestration outside strategy-local requirements.
- Portfolio, risk, compliance, reporting, disaster recovery, venue execution, broker mutation, monitoring enforcement, production deployment enforcement, and live operations enforcement remain owned by their designated external modules.
- Strategy requirements that reference external domains shall be interpreted as strategy declarations, metadata, assumptions, diagnostics, or intent annotations unless an active architecture document explicitly assigns enforcement to the strategy module.

## 3. API

### 3.1 Public Capabilities

Public capabilities are contract placeholders until their exact Python names and schemas are approved. Builders shall not invent API shapes from descriptive bullets.

| Capability | Stability | Intended consumers | Inputs | Outputs | Error codes | Side effects |
|---|---|---|---|---|---|---|
| `validate_strategy_ref` | Draft public orchestration contract | Orchestration, simulation, research, API | Strategy id, version or version constraint, environment, request metadata | Resolved immutable registry entry, lifecycle status, interface version, configuration policy, required data and indicator manifest | `STRATEGY_NOT_FOUND`, `STRATEGY_VERSION_CONSTRAINT_UNSATISFIABLE`, `STRATEGY_DEPRECATED`, `STRATEGY_LIFECYCLE_NOT_APPROVED`, `STRATEGY_UNAPPROVED_MODULE`, `STRATEGY_INVALID_CONFIG` | None |
| `validate_strategy_config` | Draft public orchestration contract | Orchestration, simulation, research, API | Resolved strategy reference, configuration payload, schema version | Validated configuration, normalized defaults, configuration hash, invalid-field diagnostics | `STRATEGY_SCHEMA_VALIDATION_FAILED`, `STRATEGY_INVALID_CONFIG` | None |
| `run_vectorized_strategy_signals` | Draft strategy execution contract | Simulation, research | Validated strategy reference, normalized market data, indicator outputs, timing policy, seeded execution context | Ordered batch of timestamped signals and `TradeIntent` objects, diagnostics, replay metadata | `STRATEGY_DATA_NOT_READY`, `STRATEGY_INDICATOR_NOT_READY`, `INDICATOR_MODULE_ERROR`, `STRATEGY_LOOKAHEAD_DETECTED`, `STRATEGY_TIMEOUT` | Strategy-local state only when declared |
| `run_event_strategy_hook` | Draft strategy execution contract | Simulation, orchestration | Hook name, typed event payload, read-only state snapshot, seeded execution context | Ordered hook result containing decisions, diagnostics, state updates, `TradeIntent` objects, checkpoint request where supported | `STRATEGY_UNSUPPORTED_TIMING_POLICY`, `STRATEGY_DATA_NOT_READY`, `INDICATOR_MODULE_ERROR`, `STRATEGY_DUPLICATE_INTENT`, `STRATEGY_TIMEOUT` | Strategy-local state only when declared |
| `build_trade_intent` | Draft public support contract | Strategy runtime, simulation | Strategy decision metadata, symbol, side, intent type, sizing hint, timing metadata, optional rationale and execution preferences | Schema-valid `TradeIntent`, deterministic `intent_id`, `decision_id`, idempotency key, lineage metadata | `STRATEGY_INVALID_CONFIG`, `STRATEGY_DUPLICATE_INTENT`, `STRATEGY_SCHEMA_VALIDATION_FAILED` | None |
| `create_strategy_replay_manifest` | Draft public support contract | Simulation, research, audit/reporting | Strategy id/version, config hash, data checksum, indicator manifest, simulation config hash, seed, interface version | Replay manifest suitable for deterministic replay and audit references | `STRATEGY_ARTIFACT_HASH_MISMATCH`, `STRATEGY_DEPENDENCY_HASH_MISMATCH`, `STRATEGY_INTERNAL_ERROR` | None |
| `export_strategy_diagnostics` | Draft public support contract | Orchestration, simulation, risk, portfolio, audit/reporting | Strategy execution context, bounded diagnostics, redaction policy | Redacted structured diagnostics, metrics payload, safe rationale references | `STRATEGY_INTERNAL_ERROR`, `STRATEGY_RESOURCE_LIMIT_EXCEEDED` | None |
...
These schema contracts are the v1.0 Builder-handoff baseline. Field names, requiredness, and error-envelope shape are normative for implementation. Numeric precision rules, enum expansions, and environment-specific limits may be tuned only through a documented schema-version change.
...
#### 1.4 Signal Timing and No-Lookahead
...
#### 1.5 Strategy Registry and Configuration
...
#### 1.6 Strategy Execution Security
...
### Business Rules Integrated into Functional Scope
...
### 9. Strategy Lifecycle and Governance
...
### 11. TradeIntent Identity, Idempotency, and Lineage
...
### 12. Data, Indicator, and Feature Readiness
...
### 13. Strategy Observability and Diagnostics
...
### 14. Strategy Validation and Research Controls
...
### 15. Performance and Resource Controls
...
### 16. Artifact Integrity and Supply-Chain Controls
...
### 17. Failure Isolation and Recovery
...
### 18. Execution Environment Gating
...
### 19. API Compatibility and Deprecation
...
### 20. Deterministic Calculation Semantics
...
### 22. Market Microstructure and Execution Constraints
...
### 23. Multi-Strategy Portfolio Coherence
...
### 24. Production Operations and Monitoring
...
### 25. Regulatory Compliance Controls
...
### 26. Data Quality and Safety Gates
...
### 27. Performance Service Level Objectives
...
### 28. Strategy Calibration and Stability
...
### 30. Model Governance and Continuous Improvement
...
### 31. Legal and Compliance Documentation
...
### 32. Feature Store and ML Model Governance
...
### 33. Secrets Management and Credential Isolation
...
### 34. Order Book and Microstructure Data Constraints
...
### 36. Strategy Interface and Processing Anatomy
...
### Example 1 - Registered vectorized strategy backtest flow
...
### Example 2 - Event strategy lifecycle
...
### Example 3 - TradeIntent shape

```json
{
  "schema_version": "strategy.trade_intent.v1",
  "intent_id": "strategy-ema-2026-01-01T00:00:00Z-000001",
  "decision_id": "decision-000001",
  "idempotency_key": "idem-strategy-ema-000001",
  "strategy_id": "ema_trend_v1",
  "strategy_version": "1.0.0",
  "symbol": "EURUSD",
  "side": "BUY",
  ...
  "allow_partial_fills": true,
  "min_fill_size": "0.01",
  "rationale_ref": "rationale-ema-000001",
  "lineage": {
    "signal_source": "ema_cross_previous_closed_bar"
  }
}
```

### Example 4 - Rejected unsafe strategy input

```json
{
  "schema_version": "strategy.diagnostics.v1",
  "strategy_id": "unknown",
  ...
  "status": "error",
  "error_code": "SIM_ARBITRARY_CODE_REJECTED",
  "details": {
    "reason": "Raw Python strategy code is not accepted by run_backtest in Phase 1."
  },
...
### Example 5 - Invalid configuration response
...
```json
{
  ...
  "decision_timestamp": null,
  "status": "error",
  "error_code": "STRATEGY_SCHEMA_VALIDATION_FAILED",
  "details": {
    "invalid_fields": ["config.fast_ema"],
    "reason": "fast_ema must be a positive integer."
  },
...
### Example 6 - Lookahead rejection response
...
```json
{
  ...
  "decision_timestamp": "2026-01-01T00:00:00Z",
  "status": "error",
  "error_code": "STRATEGY_LOOKAHEAD_DETECTED",
  "details": {
    "reason": "Current incomplete bar data is unavailable under BAR_OPEN_PREVIOUS_CLOSE."
  },
  "redaction_status": "not_required",
  "dependency_health": {
    "data": "healthy",
...
### Example 7 - Duplicate intent response
...
```json
{
  ...
  "decision_timestamp": "2026-01-01T00:00:00Z",
  "status": "error",
  "error_code": "STRATEGY_DUPLICATE_INTENT",
  "details": {
    "reason": "The generated intent_id already exists for this strategy decision."
  },
  "redaction_status": "not_required",
  "dependency_health": {}
}
...
### Example 8 - Checkpoint restore failure response
...
```json
{
  ...
  "decision_timestamp": null,
  "status": "error",
  "error_code": "STRATEGY_CHECKPOINT_INVALID",
  "details": {
    "reason": "Checkpoint checksum does not match the replay manifest."
  },
  "redaction_status": "not_required",
  "dependency_health": {}
}
...
### Example 9 - Orchestration error handling flow
...
This document is a strong strategy-domain baseline, but it is not Builder-ready for implementation until the traceability, public capability contracts, applicability tags, acceptance criteria, and test mappings below are completed. Until those readiness items are complete, this file is approved for planning and documentation refinement only, not direct implementation handoff.

Prerequisites for any Strategy implementation are explicit public API signatures, versioned input/output schemas, error decision tables, traceability matrix, sandbox policy for any code-based execution path, lifecycle promotion gates, and measurable non-functional targets approved by the owner or architecture process.
...
| Area | Strategy module owns | Strategy module does not own |
|---|---|---|
| Decisions | Strategy decision logic, signal generation rules, trade-intent creation, strategy-local decision state, checkpoints, and replay metadata. | Official risk approval, official sizing, official order creation, broker submission, fills, deals, account state, position state, margin, equity, journals, or reports. |
| Registry | Strategy registry entries, identifiers, versions, configuration schemas, manifests, lifecycle status metadata, artifact hashes, dependency hashes, build provenance references, and validation artifact references. | External governance approval systems, CI/CD enforcement, deployment rollout, or compliance record systems. |
| Inputs | Strategy-local declarations for data requirements, indicator requirements, timing policy, environment permissions, resource declarations, execution assumptions, optional risk assumptions, and read-only querying of execution state such as fills and open positions through approved snapshot interfaces. | Data normalization, indicator calculation internals, source readiness enforcement, broker adapter behavior, or portfolio/risk enforcement. |
| Diagnostics | Strategy diagnostics, rationales, observability metadata, health declarations, drift/performance metadata, runbook metadata, and safe governance metadata. | Official audit reports, regulatory filings, production alert routing, incident command, disaster-recovery orchestration, or live operations enforcement. |

The strategy module may annotate decisions with metadata about external domains, but it never enforces rules, mutates state, or performs actions belonging to those domains.
...
This matrix is the v1.0 planning baseline. It maps every stable Strategy requirement ID to an acceptance-evidence family and at least one required test-id family. Before implementation acceptance, each family-level test id shall be expanded into concrete test cases with exact file paths and runnable test names.

| Requirement IDs | Area | Applicability | Acceptance evidence | Required test IDs |
|---|---|---|---|---|
| `REQ-STRAT-001` - `REQ-STRAT-010` | Public capabilities and schema contracts | `BACKTEST_CORE`, `REPLAY`, `PAPER`, `SHADOW`, `LIVE` | Exact signatures, schema-versioned models, deterministic error envelopes, examples validating against schema | `STRAT-CONTRACT-001`, `STRAT-SCHEMA-001`, `STRAT-ERROR-001` |
| `REQ-STRAT-011` - `REQ-STRAT-016` | Requirement metadata and traceability | Documentation only until implementation scope is approved | Stable IDs, applicability tags, acceptance criteria, linked test ids, reviewed matrix | `STRAT-TRACE-001`, `STRAT-DOC-001` |
| `REQ-STRAT-017` - `REQ-STRAT-022` | Architectural invariants | `BACKTEST_CORE`, `REPLAY`, `PAPER`, `SHADOW`, `LIVE` | Boundary review proving Strategy emits intents/diagnostics and does not mutate official external state | `STRAT-BOUNDARY-001`, `STRAT-CONTRACT-002` |
| `REQ-STRAT-023` - `REQ-STRAT-096` | Functional strategy behavior | `BACKTEST_CORE`, `REPLAY`, `PAPER`, `SHADOW`, `LIVE`; limited `FUTURE` where sandboxed code is mentioned | Registry, lifecycle, timing, read-only state, replay, security, and state-transition behavior pass contract tests | `STRAT-FUNC-001`, `STRAT-REGISTRY-001`, `STRAT-NOLOOK-001`, `STRAT-STATE-001`, `STRAT-SECURITY-001`, `STRAT-REPLAY-001` |
| `REQ-STRAT-097` - `REQ-STRAT-119` | Business rules, inputs, and outputs | `BACKTEST_CORE`, `REPLAY`, `PAPER`, `SHADOW`, `LIVE` | Inputs and outputs validate against public schema contracts and business-rule suppression behavior | `STRAT-IO-001`, `STRAT-SCHEMA-002`, `STRAT-SUPPRESS-001` |
| `REQ-STRAT-120` - `REQ-STRAT-302` | Governance, risk declarations, data readiness, observability, validation, operations, compliance, ML, microstructure, document governance, and interface anatomy | `BACKTEST_CORE`, `REPLAY`, `PAPER`, `SHADOW`, `LIVE`, `ML_ONLY`, `L2_L3_ONLY`, `REGULATED_MARKET_ONLY`, `FUTURE` as applicable | Lifecycle gates, declarations, metadata, diagnostics, and external ownership boundaries are documented, validated, and blocked where pending owner approval is required | `STRAT-GOV-001`, `STRAT-RISKDECL-001`, `STRAT-DATAREADY-001`, `STRAT-OBS-001`, `STRAT-VALIDATION-001`, `STRAT-COMPAT-001`, `STRAT-ML-001`, `STRAT-MICRO-001`, `STRAT-DOCGOV-001` |
| `REQ-STRAT-303` - `REQ-STRAT-323` | Non-functional requirements | `BACKTEST_CORE`, `REPLAY`, `PAPER`, `SHADOW`, `LIVE` | Provisional latency, memory, checkpoint, diagnostic, config, timeout, determinism, and dependency-behavior targets measured on approved benchmark profile | `STRAT-NFR-001`, `STRAT-PERF-001`, `STRAT-RESOURCE-001`, `STRAT-DETERMINISM-001`, `STRAT-DEPENDENCY-001` |
| `REQ-STRAT-324` - `REQ-STRAT-350` | Edge cases and failure handling | `BACKTEST_CORE`, `REPLAY`, `PAPER`, `SHADOW`, `LIVE` | Focused failure-path coverage for stale/partial data, dependency failures, timezone/DST, clock drift, duplicates, restore failures, redaction, and concurrency | `STRAT-EDGE-001`, `STRAT-DATA-QUALITY-001`, `STRAT-CLOCK-001`, `STRAT-CONCURRENCY-001`, `STRAT-REDACTION-001` |
| `REQ-STRAT-351` - `REQ-STRAT-386` | Confirmed error codes | `BACKTEST_CORE`, `REPLAY`, `PAPER`, `SHADOW`, `LIVE` | Every error code has a triggering scenario and full `StrategyDiagnostics` schema assertion | `STRAT-ERROR-002`, `STRAT-DIAG-001` |
| `REQ-STRAT-387` - `REQ-STRAT-426` | Test requirements | Documentation only until concrete implementation scope is approved | Test plan review confirms each required test family exists or carries a non-implementation rationale | `STRAT-TESTPLAN-001`, `STRAT-TRACE-002` |
| `REQ-STRAT-427` - `REQ-STRAT-443` | Documentation requirements and assumptions | Documentation only until implementation scope is approved | Documentation review confirms public contracts, examples, replay metadata, assumptions, and pending owner decisions are separated | `STRAT-DOC-002`, `STRAT-ASSUMPTION-001` |
...
| Item | Owner | Target version | Blocks |
|---|---|---|---|
| Sandbox policy, approval workflow, vetting artifacts, and prohibited-operation lists. | Owner/Architect/Security | Pending | Any code-based strategy execution. |
| Stable requirement IDs, acceptance criteria, applicability tags, and test traceability matrix. | Owner/Architect | Baseline established; review required for v1.0 | Builder handoff for Strategy implementation. |
| Exact public API signatures and error decision tables. | Architect | Pending | Builder handoff and external-module integration. |
| Final quantitative SLOs and reference performance environment. | Owner/Architect | Pending; provisional v1.0 baselines documented | Performance tests and production-readiness claims. |
