...
> **Scope:** The implemented HaruQuantAI composability foundation and the permanent delivery standard for every new, migrated, or changed feature in `app/`, `tests/`, `scripts/`, `pyproject.toml`, and `.importlinter`
...
flowchart LR
    ...
    Interface --> Usage[8. Required _usage.py executable scenarios]
...
- comprehensive core-module API documentation and one required executable `_usage.py` per service feature;
- executable conformance, documentation-drift, lifecycle, replacement, and physical-removal checks.

`D-UI` follows the same contract-first ownership, feature identity, dependency, removal, and evidence principles through the TypeScript/React variant in §4.10. It does not use Python entry points or pretend that rendered production code is test evidence.
...
When an approved normalized legacy bundle is available, use it as a pinned behavioral
donor before Phase 1. The tracked provenance and disposition format is defined in
[`LEGACY_MIGRATION.md`](LEGACY_MIGRATION.md). Donor material is optional evidence,
never implementation authority. Donor maturity changes how much behavior must be
rediscovered; it does not change the target architecture or remove any phase,
artifact, test, usage, removal, or quality gate in this guide.

Before implementation that consumes donor evidence, the Planner and Executor must
have verified:

1. the donor repository, immutable commit, source/test tree identities, staged file
   inventory, exclusions, and drift check;
2. the relevant donor implementation, tests, fixtures, usage evidence, and owning
   legacy specification are available without caches, secrets, build products, or
   mutable runtime data;
3. every distinct donor product behavior maps to exactly one current V3 foundation
   or `FEAT-*`/`FR-*` owner, with cross-owner legacy units split into independent
   migration units;
4. every behavior has one closed ledger outcome: `COVERED`, `ADAPT`, `MERGE`,
   `REPLACED_WITH_PARITY`, or `ADD_TO_V3`; `RETIRE_MECHANISM_ONLY` may remove only
   obsolete machinery whose product behavior has another named parity target;
5. every `ADD_TO_V3` behavior has an accepted owning `SPEC-GAP-*` specification
   Task before implementation; no Task starts with unmatched behavior or a pending
   gap;
6. preserved behavior, intentionally changed behavior, superseded behavior, and
   donor tests/fixtures to port or supersede are identified before code changes;
7. the Task declares exactly one normalized feature/slice bundle, its pinned source
   manifest, any shared consumers, restore source, and deletion gate;
8. the migration Task is one atomic current-owner slice in the ratified dependency
   order, not a bulk copy of a legacy directory or domain.

The `.migration/` tree is a gitignored donor boundary. Raw roots remain read-only
until a separately approved intake Task normalizes them. A feature Executor may read
only its declared bundle and may delete that exact approved bundle after all
implementation/evidence gates pass and before review. Application, test, script,
package, build, and runtime code never import `.migration` or add it to `PYTHONPATH`.
A copied donor module is not implementation evidence until it lives in its final V3
owner, uses current contracts and dependencies, and passes this entire pipeline.

If the declared normalized bundle is absent, absence alone is not a blocker and raw
staging is not a substitute. Implement the complete ratified current scope from the
owning V3 README and public contracts from scratch, run every otherwise-applicable
pipeline gate, and record `DONOR_UNAVAILABLE` as an evidence-availability limitation
in the legacy reconciliation. Do not claim preserved, superseded, or equivalent
donor behavior without inspected evidence. Missing authoritative V3 requirements,
contracts, authorization, safety facts, or acceptance evidence for a claim actually
made remain blockers.

### 2.2 Adaptation rules through the existing phases

- **Phase 1 — ownership and contracts:** current V3 owners and contracts win. A
  legacy API never justifies a duplicate contract, facade, shared package, or
  implementation import. Breaking changes follow the normal major-version and
  compatibility-window rules.
- **Phase 2 — feature package:** place selected behavior directly in the final
  feature package and reshape it into focused responsibilities. Do not preserve a
  legacy manager/registry/service hierarchy merely because its tests are mature.
- **Phase 3 — effects:** reclassify every donor side effect. Acquire reversible
  resources through `FeatureContext`/`FeatureScope`; define idempotency,
  reconciliation, audit, retention, and recovery for durable or irreversible work.
- **Phase 4 — registration and configuration:** replace legacy globals, service
  locators, import-time registration, environment/file precedence, and provider
  routing with current entry points, strict feature configuration, and capability
  selection.
- **Phase 5 — documentation:** the feature README documents the current behavior and
  removal contract. Donor provenance/disposition remains in the migration record;
  donor status never becomes current feature status.
- **Phase 6 — tests:** port applicable donor cases into the final V3 test namespace,
  record intentional expectation changes, and add all current contract,
  configuration, lifecycle, failure, replacement, readiness, and removal evidence.
  Tests execute V3 paths and never execute the donor tree.
- **Phase 7 — verification:** verify that no application, test, package, build, or
  runtime path depends on `.migration`; reconcile every in-scope donor behavior;
  prove ported/superseded evidence; confirm no unresolved gap or remaining shared
  consumer; delete the exact approved nonshared bundle; and record deterministic
  restore provenance before review.

Within one approved feature scope, implement or adapt valid donor behavior first,
then add V3-only requirement gaps. Split them into separate atomic Tasks when their
owners, risks, validation, or rollback boundaries differ.
...
├── _persistence.py      # Optional feature-local database operations
└── _usage.py            # Required executable usage scenarios
```

Use-case filenames are responsibility-driven; a feature may have multiple focused implementation
modules. They contain production behavior, never executable demonstrations. `_persistence.py` is
present only for features that perform database operations. `_usage.py` is always present and is the
sole owner of executable examples. Tests and usage scenarios remain separate artifacts.
...
CLI usage:
    uv run python -m app.services.example.custom_service._usage
"""
```
...
Add `_persistence.py` only when the feature defines database operations. It is the single
feature-local owner of their requests, transactions, and storage mapping and routes them through
public Workspace persistence contracts. It never receives a raw connection or unrestricted SQL.
Keep business policy, authorization, orchestration, and public capability definitions in their
proper owners. Declare and test the namespace, schema, migration, retention, reconciliation, and
purge rules. Stateless features omit this file.
...
### 4.10 D-UI TypeScript/React workstation variant
...
The `tests/` tree contains automated verification, not usage demonstrations. Feature tests may verify
unit behavior, lifecycle, configuration, contracts, architecture, integration boundaries, and
failure handling, but a pytest file is never the feature's public usage example. The executable usage
owner is the required `_usage.py` module defined in §4.8.
...
### Legacy adaptation when a donor exists

- [ ] Donor repository, commit, source/test tree identities, file inventory,
  exclusions, and drift verification are recorded.
- [ ] The staged donor scope contains the relevant clean implementation, tests,
  fixtures, usage evidence, and legacy specification, with generated and sensitive
  artifacts excluded.
- [ ] Every in-scope donor behavior has one current V3 owner and exactly one of
  `COVERED`, `ADAPT`, `MERGE`, `REPLACED_WITH_PARITY`, or `ADD_TO_V3`;
  `RETIRE_MECHANISM_ONLY` is limited to machinery with complete named parity.
- [ ] Every `ADD_TO_V3` behavior has an accepted owning specification and final
  `FEAT-*`/`FR-*` relationship before implementation.
- [ ] The Task consumes exactly one normalized bundle with pinned source/test/
  fixture/usage paths, declared shared consumers, deletion gate, and restore source.
- [ ] Preserved, intentionally changed, superseded, and V3-only behavior is explicit.
- [ ] Applicable donor tests/fixtures are ported into current V3 test namespaces or
  documented as superseded by stronger current evidence.
- [ ] No application, test, package, build, configuration, or runtime path imports or
  executes `.migration`.
- [ ] Temporary compatibility adapters have an owner, bounded compatibility window,
  consumer migration, removal condition, and deletion evidence.
- [ ] Donor evidence is not cited as current feature/FR implementation evidence.
- [ ] The exact nonshared bundle is physically absent before review, or a shared
  bundle remains only with named unaccepted consumers; the reconciliation record
  retains deletion and deterministic restore evidence.

### From-scratch delivery when donor material is unavailable

- [ ] The owning V3 README and public contracts define a complete ratified scope;
  missing authoritative current requirements remain a blocker.
- [ ] No raw `.migration` staging is read as a substitute, imported, executed, or
  cited as implementation evidence.
- [ ] The complete current scope passes every applicable implementation, usage,
  lifecycle, failure, removal, and quality gate in this guide.
- [ ] Legacy reconciliation records `DONOR_UNAVAILABLE`, identifies the missing
  bundle, cites independent V3 evidence, and makes no donor-parity, preservation,
  supersession, deletion, or restore claim.
...
- [ ] Every service feature has one `_usage.py` owning a bounded `if __name__ == "__main__":` harness, and that harness passes.
...
2. When an approved normalized donor bundle is available, freeze and verify its
   provenance, staged-tree parity, exclusions, and manifest for source, tests,
   fixtures, usage, and specification evidence. When it is absent, select the
   from-scratch path, prohibit raw-staging substitution and donor-parity claims, and
   plan truthful `DONOR_UNAVAILABLE` reconciliation.
3. For available donor evidence, inventory behavior/effects and record one current
   owner plus one closed parity outcome for every behavior; mechanism retirement
   never substitutes for product parity.
4. Resolve every `ADD_TO_V3` through an accepted owning specification Task before
   build work; split cross-owner or independently reviewable units into separate
   Tasks.
5. For available donor evidence, identify preserved, intentionally changed,
   superseded, and V3-only behavior and the donor cases that will be ported or
   superseded.
6. Identify the owning domain, stable feature ID, public capability, consumers,
   state, effects, readiness impact, and removal result.
7. Update public contracts and typed events first.
8. Update the feature README and `FeatureSpec` together.
9. Update strict configuration parsing and `config_keys` together.
10. Implement or adapt the smallest focused use-case modules in the final V3 owner.
11. Add or update comprehensive module-header documentation and the required
...
17. Run focused tests while iterating.
18. Verify no application/test/build/runtime dependency reaches `.migration`.
19. For available donor evidence, reconcile all outcomes, prove ported/superseded
    evidence and zero unresolved behaviors/gaps/shared consumers, then delete the
    exact approved nonshared bundle. For unavailable donor evidence, record the
    limitation and independent V3 evidence without claiming parity or deletion.
20. Run targeted feature and applicable donor-bundle absence/removal verification.
21. Run the complete repository gate.
22. Mark documentation complete only after usage, executable verification, and the
    applicable available-donor or `DONOR_UNAVAILABLE` reconciliation gates pass.

For D-UI, apply the §4.10 substitutions, register the typed UI contribution, and use §9.5 evidence; every ownership, contract, dependency, removal, and completion rule remains in force.
