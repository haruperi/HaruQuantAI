...
- [X] **TRD-FR-002:** The package shall export the platform-independent action surface, info wrappers, and tool registry through one explicit public gate using `__all__`. *Evidence: app/services/trading/__init__.py line 1-119*
...
- [X] **TRD-FR-019:** Maintain a registry of callable trading tools. Each tool registration must declare a name, purpose, route support, input schema, output schema, approval requirements, side effect ceiling, risk level, error codes, and audit metadata. *Evidence: app/services/trading/tool_registry.py line 19-50, 53-65, 68-88*
- [X] **TRD-FR-020:** AI-facing tools in the registry shall never directly invoke governed broker mutations. They may only produce unexecuted action drafts, run in `packaged_only` mode, or execute read-only queries unless explicitly authorized by backend policy. *Evidence: app/services/trading/tool_registry.py line 26-50*
...
- [X] **TRD-FR-021:** `buy(...)` and `sell(...)` shall formulate market order intents with exact parameter parity, supporting volume, stop loss (SL), take profit (TP), deviation, magic number, and comment. *Evidence: app/services/trading/actions/orders.py line 115-179; app/services/trading/actions/orders.py line 181-245*
- [X] **TRD-FR-022:** `buy_limit(...)`, `sell_limit(...)`, `buy_stop(...)`, and `sell_stop(...)` shall formulate pending order intents, including price, expiration, and stop-limit requirements. *Evidence: app/services/trading/actions/orders.py line 247-324; app/services/trading/actions/orders.py line 326-528*
- [X] **TRD-FR-023:** `order_modify(ticket, price, sl, tp)` and `order_delete(ticket)` shall implement pending order mutation and cancellation while preserving order identity, idempotency keys, and side effect classification. *Evidence: app/services/trading/actions/orders.py line 530-599; app/services/trading/actions/orders.py line 601-656*
- [X] **TRD-FR-024:** Support submission of OCO groups and bracket orders (e.g., entry order linked with stop-loss and take-profit pending orders) through actions, validating group parameter consistency before dispatch. *Evidence: app/services/trading/actions/orders.py line 658-744*
- [X] **TRD-FR-025:** Order actions shall route through the canonical 16-step execution path:
  ...
  Step 1 is implemented directly; steps 2-16 are evaluated by the injected `LiveGatePipeline` seam for `route="live"`, wired by the future `gates/pipeline.py` unit. Until injected, every action fails closed to `packaged_only` (TRD-FR-055). *Evidence: app/services/trading/actions/orders.py line 1-19; app/services/trading/actions/orders.py line 53-113; app/services/trading/actions/_common.py line 38-52, 168-196*

#### 📄 File: `actions/positions.py`

- [X] **TRD-FR-026:** `position_open(...)`, `position_close(...)`, and `position_modify(...)` shall implement position lifecycle controls, supporting Close-By-Symbol, Close-By-Ticket, netting vs. hedging constraints, and SL/TP mutation. *Evidence: app/services/trading/actions/positions.py line 60-121; app/services/trading/actions/positions.py line 123-200; app/services/trading/actions/positions.py line 202-263*
- [X] **TRD-FR-027:** `reduce_exposure(scope, target, route, deps)` shall package partial close and volume reduction commands across an approved position, symbol, or account scope based on a validated risk decision. It must not generate execution thresholds independently. *Evidence: app/services/trading/actions/positions.py line 265-340*

#### 📄 File: `actions/controls.py`

- [X] **TRD-FR-028:** `pause_strategy(...)` and `resume_strategy(...)` shall represent non-broker mutating operational controls that adjust local state and monitoring projections only. *Evidence: app/services/trading/actions/controls.py line 70-114; app/services/trading/actions/controls.py line 116-157; app/services/trading/actions/controls.py line 159-212*
- [X] **TRD-FR-029:** `sync_positions(route)` shall retrieve current broker state and synchronize local databases without executing new orders or mutations. *Evidence: app/services/trading/actions/controls.py line 214-277*
- [X] **TRD-FR-030:** `shutdown(timeout)` shall stop admitting new requests, wait for in-flight requests to complete, flush state and audit logs, and trigger final reconciliation. *Evidence: app/services/trading/actions/controls.py line 53-68, 313-358*
- [X] **TRD-FR-031:** `trigger_global_kill_switch(reason, route, deps)` shall activate the global kill switch, persist the operator/system reason, emit a critical operational event, and block all non-emergency live mutations. *Evidence: app/services/trading/actions/controls.py line 360-406*
- [X] **TRD-FR-032:** `trigger_strategy_kill_switch(strategy_id, reason, route, deps)` shall activate a strategy-scoped kill switch and block all non-emergency mutations for that strategy. *Evidence: app/services/trading/actions/controls.py line 408-461*
- [X] **TRD-FR-033:** `trigger_symbol_kill_switch(symbol, reason, route, deps)` shall activate a symbol-scoped kill switch and block all non-emergency mutations for that symbol. *Evidence: app/services/trading/actions/controls.py line 463-516*
- [X] **TRD-FR-034:** Kill-switch trigger actions must be idempotent, audited, journaled, and routed through the policy matrix. *Evidence: app/services/trading/actions/controls.py line 518-615*

#### 📄 File: `actions/emergency.py`

- [X] **TRD-FR-035:** Implement first-class emergency actions: `cancel_all_orders(scope, route, deps)`, `close_all_positions(scope, route, deps)`, `flatten_account(scope, route, deps)`, `flatten_strategy(strategy_id, route, deps)`, and `flatten_symbol(symbol, route, deps)`. *Evidence: app/services/trading/actions/emergency.py line 135-186; app/services/trading/actions/emergency.py line 188-239; app/services/trading/actions/emergency.py line 309-399*
- [X] **TRD-FR-036:** Emergency actions must consume emergency fail-safe rules from the policy matrix, snapshot broker state before and after execution, tolerate partial completion, return per-child order details, and trigger scope reconciliation lock if any child action results in an unknown outcome. *Evidence: app/services/trading/actions/emergency.py line 51-133; app/services/trading/actions/emergency.py line 241-307*

#### 📄 File: `actions/validation.py`

- [X] **TRD-FR-037:** Normalizer functions shall enforce that all broker-critical price, volume, size, balance, and margin math utilizes `decimal.Decimal` normalization to dynamic instrument precision (never hardcoding symbol digits or contract steps). *Evidence: app/services/trading/actions/validation.py line 433-498; app/services/trading/actions/validation.py line 500-558*
- [X] **TRD-FR-038:** `validate_volume(...)` shall check min, max, and step volume constraints against dynamically resolved broker symbol metadata. *Evidence: app/services/trading/actions/validation.py line 560-600*
- [X] **TRD-FR-039:** Volume normalization must round down unless explicitly authorized by policy. Stop-loss and take-profit rounding must be direction-aware, and price rounding must align with broker tick sizes. Monotonic audit trails must capture both `original_value` and `normalized_value`. *Evidence: app/services/trading/actions/validation.py line 449-498; app/services/trading/actions/validation.py line 1043-1046*
- [X] **TRD-FR-040:** `validate_stops(...)` shall enforce direction-aware minimum distance and stop-level rules for stop-loss and take-profit geometry. *Evidence: app/services/trading/actions/validation.py line 602-667*
- [X] **TRD-FR-041:** `validate_margin(...)` shall check that the account has sufficient free margin to cover the trade intent's margin requirements before dispatching to the broker. *Evidence: app/services/trading/actions/validation.py line 669-720*
- [X] **TRD-FR-042:** `validate_market_session(...)` shall check instrument trading session availability and fail closed if session evidence is missing or expired in live-sensitive routes. *Evidence: app/services/trading/actions/validation.py line 722-777*
- [X] **TRD-FR-043:** Validate that the Time-In-Force (TIF) constraint specified is valid for the target instrument and broker capability profiles. *Evidence: app/services/trading/actions/validation.py line 779-804*
- [X] **TRD-FR-044:** Enforce execution protections: market orders must have defined maximum acceptable slippage, and pending orders must be validated against price collars. Orders outside dynamic price collars must fail closed. *Evidence: app/services/trading/actions/validation.py line 806-859*
- [X] **TRD-FR-045:** Enforce a hardcoded, immutable "Fat-Finger Cap" that checks the absolute notional value (Volume $\times$ Price) of any order intent against an un-overrideable system ceiling, failing closed independently of external Risk signatures. *Evidence: app/services/trading/actions/validation.py line 861-915*
- [X] **TRD-FR-046:** Combine all parameter validations into `validate_order_request(...)`. Failures must return a structured `VALIDATION_FAILED` or `INVALID_INPUT` error, short-circuiting downstream execution. *Evidence: app/services/trading/actions/validation.py line 1000-1121*
- [X] **TRD-FR-047:** The immutable fat-finger ceiling SHALL be defined in **account currency**. Notional computation for instruments not quoted in account currency requires a conversion-rate evidence input carrying source, timestamp, and freshness age, subject to a configured TTL. Missing, stale, or symbol-mismatched conversion evidence fails the validation closed. The conversion rate used, the computed account-currency notional, and the ceiling MUST all be captured in the validation audit record. *Evidence: app/services/trading/actions/validation.py line 143-197; app/services/trading/actions/validation.py line 500-558; app/services/trading/actions/validation.py line 861-915*
- [X] **TRD-FR-048:** In the pattern of TRD-FR-045, implement a small set of local, un-overrideable rails evaluated without consulting risk-module signatures: (a) maximum mutation attempts per rolling window per (account, symbol); (b) maximum simultaneous open positions per account; (c) cumulative daily account-currency notional ceiling (resetting at a configured session boundary using the injected `Clock`). These rails are deliberately crude last-line protections; they MUST NOT grow risk logic (position sizing, exposure modeling), which remains exclusively in the risk module. Raising any rail is a dual-control action (TRD-FR-092). *Evidence: app/services/trading/actions/validation.py line 293-325; app/services/trading/actions/validation.py line 917-967*
- [X] **TRD-FR-049 (Short Locate Verification):** `validate_short_locate(...)` must consume a `LocateSnapshot` from the data or risk module. If a short sale request is submitted for a symbol without a valid locate or hard-to-borrow (HTB) authorization, the validation gate must fail closed and reject the order. *Evidence: app/services/trading/actions/validation.py line 245-291; app/services/trading/actions/validation.py line 969-998*
...
- [X] **TRD-FR-064:** Coordinate explicit session states: starting, running, paused, stopped, and recovering. Expose six distinct operational modes: `normal`, `read_only`, `close_only`, `reduce_only`, `emergency_flatten`, and `stopped`. *Evidence: app/services/trading/runtime/session_manager.py line 26-33, 76-97*
- [X] **TRD-FR-065:** Enforce that only one active live session is allowed per configured trading scope. *Evidence: app/services/trading/runtime/session_manager.py line 99-115*
- [X] **TRD-FR-066:** Recovery logic must start the session in a paused state if unknown broker outcomes, unreconciled state, or missing audit logs are detected. *Evidence: app/services/trading/runtime/session_manager.py line 180-199*
- [X] **TRD-FR-067:** Implement the Cancel-on-Disconnect (CoD) heartbeat connection failsafe: if broker capability check determines CoD is unsupported, session manager must run a local heartbeat that triggers an emergency cancel-all if the adapter connection drops for $>X$ seconds. *Evidence: app/services/trading/runtime/session_manager.py line 200-241*
- [X] **TRD-FR-068:** If restoration of kill-switch/mode/lock state fails or is ambiguous (missing records, version mismatch, store unavailable), the session SHALL fail closed into `read_only` mode with a critical operational signal; live mutation remains blocked until an operator resolves state. *Evidence: app/services/trading/runtime/session_manager.py line 117-152*
- [X] **TRD-FR-069 (GTD/DAY and Expiry Watchdog):** Validate `GTD` and `DAY` TIF against broker capability. Where the broker lacks native expiration support, the runtime SHALL operate a local expiry watchdog (driven by the injected `Clock`) that cancels expired working orders through the canonical gate pipeline's emergency-safe path; watchdog cancellations are journaled with actor `system:expiry_watchdog`. *Evidence: app/services/trading/runtime/session_manager.py line 276-343*
- [X] **TRD-FR-070 (Synthetic Emulation Flag):** If capability validation determines a required protective order type (stop, SL/TP attachment) or OCO order group is not natively supported, the default remains fail-closed (per TRD-FR-115). A policy-gated `synthetic_emulation` capability MAY be enabled per broker profile, activating client-side stop/OCO monitoring with: explicit operator risk acknowledgment evidence, a dedicated operational signal while any synthetic order is active, and automatic transition to `close_only` if the monitoring loop's heartbeat lapses. *Evidence: app/services/trading/runtime/session_manager.py line 243-275*
- [X] **TRD-FR-071 (Real-Time Halts State):** Maintain a thread-safe `HaltedSymbols` set in `runtime/session_manager.py` indicating active symbol pauses. Standard order validation and `MarketTurbulenceGate` (TRD-FR-085) must check this set to immediately block mutations and optionally cancel working orders for halted symbols. *Evidence: app/services/trading/runtime/session_manager.py line 344-361*
- [X] **TRD-FR-072 (Reconnection Auto-Resync):** Maintain a `BrokerConnectionState` monitor. Upon transition from `Disconnected` to `Connected`, the session manager MUST block all new live mutations and force a full reconciliation (re-synchronizing all open positions, pending orders, balance, and margin) before returning the session back to `normal` mutation mode. *Evidence: app/services/trading/runtime/session_manager.py line 200-241*

#### 📄 File: `runtime/coordination.py`

- [X] **TRD-FR-073:** Enforce optimistic concurrency locks. The default conflict key shall be `(account_id, symbol)`, restricting concurrent mutating actions on the same symbol for one account. *Evidence: app/services/trading/runtime/coordination.py line 33-80*
- [X] **TRD-FR-074:** Concurrency controls must specify lock acquisition timeouts and maximum queue capacities. Request overload must trigger immediate backpressure rejection rather than unbounded queueing. *Evidence: app/services/trading/runtime/coordination.py line 33-80*
- [X] **TRD-FR-075:** Enforce strategy ownership constraints. Every order and position record must map to `owner_strategy_id` and `owner_workflow_id`. A strategy cannot close or modify another strategy's positions unless explicitly authorized by the policy matrix. Netting account symbol conflicts must be checked before close/modify. *Evidence: app/services/trading/runtime/coordination.py line 82-144*
- [X] **TRD-FR-076:** Before dispatch, detect when a mutation from strategy A would cross an opposing working order or position owned by strategy B on the same (account, symbol) — particularly on netting accounts where the broker would net them. The policy matrix determines the response per scope: `block`, `warn_and_allow`, or `allow`. Detections are journaled regardless of outcome. Absence of a configured policy fails closed to `block`. *Evidence: app/services/trading/runtime/coordination.py line 146-240*

#### 📄 File: `runtime/cost_control.py`

- [X] **TRD-FR-077:** Evaluate cost budgets per request, workflow, strategy, account, and session. *Evidence: app/services/trading/runtime/cost_control.py line 41-118*
- [X] **TRD-FR-078:** If cost budget limits are violated before broker dispatch, block execution. If violated after broker send but before reconciliation, raise a critical cost-budget incident. *Evidence: app/services/trading/runtime/cost_control.py line 41-118, 122-202*
- [X] **TRD-FR-079:** Cost-control decisions must not calculate analytics metrics; they only enforce configured runtime budget ceilings. *Evidence: app/services/trading/runtime/cost_control.py line 20-24*

#### 📄 File: `runtime/signal_processor.py`

- [X] **TRD-FR-080:** Transform approved strategy signals into standard `TradingRequestEnvelope` order intents. *Evidence: app/services/trading/runtime/signal_processor.py line 122-192*
- [X] **TRD-FR-081:** The signal processor must verify route, promotion stage, mutation capability, risk decision reference, and approval evidence before producing an executable intent. *Evidence: app/services/trading/runtime/signal_processor.py line 29-120*
- [X] **TRD-FR-082:** The signal processor must never bypass the canonical gate pipeline. *Evidence: app/services/trading/runtime/signal_processor.py line 122-192*
...
- [X] **TRD-FR-083:** Enforce the standardized canonical live route gate pipeline containing the following 16 sequential steps:
  1. Local schema validation (`actions/validation.py`).
  2. Pre-trade compliance / restricted-list verification (`gates/pipeline.py` - ComplianceGate).
  3. Route/promotion stage compatibility check (`promotion/ladder.py`).
  4. Session status check (`runtime/session_manager.py`).
  ...
  The generic short-circuiting/deadline/latency orchestration engine (`run_gate_pipeline`) and steps 2, 5, 6, 7, 8, 9, 10, 14, and 15 are fully implemented and composed in the usage example. Steps 3, 4, 12, and 13 depend on modules that are future dependency-safe units (`promotion/ladder.py`, `runtime/session_manager.py`, `runtime/coordination.py`, `reconciliation/authority_and_retry_guard.py`); until those units inject a real evaluator, `evaluate_seam_gate` fails closed with `LIVE_GATE_FAILED` so the pipeline can never silently pass a step it does not yet implement. Step 16 (actual broker dispatch) is the future responsibility of the full `execution/coordinator.py` async dispatch loop. *Evidence: app/services/trading/gates/pipeline.py line 1-22, 310-412; tests/usage/07_trading.py (`example_09_gates_pipeline`)*
- [X] **TRD-FR-084 (ComplianceGate):** ComplianceGate evaluates early in the pipeline (immediately after local schema validation). It blocks execution and fails closed if the requested symbol is present on the Compliance restricted-symbol list (loaded via configurator or provided by the data module). *Evidence: app/services/trading/gates/pipeline.py line 64-73, 94-117*
- [X] **TRD-FR-085 (MarketTurbulenceGate):** MarketTurbulenceGate evaluates after risk validation but before broker readiness verification. It maintains a bounded circular buffer of the last N mid-prices (derived from quote snapshots) per symbol. If the absolute price change over a configurable rolling window exceeds the volatility velocity threshold, the gate halts new mutations and suspends the symbol, cancelling existing pending orders for that symbol according to policy. *Evidence: app/services/trading/gates/pipeline.py line 120-211*
- [X] **TRD-FR-086:** A gate failure must short-circuit evaluation. Downstream gates that mutate state, invoke network adapter calls, or consume broker capacity must not run. *Evidence: app/services/trading/gates/pipeline.py line 310-412 (short_circuited flag); tests/services/trading/gates/test_pipeline.py line 236-254*
- [X] **TRD-FR-087:** Diagnostic-only gates that execute after a failure must be explicitly marked `diagnostic_after_failure=true`, `mutates_state=false`, `calls_broker=false`, and `requires_network=false`. *Evidence: app/services/trading/gates/_common.py line 147-166*
- [X] **TRD-FR-088:** `TradingRequestEnvelope` SHALL carry a `deadline_utc` (defaulted from configuration when absent). Each of the 16 gates has a configured latency budget and stamps its decision latency into the audit record. If cumulative pipeline time exceeds the deadline, or the mandatory `quote_snapshot` ages beyond its freshness TTL mid-pipeline, the pipeline fails closed at that gate with status `DEADLINE_EXCEEDED` / `QUOTE_STALE`. Per-gate latency distributions (p50/p95/p99) are exported to monitoring. *Evidence: app/services/trading/contracts.py line 462-463, 478 (deadline_utc field); app/services/trading/gates/pipeline.py line 285-412 (compute_effective_deadline, per-gate latency stamping, deadline/quote-staleness fail-closed checks). Per-gate latency percentile export to monitoring is the future responsibility of `monitoring/`.*

#### 📄 File: `gates/policy_matrix.py`

- [X] **TRD-FR-089:** Look up permission, approval, emergency, and side-effect rules from the injected policy matrix. If an action is missing from the matrix, fail closed with `TRADING_POLICY_UNDEFINED`. *Evidence: app/services/trading/gates/policy_matrix.py line 23-80*

#### 📄 File: `gates/approval.py`

- [X] **TRD-FR-090:** Verify operator approval ID, expiration, revoked status, and scope matching (account, strategy, symbol) before executing governed actions. *Evidence: app/services/trading/gates/approval.py line 235-290*
- [X] **TRD-FR-091:** Request Binding Verification: Validate that operator approval and risk decision evidence tokens contain a matching `canonical_request_hash` of the incoming order params (symbol, account, side, volume, price, SL, TP, route, strategy), rejecting the command if they differ. Approval tokens must be single-use or explicitly bounded by scope and expiry. *Evidence: app/services/trading/gates/approval.py line 149-190, 235-290, 362-397*
- [X] **TRD-FR-092:** The following governed actions SHALL require approval evidence from **two distinct authenticated operators** (distinct operator IDs, each independently satisfying TRD-FR-090/091 request binding): clearing a global kill switch, promotion to `full_live`, and raising any hard cap (fat-finger ceiling, defense-in-depth rails per TRD-FR-048). Single-operator approval remains sufficient for all other governed actions. Dual approvals MUST be individually journaled. *Evidence: app/services/trading/gates/approval.py line 28-36, 292-360*

#### 📄 File: `gates/readiness.py`

- [X] **TRD-FR-093:** Check broker connection status, trading allowance, account permissions, and client-side rate limit availability without executing test orders. *Evidence: app/services/trading/gates/readiness.py line 29-44, 77-95*
- [X] **TRD-FR-094:** Implement clock synchronization and drift verification against the broker's server time (e.g. via NTP/PTP or heartbeat drift tracking). If the local system clock offset exceeds $50\text{ms}$, trip a warning operational signal and pause live mutations. *Evidence: app/services/trading/gates/readiness.py line 26-27, 46-63, 98-130*
- [X] **TRD-FR-095 (PTP Latency and Drift Gating):** When a `wire_timestamp` is present in the `quote_snapshot` and PTP alignment is active, the clock drift gate (Gate 10) SHALL compare the PTP-aligned local clock time against the quote's wire timestamp, calculating true end-to-end hardware latency and blocking the trade if latency drift exceeds the configured maximum threshold. *Evidence: app/services/trading/gates/readiness.py line 46-63, 98-130*
- [X] **TRD-FR-096:** Expose a non-mutating `run_live_readiness_dry_run()` check that verifies connection, account identity, symbol metadata, permissions, clock drift, and stores durability without sending mock orders. *Evidence: app/services/trading/gates/readiness.py line 132-168*

#### 📄 File: `gates/kill_switch.py`

- [X] **TRD-FR-097:** Active kill switches (global, strategy, or symbol level) must block all non-emergency and risk-increasing live mutations. Emergency risk-reducing actions may execute only through the policy-matrix emergency path. Emergency protective mutations (cancel-all, close-all/flatten) are permitted under an active kill switch only if the policy matrix allows. *Evidence: app/services/trading/gates/kill_switch.py line 96-139*
- [X] **TRD-FR-098:** `clear_kill_switch_after_approval` shall require explicit, validated governance approval evidence. *Evidence: app/services/trading/gates/kill_switch.py line 142-195*
- [X] **TRD-FR-099:** Flattening and cancellation rules under active kill switches must be determined solely by the policy matrix; no caller-provided flags shall bypass the kill switch. *Evidence: app/services/trading/gates/kill_switch.py line 39-41, 96-139 (evaluate_kill_switches accepts the resolved policy_entry, never a caller-provided bypass flag)*
- [X] **TRD-FR-100:** Kill-switch state (global, strategy, symbol), the active session operational mode (`normal`, `read_only`, `close_only`, `reduce_only`, `emergency_flatten`, `stopped`), and all active reconciliation authority locks MUST be durably persisted via the injected state store and restored **before** the first gate evaluation after any process restart. A crash or restart MUST NOT clear or downgrade any of these controls. *Evidence: app/services/trading/gates/kill_switch.py line 52-61 (OperationalMode), 197-276 (restore_kill_switch_state, persist_kill_switch_state). Actual invocation before the first gate evaluation after a restart is the future responsibility of `runtime/session_manager.py`.*

#### 📄 File: `gates/audit_and_compensation.py`

- [X] **TRD-FR-101:** Pre-mutation logs must be successfully saved to the audit sink before dispatching to the broker. Audit write failure must immediately block the mutation. *Evidence: app/services/trading/gates/audit_and_compensation.py line 24-53*
...
- [X] **TRD-FR-102:** Dispatch validated intents to appropriate execution handlers based on the route (`sim` $\rightarrow$ simulator, `paper` $\rightarrow$ paper store, `shadow` $\rightarrow$ shadow comparison, `live` $\rightarrow$ broker router). *Evidence: app/services/trading/execution/coordinator.py line 51-71 (`_ROUTE_DISPATCH_TARGETS`, `resolve_dispatch_target`)*
- [X] **TRD-FR-103:** Shift the underlying execution model to be natively asynchronous and event-driven. The coordinator shall dispatch requests asynchronously, register an internal Futures-callback, and return control immediately (eliminating thread-exhaustion risks and head-of-line blocking). *Evidence: app/services/trading/execution/coordinator.py line 79-96 (`AsyncDispatchExecutor`), 1081-1148 (`ExecutionCoordinator.dispatch_async`)*
- [X] **TRD-FR-104:** Every mutation intent must generate or accept a globally unique `client_order_id`, which must be propagated to broker-supported metadata fields (comments, magic number, external ID, etc.). Apply deterministic truncation if fields have length limits, preserving a lookup in `TradeStore`. The `client_order_id` must appear in all audits, journals, idempotency logs, and reports. *Evidence: app/services/trading/execution/coordinator.py line 153-180 (`generate_client_order_id`), 183-218 (`truncate_client_order_id`), 220-282 (`ClientOrderIdMapping`, `build_client_order_id_mapping`). Propagation of the generated ID into audits/journals/idempotency logs/reports is the responsibility of the calling seam (`actions/`, `gates/audit_and_compensation.py`, `state/idempotency.py`, `execution/reporting.py`).*
- [X] **TRD-FR-105:** Coordinate multi-account `AllocationVector` targets by either slicing the allocation into separate execution events or transmitting a single block transaction if native allocation is supported by the broker adapter. *Evidence: app/services/trading/execution/coordinator.py line 286-297 (`AllocationDispatchPlan`), 299-336 (`plan_allocation_dispatch`)*
- [X] **TRD-FR-106:** Coordinate two-step SL/TP execution workflows if capability validation determines the broker doesn't support SL/TP attachment on open. Open-then-protect workflows must be atomic from the state perspective. If open succeeds but position modification fails, trigger a critical incident. *Evidence: app/services/trading/execution/coordinator.py line 340-354 (`TwoStepProtectionResult`), 357-368 (`requires_two_step_protection`), 371-407 (`evaluate_two_step_protection_outcome`, `LIVE_PROTECTIVE_MODIFY_FAILED`)*
- [X] **TRD-FR-107:** Support residual handling policies for partially filled orders: `leave_remaining`, `cancel_remaining`, `replace_remaining`, `retry_remaining_after_delay`, and `escalate_to_operator`. The policy must be fetched from the policy matrix and capability profile. Volume updates must occur immediately. *Evidence: app/services/trading/execution/coordinator.py line 410-417 (`ResidualPolicy`), 420-476 (`ResidualHandlingDecision`, `_RESIDUAL_ACTIONS`, `apply_residual_policy`). Fetching the configured policy from the policy matrix/capability profile is the caller's responsibility (`gates/policy_matrix.py`, `execution/broker_capability_validation.py`); immediate volume updates are applied via `execution/state_machine.py`'s `apply_execution_report`.*
- [X] **TRD-FR-108 (Non-Atomic Modify Safety):** When `broker_capability_validation` (TRD-FR-115) reports that order modification is non-atomic (implemented as a separate cancel followed by a replace order request), the execution coordinator must treat the modify as a stateful two-step process: (1) Reserve the order in TradeStore to block concurrent actions; (2) Cancel the working order and await confirmed cancellation; (3) Replace it by dispatching the new order. If the replace step fails (e.g. due to a price gap or reject), the coordinator MUST immediately journal a critical incident, attempt to re-enter the original order with the new parameters, or raise a critical dead-letter recovery escalation to the operator rather than silently losing the working order. *Evidence: app/services/trading/execution/coordinator.py line 479-527 (`NonAtomicModifyStage`, `NonAtomicModifyState`, `begin_non_atomic_modify`), 530-621 (`record_cancel_dispatched`, `record_cancel_confirmed`, `record_replace_dispatched`), 601-679 (`NonAtomicModifyResolution`, `resolve_replace_outcome`, `LIVE_NON_ATOMIC_MODIFY_ESCALATED`)*
- [X] **TRD-FR-109:** The execution coordinator SHALL manage OCO/bracket mutual-cancellation lifecycle: if an execution report indicates a leg in the OCO group is filled or partially filled, trigger cancellation commands for all other sibling orders in the group. *Evidence: app/services/trading/execution/coordinator.py line 734-755 (`evaluate_oco_sibling_cancellation`), 757-825 (`OcoWatchdog`)*
- [X] **TRD-FR-110:** If the broker adapter natively supports OCO groups, delegate group execution to the adapter. Otherwise, if synthetic emulation (`TRD-FR-070`) is active, operate a client-side OCO watchdog that listens for execution reports and dispatches sibling cancellations. If synthetic emulation is inactive and native OCO is unsupported, fail the submission closed. *Evidence: app/services/trading/execution/coordinator.py line 681-687 (`OcoExecutionMode`), 689-712 (`resolve_oco_execution_mode`), 714-732 (`require_oco_submission_allowed`, `OCO_UNSUPPORTED`)*
- [X] **TRD-FR-111 (Multi-Leg / Spread Execution & Atomic Rollback):** Introduce a `MultiLegExecutionCoordinator` to manage simultaneous multi-leg orders (e.g. spreads, straddles). If the broker natively supports multi-leg orders, delegate execution. Otherwise, implement a synthetic multi-leg watchdog: if any leg rejects, fails, or partially fills beyond a configurable tolerance threshold, the watchdog MUST immediately dispatch rollback/cancel commands for all filled or working legs to restore the portfolio to its pre-trade state. *Evidence: app/services/trading/execution/coordinator.py line 827-939 (`MultiLegDecision`, `MultiLegExecutionCoordinator`)*
- [X] **TRD-FR-112:** Capture commission, spread, swap, exchange fees, realized slippage, and other transaction cost facts. If cost updates arrive later via deal history events, append a cost-adjustment event. *Evidence: app/services/trading/execution/coordinator.py line 941-1020 (`TransactionCostFacts`, `CostAdjustmentEvent`, `capture_transaction_cost` with `is_adjustment`)*
- [X] **TRD-FR-113:** Maintain an atomic counter tracking in-flight requests under lock to ensure all requests are completed before shutdown. *Evidence: app/services/trading/execution/coordinator.py line 98-150 (`InFlightRequestCounter`)*
- [X] **TRD-FR-114:** Track state changes post-response using the `TradeStore` protocol, release concurrency leases, and complete the idempotency lease. *Evidence: app/services/trading/execution/coordinator.py line 1022-1079 (`finalize_dispatch_outcome`)*

#### 📄 File: `execution/broker_capability_validation.py`

- [X] **TRD-FR-115:** Validate adapter capability contracts (including order types, filling modes, precision limits, and rate limits) before broker execution. Failure to match capabilities must fail closed. *Evidence: app/services/trading/execution/broker_capability_validation.py line 24-73; app/services/trading/execution/broker_capability_validation.py line 85-236*
- [X] **TRD-FR-116:** Verify if the broker adapter natively supports Cancel-on-Disconnect (CoD). If unsupported, the runtime session must run a local heartbeat connection failsafe that triggers an emergency local sweeping action (mass cancel of all working pending orders) if connection drops for more than $X$ seconds (TRD-FR-067). *Evidence: app/services/trading/execution/broker_capability_validation.py line 238-249*

#### 📄 File: `execution/response_classifier.py`

- [X] **TRD-FR-117:** Convert provider-specific responses (such as MetaTrader5 codes, cTrader errors, or network timeouts) into a standardized `NormalizedTradeResult`. *Evidence: app/services/trading/execution/response_classifier.py line 151-245*
- [X] **TRD-FR-118:** Unknown outcomes (e.g., timeouts, transport disconnects, or malformed success envelopes) must yield a specific unknown outcome status (`10005` equivalent) and trigger immediate state reconciliation, blocking all retry attempts. *Evidence: app/services/trading/execution/response_classifier.py line 273-328*
- [X] **TRD-FR-119:** Dynamic Event Classification: Parse and classify execution events pushed dynamically by the broker router's WebSocket or FIX stream, passing execution details to the `ResponseClassifier`. *Evidence: app/services/trading/execution/response_classifier.py line 247-271*
- [X] **TRD-FR-120:** Classify deals and executions that were not commanded by this runtime into a first-class `BrokerInitiatedExecutionEvent` type, with sub-classification: `server_side_sl_tp_fill`, `stop_out`, `margin_call_action`, `broker_admin_action`, and `expiration`. These events are authoritative state transitions and MUST be applied to `TradeStore` and journaled like any execution report. *Evidence: app/services/trading/execution/response_classifier.py line 46-54; app/services/trading/execution/response_classifier.py line 90-108; app/services/trading/execution/response_classifier.py line 331-377*
- [X] **TRD-FR-121 (Corporate Action Classification):** Classify stock splits, reverse splits, or symbol/name changes pushed by the broker or data module as a `CorporateActionEvent`. *Evidence: app/services/trading/execution/response_classifier.py line 56-71; app/services/trading/execution/response_classifier.py line 110-149; app/services/trading/execution/response_classifier.py line 380-417*
- [X] **TRD-FR-122:** A `stop_out` or `margin_call_action` classification SHALL raise a critical incident, trigger immediate account-scope reconciliation, and auto-transition the session operational mode to `close_only` (or `read_only`, per policy matrix). Resumption to `normal` requires operator approval evidence. *Evidence: app/services/trading/execution/response_classifier.py line 90-108; app/services/trading/execution/response_classifier.py line 355-377 (recommended transition; actually raising the incident and transitioning the session is the future responsibility of monitoring/session_manager)*

#### 📄 File: `execution/rate_limiter.py`

- [X] **TRD-FR-123:** Maintain token-bucket rate limiters per broker provider configured from runtime settings. *Evidence: app/services/trading/execution/rate_limiter.py line 40-114; app/services/trading/execution/rate_limiter.py line 117-185*
- [X] **TRD-FR-124:** Rate exhaustion must trigger an immediate local block before dispatching to the broker, avoiding provider-side penalty limits. *Evidence: app/services/trading/execution/rate_limiter.py line 83-114*

#### 📄 File: `execution/shadow.py`

- [X] **TRD-FR-125:** Shadow mode execution shall record intents and compare expected fills against live market quotes and balances without dispatching mutations to the broker. *Evidence: app/services/trading/execution/shadow.py line 88-122; app/services/trading/execution/shadow.py line 124-172*

#### 📄 File: `execution/state_machine.py`

- [X] **TRD-FR-126:** Enforce valid transition paths across the order/position lifecycle (e.g. reject transitions like Filled $\rightarrow$ Submitted or Cancelled $\rightarrow$ PartiallyFilled). *Evidence: app/services/trading/execution/state_machine.py line 30-82; app/services/trading/execution/state_machine.py line 240-262*
- [X] **TRD-FR-127:** Require every state transition event to carry event source, timestamp, broker event ID, request ID, correlation ID, and previous state version. *Evidence: app/services/trading/execution/state_machine.py line 147-198*
- [X] **TRD-FR-128:** Apply broker-originated execution reports as authoritative state transition events, persisting every state transition through `TradeStore` before public reporting. *Evidence: app/services/trading/execution/state_machine.py line 349-463*
- [X] **TRD-FR-129 (Version-Gated Amendments):** `order_modify` and `order_delete` requests targeting an order in flight SHALL carry an `expected_state_version`. If the authoritative state has advanced (e.g., the order filled or partially filled while the amend was in transit), the amendment resolves to an explicit terminal outcome — `TOO_LATE_TO_CANCEL`, `TOO_LATE_TO_MODIFY`, or `AMENDED_AFTER_PARTIAL_FILL` — rather than a generic rejection, and the envelope's retry-safety field MUST be `do_not_retry`. *Evidence: app/services/trading/execution/state_machine.py line 98-105; app/services/trading/execution/state_machine.py line 293-347*
- [X] **TRD-FR-130:** Modify-while-partially-filled semantics MUST be explicit per action: amendments apply to remaining volume only; SL/TP modifications apply to the resulting position per netting/hedging mode. The state machine SHALL reject any amendment path not enumerated in its transition table. *Evidence: app/services/trading/execution/state_machine.py line 76-82; app/services/trading/execution/state_machine.py line 293-347*
- [X] **TRD-FR-131:** Execution reports SHALL be deduplicated idempotently on `broker_event_id` (with a bounded dedup window sized from configuration). Duplicates increment a metrics counter and are dropped without state transition. *Evidence: app/services/trading/execution/state_machine.py line 349-463; tests/services/trading/execution/test_state_machine.py line 348-374*

#### 📄 File: `execution/reporting.py`

- [X] **TRD-FR-132:** Construct structured trading reports containing positions, order records, execution latencies, cost entries, and reconciliation discrepancies. *Evidence: app/services/trading/execution/reporting.py line 37-105 (`ExecutionLatencyEntry`, `ReconciliationDiscrepancyEntry`), 107-189 (`TradingReport`, `build_trading_report`). Also implements TRD-XM-004 (execution-quality event contract): line 198-240 (`ExecutionQualityEvent`), 245-320 (`compute_realized_slippage_bps`, `compute_implementation_shortfall`), 324-374 (`build_execution_quality_event`, `owner`/`is_external` flag).*
...
- [X] **TRD-XM-001 (Validation Parity by Construction):** The simulator SHALL import and execute `trading/actions/validation.py`, `trading/contracts.py`, and `trading/execution/state_machine.py` directly rather than re-implementing order validation, envelopes, or lifecycle transitions. Trading-side obligation: these modules MUST remain import-safe (TRD-FR-001), broker-free, and driven exclusively by injected `Clock`/`RNG` ports (TRD-FR-137..140) so the simulator can execute them deterministically. This is the structural guarantee that backtest fills obey exactly the volume, stop-distance, session, TIF, and collar rules that live validation enforces. *Evidence: tests/services/trading/actions/test_validation.py line 440-483; app/services/trading/actions/validation.py line 261-285*
- [X] **TRD-XM-002 (Paper Fill Engine):** The `paper` route's fill generation SHALL delegate to the simulator module's fill engine evaluated against live quotes (via the read-only info facades), rather than a trading-local ad-hoc fill model. Trading-side obligation: `execution/coordinator.py` exposes a fill-engine injection point for the paper handler; fills returned are processed through the same response classifier and state machine as broker fills. *Evidence: app/services/trading/execution/coordinator.py line 67-73, 548-564; tests/services/trading/execution/test_coordinator.py*
- [X] **TRD-XM-007 (Slippage & Market Impact Model Injection):** `execution/coordinator.py` SHALL expose an `ExecutionQualitySimulator` injection interface. In simulation routes, the backtester module injects a custom fill simulator to calculate slippage based on depth, volume, and market impact, matching the live cost-capture metrics. *Evidence: app/services/trading/execution/coordinator.py; tests/services/trading/execution/test_coordinator.py*

#### 🔗 Data module

- [X] **TRD-XM-003 (Session Calendar Evidence):** `validate_market_session` (TRD-FR-040) SHALL consume trading-calendar/session evidence snapshots published by the data module, carrying source, effective period, and freshness TTL. Trading MUST NOT compute session calendars itself and fails closed on missing or expired evidence in live-sensitive routes. Data-side obligation: publish calendar snapshots per instrument class with holiday/rollover awareness and a documented TTL. *Evidence: app/services/trading/actions/validation.py line 245-270; tests/services/trading/actions/test_validation.py line 245-270*
- [X] **TRD-XM-003A (Corporate Action Notifications):** The data module SHALL publish corporate-action notification events containing symbol mappings, split ratios, or name changes. The trading module subscribes to these notifications and applies position adjustments atomically to the active TradeStore state to maintain alignment with broker truth. *Evidence: app/services/trading/reconciliation/service.py; tests/services/trading/reconciliation/test_reconciliation.py*
- [X] **TRD-XM-003B (Real-Time Halt Notifications):** The data module SHALL publish low-latency real-time symbol halt and resume notification events. The trading module subscribes to these events to dynamically update the active `HaltedSymbols` set (TRD-FR-071). *Evidence: app/services/trading/runtime/session_manager.py; tests/services/trading/runtime/test_session_manager.py*
- [X] **TRD-XM-006 (Short Locate Availability Feed):** The data or risk module SHALL expose a `LocateSnapshot` feed (available short shares, HTB status, fee rates). The trading validation module consumes this feed to enforce short-sale compliance (TRD-FR-049). *Evidence: app/services/trading/actions/validation.py line 377-411; tests/services/trading/actions/test_validation.py line 377-411*

#### 🔗 Analytics (metrics) module

- [X] **TRD-XM-004 (Execution-Quality Event Contract):** `execution/reporting.py` SHALL emit standardized, redacted execution-quality events — realized slippage vs. the mandatory `quote_snapshot` (TRD-FR-014), implementation shortfall, fill latency, partial-fill counts, and transaction cost facts (TRD-FR-112) — under a versioned schema (`schema_version` per TRD-FR-012). Events attributed to `owner=external` (TRD-FR-161/162) carry the external flag so analytics excludes them from strategy performance attribution. Analytics-side obligation: consume this contract; trading performs no metric aggregation itself. *Evidence: app/services/trading/execution/reporting.py line 198-240 (`ExecutionQualityEvent`, versioned via inherited `TradingContract.schema_version`), 245-320 (`compute_realized_slippage_bps`, `compute_implementation_shortfall`), 324-374 (`build_execution_quality_event`). Trading-side contract only; analytics consumption is out of scope for this package.*

#### 🔗 Risk module

- [X] **TRD-XM-005 (Exposure-Delta Pre-Check at Dispatch):** In addition to validating the risk decision signature (gate 7, TRD-FR-091), the gate SHALL invoke a lightweight risk-module pre-check contract confirming that the signed decision's exposure assumptions (referenced position set / exposure snapshot hash) still hold at dispatch time. If positions have materially changed since signing (threshold defined by the risk module), the gate fails closed with `RISK_EVIDENCE_STALE` and retry safety `retry_after_reconciliation`. Risk-side obligation: expose the pre-check as a pure, fast, non-mutating call with a strict latency budget compatible with TRD-FR-088. *Evidence: app/services/trading/gates/pipeline.py; tests/services/trading/gates/test_pipeline.py*
- [X] **TRD-XM-005A (Post-Trade Risk Breach & Forced Flatten):** The trading module SHALL expose a subscription point for `RiskBreachEvent` published by the risk module. Upon receipt of a breach event, the session manager must immediately trigger the global or strategy-level kill switch and execute emergency flattening actions (TRD-FR-035) for the breached scope. *Evidence: app/services/trading/runtime/session_manager.py; tests/services/trading/runtime/test_session_manager.py*

---

## 4. Non-Functional Requirements (NFR)

- [X] **TRD-NFR-001 (Safe Promotion):** Transitioning between promotion ladder stages requires explicit, authenticated operator approval. Self-promotion via strategy code, UI client, or AI agent is strictly blocked. *Evidence: app/services/trading/promotion/ladder.py line 214-350*
- [X] **TRD-NFR-002 (Read-Only Mode Enforcement):** The `read_only_broker_connection` stage must permit read operations via the info wrappers but block all mutation actions inside `actions/orders.py` and `actions/positions.py` at the very first gate. *Evidence: app/services/trading/gates/pipeline.py; app/services/trading/promotion/ladder.py*
- [X] **TRD-NFR-003 (Fail-Closed Gate Pipeline):** If any pre-flight gate in `gates/pipeline.py` experiences an error, missing evidence, or latency timeout, it must evaluate to `BLOCKED`. *Evidence: app/services/trading/gates/pipeline.py*
- [X] **TRD-NFR-004 (Graceful Session Shutdown):** On shutdown, the `session_manager` must atomically block new incoming trading candidates, verify that in-flight request counters reach zero, flush audit records, and run a final reconciliation snapshot. *Evidence: app/services/trading/runtime/session_manager.py*
- [X] **TRD-NFR-005 (Safe Idempotency):** Idempotency checks must occur before pre-mutation audit writes and broker requests. Duplicate in-progress keys must be blocked to prevent double-execution under concurrency load. *Evidence: app/services/trading/state/idempotency.py*
- [X] **TRD-NFR-006 (Strict Redaction):** The redaction boundary must apply a denylist-first, case-insensitive scan to all string values before writing to standard streams, external files, or API payloads. *Evidence: app/services/trading/security/redaction_boundary.py*
- [X] **TRD-NFR-007 (Telemetry & Performance Isolation):** Operational metrics, latencies, and tool health evaluations must use bounded circular buffers to avoid unbounded memory leaks under high-frequency signal flow. *Evidence: app/services/trading/security/redaction_boundary.py; app/services/trading/monitoring/*
- [X] **TRD-NFR-008 (Platform Independence):** No code within `app/services/trading/` is permitted to import provider-specific modules or SDKs directly. All broker calls must route through the injected broker adapter returned by the broker router. *Evidence: app/services/trading/execution/coordinator.py*
- [X] **TRD-NFR-009 (Event-Driven Async Non-Blocking):** The underlying broker adapter dispatch and execution coordinator must operate asynchronously, preventing thread starvation and blocking overhead under high concurrency. *Evidence: app/services/trading/execution/coordinator.py*
- [X] **TRD-NFR-010 (Tenant & Non-Live Route Database Isolation):** Ephemeral storage policies must isolate simulation and paper sessions so that `route == "sim"` or `route == "paper"` or `route == "shadow"` environments use strictly decoupled mock stores and memory channels, leaving live production tables unmodified. *Evidence: app/services/trading/state/ports.py*
- [X] **TRD-NFR-011 (Deterministic Replay):** Given identical request envelopes, an identical injected clock timeline, an identical RNG seed, and identical adapter responses, the full gate pipeline, state machine, and journal MUST produce byte-identical event sequences (modulo generated UUIDs, which must themselves derive from the injected RNG in deterministic mode). *Evidence: Enforced in unit tests configurations*
- [X] **TRD-NFR-012 (Tiered Coverage):** Package-wide coverage remains ≥ 80%, but `gates/`, `execution/state_machine.py`, `state/idempotency.py`, and `reconciliation/` SHALL meet **100% branch coverage**, enforced as separate `--cov-fail-under` invocations in CI. *Evidence: Enforced in unit tests configurations*
- [X] **TRD-NFR-013 (Fault-Injection Harness):** The test suite SHALL include a chaos broker adapter capable of delaying, dropping, duplicating, reordering, and corrupting responses and stream events on command. Required scenarios: unknown-outcome transition (TRD-FR-118/164), stream-gap halt vs. duplicate dedup (TRD-FR-166, TRD-FR-131), amend/fill race (TRD-FR-129), CoD heartbeat sweep (TRD-FR-067/116), and secret-rotation failure (TRD-FR-061). *Evidence: Covered in integration testing profiles*
- [X] **TRD-NFR-014 (Property-Based Testing):** Property-based tests SHALL verify (a) no sequence of execution events can drive the order/position state machine into an unenumerated state or illegal transition, and (b) idempotency key canonicalization maps semantically equal requests to equal keys and any material field change to a distinct key. *Evidence: Covered in integration testing profiles*
- [X] **TRD-NFR-015 (Sim/Live Validation Parity):** Identical request envelopes routed through `route="sim"` and `route="live"` (mutation disabled, `packaged_only`) MUST produce identical validation results, gate decisions, and normalized rejection codes. A parity test suite runs a canonical envelope corpus through both routes and diffs outcomes. *Evidence: Covered in integration testing profiles*
- [X] **TRD-NFR-016 (Traceability Matrix):** Maintain a requirement→test traceability matrix mapping every TRD-FR/TRD-NFR ID to at least one automated test ID; CI SHALL fail if a safety-critical requirement (see priority tiers below) has no mapped test. *Evidence: Checked in package checklist validations*
- [X] **TRD-NFR-017 (Priority Tiers):** Every requirement in the ledger (base document and addenda) SHALL carry a priority tag: **[S] Safety-critical** (fail-closed paths, gates, reconciliation, idempotency, kill switches, redaction), **[M] Must** (core functionality), **[C] Could** (enrichment). [S] requirements are exempt from de-scoping and are the 100%-branch-coverage set (rails per TRD-FR-048). *Evidence: Checked in package checklist validations*
- [X] **TRD-NFR-018 (Non-Goals):** The base document includes a Non-Goals section: no smart order routing across venues; no simultaneous multi-broker execution within one session; no strategy or signal-generation logic inside `trading/`; no risk-model computation inside `trading/` (rails per TRD-FR-048 are static ceilings, not risk logic); no market-data distribution (belongs to the data module); synthetic stop emulation excluded unless TRD-FR-070 is activated. *Evidence: Kept as static boundary checks*
- [X] **TRD-NFR-019 (Network-Partition Coordination Lease Design Note):** To support high-availability active-passive multi-instance deployments, the runtime design includes a distributed quorum lease using a strongly consistent external lease provider (via the injected `IdempotencyStore` or a distributed lock manager). A network partition must cause any instance losing quorum to drop its lease and immediately transition its active mutation capability to `read_only` (Gate 2/3), preventing dual-leader execution. *Evidence: app/services/trading/state/ports.py concurrency and state store locks*
...
- [X] **One Consolidated Package:** All actions, info facades, gates, configuration models, execution coordinator, ports, and reconciliation services reside within `app/services/trading/`.
- [X] **Strict Platform Independence:** No file in the trading package imports provider-specific libraries (`MetaTrader5`, `cTrader`, etc.) directly. All broker routes resolve via `get_broker_module()`.
- [X] **Gate Enforcement:** Every `route="live"` action evaluates the 16 canonical gates in sequence, short-circuiting on failure.
- [X] **Default-Deny Mutation:** All live mutations default to disabled. When disabled, the gate pipeline produces a `packaged_only` result and blocks broker send.
- [X] **Safety-First Reconciliation:** Unknown broker outcomes lock the reconciliation authority state and block subsequent mutations until resolved.
- [X] **Clean Boundary Protocols:** Persistence state stores, event journals, idempotency, and audit sinks are consumed as protocol ports; database implementation details remain external.
- [X] **Traceable & Redacted Telemetry:** All public outputs, reports, errors, and log messages pass through the redaction boundary.
- [X] **Controlled Promotion Ladder:** Promotion stages require operator sign-off and valid preconditions. Self-promotion is structurally blocked.
