...
- [ ] Cover rejected-code and partial-fill scenarios in `run_backtest` contract tests.
...
- [ ] Detect and deterministically reject or flag data-quality edge cases before indicator calculation, signal generation, or execution.
  - Empty data.
...
- [ ] Enforce risk and concentration limit edge cases deterministically during execution.
  - Stops-level violation.
  - Freeze-level violation.
  - Correlation limit exceeded.
  - Concentration limit exceeded.
- [ ] Handle market-session and gap edge cases deterministically.
  - Market closed.
...
- [ ] Handle order-fill and lookup edge cases deterministically.
  - Unsupported fill policy.
  - Partial-fill remainder.
  - Limit order touched but queue not filled.
  - Order not found.
- [ ] Handle sizing, commission, and analysis-failure edge cases deterministically.
  - Sizing requires stop loss.
  - Commission calculation failure.
  - Monte Carlo failure.
  - Optimization failure.
- [ ] Handle corporate-action and continuous-data edge cases deterministically.
...
- [ ] Handle infrastructure and sandbox-integrity edge cases deterministically.
...
- [ ] Handle timezone and DST-boundary edge cases deterministically.
...
- [ ] Handle authorization and manifest-integrity edge cases deterministically.
...
- [ ] Define required and optional simulator inputs covering market data, manifests, and run configuration.
...
- [ ] Produce required simulator outputs covering trade history, exposure, and run diagnostics.
  - Orders history.
  - Deals history.
  - Trade list.
...
- [ ] Handle strategy-registry and portfolio-risk rejection edge cases deterministically.
...
- [ ] Produce strategy-injection inputs and diagnostics outputs for AI Tool boundaries.
...
- [ ] Handle commission, accounting, and request-validation edge cases deterministically.
...
- [ ] Produce structured simulator result outputs covering account, curve, and diagnostic artifacts.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Handle tick model and tick-volume validation edge cases deterministically.
...
- [ ] Verify synthetic tick generation remains deterministic across resumed, parallelized, and out-of-order processing edge cases.
...
- [ ] Verify tick batching stops safely at every boundary edge case.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Handle spread-model edge cases deterministically.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Handle slippage-model edge cases deterministically.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Handle liquidity-model edge cases deterministically.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Handle borrow-fee edge cases deterministically.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Handle swap-model edge cases deterministically.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Handle insufficient-margin edge cases deterministically.
  - Insufficient margin.
...
- [ ] Handle config, schema, and data-validation edge cases deterministically.
  - Invalid date range.
  - Missing symbol.
  ...
  - Invalid volume.
  - Invalid enum casing.
- [ ] Handle position-sizing input edge cases deterministically.
  - Invalid stop-loss or take-profit direction.
  - Invalid ATR sizing input.
  - Invalid Kelly sizing input.
- [ ] Handle missing asset-class data edge cases deterministically.
...
- [ ] Handle FX conversion-rate edge cases deterministically.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Handle journal-durability and access-permission edge cases deterministically.
...
- [ ] Handle event-priority conflict edge cases deterministically.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Handle corrupted-artifact-manifest edge cases deterministically.
...
- [ ] Produce production-run governance artifacts covering configuration, retention, disclosure, and observability metadata.
