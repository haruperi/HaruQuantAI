...
- [ ] Write the Strategy Service operating manual covering regulatory documentation retention periods, the strategy input modes approved for `run_backtest`, and the explicit `Documentation Only`/`Future`/`Not Implemented` rationale required for any requirement without implementation scope.
...
- [ ] Require strategies to not assume infinite liquidity at the best bid or ask.
...
- [ ] Enforce strategy configuration schema validation, default/unknown-field/type-coercion policy, and rejection of configuration-injection patterns before execution.
...
- [ ] Reject duplicate or malformed registry entries deterministically.
...
- [ ] Resolve strategy version constraints deterministically and enforce deprecation/lifecycle/checkpoint-compatibility gating before execution.
...
- [ ] Define provisional v1.0 performance and resource baselines for strategy execution, including the reference environment used to measure them.
...
- [ ] Define the `TradeIntent` schema and the reproducibility/replay/audit metadata required for every strategy decision.
...
- [ ] Define the registered strategy lifecycle status enum.
...
- [ ] Version and compatibility-test public capability input/output schemas before they are consumed downstream.
...
- [ ] Write strategy registry validation tests covering resolution, rejection, and contract coverage.
...
- [ ] Document strategy registry and configuration schema behavior.
...
- [ ] Restrict Phase 1 strategy execution to registered strategies and validated configuration only.
  - Phase 1 strategy execution shall allow registered strategies and validated configuration only.
- [ ] Resolve strategy files and module paths only through approved registries or allowlisted roots, never arbitrary user-supplied filesystem paths.
...
- [ ] Declare explicit technology stack version constraints for production-eligible strategy execution.
...
- [ ] Define the strategy module's ownership boundary, location, and separation from simulation/indicator/data module responsibilities.
...
- [ ] Declare third-party dependency and capacity assumptions for production-eligible strategy execution.
...
- [ ] Document each public capability's exact Python signature, callable name, stability level, intended consumers, schemas, error codes, side-effect policy, idempotency behavior, and compatibility guarantees.
...
- [ ] Enforce strategy determinism, diagnostic redaction, multiprocess isolation, and dependency-call timeout/retry behavior.
...
- [ ] Restrict strategies to producing decisions, signals, trade intents, or state updates without mutating or finalizing official trading state.
...
- [ ] Define the `TradeIntent` emission contract, including alpha/cost threshold suppression, partial-fill guidance, and timing/sizing handoff to the simulation engine.
...
- [ ] Enforce the default `BAR_OPEN_PREVIOUS_CLOSE` signal timing policy and previous-closed-bar-only data access at bar open.
...
- [ ] Enforce point-in-time correctness for feature/indicator lookups and declare data-latency tolerance.
...
- [ ] Prohibit arbitrary code execution through `run_backtest` and enforce resource controls on approved strategy code.
...
- [ ] Define strategy decision-state management, serialization, and isolation rules.
...
- [ ] Define the strategy execution input/output data contract fields.
...
- [ ] Define strategy-level risk profile fields and lifecycle-promotion evidence requirements.
...
- [ ] Define `TradeIntent` idempotency and lineage fields.
...
- [ ] Declare indicator-readiness and missing/stale-data handling policies.
...
- [ ] Emit structured diagnostics with cross-module trace correlation.
...
- [ ] Produce and validate parameter-optimization validation artifacts.
...
- [ ] Declare strategy computational complexity, concurrency model, and resource budgets.
...
- [ ] Produce strategy artifacts through an approved build pipeline with security and SBOM gates.
...
- [ ] Isolate strategy failures and support an external hard-kill signal.
...
- [ ] Gate paper/live execution behind validation gates, explicit approval, and deterministic-decision-input rules.
...
- [ ] Require a runbook for every production-eligible strategy, including turn-off and onboarding metadata.
...
- [ ] Declare strategy execution assumptions, execution algorithms, spread/volume limits, and venue eligibility.
...
- [ ] Declare deterministic halt-like market-state policies and surface them in diagnostics.
...
- [ ] Declare portfolio-interaction mode and exposure/allocation assumptions without owning portfolio-level enforcement.
...
- [ ] Declare strategy health checks, circuit-breaker inputs, deployment/rollback metadata, performance-review bands, and drift/canary diagnostics without owning their enforcement.
...
- [ ] Declare regulatory regime, reporting, and best-execution assumptions without owning regulatory enforcement.
...
- [ ] Declare data-gap, corporate-action, startup-readiness, and delisted-symbol handling assumptions.
...
- [ ] Define strategy latency, throughput, recovery, and resource SLOs.
...
- [ ] Define strategy calibration, parameter-stability, and overfitting-control policies.
...
- [ ] Define strategy-local checkpoint/restore assumptions and degradation communication metadata.
...
- [ ] Declare market-closure and emergency-liquidation assumptions without owning liquidation enforcement.
...
- [ ] Define strategy performance-review cadence, A/B testing, shadow testing, and kill criteria.
...
- [ ] Document strategy IP classification, licensing, and regulatory-filing compliance.
...
- [ ] Define ML model-artifact serialization, feature-store dependency, and drift-detection requirements.
...
- [ ] Prohibit hardcoded secrets and enforce secrets-manager-only access.
...
- [ ] Declare order-book depth support and deterministic behavior for degraded order-book data.
...
- [ ] Require traceability and sign-off before a strategy is considered production-ready.
...
- [ ] Define the standard strategy processing anatomy and hook contract.
...
- [ ] Fail deterministically on invalid or unknown strategy configuration fields.
...
- [ ] Define deterministic error behavior for data-dependency failures and concurrent read-only state access.
...
- [ ] Require traceability, executable usage examples, and Builder-handoff documentation for every requirement.
...
- [ ] Write the full strategy test suite covering built-ins, timing, fuzz, performance, snapshot, mutation, boundary, security, and clock-drift cases.
...
- [ ] Confirm the strategy module emits only signals/diagnostics and never mutates external authority state.
...
- [ ] Reuse canonical error codes from `app.utils.errors` and map lower-level lookahead errors to the canonical strategy-domain code.
...
- [ ] Apply deterministic disablement/escalation policy and return safe, structured errors for invalid strategy input.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Write contract, error-code-mapping, and dependency-failure tests with a decision table for every public capability.
...
- [ ] Reject raw strategy-code injection attempts deterministically with safe journaling and diagnostics.
...
- [ ] Fail deterministically on resource exhaustion and secret exposure, journaling security rejections with redaction.
...
- [ ] Fail deterministically on deprecated/revoked strategies and preserve the simulation/risk engine as the final risk authority.
...
- [ ] Fail deterministically on duplicate intent/idempotency collisions or non-monotonic sequence numbers.
...
- [ ] Fail deterministically on malformed, missing, timezone-inconsistent market data, or clock drift beyond tolerance.
...
- [ ] Invalidate strategy approval on provenance changes and define deterministic orchestration failure policies.
...
- [ ] Fail deterministically on corrupt, incompatible, or unauthorized checkpoints before execution.
  - Corrupt, incompatible, or unauthorized checkpoints shall fail deterministically before execution.
- [ ] Declare data-failover, backup-venue, and recovery assumptions, and require post-mortem documentation for strategy failures.
...
- [ ] Fail deterministically on unknown, empty, or unapproved strategy identifiers and modules.
...
- [ ] Write failure-path, scenario, and security tests for every confirmed error code.
...
- [ ] Define the vectorized signal-strategy processing contract: indicator/signal computation, current-bar shifting, output batching, and exclusion of execution/accounting/risk/journal steps.
...
- [ ] Enforce atomic lookahead detection and a stable decision clock for vectorized batches.
...
- [ ] Run CPU-bound vectorized strategies in isolated worker processes per their declared execution profile.
...
- [ ] Define deterministic hook execution order across vectorized, event-driven, replay, checkpoint-restore, and shutdown runs.
...
- [ ] Document vectorized/event-driven examples and no-lookahead strategy timing.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Write vectorized-strategy and no-lookahead property-based test coverage.
...
- [ ] Support stateful event strategies that respond to lifecycle events only through controlled, read-only interfaces.
...
- [ ] Apply atomic per-decision-event state updates with deterministic rollback on failure.
  - Strategy-local state updates shall be atomic per decision event or shall fail with a deterministic rollback diagnostic.
- [ ] Declare strategy risk/correlation assumptions during stress events and link every `TradeIntent` to its originating decision event.
...
- [ ] Emit strategy execution metrics covering intents, decisions, data quality, and latency.
...
- [ ] Enforce bounded execution, reentrancy/thread-safety declarations, and a stable deterministic event-ordering policy.
...
- [ ] Declare wash-trade prevention rules and behavior during microstructure market-state events.
...
- [ ] Define the standard event-strategy lifecycle hook interface.
...
- [ ] Define measurable resource limits, backpressure behavior, and async/sync compatibility contracts per environment.
...
- [ ] No file-specific non-functional requirements defined.
- [ ] Gate strategy execution by approved lifecycle status.
  - A strategy shall not execute in an environment higher than its approved lifecycle status.
  - `STRATEGY_LIFECYCLE_NOT_APPROVED`
- [ ] Write the event-driven test suite covering stress, chaos, memory-leak, boundary, and concurrency cases.
...
- [ ] Restrict strategy code execution to registered, validated, or explicitly sandboxed/vetted-and-approved paths.
...
- [ ] Fail deterministically on disallowed environment-variable access, missing sandbox metadata, or expired sandbox approval.
...
- [ ] Document sandbox and vetting requirements if code-based strategy execution is ever enabled.
