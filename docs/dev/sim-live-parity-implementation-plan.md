...
**Per-unit status:** 14b Completed 2026-08-16 under the owner's goal-wide standalone approval. `FR-SIM-146`–`150`, `FR-SIM-195`, `FR-SIM-197`, `FR-SIM-198`, and `FR-SIM-215`–`217` make request v2 the canonical async path: the run context holds approved Trading requests and awaits the injected public Trading action at `app/services/simulator/run/orchestrator.py:436`, while the retained synchronous bridge is explicitly separate at `app/services/simulator/run/orchestrator.py:594`. One complete authority snapshot and exclusive-or-contiguous activity evidence are validated at `app/services/simulator/state/runtime.py:34` and `app/services/simulator/state/runtime.py:80`; identity disagreement is proven at `tests/simulator/integration/test_initial_authority_state.py:22`–`35`, and exclusive, complete replay, unknown, missing, gap, and conflict cases at `tests/simulator/integration/test_foreign_activity_guard.py:8`–`39`. Hashed terminal policy is applied through the injected Trading terminal action at `app/services/simulator/run/orchestrator.py:475` and proven on/off at `tests/simulator/integration/test_terminal_liquidation_policy.py:29`. Protection and close effects emit `protection_trigger` plus provider-shaped `authority_deal` evidence at `app/services/simulator/execution/engine.py:365` and `app/services/simulator/execution/engine.py:494`. Async unchanged-request delivery and no-duplicate cancellation evidence are at `tests/simulator/integration/test_trading_cutover.py:26` and `tests/simulator/integration/test_trading_cutover.py:62`. Usage functions `fr_sim_146()`–`fr_sim_150()`, `fr_sim_195()`, `fr_sim_197()`, `fr_sim_198()`, and `fr_sim_215()`–`fr_sim_217()` are at `tests/simulator/usage/features/07_run.py:398`–`454` and passed directly. Ruff format/check and full Simulator mypy passed; the four mandated files passed 14 tests with `--no-cov`, 31 focused compatibility tests passed, usage registry evidence passed 29 tests, and the complete Simulator behavior gate passed 446 tests. The bounded blocker recommendation treats the injected account-activity port as the owner of complete foreign/manual replay material and admits it only after Simulator proves a contiguous source sequence; missing, unknown, gapped, or exclusive/replay-conflicting evidence blocks execution. V1 retains its labelled direct-intent and unconditional-terminal compatibility behavior; request v2 never invokes that branch. No Trading production file, real account, provider IO, realism formula, or legacy API outside its documented status changed. Rollback removes the four integration tests, v2 dependency ports and async branch, authority snapshot/activity validators, provider-deal categories, new usages/requirements, and restores the prior v1-only orchestration before rerunning Simulator and Trading paired gates.
...
- [x] Commit remains unauthorized, or its separately authorized hash is recorded. Evidence: owner goal-wide commit authorization; Unit 14a committed as `54d46db4` with the exact prescribed message and Unit 14b hash will be reconciled in Unit 15a.
...
**Unit 14b coverage note:** The four-file literal command passed all 14 behaviors and exited 1 only because repository-wide subset coverage was 18.82%; its exact `--no-cov` behavioral counterpart passed.

The literal Unit 14b full Simulator command passed all 446 behaviors and exited 1 only on the repository-wide 29.47% coverage floor; the full `--no-cov` behavior gate passed.
