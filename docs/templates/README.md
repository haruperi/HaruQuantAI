...
app/services/persistence/[domain].py   # only when the domain persists state
tests/services/[domain]/[feature_1]/
tests/examples/[domain_number]_[domain].py
```

Each `app/services/[domain]/[feature].py` module is one cohesive feature and physical removal unit.
...
`app/contracts/[domain].py` module. Feature modules never import sibling implementations. They
...
consolidated in `app/services/persistence/[domain].py`. Feature modules consume focused persistence
...
[Describe the high-level purpose and business outcome this domain delivers.]
...
- [Explicitly unsupported concern]

### Shared contracts

The domain's public boundary is `app/contracts/[domain].py`. A counterparty may be a producer,
...
| --- | --- | --- | --- | --- |
| Missing | `[domain].[capability-name]@1` | `[ProtocolName]` | `1` | [Purpose] |
...
| --- | --- | --- | --- | --- | --- |
| Missing | `[domain].[feature_partition]` | `FEAT-[DOMAIN]-[ACTION_OBJECT]` | `sqlite` | `retain` | `[capability]` |
...
| --- | --- | --- | --- | --- | --- |
| `FEAT-[DOMAIN]-[ACTION_OBJECT]` | [Value] | `app/services/[domain]/[feature].py` | `[domain].[capability]@1` | [Keys or `None`] | Missing |
...
- **Lead owner:** `FEAT-[DOMAIN]-[ACTION_OBJECT]`
...
- **Domain persistence module:** `app/services/persistence/[domain].py` or `None`
- **Namespace:** `[domain].[feature]` or `None`
...
| Missing | `ARCH-004` | Public contracts live in `app/contracts/[domain].py`. | Import/contract checks |
| Missing | `ARCH-005` | Feature modules never import sibling implementations. | Import checks |
| Missing | `ARCH-006` | SQL and schema operations live in `app/services/persistence/[domain].py`. | Architecture and schema checks |
...
- [ ] One cohesive `app/services/[domain]/[feature].py` implementation.
- [ ] Public contracts in `app/contracts/[domain].py`.
...
2. Update `app/contracts/[domain].py` first when the public boundary changes.
3. Update the cohesive feature module and its immutable `SPEC`.
4. Update `app/services/persistence/[domain].py` only when database operations change.
