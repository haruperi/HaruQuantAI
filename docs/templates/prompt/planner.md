...
When Additional context contains `Goal unattended assumption policy:`, make one bounded attempt to resolve non-critical ambiguity through a reversible repository-grounded assumption before returning `BLOCKED`. Never assume protected authority or external facts. Add an exact `### Assumptions for Human Review` section to the dry run, using `- NONE` when no assumption is applied; otherwise record the blocker, assumption, evidence, affected scope, risk, validation, and revisit trigger.
...
The gate label is invariant. The deterministic orchestrator—not Planner—decides whether it is satisfied by an exact interactive `OWNER_MESSAGE` or frozen `RUN_PREAUTHORIZATION`; do not claim either source in the pre-gate dry run.
For Planner `BLOCKED`, keep the same iteration, target Planner, set `requires_owner_gate=false`, and leave `owner_gate` empty. Under the unattended assumption policy, use this handoff only for a protected/external blocker or after the bounded assumption attempt fails; otherwise the workflow stops until the owner resolves the documented external cause.
