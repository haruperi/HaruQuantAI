...
* **Owns**: Structured and bound-context logging, UTC time policy and formatting, shared `AuthContext` and `AuditEvent` contracts, shared base errors and immutable metadata, injected error-event routing, ID generation, canonical serialization, settings loading and secret resolution (including resolving broker secrets into injected `BrokerConnectionConfig` instances at the composition root), redaction, password hashing, caller-keyed Fernet encryption, and in-memory active-secret-version selection.
...
| Partial | `AuthContext`                            | `v1`  | `Utils`        | `UI/API`                                                                                      | `All governed domains`                                                                                                                                                         | Shared authenticated principal and trace context                                                                                                                                                                                                                      | principal ID, roles, scopes, request/correlation IDs, issued timestamp                                                                                                                                                                                | Missing or invalid context causes the receiving domain to fail closed                                                                                                                         |
...
| Partial | `AuditEvent` common envelope             | `v1`  | `Utils`        | `Every domain`                                                                                | `Data` durable audit storage; Risk and UI/API query only through Data-owned query contracts                                                                                    | Redacted trace record for governed actions; each emitting domain owns its payload fields                                                                                                                                                                              | event ID, timestamp, domain, action, principal ID, correlation ID, redacted payload                                                                                                                                                                   | Emission or persistence failure is surfaced and never silently swallowed                                                                                                                      |
...
| Completed | `LOG_DIRECTORY`                                                            | `Path` | `data/logs`                                 | No          | `Utils`   | All domains                                                                          | Directory created during explicit configuration for app/access/debug/error logs                                                                                                 |
...
| Completed | `LOG_ENQUEUE`                                                              | `bool` | `true`                                      | No          | `Utils`   | All domains                                                                          | In-process queued logging with explicit non-destructive synchronization and flush/stop lifecycle                                                                                |
