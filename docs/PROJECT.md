...
- D-UI presents one Vite + React workstation canvas. Browser routes select authorized workstation/workspace entry points; feature-owned widgets consume D-IFACE/public contracts, workspaces arrange widget instances spatially, and explicit source/clock/cursor context coordinates temporal presentation without making the UI authoritative for business time or state.
...
| Missing | UI view models, commands, widget descriptors/instances, workspace templates/layouts, temporal contexts, and extension lifecycle events | `v1` | User Interface | D-UI adapters over D-IFACE/public domain contracts | Feature-owned React widgets and D-UI support adapters | Type-safe spatial/temporal presentation and interaction contracts without duplicating business policy | `app/contracts/ui/` | Incompatible or unavailable capabilities and time domains render explicit unavailable/degraded/stale/resync states and disable unsafe commands. |
...
| Missing | Client-only drafts, navigation, focus, widget-instance state, Dockview layout snapshots, workspace templates/preferences, and temporal presentation cursors; no authoritative business state. | User Interface | Feature-owned D-UI widgets and documented support adapters through `app/contracts/ui/` | User Interface only | Persisted presentation state, when enabled, is actor/workspace/capability/schema scoped and migratable; source clocks/timestamps/sequences remain projections, and server-owned entities remain read-only until submitted through public commands. |
...
| --- | --- |
| 142 product features: 125 service + 17 UI | The fifteen owning domain/UI READMEs; each final delivery checkbox appears exactly once in `IMPLEMENTATION_ORDER.md`. UI widget types and instances are contributions owned by those 17 features, not additional product features. |
...
| Delivery-phase coverage | Exact feature/FR allocation, the ordered UI workstation foundation, widget slices, and later de-mock gates in `IMPLEMENTATION_ORDER.md`; product completion remains governed by owner acceptance plus §15. |
