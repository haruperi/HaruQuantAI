...
| Completed | Brokers persisted state | `Brokers` | `broker_health_history`, `broker_route_recovery`, `broker_environment_permissions`, `broker_event_checkpoints` | Temporary bounded operational checkpoints pending later Brokers refactor children; instrument identity and provider mappings are Catalogue-owned | Credentials, SDK/session objects, balances, orders, fills, positions, instrument identity, and raw provider payloads are never persisted. Health cannot authorize trading, permissions default deny, and recovery/event cursors cannot trigger command resubmission. |
