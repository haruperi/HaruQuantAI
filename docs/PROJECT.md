...
| Missing | `SYS-WF-002` | Signal to live execution                     | Market data update / scheduled evaluation | `Data → Indicators → Strategy → Risk → Trading(live) → Brokers → Analytics → UI/API`          | Approved order executed at broker, reconciled and recorded; unknown broker state is frozen and submitted for critical alert delivery | `tests/system/integration/test_signal_to_live.py`       |
| Partial | `SYS-WF-003` | Parameter optimization and approved adoption | Optimization request                      | `Data → Optimization → Strategy → Simulation → Analytics → Optimization → UI/API → Strategy` | The deterministic advisory Optimization core is implemented and tested; UI/API approval and Strategy adoption remain pending | `tests/system/integration/test_optimization.py`         |
...
| Missing | `SYS-WF-005` | Operator monitoring and kill switch          | Operator action via UI/API                | `UI/API → Risk → Trading → UI/API`                                                                | Kill switch engaged; all execution halted fail-closed; canonical activation submitted for critical alert delivery | `tests/system/integration/test_kill_switch.py`          |
...
**Output boundary:** Broker execution receipt reconciled and recorded (or an audited rejection), visible to Analytics and UI/API. An unknown broker state remains frozen in Trading and produces critical operational evidence for channel-neutral UI/API alert delivery.
...
|     8 | `UI/API`     | Build and attempt channel-neutral critical alert delivery only for authoritative unknown-broker-state evidence | Critical `BROKER_STATE_UNKNOWN` `OperationalEvent` | `CriticalOperationalAlert` + delivery result |
...
5. When Trading first locks a conflict scope for an unknown broker state, it emits one critical `BROKER_STATE_UNKNOWN` `OperationalEvent`. UI/API derives a deterministic `CriticalOperationalAlert` from that event and performs one delivery attempt through its injected channel-neutral sink (`WF-API-014`).
...
| Partial     | `OperationalEvent`                       | `v1`  | `Trading`      | `Trading`                                                                              | `UI/API`                                                                                                                                                                       | Redacted bounded execution health, dependency, staleness, timeout, latency, cost, and incident evidence; critical `BROKER_STATE_UNKNOWN` production remains missing                                                                                                   | event ID/type/severity, route/session/order references, UTC timestamp, bounded measurements, trace IDs, redacted facts                                                                                                                                                             | Invalid/unredacted event or delivery failure returns structured Trading failure and never changes execution truth                                                                             |
