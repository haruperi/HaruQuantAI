...
> **Status:** `In Progress` — of 253 registered application features, 245 are implemented and structurally reconciled (96.84%); 8 are `Pending` and none are `Partial`. Deployment, external-provider readiness, and separately registered system workflows remain distinct runtime concerns.
...
- Formulation and dispatch of order intents across simulation, demo, and live routes, including reconciliation and emergency controls.
...
flowchart TD
    ...
    TRADE --> TRADE_CAP[Execute approved actions on sim, demo, and live routes]
...
* **Status**: `Completed`. Twelve focused Brokers features are implemented: instrument profiles, capability metadata, five direct provider channels, reconciliation, environment isolation, event normalization, conformance, and provider specification snapshots (parity-programme Phase 4a). Provider release policy and production authorization remain separate runtime gates.
...
* **Owns**: Historical market and account data storage/persistence, durable audit storage, shared database infrastructure, connections, locking, SQLite migration execution framework, real-time acquisition cadence and feed handling, shared stream sequencing/fan-out/resume/backpressure policy, data-source selection and cross-provider fallback policy, consumption of Catalogue identity/provider mappings to obtain exact provider-native symbols before a Brokers call, normalization of raw broker/provider reads into `MarketDataset` / `AccountStateSnapshot`, multi-timeframe alignment, deterministic series-level market-data quality inspection producing scored issue, severity, and remediation evidence, and deterministic tick-series derivation from real bar or tick evidence under approved tick and spread models (distinct from GBM synthetic generation, which is fixtures-only and never reaches an official simulation run).
...
* **Boundaries**: Emits proposals (which may include sizing proposals), never broker orders. Does not own live/demo runtime orchestration, risk enforcement, final position sizing approval (Risk owns the final approved size), order routing, official fills, or data normalization.
...
* **Responsibility**: Orchestrate live and demo evaluation workflows, convert approved risk decisions into deterministic order intents, and execute them on the selected route (`sim`, `demo`, `live`) with reconciliation, monitoring, and emergency controls.
* **Inputs**: Live/demo evaluation triggers, strategy references, route/profile configuration, runtime gate configuration, approved `RiskDecision`s with approval tokens, Risk-owned `ActionPolicyVerdict`s, and `KillSwitchState`.
...
* **Key Limits**: No hidden numeric defaults; portfolio size, weight caps, evidence freshness, drift thresholds, schedules, and decision expiry are required profile values. Missing/stale evidence fails closed. Live/demo activation requires authenticated human approval plus Risk authorization.
...
* **Boundaries**: UI never becomes authoritative for backend policy, safety, calculations, persistence, or execution. Client checks are advisory. `FEAT-UI-*` uses executable component/integration evidence instead of standalone usage programs. Employs Spatiotemporal Composability ("everything is a plugin"): a single-page composable canvas hosting dynamic workspaces where all functional capabilities are pluggable widgets in `src/widgets/` that can be added, removed, docked, or split. Markets and Watchlists reside in focused `src/widgets/markets/` and `src/widgets/watchlists/` folders.
...
`sim`, `demo`, and `live` use the same Trading orchestration and differ only at an injected
...
market-evidence class, initial authority state, and evidence sources. `demo` names only the
...
| **L5-MT5-Operational · Shared operational certification** | Verified demo evidence passes every admitted operational invariant; the resulting certificate applies only to MT5 semantics shared by demo and live credential routes |

**Published certificate.** `l5-mt5-operational-btcusd-20260816-04` completed
L5-MT5-Operational on 2026-08-16 from an independent MT5 demo `BTCUSD` trace. It is valid through
2027-08-14 unless invalidated, applies only to the shared deterministic operational semantics in
Envelope v2, and carries no live empirical claim. The generated nine-member evidence bundle remains
under `artifacts/sim_live_parity/mt5-operational/v2/` and is intentionally ignored by source control.

**L5-MT5-Operational is one bounded operational certificate.** Demo evidence may certify the MT5
order, lifecycle, event, policy, accounting-rule, and persistence semantics shared by demo and live;
it never transfers demo spreads, latency, fills, liquidity, slippage, execution-price distributions,
calibration, or performance claims to live. A parity certificate is a revocable lease: build, contract,
...
explicit participant when Trading dispatches `demo`/`live` mutations. Utils is shared
...
**Actor / trigger:** Live/demo market data update or scheduled strategy evaluation under an authenticated principal.
...
1. `Trading` owns the live/demo runtime loop and coordinates only through public domain APIs:
...
|     3 | `Simulation`   | Run deterministic backtests per candidate (via SYS-WF-001 core)               | `SimulationBacktestRequest` per candidate + history | `SimulationResult`s                                   |
...
| Order | Domain               | Responsibility                                                                                         | Input                                                   | Output                                               |
| ----: | -------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------- | ---------------------------------------------------- |
|     1 | `UI/API`           | Authenticate, collect explicit approval for demo/live activation, submit, and present status          | Operator request                                        | `PortfolioConstructionRequest` / approval evidence |
...
|     5 | `Simulation`       | Run the portfolio backtest under simulation policy                                                     | Self-contained `PortfolioBacktestRequest` projection | `PortfolioSimulationResult`                        |
...
|     5 | `Brokers`          | Perform demo/live mutations only when called by Trading                                           | Canonical broker operations            | Provider-truth results                                       |
...
| Completed | `ProviderSpecificationSnapshot`          | `v1`  | `Brokers`      | `Brokers`                                                                               | `Data` (effective-dated history via `FEAT-DATA-02` Phase 4b), `Simulation` (revision sets), UI/API (views)                                                                     | Typed **current** provider specification observation for one symbol/account: execution/order/filling/expiration/GTC modes, stops/freeze, directional volume limit, calculation mode, margin/swap evidence, instrument scalars, and account permission evidence with explicit `unverified` exclusions | broker/server/redacted account digest/environment/terminal build/source revision, aware-UTC `observed_at`, retrieval provenance, canonical SHA-256 checksum; separate typed dynamic cost-evidence reference                                                        | Missing required fields fail closed (`BROKER_RESPONSE_INVALID`); no effective bounds exist (current observation only); unmapped provider values become `UNKNOWN` and fail canonical eligibility; raw account IDs never stored |
...
| Completed   | `OrderIntent`                            | `v1`  | `Trading`      | `Trading`                                                                              | `Simulation` for `sim`; `Brokers` via `BrokerAdapter` mutation operations for `demo/live`; `UI/API` (views)                                                         | Deterministic, idempotent executable order request                                                                                                                                                                                                                    | client order ID, route, symbol, side, approved volume, idempotency hash                                                                                                                                                                                                            | Duplicate hash deduplicated; gate failure causes no dispatch                                                                                                                                  |
...
| Completed   | `FastResearchRequest`                    | `v1`  | `Simulation`   | `UI/API`                                                                         | `Simulation`                                                                                                                                                                   | Explicit non-canonical approximation request for bounded research only                                                                                                                                                                                                  | reference-based strategy/data/execution/Risk identity, `fast_research` profile, `sim` route, `canonical=false`, and a dedicated config hash                                                                                                                            | Canonical claims, disabled research policy, unsafe values, or incompatible references are rejected with structured `SIM_*` errors |
| Completed   | `SimulationBacktestRequest`              | `v2`  | `Simulation`   | `UI/API`; `Optimization` via its internal backtest-adapter port                      | `Simulation`                                                                                                                                                                   | Sole canonical backtest request with complete bound execution identity; execution is asynchronous through `run_backtest_async`                                                                                                                                          | required execution-model reference/hash, calculation and evidence hashes, separate source/tick lineage hashes, market-evidence class, decision-instant policy, provider-specification revisions, complete initial-authority-state hash, certification target (`demo`/`live`), and terminal-close policy | Missing execution identity, relabelled demo evidence as live, retroactive snapshots, or synchronous invocation are rejected with structured `SIM_*` errors |
| Completed   | `ParityEnvelope` / parity certificate   | `v2`  | `Simulation`   | `Simulation`                                                                          | `Trading, Brokers, Data, UI/API (published scope only)`                                                                                                                        | Versioned falsifiable matrix for certificate `l5-mt5-operational-btcusd-20260816-04`, covering only MT5 operational semantics shared by demo/live routes; empirical evidence remains route-scoped                                                                                                                               | evidence route, applicable provider routes, certified operational semantics, excluded empirical claims, provider/environment/server mode, initial-authority-state hash, tolerances, ignored-field registry, validity and invalidation triggers | Work outside the matrix fails canonical eligibility; certificate invalidates on bound identity or drift change; no demo empirical artifact may be relabelled live |
...
| Completed   | `PortfolioBacktestRequest`               | `v1`  | `Simulation`   | `Portfolio`                                                                            | `Simulation`                                                                                                                                                                   | Deterministically simulate a complete portfolio candidate through a receiver-owned self-contained projection                                                                                                                                                          | portfolio/result IDs and versions, ordered canonical child requests, strategy/data/FX references and hashes, bounded UTC range, execution/Risk versions, seed, config hash, `simulation` profile, `sim` route                                                                    | Unsafe objects, Portfolio contract instances, stale/incompatible references, invalid range, or non-deterministic configuration are rejected before execution                                  |
...
| Completed | No durable Brokers state; provider adapters and gateway are strictly stateless in memory | `Brokers` | None | All historical `broker_*` tables retired and deleted; instrument identity/mappings are Catalogue-owned, admission is Workspace-owned, reconciliation is Trading-owned, checkpoints are Data-owned | Credentials, SDK/session objects, balances, orders, fills, positions, instrument identity, and raw provider payloads are never persisted. Health is transient in-memory provider truth only. |
...
| Missing | `P-SYS-002`   | Reliability     | `RUNTIME_PROFILE` and `EXECUTION_ROUTE` are validated as a compatible pair per the §6 compatibility table (`research`→`none`, `simulation`→`sim`, `demo`→`demo`, `live`→`live`); an incompatible pair fails closed at initialization.                                                                                                                                                                                                                                                                     | Initialization/config unit + integration tests                                     |
...
live/demo loop. Agentic orchestration runs in a separately controllable worker, and
...
| ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------------------------------------------ | --------------------------------------------------------------------- |
| Backend process (FastAPI +`app/utils` + `app/services/*`) | Utils, Brokers, Data, Indicators, Strategy, Risk, Trading, Simulation, Analytics, Optimization, Research, Portfolio, API gateway | dev / demo / live | `uv run` (single entry point)                                    | Single instance — SQLite write locking and one MT5 terminal per host |
| Trading runtime loop (background worker)                      | Trading orchestrating Data, Indicators, Strategy, Risk; dispatching via Brokers                                                  | demo / live only  | Backend process, gated by`RUNTIME_PROFILE` / `EXECUTION_ROUTE` | Single instance                                                       |
| Agentic worker                                                | `app/agentic` Google ADK adapter, workflows, agents, context, permissions, and operations                                        | dev / sandbox / demo / separately approved live proposal mode | Backend composition starts a separately cancellable worker, gated by `AGENTIC_ENABLED` and firm mandate | Bounded concurrency; no broker credential or direct broker route |
...
| MT5 terminal                                                  | External broker gateway                                                                                                          | demo / live       | Operator                                                           | One per broker account                                                |
