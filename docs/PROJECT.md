...
* **Inputs**: Raw log records, error conditions, audit payloads, caller-supplied credential material, and environment settings.
* **Outputs**: Structured and specialized logs, mapped/routed errors, prefixed IDs, canonical JSON, shared context/audit contracts, protected credential values, and loaded settings.
* **Owns**: Structured and bound-context logging, UTC time policy and formatting, shared `AuthContext` and `AuditEvent` contracts, shared base errors and immutable metadata, injected error-event routing, ID generation, canonical serialization, the sole repository `.env` loading base for typed settings, explicit secret-reference resolution, redaction, password hashing, caller-keyed Fernet encryption, and in-memory active-secret-version selection. Composition roots consume typed `SecretStr` settings and inject Brokers-owned `BrokerConnectionConfig` instances.
* **Boundaries**: Owns no durable business state and makes no business decisions. Does not own authentication, identity verification, credential persistence, key storage or rotation, strategy logic, risk rules, broker operations, persistence, or any domain contract base classes (domains own their own contracts locally).
...
* **Inputs**: Canonical broker requests (market data reads, account state reads, order mutations, execution-state reads, subscriptions), `BrokerConnectionConfig` constructed by the composition root with caller-selected environment and Utils-resolved in-memory credentials. Data and Trading may receive an injected capability-scoped adapter, but neither constructs, resolves secrets for, or mutates the configuration.
...
* **Boundaries**: Pure passthrough with zero business logic — no business validation (structural/transport validation only), no risk checks, no decision-making, no data enrichment, no business retry/replay (transport-level flow control and connection recovery are permitted; mutations are never retried), and no state management beyond the live session (no durable state). Owns no credential vault and performs no credential persistence or database access — Utils resolves secret references before the composition root constructs `BrokerConnectionConfig`; only resolved secret values live in memory for the adapter lifecycle. Data owns canonical market identity, friendly names, and every cross-provider/provider alias mapping; Brokers accepts and reports exact provider-native symbol strings only and owns no alias resolution. Brokers does not normalize into `MarketDataset` / `AccountStateSnapshot` and never leaks raw SDK objects across its boundary. Only Trading may invoke mutation operations; Data's use is strictly read-only; Risk and all other domains have no Brokers dependency. The read/write split is enforced by capability-trait scoping (`MarketDataProvider`, `TradeExecutionProvider`, `AccountProvider`, `CalculationProvider`).
...
| Missing | `BrokerConnectionConfig`                 | `v1`  | `Brokers`      | Composition root (secret references and provider enable flags resolved first by Utils/settings) | `Brokers`                                                                                                                                                                      | Explicit provider, enablement, account, environment, resolved in-memory credentials, and transport tuning for one adapter session                                                                                                                                     | broker ID, environment (`LIVE`/`DEMO`/`TESTNET`/`SANDBOX`), required `provider_enabled`, `SecretStr` credential mapping, timeouts, buffer and circuit-breaker bounds                                                                      | Disabled provider, environment mismatch, or invalid configuration →`BROKER_CONFIGURATION_INVALID`; no implicit default to `LIVE`; references are never resolved inside Brokers           |
...
| Missing | `AuditEvent` common envelope             | `v1`  | `Utils`        | `Every domain`                                                                                | `Data` durable audit storage; Risk and UI/API query only through Data-owned query contracts                                                                                    | Redacted trace record for governed actions; each emitting domain owns its payload fields                                                                                                                                                                              | event ID, timestamp, domain, action, principal ID, correlation ID, redacted payload                                                                                                                                                                   | Emission or persistence failure is surfaced and never silently swallowed                                                                                                                      |
