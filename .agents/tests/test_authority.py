...
@pytest.mark.parametrize(
    "path", [".gitignore", ".gitattributes", ".github/workflows/ci.yml", ".gitmodules"]
)
def test_path_authority_allows_git_prefixed_repository_files(
    orc: ModuleType, path: str
) -> None:
    assert orc._normalize_path_list([path]) == [path]
