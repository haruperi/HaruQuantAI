...
def _render(spec: dict[str, Any]) -> str:
    ...
    if "child_additional_context" in spec:
        lines.append(
            f"child_additional_context = {_q(str(spec['child_additional_context']))}"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Common coordination context injected into every child Task.",
    )
    parser.add_argument(
        "--listed-order",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Under unattended runtime policy, give each Planner one bounded "
            "repository-grounded assumption retry before pausing."
        ),
    )
