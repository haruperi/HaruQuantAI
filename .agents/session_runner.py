...
SHARED_SESSION_MODES = frozenset({"solo", "solo-headless"})
...
def _configure_utf8_stdio() -> None:
    """Configure forwarded vendor output streams for explicit UTF-8 text."""
...
def _empty_ledger() -> dict[str, Any]:
    return {"schema_version": SESSION_SCHEMA_VERSION, "generations": {}}
...
def _load_ledger(path: Path) -> dict[str, Any]:
    ...
    if schema == 1:
        ...
        if not isinstance(sessions, dict):
            raise SessionContinuityError("Legacy role-session ledger is invalid.")
        return {
            "schema_version": SESSION_SCHEMA_VERSION,
            "generations": {"normal": {"mode": "multi-delegate", "sessions": sessions}},
        }
    ...
    if not isinstance(generations, dict):
        raise SessionContinuityError(
            "Role-session ledger has invalid generations data."
        )
...
def _generation_sessions(
    ledger: dict[str, Any], *, mode: str, generation: str
) -> dict[str, Any]:
    """Return/create one mode-bound session generation."""
    generations = cast("dict[str, Any]", ledger["generations"])
    raw = generations.get(generation)
    if raw is None:
        raw = {"mode": mode, "sessions": {}}
        generations[generation] = raw
    if not isinstance(raw, dict) or raw.get("mode") != mode:
        raise SessionContinuityError(
            f"Session generation {generation!r} mode mismatch."
        )
    sessions = raw.get("sessions")
    if not isinstance(sessions, dict):
        raise SessionContinuityError("Session generation has invalid sessions data.")
...
def _existing_record(
    ...
    role: str,
    *,
    mode: str = "delegate-multi",
    generation: str = "normal",
) -> dict[str, Any] | None:
    sessions = _generation_sessions(ledger, mode=mode, generation=generation)
    record = sessions.get("SOLO" if mode in SHARED_SESSION_MODES else role)
    if record is None:
        return None
...
def _record_session(
    ...
    *,
    ...
    sessions = _generation_sessions(ledger, mode=mode, generation=generation)
    record_key = "SOLO" if mode in SHARED_SESSION_MODES else identity.role
    existing = _existing_record(ledger, identity.role, mode=mode, generation=generation)
    if existing is not None:
        ...
        last_iterations = existing.setdefault("last_iterations", {})
        if not isinstance(last_iterations, dict):
            raise SessionContinuityError("Session iteration ledger is invalid.")
        previous = int(last_iterations.get(identity.role, 0))
        if identity.iteration < previous:
            raise SessionContinuityError("Role session iteration moved backwards.")
        last_iterations[identity.role] = identity.iteration
        existing["last_iteration"] = max(
            int(existing.get("last_iteration", 0)), identity.iteration
        )
        return
    for other_role, value in sessions.items():
        if (
            mode not in SHARED_SESSION_MODES
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--mode",
        default="delegate-multi",
        choices=tuple(sorted(HEADLESS_MODES)),
    )
    parser.add_argument("--generation", default="normal")
