    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                console.error(err)
                setError("Failed to calculate hourly period analysis.")
            } finally {
                setLoading(false)
            }
        }
    ...
    if (!selectedBacktest) {
        return (
            <div className="flex flex-col h-full w-full">
                <PerformancePageHeader title="Hourly Period Analysis" />
                <div className="flex-1 flex items-center justify-center p-6 bg-muted/10">
                    <div className="text-center">
                        ...
                        <p className="text-sm text-muted-foreground mt-2">Please select a backtest run to view hourly period analysis.</p>
                    </div>
                </div>
            </div>
        )
