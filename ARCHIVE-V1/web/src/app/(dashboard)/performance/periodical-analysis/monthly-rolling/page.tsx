...
export default function Page() {
    ...
    const [data, setData] = useState<RollingPeriodData[] | null>(null)
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true)
                setError(null)
                const result = await strategyApi.getPeriodRollingAnalysis(
                    'monthly',
                    trades,
                    initialBalance
                )
                ...
                console.error(err)
                setError("Failed to calculate monthly rolling period analysis.")
            } finally {
                setLoading(false)
            }
        }
    ...
    if (!selectedBacktest) {
        return (
            <div className="flex flex-col h-full w-full">
                <PerformancePageHeader title="Monthly Rolling Period Analysis" />
                <div className="flex-1 flex items-center justify-center p-6 bg-muted/10">
                    <div className="text-center">
                        ...
                        <p className="text-sm text-muted-foreground mt-2">Please select a backtest run to view monthly rolling period analysis.</p>
                    </div>
                </div>
            </div>
        )
                                ...
                                <table className="w-full text-sm caption-bottom">
                                    <thead className="[&_tr]:border-b sticky top-0 bg-background z-10">
                                        <tr className="border-b transition-colors hover:bg-muted/50">
                                            ...
                                            <th className="h-12 px-3 text-right align-middle font-medium text-muted-foreground">Profit Change</th>
                                            ...
                                            <th className="h-12 px-3 text-right align-middle font-medium text-muted-foreground">PF Change</th>
                                            <th className="h-12 px-3 text-right align-middle font-medium text-muted-foreground">Win Rate</th>
                                            <th className="h-12 px-3 text-right align-middle font-medium text-muted-foreground">Win Rate Change</th>
                                            ...
                                            <th className="h-12 px-3 text-right align-middle font-medium text-muted-foreground">Return % Change</th>
                                    ...
                                    <tbody className="[&_tr:last-child]:border-0">
                                        {data.map((row, index) => (
                                            <tr key={index} className="border-b transition-colors hover:bg-muted/50">
                                                ...
                                                <td className={`p-2 px-3 align-middle text-right font-mono text-xs ${getChangeColor(row.net_profit_change)}`}>
                                                    {formatCurrency(row.net_profit_change)}
                                                </td>
                                                ...
                                                <td className={`p-2 px-3 align-middle text-right font-mono text-xs ${getChangeColor(row.profit_factor_change)}`}>
                                                    {formatNumber(row.profit_factor_change)}
                                                </td>
                                                <td className="p-2 px-3 align-middle text-right font-mono">
                                                    {formatPercent(row.win_rate)}
                                                </td>
                                                <td className={`p-2 px-3 align-middle text-right font-mono text-xs ${getChangeColor(row.win_rate_change)}`}>
                                                    {formatPercent(row.win_rate_change)}
                                                </td>
                                                ...
                                                <td className={`p-2 px-3 align-middle text-right font-mono text-xs ${getChangeColor(row.return_pct_change)}`}>
                                                    {formatPercent(row.return_pct_change)}
                                                </td>
                                                <td className="p-2 px-3 align-middle text-right font-mono">
