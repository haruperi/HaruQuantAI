...
export default function Page() {
    ...
    const [data, setData] = useState<WinningTradesData | null>(null)
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true)
                setError(null)
                const result = await strategyApi.getWinningTradesAnalysis(
                    trades,
                    initialBalance
                )
                ...
                console.error(err)
                setError("Failed to generate winning trades chart.")
            } finally {
                setLoading(false)
            }
        }
    ...
    if (!selectedBacktest) {
        return (
            <div className="flex flex-col h-full w-full">
                <PerformancePageHeader title="Winning Trades" />
    ...
    return (
        <div className="flex flex-col h-full w-full overflow-hidden">
            ...
            <div className="flex-1 overflow-auto p-4 md:p-6 bg-muted/10">
                <Card className="h-full border-t-4 border-t-green-500/50 flex flex-col">
                        ...
                        {loading ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    ...
                                    {/* Regular winning trades - light gray dots */}
                                    <Scatter
                                        name="Winning Trades"
                                        data={regularTrades}
                                        fill="#cbd5e1"
                             ...
                             !loading && <div className="flex items-center justify-center h-full text-muted-foreground">No winning trades to display.</div>
                        )}
                    </CardContent>
                </Card>
            </div>
