...
interface TradePoint {
    trade_number: number
    profit_loss: number
    is_winning_outlier: boolean
    is_losing_outlier: boolean
    is_open: boolean
    type: string | null
}

interface TotalTradesData {
    trades: TradePoint[]
    average: number
}
            ...
            <div className="bg-slate-950 border border-slate-800 p-3 rounded-lg shadow-xl text-sm">
                ...
                <div className="mt-2 space-y-1">
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <span className={`font-mono ${data.profit_loss >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                    ...
                    {data.is_winning_outlier && (
                        <div className="text-green-400 text-xs">Winning Outlier</div>
                    )}
                    {data.is_losing_outlier && (
                        <div className="text-red-400 text-xs">Losing Outlier</div>
                    )}
                    {data.is_open && (
                        <div className="text-blue-400 text-xs">Open Trade</div>
                    )}
                </div>
            </div>
        )
...
export default function Page() {
    ...
    const [data, setData] = useState<TotalTradesData | null>(null)
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true)
                setError(null)
                const result = await strategyApi.getTotalTradesAnalysis(
                    trades,
                    initialBalance
                )
                ...
                console.error(err)
                setError("Failed to generate total trades chart.")
            } finally {
                setLoading(false)
            }
        }
    ...
    if (!selectedBacktest) {
        return (
            <div className="flex flex-col h-full w-full">
                <PerformancePageHeader title="Total Trades" />
    ...
    // Separate trades into different categories for rendering
    const regularTrades = data?.trades.filter(t => !t.is_winning_outlier && !t.is_losing_outlier && !t.is_open) || []
    const winningOutliers = data?.trades.filter(t => t.is_winning_outlier) || []
    const losingOutliers = data?.trades.filter(t => t.is_losing_outlier) || []
    const openTrades = data?.trades.filter(t => t.is_open) || []
                        ...
                        {loading ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    ...
                                    {/* Regular trades - light gray dots for visibility */}
                                    <Scatter
                                        name="Profit/Loss"
                                        data={regularTrades}
                                        fill="#cbd5e1"
                                    ...
                                    {/* Winning outliers - bright green squares */}
                                    {winningOutliers.length > 0 && (
                                        <Scatter
                                            name="Winning Outliers"
                                            data={winningOutliers}
                                            fill="#4ade80"
                                            shape="square"
                                        />
                                    )}
