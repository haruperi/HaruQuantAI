...
class PositionInfo:
    ...
    def select(self, symbol: str) -> bool:
        ...
        if positions:
            ...
            logger.debug("Selected open position for symbol '%s'.", symbol)
            return True
        self._data = None
        logger.debug("No open position found for symbol '%s'.", symbol)
        return False
    ...
    def select_by_ticket(self, ticket: int) -> bool:
        ...
        if positions:
            self._data = positions[0]
            logger.debug("Selected open position ticket %s.", ticket)
            return True
        self._data = None
        logger.debug("No open position found for ticket %s.", ticket)
        return False
