...
class SymbolInfo:
    ...
    def refresh(self) -> bool:
        ...
        """
        if not self._name:
            logger.debug("Symbol refresh skipped: no symbol name set.")
            return False
        broker = get_broker_module()
        self._data = broker.get_symbol_info(self._name)
        logger.debug("Refreshed symbol specifications for '%s'.", self._name)
    ...
    def select(self, select: bool) -> bool:
        ...
        """
        if not self._name:
            logger.debug("Market Watch select skipped: no symbol name set.")
            return False
        broker = get_broker_module()
        client = (
        ...
        if hasattr(client, "subscribe_spots") and select:
            try:
                ...
                logger.debug("Subscribed to spots for symbol '%s'.", self._name)
                return True
            except Exception:
                logger.warning(
                    "Failed to subscribe to spots for symbol '%s'.", self._name
                )
                return False
        return True
