...
from app.services.trading.account_info import AccountInfo
from app.services.trading.concurrency import ConcurrencyQueue
from app.services.trading.idempotency import IdempotencyService
from app.services.trading.position_info import PositionInfo
from app.services.trading.rate_limiter import get_rate_limiter
from app.services.trading.readiness import ReadinessService
from app.services.trading.reconciliation import ReconciliationService
from app.services.trading.result import (
    BrokerResponseNormalizer,
    NormalizedTradeResult,
    ResultBuilder,
)
from app.services.trading.store import TradeStore, get_default_store
from app.services.trading.symbol_info import SymbolInfo
from app.services.trading.terminal_info import TerminalInfo
from app.services.trading.validation import ValidationService
...
class Trade:
    ...
    def _send_request(self, request: dict[str, Any]) -> bool:
        ...
        """
        logger.info(
            "Processing trade request for symbol '%s' (action=%s).",
            request.get("symbol", self._symbol),
            request.get("action", 1),
        )
        if Trade._kill_switch_active and not Trade._bypass_kill_switch:
            logger.warning("Trade request blocked by active kill switch.")
        ...
        with Trade._in_flight_lock:
            if Trade._is_shutting_down:
                logger.warning("Trade request rejected: service is shutting down.")
                self._result = ResultBuilder.failure(
        ...
        try:
            ...
            if not symbol:
                ...
                if order_ticket:
                    from app.services.trading.order_info import OrderInfo
            ...
            if existing:
                if existing["status"] == "in_progress":
                    logger.warning(
                        "Duplicate request %s already in progress.", idem_key
                    )
                    self._result = ResultBuilder.failure(
                ...
                if existing["status"] == "completed":
                    logger.info(
                        "Returning cached result for duplicate request %s.", idem_key
                    )
            ...
            if (
                ...
                try:
                    ...
                    logger.error("Startup reconciliation pass failed: %s", e)
                    self._result = ResultBuilder.failure(
            ...
            with self._concurrency.lock_sync(account_id, symbol):
                ...
                if not limiter.acquire():
                    logger.warning(
                        "Rate limit exceeded for provider '%s'.", active_broker
                    )
                    self._result = ResultBuilder.failure(
                ...
                try:
                    ...
                    logger.error(
                        "Broker call timed out after 5s; flagged as Unknown Outcome."
                    )
                    self._result = ResultBuilder.failure(
                    ...
                    logger.exception("Broker adapter execution error.")
                ...
                if is_success:
                    logger.info(
                        "Trade request succeeded (deal=%s, order=%s, retcode=%s).",
                        self._result.deal,
                        self._result.order,
                    ...
                    logger.warning(
                        "Trade request failed (retcode=%s): %s",
                        self._result.retcode,
                        self._result.comment,
                    )
    ...
    def buy(
        ...
        if not target_symbol:
            logger.warning("No symbol provided or set; aborting trade operation.")
            return False
    ...
    def position_close(self, symbol_or_ticket: str | int, deviation: int = -1) -> bool:
        ...
        if isinstance(symbol_or_ticket, int):
            if not pos.select_by_ticket(symbol_or_ticket):
                logger.warning(
                    "Position with ticket %s not found.", symbol_or_ticket
                )
                return False
        elif not pos.select(symbol_or_ticket):
            logger.warning("Position for symbol '%s' not found.", symbol_or_ticket)
            return False
    ...
    def set_kill_switch(cls, active: bool, flatten_positions: bool = False) -> None:
        ...
        Args:
            active: True to activate the kill switch, False to deactivate.
            flatten_positions: If True, close all open positions on activation.
        """
    ...
    def shutdown(cls, timeout: float = 5.0) -> None:
        ...
        try:
            from app.services.brokers.router import get_broker_module
            from app.services.trading.reconciliation import ReconciliationService
            from app.services.trading.store import get_default_store
