...
The service is compacted into focused modules:

1. **[__init__.py](file:///c:/Users/rharu/Documents/MyApplications/Quant/app/services/data/__init__.py)**: Re-exports every public class and function across the data modules (see `__all__`). It contains no implementation details. Eight names are defined in two modules each (`get_market_hours`, `get_trading_sessions`, `get_symbol_metadata`, `get_data_availability`, `resample_ohlcv`, `align_multitimeframe_data`, `clear_data_cache`, `get_cached_data`); the tool-layer binding (`market_tools`/`csv`) wins at the package level, and the duplicate definitions in `calendar`, `gateway`, `storage`, and `transforms` remain reachable through their own submodules.
2. **[gateway.py](file:///c:/Users/rharu/Documents/MyApplications/Quant/app/services/data/gateway.py)**: Router gateway and source adapters. Orchestrates historical, real-time, local, synthetic, and broker data queries. Local/synthetic sources use `CSVAdapter`, `ParquetAdapter`, and `SyntheticAdapter`; all broker sources (MT5, cTrader, Dukascopy, Binance, Yahoo) are served by a single generic `BrokerAdapter` configured per source.
...
6. **[transforms.py](file:///c:/Users/rharu/Documents/MyApplications/Quant/app/services/data/transforms.py)**: Pure mathematical and structural operations. Manages timeframe conversion, incremental bar aggregation, tick aggregation, pandas resampling, timeframe alignment, indicator labeling, and NumPy type cleaning.
7. **[validation.py](file:///c:/Users/rharu/Documents/MyApplications/Quant/app/services/data/validation.py)**: Enforcement of safety boundaries, including timeframe format checks and volume/spread limits validation. (Licensing rules live in `licensing.py`; market hours / session windows in `calendar.py`.)
8. **[generators.py](file:///c:/Users/rharu/Documents/MyApplications/Quant/app/services/data/generators.py)**: Signal-aware tick generation from real broker bars/ticks for backtesting, across four models with native/fixed/variable spreads. Fully vectorized, with DataFrame and streamed-Parquet outputs.
...
#### `list_symbols`
Discovers available symbols from local CSV/Parquet archives. Broker-backed
symbol discovery lives in `app.services.brokers`.
```python
...
Align lower and higher timeframe OHLCV records on timestamps.
```python
from app.services.data import align_multitimeframe_data

aligned = align_multitimeframe_data(
    datasets={"5m": five_min_bars, "1h": hourly_bars},
    target_timestamps=[bar["timestamp"] for bar in five_min_bars],
)
```

### Tick Generation (`generators.py`)

Reconstruct tick streams from **real** broker data for tick-level backtesting.
Four models select the source/shape, via `TICK_MODEL_REAL`, `TICK_MODEL_GENERATED`,
`TICK_MODEL_TRADING_BAR`, and `TICK_MODEL_OHLC_M1`. Spread models are
`native_spread` (default), `fixed_spread`, and `variable_spread`.

| Model | Input | Behavior |
| --- | --- | --- |
| `TICK_MODEL_TRADING_BAR` | trading-tf `bars` | 4 ticks/bar: O->L->H->C (bull) / O->H->L->C (bear) |
| `TICK_MODEL_GENERATED` | trading-tf `bars` | `tick_volume` ticks/bar (mql5 article #75 path) |
| `TICK_MODEL_OHLC_M1` | `m1_bars` + `bars` | 4 ticks per M1 bar; signals merged from trading-tf bars |
| `TICK_MODEL_REAL` | `real_ticks` + `bars` | Standardize real ticks; signals merged from trading-tf bars |

#### `generate_ticks` / `generate_ticks_frame`
`generate_ticks` returns `list[dict]` (small/demo); `generate_ticks_frame` returns a
columnar `DataFrame` and is preferred for large inputs (no per-tick serialization).
```python
from app.services.data import generate_ticks_frame
from app.services.data.generators import TICK_MODEL_GENERATED

ticks = generate_ticks_frame(
    model=TICK_MODEL_GENERATED,
    trading_timeframe="H1",
    bars=h1_bars,  # real broker bars
)
```

#### `generate_ticks_to_parquet`
Stream ticks to a Parquet file with bounded memory (chunked row groups), the
preferred path for years of data.
```python
from app.services.data import generate_ticks_to_parquet
from app.services.data.generators import TICK_MODEL_GENERATED

meta = generate_ticks_to_parquet(
    model=TICK_MODEL_GENERATED,
    trading_timeframe="H1",
    path="artifacts/ticks/eurusd_h1.parquet",
    bars=h1_bars,
    chunk_size=100_000,
)
# meta -> {"path": ..., "rows": ..., "columns": [...]}
```
