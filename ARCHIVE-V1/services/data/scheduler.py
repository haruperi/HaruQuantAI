...
def _get_or_create_event_loop() -> asyncio.AbstractEventLoop:
    ...
    """
    logger.debug("Resolving active asyncio event loop or setting a new event loop policy.")
    try:
...
def _filter_in_memory_feeds(
    ...
    """
    logger.debug(
        f"Filtering active in-memory feeds matching feed_id={feed_id}, "
        f"source={source}, symbol={symbol}, data_kind={data_kind}."
    )
    matching_feeds = []
    if feed_id:
