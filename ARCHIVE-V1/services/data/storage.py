...
class Bar:
    """Canonical OHLCV bar used by data-storage helpers."""
...
DB_FILE_PATH: str = "data/data/data_service.db"
...
class DatabaseHelper:
    ...
    def __init__(self, db_path: str = DB_FILE_PATH) -> None:
        ...
        self.init_database()
        logger.debug("Initialized data database helper at %s.", self.db_path)
    ...
    def init_database(self) -> None:
        ...
        Returns:
            None.
        """
        logger.debug(f"Initializing SQLite database schema at path: {self.db_path}")
...
def _parse_timezone_aware_datetime(val: str) -> datetime:
    ...
    Returns:
        datetime.
    """
    logger.debug(f"Parsing ISO-8601 timestamp string: '{val}'")
...
def load_ohlcv_csv(path: str | Path) -> tuple[Bar, ...]:
    ...
    Returns:
        tuple[Bar, ...].
    """
    logger.debug(f"Loading canonical OHLCV bars from CSV file path: {path}")
