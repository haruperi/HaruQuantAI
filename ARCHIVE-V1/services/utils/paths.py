...
def _coerce_path(value: str | Path, *, field_name: str) -> Path:
    """Coerce a string or ``Path`` into an expanded ``Path``.

    Args:
        value: String or ``Path`` value to coerce.
        field_name: Name used in validation error messages.

    Returns:
        An expanded ``Path`` (``~`` resolved to the user home).

    Raises:
        ValidationError: If ``value`` is empty or not a ``str``/``Path``.
    """
    if isinstance(value, str):
        ...
        res = Path(value.strip()).expanduser()
        ...
        res = value.expanduser()
    else:
        _raise_validation(
            f"{field_name} must be a string or pathlib.Path.",
            field_name=field_name,
        )
    logger.debug("Implemented path coercion")
    return res
...
def _resolve_path(path: Path) -> Path:
    ...
    logger.debug("Implemented resolving path")
    return res
...
def _strip_windows_extended_prefix(path: Path) -> Path:
    ...
    if text.startswith("\\\\?\\UNC\\"):
        ...
        res = path
    logger.debug("Implemented stripping Windows extended prefix")
    return res


def _ensure_within_base(path: Path, base_dir: Path) -> None:
    """Enforce that a resolved path stays inside a base directory.

    Args:
        path: Resolved candidate path to check.
        base_dir: Resolved base directory the path must stay within.

    Returns:
        ``None`` when ``path`` is inside ``base_dir``.

    Raises:
        SecurityError: If ``path`` escapes ``base_dir``.

    Side effects:
        Emits a warning log when a traversal attempt is rejected.
    """
    ...
    *,
    ...
    if base_dir is None:
        ...
        logger.debug(
            ...
            extra={"event_name": "path_normalized", "bounded": False},
        )
        return resolved
    ...
    logger.debug(
        "Implemented base-bounded path normalization",
        extra={"event_name": "path_normalized", "bounded": True},
    )
    return normalized
...
def safe_join(base_dir: str | Path, *parts: str | Path) -> Path:
    ...
    _ensure_within_base(normalized, normalized_base)
    logger.debug(
        "Implemented safe path join",
        extra={"event_name": "safe_join", "parts": len(parts)},
    )
    return normalized
...
def validate_path_within_root(
    ...
    logger.debug(
        "Implemented path-within-root validation",
        extra={"event_name": "path_within_root_validated"},
    )
    return normalized
