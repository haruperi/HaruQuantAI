...
All 16 modules share a single public contract: other services throughout the application import exclusively from `app.services.utils`
(e.g. `from app.services.utils import logger, success_response`) rather than from sub-files directly. Heavy optional dependencies such
...
4. **Unified Entry Point**: All public interfaces must be registered and re-exported in `app/utils/__init__.py`. Other services
   must import from `app/utils` directly, not from sub-files (e.g. `from app.services.utils import logger`, not
...
Import exclusively from the `app.services.utils` package, never from sub-files:

```python
# Correct — unified public registry
from app.services.utils import logger, success_response, build_metadata, ValidationError

# Incorrect — bypasses the registry and violates the design contract
from app.services.utils.logger import logger             # Do not do this
from app.services.utils.standard import success_response  # Do not do this
```
...
```python
from app.services.utils import logger, build_metadata, success_response
...
```python
from app.services.utils import build_metadata, success_response, error_response, response_from_exception
...
from app.services.utils import utc_now, parse_datetime, to_naive_utc, to_utc_datetime, is_stale, format_utc_timestamp
...
```python
from app.services.utils import hash_password, verify_password, generate_encryption_key, encrypt_text, decrypt_text, redact_payload
...
```python
import pandas as pd
from app.services.utils import inspect_ohlcv_quality, validate_ohlcv_quality
...
```python
from app.services.utils import InMemoryEventBus, build_event_envelope, publish_event
...
```python
from app.services.utils import CircuitBreaker, MetricRegistry, record_tool_call_metric, export_prometheus_metrics
...
```python
from app.services.utils import FakeNotificationAdapter, NotificationRouter, route_notification
...
```python
from app.services.utils import validate_input_schema, validate_numeric_range, ValidationError
...
from app.services.utils import ensure_parent_dir, safe_join, validate_path_within_root
...
from app.services.utils import settings, validate_config
...
```python
from app.services.utils import InMemoryEventBus, ErrorRouter
...
```python
from app.services.utils import FakeNotificationAdapter, NotificationRouter, broadcast_notification
...
```python
from app.services.utils import AlertDeduplicator
...
```bash
python -c "import app.services.utils; print('import-safe: OK')"
```
