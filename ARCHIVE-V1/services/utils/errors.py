...
if TYPE_CHECKING:
    from app.services.utils.event_bus import InMemoryEventBus
...
_ERROR_SEVERITIES: frozenset[str] = frozenset({"info", "warning", "error", "critical"})
...
class Error(Exception):
    ...
    Attributes:
        code: Stable approved deterministic error code.
        severity: Deterministic severity classification.
        retryable: Whether the failure is safe to retry.
        field_path: Optional dotted path of the offending input field.

    Args:
        message: Human-readable error details.
        code: Optional approved deterministic error code.
        field_path: Optional dotted path to the offending field.
        severity: Optional severity classification.
        retryable: Optional retryability flag.
        details: Optional raw details; redacted on access.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the error with a deterministic message and code.

        Args:
            message: Human-readable error details. Must not contain
                secret values, credentials, or private broker payloads.
            code: Optional approved deterministic error code. When
                provided, it is normalised via ``normalize_error_code``
                before being stored. Unknown codes are silently mapped
                to ``UNKNOWN_ERROR``.
            field_path: Optional dotted path identifying the offending
                input field, for example ``"order.volume"``.
            severity: Optional severity classification. Defaults to the
                class-level severity (``"error"``).
            retryable: Optional retryability flag. Defaults to the
                class-level value (``False``).
            details: Optional raw diagnostic details. Returned only via
                :meth:`redacted_details`, which strips secrets.

        Raises:
            ValidationError: If ``severity`` is not an approved value.
            TypeError: If ``message`` is not a string (propagated from
        ...
        if severity is not None:
            if severity not in _ERROR_SEVERITIES:
                raise ValidationError(
                    f"severity must be one of {sorted(_ERROR_SEVERITIES)}."  # noqa: EM102
                )
    ...
    def redacted_details(self) -> str:
        """Return secret-safe details suitable for logs and envelopes.

        Falls back to the error message when no explicit details were
        provided. Sensitive values are removed before returning.

        Returns:
            Redacted, secret-safe details string.

        Side effects:
            None.
        """
        from app.services.utils.security import redact_text

        raw = self._details if self._details is not None else self.message
...
class InfrastructureError(Error):
    """Database, migration, connection, or storage infrastructure failure."""
    ...
    def __init__(
        ...
        Args:
            descriptor: The normalized error descriptor payload.
            details: Human-readable explanation or raw traceback details.
        """
        msg = details if details is not None else descriptor.message
        code_name = descriptor.name
        if code_name not in APPROVED_ERROR_CODES:
            code_name = "DATABASE_ERROR"
...
class ErrorRouter:
    ...
    def route_error(
        ...
        *,
        ...
        """
        from app.services.utils.event_bus import build_event_envelope
...
def _validate_code_text(code: object, field_name: str = "code") -> str:
    ...
    Args:
        code: Candidate code value. Must be a non-empty string.
        field_name: Field label used in the error message.

    Returns:
        Uppercased, stripped string when ``code`` is valid.

    Raises:
        ValidationError: If ``code`` is not a non-empty string.
    """
