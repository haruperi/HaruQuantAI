...
- **Manifest Authority Transition**: During the provider decoupling transition, provider capability declarations are authored in static `manifest.toml` files per provider folder. Feature-level domain README `### Feature Registry` tables remain authoritative for business capability status and requirements until the manifest-generation migration is formally enacted in Phase 16.
