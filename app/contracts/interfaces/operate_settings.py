...
class OperateDiagnosticsRequest:
    """One bounded diagnostics translation request."""
...
OperateDiagnosticsResult = (
    DiagnosticSnapshot | BenchmarkComparison | DiagnosticBundleRef | InterfaceFailure
)


@runtime_checkable
class OperateSettingsCapability(Protocol):
    """Public settings and diagnostics translation boundary."""

    async def administer_settings(
        self,
        request: OperateSettingsRequest,
    ) -> OperateSettingsSuccess | InterfaceFailure:
        """Translate one versioned settings operation."""
        ...
    ...
    async def diagnostics(
        ...
        """Translate one bounded diagnostics operation."""
        ...
