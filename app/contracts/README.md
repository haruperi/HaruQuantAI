...
`plugins.sandbox-permissions@1` retains one async method and now completes its
previously missing operation envelope. `GRANT` carries the immutable manifest,
package hash, requested permissions, and limits; `INSPECT` reads the exact
versioned grant; `EXECUTE` carries bounded `JsonObject` input and returns only a
validated `JsonObject` output. Package roots and secret values are never wire
fields. The provider resolves package-hash and `SecretRef`-to-environment-name
bindings process-locally, and failures include the precise
`PLUGIN_SANDBOX_EXECUTION_FAILED` code for worker/protocol/timeout faults.
