...
from app.contracts.orchestration.local_workers import (
    LocalWorkDescriptor,
    LocalWorkResult,
    SharedInputHandle,
)
...
class ExecuteLocalWorkService:
    ...
    def __init__(
        ...
        self._active: dict[str, tuple[object, object]] = {}
        self._closed = False
        self._ctx = multiprocessing.get_context("spawn")

    async def stage_input(self, content: bytes) -> SharedInputHandle:
        """Stage one bounded immutable input into owned shared memory."""
        self._ensure_open()
        if not content or len(content) > self._max_input:
            raise ValueError("LOCAL_INPUT_SIZE_INVALID")
        shm = shared_memory.SharedMemory(create=True, size=len(content))
        shm.buf[: len(content)] = content
        handle_id = f"shared-{uuid.uuid4().hex}"
        self._handles[handle_id] = shm
    ...
    async def execute(self, descriptor: LocalWorkDescriptor) -> LocalWorkResult:
        """Execute one fenced work descriptor under shared resource admission."""
        self._ensure_open()
        ...
        if (
            current.current_attempt.attempt_id != descriptor.attempt_id
            or current.current_attempt.fence != descriptor.fence
        ):
            return LocalWorkResult(
                descriptor.work_id,
        ...
        admission = await self._resources.admit(
            ResourceAdmissionRequest(
                request_id=f"local:{descriptor.work_id}",
                owner_id="FEAT-ORCH-EXECUTE_LOCAL_WORK",
                work_id=descriptor.work_id,
                idempotency_key=(
                    f"local:{descriptor.job_id}:{descriptor.attempt_id}:"
                    f"{descriptor.work_id}"
                ),
                profile=FiniteResourceProfile(
                    memory_bytes=descriptor.input_handle.byte_count,
                    thread_count=1,
                    ready_descriptors=1,
                ),
            )
        ...
        if admission.status is not AdmissionStatus.ADMITTED or admission.lease is None:
            return LocalWorkResult(
                ...
                b"",
                "",
                f"RESOURCE_{admission.status.value.upper()}",
            )
        ...
        process = self._ctx.Process(
            ...
            args=(
                ...
                descriptor.input_handle.shared_memory_name,
                descriptor.input_handle.byte_count,
                descriptor.input_handle.content_hash,
                descriptor.seed,
                cancel,
                queue,
            ),
            name=f"hq-{descriptor.work_id}",
        )
        self._active[descriptor.work_id] = (process, cancel)
        process.start()
        try:
            ...
            return LocalWorkResult(
                ...
                str(status),
                output,
                hashlib.sha256(output).hexdigest() if output else "",
                str(reason),
            )
        finally:
            self._active.pop(descriptor.work_id, None)
            queue.close()
    ...
    async def cancel(self, work_id: str) -> bool:
        """Cooperatively cancel one active worker then terminate after the bound."""
        active = self._active.get(work_id)
        if active is None:
            return False
        process, cancel = active
        cancel.set()
        await asyncio.sleep(self._grace)
        if process.is_alive():
            process.terminate()
    ...
    async def release_input(self, handle: SharedInputHandle) -> None:
        """Release one owned shared-memory mapping idempotently."""
    ...
    def _ensure_open(self) -> None:
        if self._closed:
            raise RuntimeError("execute-local-work service is closed")

    async def close(self) -> None:
        """Stop workers and release all mappings."""
        if self._closed:
            return
        self._closed = True
        for work_id in tuple(self._active):
