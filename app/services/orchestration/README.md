...
#### Ratified v1 public records (25)

Table anchors: `projects(name_normalized UNIQUE)`, `project_versions(project_id,version UNIQUE)`, `project_runs(project_version_id,run_number UNIQUE)`, `task_runs(project_run_id,task_key,logical_iteration UNIQUE)`, `task_attempts(task_run_id,attempt_no UNIQUE)`, `variable_assignments(project_run_id,name,sequence UNIQUE)`. Task-state machine is the §4.3/§5.1 canonical set; terminal states immutable.

| # | Record | Exact wire fields | FRs / rules |
|---|---|---|---|
| R1 | `ProjectRef` | `project_id: Uuid7`; `schema_version: Literal[1] = 1`. | — |
| R2 | `ProjectVersion` | `project_version_id: Uuid7`; `project_id: Uuid7`; `version: int >= 1`; `name: str 1..160`; `project_type: Literal[MANUAL_RESEARCH] = "MANUAL_RESEARCH"`; `graph: ProjectGraph`; `inputs: tuple[ProjectVariable, ...] = ()`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable; editing creates a new version; rejected versions cannot be published through any transport. | FR-ORCH-DEFINE_PROJECT_GRAPHS. |
| R3 | `ProjectGraph` | `graph_id: Uuid7`; `tasks: nonempty tuple[TaskDefinition, ...]`; `transitions: tuple[TransitionEdge, ...] = ()` where `TransitionEdge(from_task_key: nonempty str, to_task_key: nonempty str, condition: ProjectExpression | None = None, kind: Literal[ORDERED,BRANCH,LOOP,GOTO,STOP_AND_START] = "ORDERED", cycle_bound: JsonObject | None = None)`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Cycles require an enforced count/duration/result/budget/operator-stop bound; `GOTO`/`STOP_AND_START` compile to explicit bounded transitions; static validation rejects invalid edges/missing contracts. | FR-ORCH-DEFINE_PROJECT_GRAPHS, DEFINE_TASK_TRANSITIONS, COMPILE_CONTROL_TRANSITIONS. |
| R4 | `TaskDefinition` | `task_key: nonempty str`; `task_type: nonempty str`; `settings: JsonObject` (typed per `TaskContract`); `resource_refs: tuple[Uuid7, ...] = ()`; `contract: TaskContract`; `schema_version: Literal[1] = 1`. | FR-ORCH-DEFINE_PROJECT_GRAPHS, DECLARE_TASK_CONTRACTS. |
| R5 | `TaskContract` | `task_type: nonempty str`; `contract_version: int >= 1`; `input_ports: tuple[PortSpec, ...] = ()` where `PortSpec(name: nonempty str, schema_version: int >= 1)`; `output_ports: tuple[PortSpec, ...] = ()`; `required_capabilities: tuple[CapabilityIdentifier, ...] = ()`; `supports_checkpoint: bool`; `cancellation_behavior: Literal[COOPERATIVE,IMMEDIATE,NONE]`; `permissions: tuple[nonempty str, ...] = ()`; `resource_estimator: nonempty str`; `schema_version: Literal[1] = 1`. | FR-ORCH-DECLARE_TASK_CONTRACTS. |
| R6 | `TaskState` | `task_run_id: Uuid7`; `logical_state: Literal[QUEUED,RUNNING,PAUSING,PAUSED,RESUMING,STOPPING,STOPPED,COMPLETED,FAILED,CANCELLED]`; `logical_iteration: int >= 1`; `progress: DecimalValue in [0,1] | None = None` (monotonic within an attempt; indeterminate allowed); `entered_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. Invalid transitions return stable conflict errors and create no effects; progress reconstructs from durable state after reconnect. | FR-ORCH-DEFINE_TASK_STATES, REPORT_PROJECT_PROGRESS. |
| R7 | `ProjectRunRef` | `project_run_id: Uuid7`; `project_version_id: Uuid7`; `run_number: int >= 1`; `run_manifest_id: Uuid7`; `initiator: Uuid7` (principal); `state: Literal[QUEUED,RUNNING,PAUSED,STOPPING,STOPPED,COMPLETED,FAILED,CANCELLED]`; `schema_version: Literal[1] = 1`. The run manifest pins project version, referenced object versions, effective settings, seeds, resource policy, and initiator and is complete before the first task is queued. | FR-ORCH-PIN_PROJECT_RUNS. |
| R8 | `TaskRunRef` | `task_run_id: Uuid7`; `project_run_id: Uuid7`; `task_key: nonempty str`; `logical_iteration: int >= 1`; `resolved_inputs: JsonObject = {}`; `state: TaskState | None = None`; `schema_version: Literal[1] = 1`. | FR-ORCH-DEFINE_TASK_STATES. |
| R9 | `TaskAttemptRef` | `attempt_id: Uuid7`; `task_run_id: Uuid7`; `attempt_no: int >= 1`; `retry_policy_version: int >= 1`; `failure_cause: nonempty str | None = None`; `recovery_point: JsonObject | None = None`; `started_at: UtcTimestamp | None = None`; `ended_at: UtcTimestamp | None = None`; `schema_version: Literal[1] = 1`. Retry creates a new attempt on the same logical run; history/cause/recovery point retained. | FR-ORCH-VERSION_TASK_ATTEMPTS. |
| R10 | `TaskLease` | `lease_id: Uuid7`; `task_run_id: Uuid7`; `attempt_no: int >= 1`; `worker_id: Uuid7`; `fencing_token: int >= 1` (monotonic); `acquired_at/last_heartbeat_at/expires_at: UtcTimestamp`; `state: Literal[ACTIVE,RELEASED,EXPIRED,SUPERSEDED]`; `schema_version: Literal[1] = 1`. Workers heartbeat and lose commit authority after expiry or token replacement; a stale worker cannot commit after reassignment. | FR-ORCH-FENCE_TASK_LEASES. |
| R11 | `TaskCheckpoint` | `checkpoint_id: Uuid7`; `task_run_id: Uuid7`; `attempt_no: int >= 1`; `sequence: int >= 1`; `implementation_hash: ContentHash`; `manifest_hash: ContentHash`; `state_artifact_id: Uuid7`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable, content-addressed; valid only for compatible implementation+manifest (`CHECKPOINT_INCOMPATIBLE` otherwise); corrupt checkpoints never resume. | FR-ORCH-VERSION_TASK_CHECKPOINTS. |
| R12 | `TaskOutputCommit` | `commit_id: Uuid7`; `task_run_id: Uuid7`; `attempt_no: int >= 1`; `fencing_token: int >= 1`; `output_artifact_ids: tuple[Uuid7, ...] = ()`; `committed_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. Atomic commit before downstream eligibility; fault injection never exposes partial output to a successor; commits accept only the current token before expiry. | FR-ORCH-COMMIT_TASK_OUTPUTS. |
| R13 | `ProjectVariable` | `assignment_id: Uuid7`; `project_run_id: Uuid7`; `name: nonempty str`; `value_type: Literal[BOOLEAN,INTEGER,DECIMAL,STRING,TIMEFRAME,INSTRUMENT,UUID]`; `value: JsonValue`; `sequence: int >= 1`; `source: Literal[PROJECT_INPUT,PREDECESSOR_OUTPUT]`; `schema_version: Literal[1] = 1`. Typed, scoped, immutable per assignment; missing/ambiguous/incompatible values fail before scheduling. | FR-ORCH-TYPE_PROJECT_VARIABLES, SCOPE_PROJECT_VARIABLES. |
| R14 | `ProjectExpression` | `expression_id: Uuid7`; `expression: nonempty str`; `language_version: int >= 1`; `referenced_outputs: tuple[nonempty str, ...] = ()`; `schema_version: Literal[1] = 1`. Sandboxed deterministic expression language over declared task results, counters, and variables; no filesystem/network/clock/undeclared access. | FR-ORCH-EVALUATE_PROJECT_EXPRESSIONS. |
| R15 | `DomainTaskRequest` | `request_id: Uuid7`; `task_type: Literal[BUILD,RETEST,OPTIMIZE,FILTERING,CUSTOM_ANALYSIS,CREATE_PORTFOLIO,AUTOMATIC_PORTFOLIO_BUILDER,AUTOMATIC_RETEST,APPLY_MASS_CONFIG,CLEAR_DATABANKS,LOAD_FROM_FILES,SAVE_TO_FILES,UPDATE_DATA,LOG_DATABANK_STATS]`; `delegated_capability: CapabilityIdentifier`; `pinned_selection_token_id: Uuid7 | None = None` (mass-config/clear/load/save operate on pinned selections with dry-run impact records; retry never broadens scope); `connector_sync_plan_id: Uuid7 | None = None` (UPDATE_DATA; staged data invisible downstream); `databank_snapshot: JsonObject | None = None` (LOG_DATABANK_STATS pinned membership + versioned metrics); `input_refs: tuple[Uuid7, ...] = ()`; `schema_version: Literal[1] = 1`. Delegates to the corresponding application services. | FR-ORCH-DELEGATE_DOMAIN_TASKS, PIN_TASK_SELECTIONS, SYNC_PROJECT_DATA, PIN_PORTFOLIO_INPUTS. |
| R16 | `UtilityTaskRequest` | `request_id: Uuid7`; `task_type: Literal[CALL_EXTERNAL_SCRIPT,DELETE_FILE,WAIT_FOR,NOTIFICATION,NEURAL_NETWORK_TRAINER]`; `allowlist_entry_id: Uuid7 | None = None` (CALL_EXTERNAL_SCRIPT); `arguments: tuple[str, ...] = ()`; `working_directory: str | None = None`; `environment: dict[str, str] = {}`; `timeout_seconds: int >= 1 | None = None`; `input_artifact_ids: tuple[Uuid7, ...] = ()`; `output_contract: JsonObject = {}`; `artifact_handle_id: Uuid7 | None = None` (DELETE_FILE accepts only workspace-managed artifact/temp handles with containment/reference/retention policy); `wait_duration_seconds: int >= 1 | None` and `wait_condition: ProjectExpression | None` (WAIT_FOR bounded and checkpointable/cancellable); `notification: NotificationTemplate-ref fields` (`channel_id: Uuid7 | None`, `template_id: Uuid7 | None`); `training_plan_id: Uuid7 | None` (NEURAL_NETWORK_TRAINER, Experimental §21.3); `schema_version: Literal[1] = 1`. | FR-ORCH-RUN_APPROVED_EXECUTABLES, MANAGE_WORKSPACE_TASKS, EVALUATE_DURATION_CONDITIONS, SEND_PROJECT_NOTIFICATIONS, TRAIN_NEURAL_NETWORKS. |
| R17 | `ExecutableAllowlistEntry` | `entry_id: Uuid7`; `executable_path: nonempty str`; `allowed_arguments: tuple[str, ...] = ()`; `allowed_working_directory: str | None = None`; `allowed_environment: dict[str, str] = {}`; `max_timeout_seconds: int >= 1`; `granted_by: Uuid7` (principal); `schema_version: Literal[1] = 1`. Only allowlisted executables run in isolated processes with explicit contract. | FR-ORCH-RUN_APPROVED_EXECUTABLES. |
| R18 | `NotificationChannelConfig` | `channel_id: Uuid7`; `kind: Literal[DESKTOP,SMTP_EMAIL,TELEGRAM,TWILIO_SMS]`; `is_enabled: bool = False`; `master_switch_required: Literal[True] = True`; `rate_limit: nonempty str`; `transport_policy: Literal[DISABLED,STARTTLS,TLS] = "DISABLED"` (SMTP); `sender_ref: nonempty str | None = None`; `recipient_refs: tuple[nonempty str, ...] = ()`; `secret_ref_ids: tuple[Uuid7, ...] = ()` (Workspace `SecretRef` in-memory references; opaque config); `segment_policy: JsonObject = {}`; `schema_version: Literal[1] = 1`. Strictly validated from Workspace-owned settings; delivery disabled by default requiring master + channel switch; rate limits enforced before any external call. | FR-ORCH-CONFIGURE_NOTIFICATION_CHANNELS, ENFORCE_NOTIFICATION_LIMITS. |
| R19 | `NotificationTemplate` | `template_id: Uuid7`; `version: int >= 1`; `message_kind: Literal[TRADING,POSITION,SYSTEM,CONNECTION,ERROR,PERFORMANCE,MARKET,NEWS,RISK,CUSTOM,TEST,SESSION_LOCAL]`; `body_template: nonempty str`; `content_kind: Literal[PLAIN_TEXT,HTML] = "PLAIN_TEXT"`; `redaction_policy: nonempty str`; `schema_version: Literal[1] = 1`. Versioned built-ins plus session-local custom templates. | FR-ORCH-RENDER_NOTIFICATION_TEMPLATES. |
| R20 | `NotificationSession` | `session_id: Uuid7`; `manager_instance: Literal[SHARED_SINGLETON]`; `channel_readiness: dict[Uuid7, Literal[READY,DISABLED,INVALID]]`; `opened_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. At most one thread-safe manager session; every client/timer/secret handle closed through the owning effect scope. | FR-ORCH-MANAGE_NOTIFICATION_SESSIONS. |
| R21 | `NotificationReceipt` | `receipt_id: Uuid7`; `delivery_id: nonempty str` (stable delivery identity); `channel_id: Uuid7`; `status: Literal[DELIVERED,FAILED,SUPPRESSED_RATE_LIMIT,DISABLED]`; `rendered_hash: ContentHash`; `sent_at: UtcTimestamp | None = None`; `error: nonempty str | None = None`; `schema_version: Literal[1] = 1`. Redacted rendered template; message payload never embedded. | FR-ORCH-SEND_PROJECT_NOTIFICATIONS, DELIVER_*_NOTIFICATIONS. |
| R22 | `ProjectProgress` | `project_run_id: Uuid7`; `phase: Literal[INDETERMINATE,DETERMINATE]`; `completed_units: int >= 0 = 0`; `total_units: int >= 0 | None = None`; `attempt_count: int >= 1`; `schema_version: Literal[1] = 1`. Logical progress distinguished from retry attempts. | FR-ORCH-REPORT_PROJECT_PROGRESS. |
| R23 | `ProjectHistoryEntry` | `entry_id: Uuid7`; `project_run_id: Uuid7`; `sequence: int >= 1`; `kind: Literal[STATE_TRANSITION,COMMAND,CHECKPOINT,LOG,RESOURCE_USAGE,OUTPUT,CAUSAL_LINK]`; `recorded_at: UtcTimestamp`; `detail: JsonObject`; `schema_version: Literal[1] = 1`. Operator can reconstruct why every task ran. | FR-ORCH-RETAIN_PROJECT_HISTORY. |
| R24 | `NetworkTrainingPlan` | `plan_id: Uuid7`; `dataset_artifact_ids: tuple[Uuid7, ...]`; `leakage_prevention: JsonObject`; `preprocessing: JsonObject`; `model_spec: JsonObject`; `validation_spec: JsonObject`; `seeds: tuple[nonempty str, ...]`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Experimental §21.3 contract. | FR-ORCH-TRAIN_NEURAL_NETWORKS. |
| R25 | `NetworkTrainingResult` | `result_id: Uuid7`; `plan_id: Uuid7`; `trained_model_artifact_id: Uuid7`; `validation_metrics: JsonObject`; `reproducibility_manifest: JsonObject`; `inference_artifact_id: Uuid7 | None = None`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. | FR-ORCH-TRAIN_NEURAL_NETWORKS. |

Cross-owner references used by these records (never copied): `SecretRef` IDs and artifact handles (Workspace); `ConnectorSyncPlan` (Data); databank snapshots (Analytics); delegated capability keys (Research/Analytics/Portfolio); `ProjectGraphProjection` consumer (Interfaces). No subscriptions.

#### Ratified v1 capabilities and operation envelopes

All new (universal rule; shared `OrchestrationFailure` with `code: Literal[ORCHESTRATION_VALIDATION_FAILED,PROJECT_NOT_FOUND,PROJECT_VERSION_CONFLICT,TASK_CONTRACT_INVALID,GRAPH_CYCLE_UNBOUNDED,TASK_STATE_CONFLICT,LEASE_NOT_HELD,FENCING_TOKEN_STALE,CHECKPOINT_INCOMPATIBLE,VARIABLE_RESOLUTION_FAILED,EXECUTABLE_NOT_ALLOWLISTED,NOTIFICATION_DISABLED,NOTIFICATION_RATE_LIMITED,CAPABILITY_UNAVAILABLE]`; no subscriptions):

1. `orchestration.define-projects@1` / `DefineProjectsCapability` / `define_projects` — ops `CREATE, EDIT_GRAPH, VALIDATE, PUBLISH_VERSION, COMPARE`. Success: `version: ProjectVersion | None`; `projection: ProjectGraphProjection | None` is Interfaces-owned; use `graph: ProjectGraph | None`; `findings: tuple[ValidationIssue, ...] = ()`. FRs: DEFINE_PROJECT_GRAPHS, DECLARE_TASK_CONTRACTS, DEFINE_TASK_TRANSITIONS, COMPILE_CONTROL_TRANSITIONS.
2. `orchestration.run-tasks@1` / `RunTasksCapability` / `run_tasks` — ops `START_RUN, PAUSE, RESUME, STOP, CANCEL, RETRY, REPORT_PROGRESS`. Success: `run: ProjectRunRef | None`; `state: TaskState | None`; `progress: ProjectProgress | None`. Idempotent commands persist outcomes; retries never duplicate attempts/outputs. FRs: DEFINE_TASK_STATES, RETRY_TASKS_IDEMPOTENTLY, FENCE_TASK_LEASES, VERSION_TASK_ATTEMPTS, VERSION_TASK_CHECKPOINTS, COMMIT_TASK_OUTPUTS, REPORT_PROJECT_PROGRESS.
3. `orchestration.evaluate-conditions@1` / `EvaluateConditionsCapability` / `evaluate_conditions` — ops `RESOLVE_VARIABLES, EVALUATE`. Success: `variables: tuple[ProjectVariable, ...] = ()`; `result: JsonValue | None = None`. FRs: TYPE_PROJECT_VARIABLES, EVALUATE_PROJECT_EXPRESSIONS, SCOPE_PROJECT_VARIABLES.
4. `orchestration.run-domain-tasks@1` / `RunDomainTasksCapability` / `run_domain_tasks` — ops `DELEGATE, PIN_SELECTION, DRY_RUN`. Success: `request: DomainTaskRequest | None`; `impact: JsonObject = {}`. FRs: DELEGATE_DOMAIN_TASKS, PIN_TASK_SELECTIONS, SYNC_PROJECT_DATA, PIN_PORTFOLIO_INPUTS.
5. `orchestration.run-utility-tasks@1` / `RunUtilityTasksCapability` / `run_utility_tasks` — ops `CALL_EXECUTABLE, DELETE_FILE, WAIT_FOR, NOTIFY, MANAGE_ALLOWLIST, CONFIGURE_CHANNEL, RENDER_TEMPLATE, OPEN_SESSION, CLOSE_SESSION`. Success: `request: UtilityTaskRequest | None`; `allowlist_entry: ExecutableAllowlistEntry | None`; `channel: NotificationChannelConfig | None`; `session: NotificationSession | None`; `receipt: NotificationReceipt | None`. FRs: RUN_APPROVED_EXECUTABLES…ENFORCE_NOTIFICATION_LIMITS (all notification rows).
6. `orchestration.track-run-history@1` / `TrackRunHistoryCapability` / `track_run_history` — ops `RECORD, QUERY`. Success: `entries: tuple[ProjectHistoryEntry, ...] = ()`. FRs: RETAIN_PROJECT_HISTORY.
7. `orchestration.train-networks@1` / `TrainNetworksCapability` / `train_networks` — ops `PLAN, TRAIN` (Experimental). Success: `plan: NetworkTrainingPlan | None`; `result: NetworkTrainingResult | None`. FRs: TRAIN_NEURAL_NETWORKS.
