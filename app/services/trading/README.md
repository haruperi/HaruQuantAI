...
Expose reusable execution-session and intent-policy semantics required by simulation while preserving existing Trading authority. Provide read-only, owner-authored outcome evidence for later calibration.

### Owns

Research-compatible execution session/account policies; deterministic execution/position ownership rules; authorized post-horizon outcome observation.

### Does not own

A second simulator, broker client, live route or account authority; Agentic order construction or deployment permissions. The three selected features do not remove unrelated existing Trading responsibilities.
...
| --- | --- | --- | --- | --- |
| DOCUMENTARY_BOUND | `trading.manage-execution-sessions@1` | Selected public operation/DTO surface; exact existing symbols are inventoried in the Phase 0 contract-binding projection.<br>[`app/contracts/trading/manage_execution_sessions.py`](../../contracts/trading/manage_execution_sessions.py) | 1 | Preserve governed live/simulation session boundaries |
| DOCUMENTARY_BOUND | `trading.execution-policies@1` | Selected public operation/DTO surface; exact existing symbols are inventoried in the Phase 0 contract-binding projection.<br>[`app/contracts/trading/execution_policies.py`](../../contracts/trading/execution_policies.py) | 1 | Provide pure shared execution-policy descriptors |
| DOCUMENTARY_BOUND | `trading.outcome-evidence@1` | Selected public operation/DTO surface; exact existing symbols are inventoried in the Phase 0 contract-binding projection.<br>[`app/contracts/trading/outcome_evidence.py`](../../contracts/trading/outcome_evidence.py) | 1 | Expose matured execution outcomes read-only |
...
| --- | --- | --- | --- | --- |
| `catalogue.define-trading-rules@1` | Catalogue | Required | [`FEAT-TRD-MODEL_EXECUTION_POLICIES`](#feat-trd-model-execution-policies) | Version trading costs and venue constraints |
| `workspace.manage-accounts@1` | Workspace | Required | [`FEAT-TRD-OBSERVE_OUTCOMES`](#feat-trd-observe-outcomes) | Verify accounts, principals and sessions |
...
Existing session/account and execution-policy references, plus authorized outcome projections. Live execution records stay with their existing owners; simulations cannot mutate them.

| Evidence | Owning feature | Partition / ownership class | Driver binding | Retention / read boundary |
| --- | --- | --- | --- | --- |
| PHASE0_BOUND | [`FEAT-TRD-MANAGE_EXECUTION_SESSIONS`](#feat-trd-manage-execution-sessions) | Feature-owned semantic state | Existing declared driver; no new database selected. | Retain committed evidence across deactivate/reactivate; purge only through explicit authorization, dependency/reference checks and recorded disposition. |
| PHASE0_BOUND | [`FEAT-TRD-MODEL_EXECUTION_POLICIES`](#feat-trd-model-execution-policies) | Feature-owned semantic state | Existing declared driver; no new database selected. | Retain committed evidence across deactivate/reactivate; purge only through explicit authorization, dependency/reference checks and recorded disposition. |
| PHASE0_BOUND | [`FEAT-TRD-OBSERVE_OUTCOMES`](#feat-trd-observe-outcomes) | No new durable business partition selected here | No new business driver. | Release local buffers/caches on teardown. Retaining an artifact requires the declared custody capability and an explicit owner policy. |
...
| --- | --- | --- |
| Package | Domain boundary | `app/services/trading/` |
| Module folder | Composable feature owner | `app/services/trading/manage_execution_sessions/` — [`FEAT-TRD-MANAGE_EXECUTION_SESSIONS`](#feat-trd-manage-execution-sessions) |
| File | Manifest, strict configuration, lifecycle or focused use case | `manifest.py`, `config.py`, `feature.py`, focused logic module |
| Class / function / method | One or more traced requirement behaviors | `FR-TRC-TRD-MANAGE_EXECUTION_SESSIONS-001` and its acceptance oracle |
...
| --- | --- | --- | --- | --- | --- | --- |
| [`FEAT-TRD-MANAGE_EXECUTION_SESSIONS`](#feat-trd-manage-execution-sessions) | Preserve governed live/simulation session boundaries | `app/services/trading/manage_execution_sessions/` | U2 | 2 | 1 | NOT_REVALIDATED |
| [`FEAT-TRD-MODEL_EXECUTION_POLICIES`](#feat-trd-model-execution-policies) | Provide pure shared execution-policy descriptors | `app/services/trading/model_execution_policies/` | U2 | 2 | 1 | NOT_REVALIDATED |
| [`FEAT-TRD-OBSERVE_OUTCOMES`](#feat-trd-observe-outcomes) | Expose matured execution outcomes read-only | `app/services/trading/observe_outcomes/` | U8 | 2 | 1 | NOT_REVALIDATED |

```text
app/services/trading/
├── README.md  # this domain target registry
├── __init__.py  # docstring only
├── manage_execution_sessions/  # FEAT-TRD-MANAGE_EXECUTION_SESSIONS
├── model_execution_policies/  # FEAT-TRD-MODEL_EXECUTION_POLICIES
└── observe_outcomes/  # FEAT-TRD-OBSERVE_OUTCOMES
```
...
### Domain-local reading sequence — Bind simulation execution policy

**Input boundary:** Explicit simulation environment/account policy and pinned venue rules.

**Output boundary:** Reproducible intent/position semantics with no access to a live broker client.

**Capabilities to inspect:** [`FEAT-TRD-MANAGE_EXECUTION_SESSIONS`](#feat-trd-manage-execution-sessions) → [`FEAT-TRD-MODEL_EXECUTION_POLICIES`](#feat-trd-model-execution-policies).
...
**Participants:** [`FEAT-AGT-CALIBRATE_OUTCOMES`](../agentic/README.md#feat-agt-calibrate-outcomes), [`FEAT-TRD-OBSERVE_OUTCOMES`](#feat-trd-observe-outcomes), [`FEAT-ANA-COMPUTE_METRICS`](../analytics/README.md#feat-ana-compute-metrics), [`FEAT-AGT-MANAGE_CLAIMS`](../agentic/README.md#feat-agt-manage-claims), [`FEAT-AGT-EVALUATE_PROFILES`](../agentic/README.md#feat-agt-evaluate-profiles), [`FEAT-AGT-GOVERN_TOOL_CALLS`](../agentic/README.md#feat-agt-govern-tool-calls).

**This domain contributes:** [`FEAT-TRD-OBSERVE_OUTCOMES`](#feat-trd-observe-outcomes). Every participating feature’s scoped FR/local-NFR obligations remain binding.
...
<a id="feat-trd-manage-execution-sessions"></a>
### 4.1 `manage_execution_sessions/` — `FEAT-TRD-MANAGE_EXECUTION_SESSIONS`

> **Feature ID:** `FEAT-TRD-MANAGE_EXECUTION_SESSIONS`
> **Domain:** `trading`
> **Status:** `Partial` — target documented; full-scope implementation evidence **NOT_REVALIDATED**.
> **Selected owner:** `app/services/trading/manage_execution_sessions/`
...
Preserve governed live/simulation session boundaries. Deliver the bounded behaviors in the FR table through this feature’s declared public capability; retain the domain boundary in §1.

#### Capability Declarations

**Provides:** `trading.manage-execution-sessions@1`.

**Required capabilities:**

None (root with respect to the register’s required-provider graph)..

**Optional / operation-gated capabilities:** the complete scoped provider table in the [source feature card](../../../docs/dev/Feature_Requirement_Traceability_Register.md#feat-trd-manage-execution-sessions) is normative. Declare each applicable key separately from required startup dependencies. Absence must affect only the operations requiring it, with the exact recorded denial/unavailable behavior.

**Public contract target:** [`app/contracts/trading/manage_execution_sessions.py`](../../contracts/trading/manage_execution_sessions.py). **Literal protocol/DTO/operation symbols:** the selected target, operation scope, request/result union and typed failure semantics in this card are frozen; exact existing symbols are inventoried in `docs/dev/evidence/contract-bindings.json`, and a planned contract retains this binding without claiming runtime certification.
...
| --- | --- | --- |
| Capability binding `trading.manage-execution-sessions@1` | FEAT-TRD-MANAGE_EXECUTION_SESSIONS | Unregister when the reconciler closes the feature scope. |
| Tasks, listeners, requests, workers, leases or buffers actually created by this feature | FEAT-TRD-MANAGE_EXECUTION_SESSIONS | Register exact disposers; cancel/await/release on failure or removal. A pure provider must not create unnecessary effects. |
...
**Records:** Only records/artifact references required by the functional requirements below; Existing session/account and execution-policy references, plus authorized outcome projections. Live execution records stay with their existing owners; simulations cannot mutate them.
...
| manage_execution_sessions.py | Focused production domain-logic module | Compatible contract operation binding. |
...
| --- | --- | --- | --- | --- |
| PENDING | `FR-TRC-TRD-MANAGE_EXECUTION_SESSIONS-001` | Expose immutable simulation-compatible account/mode/position-policy references for governed evaluations. | `AT-TRD-MANAGE_EXECUTION_SESSIONS-001` | A live session cannot be selected through a simulation-only request; missing policy prerequisites fail before execution. |
| PENDING | `FR-TRC-TRD-MANAGE_EXECUTION_SESSIONS-002` | Keep deterministic controls usable when Agentic or the research UI is removed. | `AT-TRD-MANAGE_EXECUTION_SESSIONS-002` | Disable the entire Agentic domain: existing Trading risk checks, session controls and broker serialization remain unchanged. |
...
| --- | --- | --- | --- | --- |
| PENDING | `NFR-TRC-TRD-MANAGE_EXECUTION_SESSIONS-001` | Removing FEAT-TRD-MANAGE_EXECUTION_SESSIONS withdraws only its declared contribution; no dependent operation may silently select a substitute provider. | `ATN-TRD-MANAGE_EXECUTION_SESSIONS-001` | Disable and physically remove manage_execution_sessions; its operation is unavailable, unrelated capabilities remain usable, and retained source objects are unchanged. |

#### Applicable Shared NFRs, Catalogue and Source Bindings

[source feature card](../../../docs/dev/Feature_Requirement_Traceability_Register.md#feat-trd-manage-execution-sessions): the exact “Applicable shared NFRs,” “Detailed catalogue families,” “Catalogue entries, algorithms and controls delivered,” “Source scope / Original source IDs,” and operation-gated provider sections are incorporated for **this feature only**. These sections remain normative; an acceptance manifest must enumerate the actual linked IDs/entries and evidence, not just cite this paragraph. No source algorithm, control, permission or release condition is weakened by this domain projection.

#### Acceptance Tests and Evidence

| Acceptance family | Intended test owner | Required evidence state |
| --- | --- | --- |
| Every AT ID in this card | `tests/services/trading/manage_execution_sessions/test_traceability.py` | PENDING: bind an actual named test and assertion to each oracle. |
| Every ATN ID in this card | `tests/services/trading/manage_execution_sessions/test_lifecycle.py` | PENDING: lifecycle/resource/numerical evidence as applicable. |
| Contract → provider → composition → Interfaces → UI → end-to-end | `docs/dev/SQX/evidence/features/FEAT-TRD-MANAGE_EXECUTION_SESSIONS/acceptance.json` | All six stages NOT_REVALIDATED; justify each genuinely inapplicable stage. |
...
```powershell
uv run --frozen python -m app.services.trading.manage_execution_sessions._usage
```
...
Disable and physically remove the actual reconciled owner of `FEAT-TRD-MANAGE_EXECUTION_SESSIONS`. Withdraw `trading.manage-execution-sessions@1` and all its scoped contributions. Required dependents become BLOCKED/unavailable through their declared contract; operation-gated consumers disable only affected operations. Retain committed source objects and evidence; no cross-provider substitute or implicit purge is permitted. Exercise the local ATN oracles and §7 gates before restoring the feature.

---

<a id="feat-trd-model-execution-policies"></a>
### 4.2 `model_execution_policies/` — `FEAT-TRD-MODEL_EXECUTION_POLICIES`

> **Feature ID:** `FEAT-TRD-MODEL_EXECUTION_POLICIES`
> **Domain:** `trading`
> **Status:** `Partial` — target documented; full-scope implementation evidence **NOT_REVALIDATED**.
> **Selected owner:** `app/services/trading/model_execution_policies/`
> **First release milestone:** `U2`; execution order remains in the [Phased Feature Implementation Plan](../../../docs/dev/Phased_Feature_Implementation_Plan.md).

#### Purpose

Provide pure shared execution-policy descriptors. Deliver the bounded behaviors in the FR table through this feature’s declared public capability; retain the domain boundary in §1.

#### Capability Declarations

**Provides:** `trading.execution-policies@1`.

**Required capabilities:**

`catalogue.define-trading-rules@1` — [`FEAT-CAT-DEFINE_TRADING_RULES`](../catalogue/README.md#feat-cat-define-trading-rules).

**Optional / operation-gated capabilities:** the complete scoped provider table in the [source feature card](../../../docs/dev/Feature_Requirement_Traceability_Register.md#feat-trd-model-execution-policies) is normative. Declare each applicable key separately from required startup dependencies. Absence must affect only the operations requiring it, with the exact recorded denial/unavailable behavior.

**Public contract target:** [`app/contracts/trading/execution_policies.py`](../../contracts/trading/execution_policies.py). **Literal protocol/DTO/operation symbols:** the selected target, operation scope, request/result union and typed failure semantics in this card are frozen; exact existing symbols are inventoried in `docs/dev/evidence/contract-bindings.json`, and a planned contract retains this binding without claiming runtime certification.
...
**Feature-specific parameter/limit obligations:** `FR-TRC-TRD-MODEL_EXECUTION_POLICIES-001`, `FR-TRC-TRD-MODEL_EXECUTION_POLICIES-002`. Their full text and test oracles below are binding; this list is an index, not a reduced schema.

#### Runtime Effects & Scope Disposal

| Effect | Owner | Disposal mechanism |
| --- | --- | --- |
| Capability binding `trading.execution-policies@1` | FEAT-TRD-MODEL_EXECUTION_POLICIES | Unregister when the reconciler closes the feature scope. |
| Tasks, listeners, requests, workers, leases or buffers actually created by this feature | FEAT-TRD-MODEL_EXECUTION_POLICIES | Register exact disposers; cancel/await/release on failure or removal. A pure provider must not create unnecessary effects. |
| Accepted owner work and committed records | Their declared semantic owner | Observation cleanup does not secretly cancel accepted work or purge committed evidence; use explicit owner commands. |
...
| config.py | Strict typed configuration with unknown-key validation | Compatible FeatureConfig.from_dict() binding; no invented accepted keys. |
| feature.py | Scoped mount adapter and zero-argument factory | create_feature(); mount through FeatureContext/FeatureScope. |
| model_execution_policies.py | Focused production domain-logic module | Compatible contract operation binding. |
| _persistence.py | Optional owner of all feature-local database operations when durable state is required | Declared storage operations through public Workspace persistence contracts; no raw connection, policy, authorization or orchestration. |
| _usage.py | Required bounded offline usage scenarios and executable __main__ harness | _run_usage_example(); scenarios map to every applicable FR without owning business logic. |
...
| Status | Requirement ID | Responsibility / required behavior | Acceptance ID | Expected result |
| --- | --- | --- | --- | --- |
| PENDING | `FR-TRC-TRD-MODEL_EXECUTION_POLICIES-001` | Expose versioned netting/hedging, ownership, duplicate/replace/reversal and permitted transition semantics as typed pure descriptors. | `AT-TRD-MODEL_EXECUTION_POLICIES-001` | A simulator request cannot widen strategy/account scope; duplicate and replacement fixtures use the same owner-declared policy. |
| PENDING | `FR-TRC-TRD-MODEL_EXECUTION_POLICIES-002` | Qualify native-compatible pure helpers with exact arithmetic and explicit input/state bounds. | `AT-TRD-MODEL_EXECUTION_POLICIES-002` | Reference/native fixtures agree without network calls, broker credentials or per-tick capability resolution. |
...
| Status | Requirement ID | Quality / removal constraint | Acceptance ID | Expected result |
| --- | --- | --- | --- | --- |
| PENDING | `NFR-TRC-TRD-MODEL_EXECUTION_POLICIES-001` | Removing FEAT-TRD-MODEL_EXECUTION_POLICIES withdraws only its declared contribution; no dependent operation may silently select a substitute provider. | `ATN-TRD-MODEL_EXECUTION_POLICIES-001` | Disable and physically remove model_execution_policies; its operation is unavailable, unrelated capabilities remain usable, and retained source objects are unchanged. |

#### Applicable Shared NFRs, Catalogue and Source Bindings

[source feature card](../../../docs/dev/Feature_Requirement_Traceability_Register.md#feat-trd-model-execution-policies): the exact “Applicable shared NFRs,” “Detailed catalogue families,” “Catalogue entries, algorithms and controls delivered,” “Source scope / Original source IDs,” and operation-gated provider sections are incorporated for **this feature only**. These sections remain normative; an acceptance manifest must enumerate the actual linked IDs/entries and evidence, not just cite this paragraph. No source algorithm, control, permission or release condition is weakened by this domain projection.

#### Acceptance Tests and Evidence

| Acceptance family | Intended test owner | Required evidence state |
| --- | --- | --- |
| Every AT ID in this card | `tests/services/trading/model_execution_policies/test_traceability.py` | PENDING: bind an actual named test and assertion to each oracle. |
| Every ATN ID in this card | `tests/services/trading/model_execution_policies/test_lifecycle.py` | PENDING: lifecycle/resource/numerical evidence as applicable. |
| Contract → provider → composition → Interfaces → UI → end-to-end | `docs/dev/SQX/evidence/features/FEAT-TRD-MODEL_EXECUTION_POLICIES/acceptance.json` | All six stages NOT_REVALIDATED; justify each genuinely inapplicable stage. |
...
```powershell
uv run --frozen python -m app.services.trading.model_execution_policies._usage
```
...
Disable and physically remove the actual reconciled owner of `FEAT-TRD-MODEL_EXECUTION_POLICIES`. Withdraw `trading.execution-policies@1` and all its scoped contributions. Required dependents become BLOCKED/unavailable through their declared contract; operation-gated consumers disable only affected operations. Retain committed source objects and evidence; no cross-provider substitute or implicit purge is permitted. Exercise the local ATN oracles and §7 gates before restoring the feature.

---

<a id="feat-trd-observe-outcomes"></a>
### 4.3 `observe_outcomes/` — `FEAT-TRD-OBSERVE_OUTCOMES`

> **Feature ID:** `FEAT-TRD-OBSERVE_OUTCOMES`
> **Domain:** `trading`
> **Status:** `Partial` — target documented; full-scope implementation evidence **NOT_REVALIDATED**.
> **Selected owner:** `app/services/trading/observe_outcomes/`
...
Expose matured execution outcomes read-only. Deliver the bounded behaviors in the FR table through this feature’s declared public capability; retain the domain boundary in §1.

#### Capability Declarations

**Provides:** `trading.outcome-evidence@1`.

**Required capabilities:**

`workspace.manage-accounts@1` — [`FEAT-WS-MANAGE_ACCOUNTS`](../workspace/README.md#feat-ws-manage-accounts).

**Optional / operation-gated capabilities:** the complete scoped provider table in the [source feature card](../../../docs/dev/Feature_Requirement_Traceability_Register.md#feat-trd-observe-outcomes) is normative. Declare each applicable key separately from required startup dependencies. Absence must affect only the operations requiring it, with the exact recorded denial/unavailable behavior.

**Public contract target:** [`app/contracts/trading/outcome_evidence.py`](../../contracts/trading/outcome_evidence.py). **Literal protocol/DTO/operation symbols:** the selected target, operation scope, request/result union and typed failure semantics in this card are frozen; exact existing symbols are inventoried in `docs/dev/evidence/contract-bindings.json`, and a planned contract retains this binding without claiming runtime certification.
...
**Feature-specific parameter/limit obligations:** `FR-TRC-TRD-OBSERVE_OUTCOMES-001`. Their full text and test oracles below are binding; this list is an index, not a reduced schema.

#### Runtime Effects & Scope Disposal

| Effect | Owner | Disposal mechanism |
| --- | --- | --- |
| Capability binding `trading.outcome-evidence@1` | FEAT-TRD-OBSERVE_OUTCOMES | Unregister when the reconciler closes the feature scope. |
| Tasks, listeners, requests, workers, leases or buffers actually created by this feature | FEAT-TRD-OBSERVE_OUTCOMES | Register exact disposers; cancel/await/release on failure or removal. A pure provider must not create unnecessary effects. |
| Accepted owner work and committed records | Their declared semantic owner | Observation cleanup does not secretly cancel accepted work or purge committed evidence; use explicit owner commands. |
...
| config.py | Strict typed configuration with unknown-key validation | Compatible FeatureConfig.from_dict() binding; no invented accepted keys. |
| feature.py | Scoped mount adapter and zero-argument factory | create_feature(); mount through FeatureContext/FeatureScope. |
| observe_outcomes.py | Focused production domain-logic module | Compatible contract operation binding. |
| _persistence.py | Optional owner of all feature-local database operations when durable state is required | Declared storage operations through public Workspace persistence contracts; no raw connection, policy, authorization or orchestration. |
| _usage.py | Required bounded offline usage scenarios and executable __main__ harness | _run_usage_example(); scenarios map to every applicable FR without owning business logic. |
...
| Status | Requirement ID | Responsibility / required behavior | Acceptance ID | Expected result |
| --- | --- | --- | --- | --- |
| PENDING | `FR-TRC-TRD-OBSERVE_OUTCOMES-001` | Resolve execution outcome references with account scope, horizon maturity, costs, revisions and integrity metadata. | `AT-TRD-OBSERVE_OUTCOMES-001` | An open horizon, ambiguous match or unauthorized account cannot produce a mature outcome. |
| PENDING | `FR-TRC-TRD-OBSERVE_OUTCOMES-002` | Expose no mutation, order, credential, kill-switch or deployment operation to an Agentic consumer. | `AT-TRD-OBSERVE_OUTCOMES-002` | Capability/schema negative tests show read projections only; removing this feature leaves non-Trading calibration routes available. |
...
| Status | Requirement ID | Quality / removal constraint | Acceptance ID | Expected result |
| --- | --- | --- | --- | --- |
| PENDING | `NFR-TRC-TRD-OBSERVE_OUTCOMES-001` | Removing FEAT-TRD-OBSERVE_OUTCOMES withdraws only its declared contribution; no dependent operation may silently select a substitute provider. | `ATN-TRD-OBSERVE_OUTCOMES-001` | Disable and physically remove observe_outcomes; its operation is unavailable, unrelated capabilities remain usable, and retained source objects are unchanged. |

#### Applicable Shared NFRs, Catalogue and Source Bindings

[source feature card](../../../docs/dev/Feature_Requirement_Traceability_Register.md#feat-trd-observe-outcomes): the exact “Applicable shared NFRs,” “Detailed catalogue families,” “Catalogue entries, algorithms and controls delivered,” “Source scope / Original source IDs,” and operation-gated provider sections are incorporated for **this feature only**. These sections remain normative; an acceptance manifest must enumerate the actual linked IDs/entries and evidence, not just cite this paragraph. No source algorithm, control, permission or release condition is weakened by this domain projection.

#### Acceptance Tests and Evidence

| Acceptance family | Intended test owner | Required evidence state |
| --- | --- | --- |
| Every AT ID in this card | `tests/services/trading/observe_outcomes/test_traceability.py` | PENDING: bind an actual named test and assertion to each oracle. |
| Every ATN ID in this card | `tests/services/trading/observe_outcomes/test_lifecycle.py` | PENDING: lifecycle/resource/numerical evidence as applicable. |
| Contract → provider → composition → Interfaces → UI → end-to-end | `docs/dev/SQX/evidence/features/FEAT-TRD-OBSERVE_OUTCOMES/acceptance.json` | All six stages NOT_REVALIDATED; justify each genuinely inapplicable stage. |
...
```powershell
uv run --frozen python -m app.services.trading.observe_outcomes._usage
```
...
Disable and physically remove the actual reconciled owner of `FEAT-TRD-OBSERVE_OUTCOMES`. Withdraw `trading.outcome-evidence@1` and all its scoped contributions. Required dependents become BLOCKED/unavailable through their declared contract; operation-gated consumers disable only affected operations. Retain committed source objects and evidence; no cross-provider substitute or implicit purge is permitted. Exercise the local ATN oracles and §7 gates before restoring the feature.
...
```powershell
uv run --frozen pytest --no-cov tests/services/trading/manage_execution_sessions
uv run --frozen ruff format --check .
uv run --frozen ruff check .
uv run --frozen mypy
uv run --frozen python scripts/architecture_check.py
uv run --frozen python scripts/validate_feature_docs.py
uv run --frozen python scripts/verify_feature_removal.py --feature FEAT-TRD-MANAGE_EXECUTION_SESSIONS --report removal-report.json
```
...
Select the intended simulation account/session policy explicitly. Do not change a live route or duplicate account state when a research evaluation starts. Environment and account scope must survive retries and restarts.

<a id="trd-policy"></a>
### 9.2 TRD-POLICY

Model legal intent transitions, netting/hedging or ownership rules, and permitted price/quantity behavior as typed deterministic policies consumed by Simulator and compatible execution consumers. Sharing a policy contract does not share a live broker object.

<a id="trd-outcomes"></a>
### 9.3 TRD-OUTCOMES

Return only authorized, owner-authored outcomes after the requested observation horizon has closed. Preserve the immutable forecast/claim and observation rule; calibration never rewrites the original outcome evidence.

<a id="trd-safety"></a>
### 9.4 TRD-SAFETY

No feature in this selected advisory/research scope grants Agentic credentials, order construction, kill-switch clearing or deployment. Removing Agentic or its outcome observer leaves existing deterministic Trading/Risk safety controls intact.
