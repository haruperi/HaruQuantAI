...
**Rules:** No provider SDK or Broker class import; sim, paper, and live share this path and differ only by facts read through Broker root getter functions. The dispatch path is `async`: Broker root mutation functions are awaited. No synchronous bridge is permitted inside a live event loop.
