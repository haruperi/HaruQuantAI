...
Trading orchestrates live and demo evaluation, converts independently approved risk decisions into deterministic order intents, and executes those intents through the selected `sim`, `demo`, or `live` route. It owns execution safety, receipts, reconciliation, monitoring evidence, and emergency controls after Risk approval. It fails closed whenever route authority, policy, state, or evidence cannot be proven.

### Owns

- Live and demo runtime orchestration through public Data, Indicators, Strategy, and Risk APIs.
...
| `BrokerAdapter` (mutation + execution-read traits)                                                     | `v1`  | Brokers    | Sole demo/live mutation boundary (`TradeExecutionProvider`) plus execution-state and account reads needed for dispatch and reconciliation; mutation operations are Trading-only.                                                                                              |
...
| `BrokerConnectionConfig`                                                                               | `v1`  | Brokers    | UI/API-composed provider/account/environment configuration with already-resolved in-memory credentials; demo and live differ only by its environment/credentials.                                                                                                               |
| `IndicatorSeries`                                                                                      | `v1`  | Indicators | Current deterministic indicator evidence during live/demo orchestration.                                                                                                                                                                                                        |
...
This section records the converged contract. `FR-TRD-093`, `FR-TRD-094`, and
`FR-TRD-113` and `FR-TRD-095` are implemented.
...
`TradingRequestV2` bound to explicit fill/time policies and the exact provider
specification checksum. It refuses to build unless Risk genuinely approved: decision
...
demo construct it from monotonic wall time; Simulation supplies scheduler
...
every other route forbids it (Phase 10b). The Brokers-owned simulation adapter
...
pairs remain the `P-SYS-002` table (`simulation`→`sim`, `demo`→`demo`,
`live`→`live`; an incompatible pair fails closed at initialization). The
Phase 6a v1→v2 order-policy migration is binding: the builder never invents
either policy dimension from legacy `time_in_force` material.

**Status handoff.** `FR-TRD-096` was completed in Phase 10b;
`FR-TRD-093`, `FR-TRD-094`, and `FR-TRD-113` were completed in Phase 14a;
`FR-TRD-095` was completed in Phase 15a.
...
| Completed | `FEAT-TRD-02` State and Deterministic Projections | `state/`          | Existing state API plus lifecycle/fill operations and route-neutral authoritative deal-to-position event correlation with restart-safe watermarks; exact declarations: Section 4.2 | Section 4.2 functional requirements including `FR-TRD-085`, `FR-TRD-086`, `FR-TRD-101`–`FR-TRD-103`, `FR-TRD-107` | `tests/trading/usage/features/02_state.py` |
...
| Completed | `FEAT-TRD-05` Reconciliation and Retry Guard      | `reconciliation/` | `create_authority_resolution`, `create_authority_snapshot`, `create_reconciliation_report`, `compare_authority_state`, `reconcile_execution_state`, `resolve_unknown_outcome`; authority-event reconciliation uses State-owned route-neutral projection/watermark operations | Section 4.5 functional requirements including `FR-TRD-108`, `FR-TRD-110` | `tests/trading/usage/features/05_reconciliation.py` |
...
├── live/                               # Live/demo session lifecycle and gates
│   ├── __init__.py
│   ├── README.md
...
│   └── runtime.py                      # Live/demo evaluation-cycle orchestration
...
- Explicit route: `sim`, `demo`, or `live`.
- Risk decision ID.
...
- `demo`: dispatch through the configured demo broker connection.
...
- A Brokers-owned adapter for `demo` or `live`.
...
Broker truth wins for demo/live and Simulation truth wins for `sim`.
...
**Sim vs live/demo — exactly which gates differ.** Common validation, the
...
**Broker-selection cross-rules (`_validate_route_environment`, `_validate_broker_selection`).** The sim route requires an enabled `sim` / `simulation` connection descriptor and uses its injected Brokers adapter as the sole mutation authority; every other route forbids `simulation`. Connection and
...
`route == "demo"` forbids it, and the adapter must declare contract `v1` /
...
6. Live requires the explicit `allow_live_mutations` flag; demo self-enables;
...
9. Pre-mutation audit is fail-closed on live/demo (`AUDIT_FAILED` stops
   dispatch).
...
| `WF-TRD-014` | `tests/trading/usage/workflows/wf_trd_014_run_live_demo_evaluation_cycle.py`         |
...
| Completed | Supporting | `WF-TRD-014` | Cross-domain | Run a live/demo evaluation cycle            | Live/demo market update or scheduled evaluation trigger                 | Neutral signal ends the cycle, or an approved`RiskDecision` enters the validate/gate/dispatch path                                  | `FR-TRD-065 → FR-TRD-012 → FR-TRD-036`                             |
...
6. Broker truth wins for demo/live; Simulation truth wins for sim —
...
### `WF-TRD-014` — Run a live/demo evaluation cycle

**Scope:** `Cross-domain`
**System workflow:** `SYS-WF-002`
**Input boundary:** A live/demo market update or scheduled strategy evaluation under an authenticated principal.
...
| Completed | `execution_positions.py` | Maintain validated current execution positions, correlate durable receipts through Broker deal/position authority, block unverifiable state, and preserve receipt watermarks across restart | Function-only memory-store, create/read/snapshot/set/transition, reconcile, serialize/restore facades | **Standard library:** `decimal`, `threading`, `typing` **Required third-party:** `pydantic>=2.13.4` **Local:** Brokers package-root reads; Trading errors; Utils logger |
...
| Completed | `FR-TRD-085` | Treat a durable exposure-changing receipt only as a refresh trigger; retrieve each provider deal and then the correlated position from Brokers authority. | `reconcile_execution_position_receipt` | Broker read; atomic process-local projection mutation | `TradingError` for invalid inputs; unverifiable facts become `UNKNOWN` | **Usage:** `tests/trading/usage/features/02_state.py::fr_trd_085()` **Unit:** `tests/trading/unit/state/test_execution_position_correlation.py` |
| Completed | `FR-TRD-086` | Never treat receipt quantities or identifiers as position authority; missing, conflicting, gapped, or disagreeing deal/position evidence produces blocking `UNKNOWN`. | `reconcile_execution_position_receipt` | Broker read; atomic process-local projection mutation | `UNKNOWN` state blocks position verbs with reconciliation required | **Usage:** `tests/trading/usage/features/02_state.py::fr_trd_086()` **Integration:** `tests/trading/integration/test_execution_position_unverifiable.py` |
| Completed | `FR-TRD-101` | Correlate one or many order receipts to the Broker deal `position_id`, supporting netting many orders into one position and distinct hedging identities. | `reconcile_execution_position_receipt` | Broker read; position projection mutation | Conflicting deal identities become `UNKNOWN` | **Usage:** `tests/trading/usage/features/02_state.py::fr_trd_101()` **Integration:** `tests/trading/integration/test_receipt_deal_position_refresh.py` |
| Completed | `FR-TRD-102` | Project partial close, full close, and reversal exclusively from the latest verified authority quantity, state, side, price, and source sequence. | `reconcile_execution_position_receipt` | Position projection mutation | Invalid or regressed authority becomes `UNKNOWN` | **Usage:** `tests/trading/usage/features/02_state.py::fr_trd_102()` **Integration:** `tests/trading/integration/test_receipt_deal_position_refresh.py` |
| Completed | `FR-TRD-103` | Deduplicate durable receipt refresh and serialize its receipt-to-position watermark so restart cannot repeat or reinterpret completed correlation. | `serialize_execution_position_store`, `restore_execution_position_store` | Deterministic state serialization/restore | Malformed state is rejected | **Usage:** `tests/trading/usage/features/02_state.py::fr_trd_103()` **Unit:** `tests/trading/unit/state/test_execution_position_correlation.py` |
| Completed | `FR-TRD-107` | Correlate complete exposure-changing deal events to position authority through one route-neutral algorithm for Simulation, demo, and live, including partial/full close, reversal, many-to-one netting, protection, and terminal liquidation. | `create_position_authority_event`, `reconcile_position_authority_event` | Atomic process-local projection and watermark mutation | Missing/conflicting identity or evidence fails closed | **Usage:** `tests/trading/usage/features/02_state.py::fr_trd_107()` **Integration:** `tests/trading/integration/test_two_route_position_reconciliation.py` |
| Completed | `FR-TRD-108` | Order authority events by source sequence; suppress exact duplicates and late events without reapplication, and block forward gaps or same-sequence identity conflicts until authority recovery. | `reconcile_position_authority_event`, `get_position_authority_watermark` | Atomic watermark mutation | `TradingError(RECONCILIATION_REQUIRED)` on gaps/conflicts | **Usage:** `tests/trading/usage/features/05_reconciliation.py::fr_trd_108()` **Unit:** `tests/trading/unit/reconciliation/test_authority_event_ordering.py` |
| Completed | `FR-TRD-109` | Preserve foreign/manual exposure as blocking `UNKNOWN` orphan state unless exact active ownership already exists; never infer or assign an owner from authority activity. | `reconcile_position_authority_event`, `detect_orphaned_trade` | Position projection mutation only | `ORPHAN_BLOCKED` disposition | **Usage:** `tests/trading/usage/features/11_trade_ownership.py::fr_trd_109()` **Integration:** `tests/trading/integration/test_foreign_orphan_block.py` |
| Completed | `FR-TRD-110` | Serialize the authority-event watermark, deal-position correlation, and position projection together so restart converges and exact replay performs no second projection or mutation. | `serialize_execution_position_store`, `restore_execution_position_store`, `reconcile_position_authority_event` | Deterministic state serialization/restore | Malformed or conflicting state is rejected | **Usage:** `tests/trading/usage/features/05_reconciliation.py::fr_trd_110()` **Integration:** `tests/trading/integration/test_reconciliation_restart.py` |
| Completed | `FR-TRD-078` | Create one validated protective stop/target bracket carrying exact quantity, OCO identity, Risk decision, and source sequence. | `create_protective_order_plan` | None | Validation failure | **Usage:** `tests/trading/usage/features/10_protective_orders.py::fr_trd_078()` **Unit:** `tests/trading/unit/test_operational_features.py` |
| Completed | `FR-TRD-079` | Transport protective-order plans as validated JSON-safe `v1` mappings through the package boundary. | `build_protective_order_plan`, `parse_protective_order_plan` | None | Invalid mapping | **Usage:** `tests/trading/usage/features/10_protective_orders.py::fr_trd_079()` **Unit:** `tests/trading/unit/test_operational_features.py` |
| Completed | `FR-TRD-080` | Prove acknowledged protection covers the exact open quantity and allow only newer residual reductions; uncertain coverage is `UNKNOWN` and exposure-increasing resize is rejected. | `verify_protective_order_coverage`, `resize_protective_orders` | None | `TradingError` | **Usage:** `tests/trading/usage/features/10_protective_orders.py::fr_trd_080()` **Unit:** `tests/trading/unit/test_operational_features.py` |
| Completed | `FR-TRD-081` | Build and parse validated `v1` ownership evidence for player, supervised-automation, or automated owners. | `build_trade_ownership`, `parse_trade_ownership` | None | Invalid mapping | **Usage:** `tests/trading/usage/features/11_trade_ownership.py::fr_trd_081()` **Unit:** `tests/trading/unit/test_operational_features.py` |
| Completed | `FR-TRD-082` | Assign one unambiguous active owner to an execution position and reject duplicate active assignment. | `create_trade_ownership_registry`, `assign_trade_ownership`, `get_trade_ownership` | Process-local ownership registry mutation | `TradingError` | **Usage:** `tests/trading/usage/features/11_trade_ownership.py::fr_trd_082()` **Unit:** `tests/trading/unit/test_operational_features.py` |
| Completed | `FR-TRD-083` | Treat missing or released ownership evidence as an orphaned trade and never infer an owner. | `detect_orphaned_trade` | None | None | **Usage:** `tests/trading/usage/features/11_trade_ownership.py::fr_trd_083()` **Unit:** `tests/trading/unit/test_operational_features.py` |
...
| Completed | Approved provider/security matrix    | contract    | Brokers README owns the`BrokerAdapter`/`BrokerFeatureFlags` contract | Yes for demo/live | `validate_adapter_capability()` | Missing API/schema/security capability blocks mutation.                                                                                             |
...
| Completed | `FR-TRD-031` | The system shall dispatch exactly one approved intent through the selected Brokers adapter for sim, demo, or live. Broker environment/account reference come only from injected `BrokerConnectionConfig`; order type/unit/instructions come only from `OrderIntent`; target identities come only from Trading state; timeout and receipt time come from injected policy/clock dependencies. | `async dispatch_order_intent(intent, connection, broker_adapter, *, operation_timeout_seconds, clock)` | One Broker-channel mutation | `TradingError`: authority unavailable, scope mismatch, timeout, or unsafe response | **Usage:** `tests/trading/usage/features/04_routing.py::fr_trd_031()` **Unit:** `tests/trading/unit/routing/test_dispatcher.py::test_dispatch_has_single_mutation_boundary()` |
...
| Completed | `FR-TRD-096` | The `sim` route shall require the exact Brokers `sim` / `simulation` connection identity; demo and live shall forbid `simulation`. | `dispatch_order_intent` | None before validated dispatch | `TradingError`: route/environment mismatch | **Usage:** `tests/trading/usage/features/04_routing.py::fr_trd_096()` **Unit:** `tests/trading/unit/routing/test_simulation_route_selection.py` |
| Completed | `FR-TRD-088` | Route every approved mutation through the same Brokers adapter boundary and response classifier. | `dispatch_order_intent` | One selected authority mutation | Fail closed on absent or mismatched authority | **Usage:** `tests/trading/usage/features/04_routing.py::fr_trd_088()` **Integration:** `tests/trading/integration/test_two_route_action_convergence.py` |
| Completed | `FR-TRD-094` | Classify business/risk gates as shared and permit only registered transport/live-authorization safety deltas. | action gate and dispatch path | None before admission | First failed shared or route safety gate | **Usage:** `tests/trading/usage/features/04_routing.py::fr_trd_094()` **Integration:** `tests/trading/integration/test_two_route_action_convergence.py` |

**Rules:** No provider SDK or Broker class import; sim, demo, and live share this path and differ only by facts read through Broker root getter functions. The dispatch path is `async`: Broker root mutation functions are awaited. No synchronous bridge is permitted inside a live event loop.
**Implementation notes:** One broker mutation boundary and response classifier consumes the injected Brokers `BrokerAdapter`; Trading owns no Simulation callback or private Simulation import.
...
**Rules:** Live/demo prefer broker truth; sim prefers Simulation truth; comparison alone never mutates broker/simulator state.
...
**Purpose:** Own live/demo enablement, startup, recovery, deterministic gating,
...
| Completed | `FR-TRD-091` | The stateful session lifecycle shall start, report, stop, and finalize Simulation without opening a network connection or requiring live-mutation authorization. | `LiveSession.start`, `LiveSession.status`, `LiveSession.stop` | Local lifecycle state and evidence persistence | `TradingError`: invalid profile, reconciliation, or shutdown evidence | **Usage:** `tests/trading/usage/features/07_live.py::fr_trd_091()` **Integration:** `tests/trading/integration/test_simulation_session_lifecycle.py` |
...
| Completed | `runtime.py`      | Build one provider-bound approved request v2 and drive one route-neutral evaluation cycle strictly through public Data/Indicators/Strategy/Risk APIs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `build_approved_trading_request`, `run_live_evaluation_cycle`                                                                               | **Standard library:** `collections.abc`, `datetime`, `hashlib`, `typing`**Required third-party:** `pydantic>=2.13.4`**Local:** Risk contracts; orders, Trading contracts, monitoring; Utils canonical JSON/logger APIs |
...
| Completed | `FR-TRD-089` | Both live and Simulation shall invoke the same public mutation action functions after approval. | public action verb family | Selected route authority mutation and Trading persistence | `TradingError`: shared action validation/gate failure | **Usage:** `tests/trading/usage/features/08_actions.py::fr_trd_089()` **Integration:** `tests/trading/integration/test_two_route_action_convergence.py` |
| Completed | `FR-TRD-090` | Both routes shall traverse the same ordered business/risk gate roles, inputs, and results. | governed order action path | None before all shared gates pass | `TradingError`: first failed shared gate | **Usage:** `tests/trading/usage/features/08_actions.py::fr_trd_090()` **Integration:** `tests/trading/integration/test_two_route_action_convergence.py` |
| Completed | `FR-TRD-092` | The action path shall preserve provider outcomes and classify accepted, rejected, neutral, malformed, and unsupported responses identically across routes. | action verbs through `dispatch_order_intent` | Exact selected authority mutation | `TradingError`: malformed, unsupported, or unknown outcome | **Usage:** `tests/trading/usage/features/08_actions.py::fr_trd_092()` **Integration:** `tests/trading/integration/test_two_route_action_convergence.py` |
| Completed | `FR-TRD-093` | Expose one root standalone builder that consumes immutable Strategy/Risk lineage and complete provider policy evidence and returns a validated request v2. | `build_approved_trading_request` | Read-only capability/state reads | `TradingError`: incomplete approval, lineage, policy, or state | **Usage:** `tests/trading/usage/features/08_actions.py::fr_trd_093()` **Unit:** `tests/trading/unit/actions/test_approved_request_builder.py` |
| Completed | `FR-TRD-095` | The shared evaluation path shall receive its async deadline authority through a required injected dependency, with no production default or ambient runtime timeout owner. | `run_live_evaluation_cycle`, `create_monotonic_deadline_factory` | Cooperative task cancellation only | `WORKFLOW_TIMEOUT` with canonical operational evidence | **Usage:** `tests/trading/usage/features/08_actions.py::fr_trd_095()` **Unit:** `tests/trading/unit/actions/test_deadline_port.py` |
| Completed | `FR-TRD-104` | Live and demo shall construct the evaluation deadline from an injected monotonic clock. | `create_monotonic_deadline_factory` | Monotonic clock read | `TimeoutError` at the exact bound | **Usage:** `tests/trading/usage/features/07_live.py::fr_trd_104()` **Unit:** `tests/trading/unit/actions/test_deadline_port.py::test_monotonic_deadline_raises_timeout_at_bound()` |
| Completed | `FR-TRD-105` | A neutral result before its deadline shall retain the canonical no-mutation response. | `run_live_evaluation_cycle` | None | Canonical upstream error only | **Usage:** `tests/trading/usage/features/08_actions.py::fr_trd_105()` **Integration:** `tests/trading/integration/test_evaluation_deadline_equivalence.py::test_neutral_result_is_identical_with_injected_deadline()` |
| Completed | `FR-TRD-106` | Deadline expiry on every route shall emit the same timeout event lineage and public error shape. | `run_live_evaluation_cycle` | Operational event publication | `WORKFLOW_TIMEOUT` | **Usage:** `tests/trading/usage/features/08_actions.py::fr_trd_106()` **Integration:** `tests/trading/integration/test_evaluation_deadline_equivalence.py::test_timeout_after_neutral_has_canonical_evidence_shape()` |
| Completed | `FR-TRD-111` | Cooperative cancellation and non-timeout upstream failures shall not be reclassified as deadline expiry. | `run_live_evaluation_cycle` | None | Original cancellation or canonical upstream error | **Usage:** `tests/trading/usage/features/08_actions.py::fr_trd_111()` **Integration:** `tests/trading/integration/test_evaluation_deadline_equivalence.py::test_upstream_error_and_cancellation_are_not_reclassified()` |
| Completed | `FR-TRD-113` | Simulation shall consume the Brokers-root adapter authority without a private mutation callback or private Simulator import. | `TradingDependencies`, action verbs | Simulation authority mutation | `TradingError`: absent or mismatched Brokers authority | **Usage:** `tests/trading/usage/features/08_actions.py::fr_trd_113()` **Integration:** `tests/trading/integration/test_two_route_action_convergence.py` |
| Completed | `FR-TRD-114` | A sim-routed order shall carry an explicit historical `simulation_session_id` and dispatch the unchanged approved `OrderIntent` through the injected direct Simulator execution port; demo/live remain broker-routed and the two authorities may never be supplied together. | `TradingRequest`, `OrderIntent`, `dispatch_order_intent`, `TradingDependencies.simulation_execution_source` | Historical Simulator mutation | Missing session/port, mixed authority, timeout, or mismatched receipt fails closed | **Unit:** `tests/trading/unit/routing/test_simulation_route_selection.py`; `app/services/trading/routing/dispatcher.py` |

**Rules:** Neutral signals never call actions; action functions do not size, approve, promote, or translate signals. Bulk policy `max_children` bounds every inspected authority child consistently for orders and positions. Demo/live bulk children require exact per-child Risk decisions/tokens and action-policy verdicts; no cancellation or reduce-only size exemption exists. Every mutation-capable verb (`FR-TRD-013`–`FR-TRD-018`, `FR-TRD-021`–`FR-TRD-023`, `FR-TRD-050`) is `async` and awaits the dispatch path; `pause_strategy`/`resume_strategy`/`sync_positions` are `async` where they touch route authority or persistence. No synchronous broker bridge is permitted.
...
CREATE TABLE trading_positions (
    ...
    environment TEXT NOT NULL CHECK (environment IN ('demo','demo','sim','live')),
    request_id TEXT NOT NULL,
    correlation_id TEXT NOT NULL,
...
| Status    | Setting / Limit                 | Type           | Default                                                | Required    | Used by                                                           | Description                                                                                                        |
| --------- | ------------------------------- | -------------- | ------------------------------------------------------ | ----------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| Completed | `EXECUTION_ROUTE`             | `str`        | `none`                                               | Conditional | validation, routing, live, actions                                | `none`, `sim`, `demo`, or `live`; action calls require sim/demo/live and compatible `RUNTIME_PROFILE`. |
...
- UI/API owns workstation routes, read models, and frontend panels. Trading intentionally contains no UI or HTTP behavior.
...
- [X] The workspace-mounted Trading panel exposes complete governed submit, cancel, and close inputs, defaults to demo, requires explicit authority references, and re-locks after each attempt. `app/ui/src/components/workflow/trading.tsx:121`
