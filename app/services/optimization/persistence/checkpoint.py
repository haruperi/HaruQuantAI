...
def load_checkpoint_with_fallback(
    ...
    """Attempt to load a checkpoint, falling back to alternative paths on failure.

    Tries ``file_path`` first, then each path in ``fallback_paths`` in order.
    Returns ``None`` when all paths fail or do not exist.

    Args:
        file_path: Primary checkpoint path to attempt first.
        fallback_paths: Ordered list of fallback paths to try when the
            primary path fails or is missing.
        base_dir: Base directory limit applied to all paths.

    Returns:
        dict[str, Any] | None: Decoded checkpoint dictionary from the
            first successful load, or ``None`` if all paths fail.
    """
    for path in [file_path, *fallback_paths]:
        try:
            return load_checkpoint(path, base_dir)
