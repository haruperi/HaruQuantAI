...
class InMemoryOptimizationRepository(OptimizationRepository):
    ...
    def update_progress(self, run_id: str, progress: float, status: str) -> None:
        """Update progress fields under lock; no-ops when run does not exist.

        Idempotent: calling with the same arguments multiple times produces
...
def retry_with_backoff(
    ...
    **kwargs: Any,  # noqa: ANN401
    ...
    if last_exc is not None:
        raise last_exc
    raise OptimizationExecutionError("Action failed after retries.")
