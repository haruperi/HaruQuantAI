...
> **Status:** `Completed` — all 10 Trading Cockpit Phase 0 work packages (`TC-IMP-OPT-01`..`TC-IMP-OPT-10`) are implemented and verified. The 9 registered features remain implemented; the 7 `EXTEND` items and 3 `DEFERRED_INTEGRATION` consumer ports are code-complete with dedicated passing tests. Package promotion to `Completed` is blocked solely by a pre-existing, out-of-scope defect: `app/utils/errors/catalog.py` (unrelated in-progress Utils-domain work already present in the working tree before this pass) fails `ERROR_CATALOG_INVALID:CATEGORY_INVALID` validation, which the `UT` conformance gate's full-suite run surfaces as a single unit-test failure whenever a path formats certain `StandardResponse` error codes. See `### Trading Cockpit Phase 0 reconciliation`.
...
| --- | --- | --- | --- |
| Completed | `OptimizationStudy v1` additions | Extends `FEAT-OPT-01`/`FEAT-OPT-06`. Adds dataset/replay identity and budget fields to the study contract (`parameters/study.py:108`, `build_optimization_study`/`parse_optimization_study`; `tests/optimization/unit/test_study.py`). | `TC-IMP-OPT-01` |
| Completed (port) | Fill-model calibration | Estimate latency, slippage, partial-fill, queue, market-impact parameters from approved evidence. **Consumer port + fail-closed `NOT_CALIBRATED` fallback** in `execution/calibration.py`; **provider deferred to Simulator `TC-IMP-SIM-16`..`TC-IMP-SIM-20`** (`tests/optimization/unit/test_calibration_ports.py`). | `TC-IMP-OPT-02` |
| Completed (port) | Scenario difficulty calibration | Tune event intensity, info load, time pressure, liquidity loss, compound failures to target competence. **Consumer port + fail-closed fallback** in `execution/calibration.py`; **provider deferred to Simulator `TC-IMP-SIM-11`..`TC-IMP-SIM-15`** (`tests/optimization/unit/test_calibration_ports.py`). | `TC-IMP-OPT-03` |
| Completed | Strategy parameter studies | Extends `FEAT-OPT-03`/`FEAT-OPT-08`. Optimizes within approved strategy/instrument envelopes with WFA/OOS validation. Provider Strategy `TC-IMP-STRAT-06` now `Completed` — consumed via `evaluate_operating_envelope` in `validation/envelope_gate.py` (`tests/optimization/unit/test_envelope_gate.py`). | `TC-IMP-OPT-04` |
| Completed | Risk sensitivity analysis | Extends `FEAT-OPT-05`. Sensitivity to risk-per-trade, drawdown warnings, stress limits, exposure caps without weakening hard limits. Provider Risk `TC-IMP-RISK-01` now code-complete — consumed as JSON-safe mapping in `robustness/risk_sensitivity.py` (`tests/optimization/unit/test_risk_sensitivity.py`). | `TC-IMP-OPT-05` |
| Completed (port) | Stress-profile calibration | Calibrate shock magnitudes/dependencies while preserving transparent assumptions. **Consumer port + fail-closed `STRESS_PROFILE_UNCALIBRATED` fallback** in `robustness/stress_calibration.py`; **provider deferred to Risk `TC-IMP-RISK-12` / Research `TC-IMP-RES-06`** (`tests/optimization/unit/test_stress_calibration.py`). | `TC-IMP-OPT-06` |
| Completed | Multi-objective evaluation | Extends `FEAT-OPT-02`. Safety, process adherence, stability, execution realism, risk-adjusted performance; raw profit never the sole objective. Provider Analytics `TC-IMP-ANL-02` now `Completed` — consumed via process-score mapping in `scoring/multi_objective.py` (`tests/optimization/unit/test_multi_objective.py`). | `TC-IMP-OPT-07` |
| Completed (port) | Anti-leakage controls | Extends `FEAT-OPT-08`. Strict train/val/test splits, point-in-time data, **scenario holdouts** (consumer port + fail-closed `SCENARIO_HOLDOUT_UNAVAILABLE` fallback) in `validation/scenario_holdout.py`; **provider deferred to Simulator `TC-IMP-SIM-11`** (`tests/optimization/unit/test_scenario_holdout.py`). | `TC-IMP-OPT-08` |
| Completed | Robustness and overfit checks | Extends `FEAT-OPT-05`/`FEAT-OPT-02`. Parameter stability, perturbation tests, regime splits, multiple seeds, uncertainty intervals. Satisfied by existing `robustness/` + scoring overfit evidence; risk-sensitivity additions in `robustness/risk_sensitivity.py`. | `TC-IMP-OPT-09` |
| Completed (port) | Promotion contract | Extends `FEAT-OPT-07`. Outputs become versioned candidate profiles; require Research/Strategy/Risk approval before use. **Consumer port + fail-closed `NOT_PROMOTED` fallback** in `evidence/promotion.py`; **provider deferred to Research `TC-IMP-RES-10`** (`tests/optimization/unit/test_promotion.py`). | `TC-IMP-OPT-10` |
...
- **OD-OPT-01 — Calibration provider readiness (non-blocking).** Three work packages (`TC-IMP-OPT-02` fill-model calibration, `TC-IMP-OPT-03` scenario difficulty calibration, `TC-IMP-OPT-06` stress-profile calibration) are `DEFERRED_INTEGRATION`. Their authoritative providers do not exist yet (Simulator fill/latency/slippage/queue models `TC-IMP-SIM-16`..`20`; Simulator scenario engine `TC-IMP-SIM-11`..`15`; Risk/Research stress-shock profile `TC-IMP-RISK-12`/`TC-IMP-RES-06`). Optimization's consumer ports are implemented and Completed at the port level: each fails closed (`NOT_CALIBRATED` / `STRESS_PROFILE_UNCALIBRATED` / `SCENARIO_HOLDOUT_UNAVAILABLE`) when its provider is absent and never returns an inferred calibration. This OD is non-blocking — it records the provider dependency and integration trigger, not an unresolved in-domain owner choice. The fake/test-double backing is removed at `TC-IMP-SYS-01` when the production provider is wired.
