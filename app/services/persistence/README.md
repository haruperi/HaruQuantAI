...
app/services/persistence/persistence.py   # only when the domain persists state
tests/services/persistence/[feature_1]/
tests/examples/[domain_number]_persistence.py
```

Each `app/services/persistence/[feature].py` module is one cohesive feature and physical removal unit.
...
`app/contracts/persistence.py` module. Feature modules never import sibling implementations. They
...
consolidated in `app/services/persistence/persistence.py`. Feature modules consume focused persistence
...
`tests/examples/[domain_number]_persistence.py`. Examples are realistic, offline, deterministic,
...
Organizing and selecting history: persistence infrastructure, banks, memberships (references, never copies), ranking/dismissal policies, copy vs. move.

### Owns

- Persistence infrastructure and database schema management.
- Strategy and result databanks, organizing historical artifact records.
- Membership references, collection tracking, and deduplication (references, never copies).
- Ranking metrics, filtering, dismissal policies, and bank copy vs. move operations.

### Does not own

- Domain-specific business logic or strategy execution.
- Raw database operations in feature service modules (ad-hoc SQL prohibited).

### Shared contracts

The domain's public boundary is `app/contracts/persistence.py`. A counterparty may be a producer,
...
| --- | --- | --- | --- | --- |
| Missing | `persistence.[capability-name]@1` | `[ProtocolName]` | `1` | [Purpose] |
...
| --- | --- | --- | --- | --- | --- |
| Missing | `persistence.[feature_partition]` | `FEAT-PERSISTENCE-[ACTION_OBJECT]` | `sqlite` | `retain` | `[capability]` |
...
| --- | --- | --- | --- | --- | --- |
| `FEAT-PERSISTENCE-[ACTION_OBJECT]` | [Value] | `app/services/persistence/[feature].py` | `persistence.[capability]@1` | [Keys or `None`] | Missing |
...
- **Lead owner:** `FEAT-PERSISTENCE-[ACTION_OBJECT]`
...
### `[feature].py` — `FEAT-PERSISTENCE-[ACTION_OBJECT]`

> **Feature ID:** `FEAT-PERSISTENCE-[ACTION_OBJECT]`
> **Status:** `[Missing | Partial | Completed]`
> **Owner module:** `app/services/persistence/[feature].py`
...
- **Provides:** `persistence.[capability]@1`
...
- **Domain persistence module:** `app/services/persistence/persistence.py` or `None`
- **Namespace:** `persistence.[feature]` or `None`
...
| Optional | `app/services/persistence/persistence.py` | Domain schema, SQL, and transactions required by this feature | [Focused repository/store symbols] |
| Missing | `tests/examples/[domain_number]_persistence.py` | Realistic offline primary-purpose example | `example_<NN>_<feature_slug>()` |
...
| Missing | `ARCH-006` | SQL and schema operations live in `app/services/persistence/persistence.py`. | Architecture and schema checks |
...
- [ ] One cohesive `app/services/persistence/[feature].py` implementation.
- [ ] Public contracts in `app/contracts/persistence.py`.
...
4. Update `app/services/persistence/persistence.py` only when database operations change.
