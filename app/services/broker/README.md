...
#### Ratified v1 public records (17)

All 17 records are ratified: 14 new records plus the three previously incomplete records (`BrokerSessionRef`, `BrokerOperationRequest`, `BrokerOperationReceipt`) completed; nothing was removed. Table anchors: `broker_adapter_profiles(stable_id UNIQUE)`, `broker_adapter_profile_versions(profile_id,version UNIQUE)`, `broker_sessions(session_ref,generation UNIQUE)`, `broker_session_transitions(session_id,sequence UNIQUE)`, `broker_operation_receipts(operation_id,attempt_no UNIQUE)`, `broker_capability_certifications(profile_version_id,capability_id,certification_version UNIQUE)`.

| # | Record | Exact wire fields | FRs / rules |
|---|---|---|---|
| R1 | `BrokerProviderProfile` | `profile_id: Uuid7`; `version: int >= 1`; `provider: ProviderRef`; `kind: Literal[MT5,CTRADER,BINANCE_SPOT,BINANCE_USD_M,BINANCE_COIN_M,DUKASCOPY,YAHOO]`; `account_ref: nonempty str`; `environment: Literal[LIVE,DEMO,TESTNET,SANDBOX,SIMULATION]` (exactly one; no default to live); `credential_ref_ids: tuple[Uuid7, ...] = ()` (Workspace `SecretRef`, values never cross); `api_version_range: nonempty str`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Missing/unsupported identity rejects admission. | FR-BRK-IDENTIFY_PROVIDER_PROFILE, RESOLVE_SESSION_CREDENTIALS, OPERATE_MT5_PROFILE, OPERATE_API_PROFILES, ENFORCE_READ_ONLY. |
| R2 | `BrokerCapabilityMatrix` | `matrix_id: Uuid7`; `profile_version_id: Uuid7`; `capabilities: tuple[BrokerCapabilityDeclaration, ...] = ()` where `BrokerCapabilityDeclaration(capability_id: CapabilityIdentifier, is_implemented: bool, is_available: bool, is_permitted: bool, is_verified: bool, is_release_approved: bool, execution_model: Literal[READ_ONLY,WRITE,EVENT_STREAM])`; `schema_version: Literal[1] = 1`. One source of truth; omitted/contradictory/partially declared dimensions reject. | FR-BRK-DECLARE_OPERATION_CAPABILITIES, VALIDATE_ORDER_POLICIES. |
| R3 | `BrokerEnvironment` | `environment: Literal[LIVE,DEMO,TESTNET,SANDBOX,SIMULATION]`; `idempotency_namespace: nonempty str`; `isolation_realm: nonempty str`; `schema_version: Literal[1] = 1`. Identities, clients, credentials, caches, event streams, receipts, and idempotency namespaces are physically or cryptographically separated; cross-environment references are rejected. | FR-BRK-ISOLATE_BROKER_ENVIRONMENTS, SEPARATE_EXECUTION_AUTHORITIES. |
| R4 | `BrokerSessionRef` (incomplete → complete) | `session_id: Uuid7`; `profile_id: Uuid7`; `profile_version: int >= 1`; `account_ref: nonempty str`; `environment: Literal[LIVE,DEMO,TESTNET,SANDBOX,SIMULATION]`; `generation: int >= 1`; `schema_version: Literal[1] = 1`. Uniqueness `(session_ref,generation)`; reconnect creates a new fenced generation; stale-generation calls/events are rejected and recorded. | FR-BRK-DEFINE_CONNECTION_STATES, RECONNECT_SESSIONS. |
| R5 | `BrokerSessionState` | `session_id: Uuid7`; `generation: int >= 1`; `connection_state: Literal[DISCONNECTED,CONNECTING,READY,DEGRADED,CLOSING,FAILED]`; `previous_state: Literal[DISCONNECTED,CONNECTING,READY,DEGRADED,CLOSING,FAILED] | None = None`; `reason: str = ""`; `transitioned_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. Every transition records previous/new state, reason, UTC time, generation; illegal transitions rejected. | FR-BRK-DEFINE_CONNECTION_STATES. |
| R6 | `BrokerSessionReadiness` | `session_id: Uuid7`; `generation: int >= 1`; `transport: Literal[NOT_READY,READY]`; `authentication: Literal[NOT_READY,READY]`; `account_authorization: Literal[NOT_READY,READY]`; `trading_permission: Literal[NOT_READY,READY]`; `subscriptions: Literal[NOT_READY,READY]`; `environment_verified: bool`; `resynchronized: bool`; `assessed_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. A boolean connection flag can never admit work; no write admitted until resynchronized. | FR-BRK-ASSESS_SESSION_READINESS, RECONNECT_SESSIONS. |
| R7 | `BrokerAccountSnapshot` | `session_id: Uuid7`; `generation: int >= 1`; `account_ref: nonempty str`; `currency: CurrencyCode`; `balances: dict[CurrencyCode, DecimalValue] = {}`; `equity: Money`; `margin: Money | None = None`; `free_margin: Money | None = None`; `permissions: tuple[nonempty str, ...] = ()`; `provider_time: UtcTimestamp | None = None`; `retrieved_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. Exact decimals and explicit units; missing fields stay explicit, never fabricated. | FR-BRK-READ_ACCOUNT_BALANCES. |
| R8 | `BrokerTradingState` | `session_id: Uuid7`; `generation: int >= 1`; `positions: tuple[ProviderRecord, ...] = ()`; `orders: tuple[ProviderRecord, ...] = ()`; `deals: tuple[ProviderRecord, ...] = ()` where `ProviderRecord(provider_id: nonempty str, record: JsonObject)` preserves provider IDs, state, quantities, prices, costs, timestamps, native metadata for Trading reconciliation; `duplicate_or_contradictory: tuple[nonempty str, ...] = ()`; `retrieved_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. | FR-BRK-READ_TRADING_STATE. |
| R9 | `BrokerMarketState` | `session_id: Uuid7`; `generation: int >= 1`; `instrument: InstrumentRef`; `provider_symbol: nonempty str`; `bid: DecimalValue | None`; `ask: DecimalValue | None`; `last: DecimalValue | None`; `market_status: Literal[OPEN,CLOSED,UNKNOWN]`; `provider_sequence: int >= 0 | None = None`; `event_time: UtcTimestamp | None = None`; `receipt_time: UtcTimestamp`; `schema_version: Literal[1] = 1`. Only genuine values; nullable missing fields; no fabricated bid/ask/last/sequence. | FR-BRK-READ_MARKET_STATE. |
| R10 | `ProviderEvent` | `event: DomainEvent` with `event_type: "broker.provider-event"`; payload `session_id: Uuid7`, `generation: int >= 1`, `kind: Literal[QUOTE,TICK,DEAL,ORDER_UPDATE,POSITION_UPDATE,MARKET_STATUS,HEARTBEAT]`, `raw_provider_hash: ContentHash`, `provider_sequence: int >= 0 | None`, `event_time: UtcTimestamp | None`, `receipt_time: UtcTimestamp`, `values: JsonObject`, `findings: tuple[ValidationIssue, ...] = ()` (sequence gaps, duplicates, late events, decode failures), `schema_version: Literal[1] = 1`. | FR-BRK-NORMALIZE_PROVIDER_EVENTS. |
| R11 | `BrokerOperationRequest` (incomplete → complete) | `operation_id: Uuid7`; `trading_operation_id: Uuid7` (Trading-owned origin); `session: BrokerSessionRef`; `operation: Literal[SUBMIT_ORDER,CANCEL_ORDER,MODIFY_ORDER]`; `provider_symbol: nonempty str` (resolved from Catalogue); `normalized_quantity: DecimalValue > 0`; `normalized_price: DecimalValue | None = None`; `policy: JsonObject` (validated against the active capability declaration before dispatch); `risk_authorization_id: Uuid7` (active Risk authorization); `idempotency_key: nonempty str`; `attempt_no: int >= 1`; `request_hash: ContentHash`; `schema_version: Literal[1] = 1`. Only Trading-owned requests accepted; unsupported type/TIF/fill policy/protection/netting rejected before dispatch. | FR-BRK-VALIDATE_TRANSPORT_REQUEST, VALIDATE_ORDER_POLICIES. |
| R12 | `BrokerOperationReceipt` (incomplete → complete) | `receipt_id: Uuid7`; `operation_id: Uuid7`; `attempt_no: int >= 1`; `profile_version_id: Uuid7`; `environment: Literal[LIVE,DEMO,TESTNET,SANDBOX,SIMULATION]`; `session_generation: int >= 1`; `request_hash: ContentHash`; `outcome: Literal[ACCEPTED,REJECTED,UNKNOWN]`; `provider_request_id: nonempty str | None = None`; `provider_client_id: nonempty str | None = None`; `provider_order_id: nonempty str | None = None`; `provider_deal_id: nonempty str | None = None`; `provider_evidence: JsonObject = {}`; `dispatched_at: UtcTimestamp | None = None`; `completed_at: UtcTimestamp | None = None`; `duration_ms: int >= 0 = 0` (monotonic); `error: ProblemDetails | None = None`; `reconciliation_keys: JsonObject = {}`; `schema_version: Literal[1] = 1`. Success and error branches mutually exclusive; timeout/disconnect after dispatch is `UNKNOWN`, never rejection/success without provider evidence; unknown blocks blind retry until Trading resolves; failure to persist the receipt fails the write discipline. Uniqueness `(operation_id,attempt_no)`. | FR-BRK-RETURN_BROKER_RESULTS, CLASSIFY_TRANSPORT_OUTCOME, JOURNAL_PROVIDER_WRITES, BLOCK_BLIND_RETRIES. |
| R13 | `BrokerOperationOutcome` | `operation_id: Uuid7`; `outcome: Literal[ACCEPTED,REJECTED,UNKNOWN]`; `receipt: BrokerOperationReceipt`; `last_known_transport_state: JsonObject = {}`; `is_reconciled: bool = False`; `schema_version: Literal[1] = 1`. Replays cannot create a second logical operation (stable idempotency/correlation identity). | FR-BRK-CORRELATE_PROVIDER_OPERATIONS, CLASSIFY_TRANSPORT_OUTCOME. |
| R14 | `ProviderCorrelation` | `correlation_id: Uuid7`; `operation_id: Uuid7`; `idempotency_key: nonempty str`; `provider_request_id: nonempty str | None = None`; `provider_client_id: nonempty str | None = None`; `provider_order_id: nonempty str | None = None`; `provider_deal_id: nonempty str | None = None`; `schema_version: Literal[1] = 1`. Stable correlation identity preserving all provider IDs. | FR-BRK-CORRELATE_PROVIDER_OPERATIONS. |
| R15 | `BrokerAdapterCertification` | `certification_id: Uuid7`; `profile_version_id: Uuid7`; `capability_id: CapabilityIdentifier`; `certification_version: int >= 1`; `api_version_range: nonempty str`; `adapter_build_hash: ContentHash`; `fixture_hashes: tuple[ContentHash, ...] = ()`; `sandbox_execution_evidence: JsonObject = {}`; `rejection_fixtures: tuple[Uuid7, ...] = ()`; `duplicate_idempotency_fixtures: tuple[Uuid7, ...] = ()`; `disconnect_unknown_fixtures: tuple[Uuid7, ...] = ()`; `approval_principal_id: Uuid7`; `issued_at: UtcTimestamp`; `expiry_rule: nonempty str`; `schema_version: Literal[1] = 1`. Version drift outside the certified range removes advertisement. | FR-BRK-TEST_ADAPTER_CONFORMANCE, VERSION_ADAPTER_CERTIFICATION. |
| R16 | `BrokerWriteCertification` | `certification_id: Uuid7`; `profile_version_id: Uuid7`; `capability_id: CapabilityIdentifier`; `is_write_released: Literal[True]`; `authenticated_execution_evidence: JsonObject`; `permission_verification: JsonObject`; `owner_approval_principal_id: Uuid7`; `approved_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. Production write requires this explicit owner-approved record. | FR-BRK-CERTIFY_BROKER_WRITES. |
| R17 | `BrokerHistoryPage` | `page_id: Uuid7`; `provider_cursor: str | None`; `requested_count: int >= 1`; `returned_count: int >= 0`; `is_truncated: bool`; `retrieved_at: UtcTimestamp`; `records: tuple[ProviderRecord, ...] = ()`; `schema_version: Literal[1] = 1`. Bounded; unbounded requests or inconsistent pagination metadata rejected. | FR-BRK-PAGE_PROVIDER_HISTORY. |

#### Ratified v1 capabilities and operation envelopes

All new (universal rule; shared `BrokerFailure` with `code: Literal[BROKER_VALIDATION_FAILED,BROKER_PROFILE_UNSUPPORTED,BROKER_ENVIRONMENT_MISMATCH,BROKER_SESSION_NOT_READY,BROKER_CAPABILITY_NOT_DECLARED,BROKER_OPERATION_REJECTED,BROKER_OUTCOME_UNKNOWN,BROKER_PAGINATION_INVALID,BROKER_CERTIFICATION_REQUIRED,CREDENTIALS_MISSING,CAPABILITY_UNAVAILABLE]`; no subscriptions — provider events are normalized observational `PUBLISH` events consumed through the Interfaces trading stream):

1. `broker.declare-capabilities@1` / `DeclareCapabilitiesCapability` / `declare_capabilities` — ops `DECLARE_PROFILE, DECLARE_MATRIX`. Success: `profile: BrokerProviderProfile | None`; `matrix: BrokerCapabilityMatrix | None`. FRs: IDENTIFY_PROVIDER_PROFILE, DECLARE_OPERATION_CAPABILITIES, HIDE_PROVIDER_INTERNALS.
2. `broker.configure-providers@1` / `ConfigureProvidersCapability` / `configure_providers` — ops `CONFIGURE, VALIDATE_CREDENTIALS`. Success: `profile: BrokerProviderProfile | None`. FRs: RESOLVE_SESSION_CREDENTIALS, OPERATE_MT5_PROFILE, OPERATE_API_PROFILES, ENFORCE_READ_ONLY.
3. `broker.isolate-environments@1` / `IsolateEnvironmentsCapability` / `isolate_environments` — ops `DECLARE_ENVIRONMENT, VERIFY_ISOLATION, RESOLVE_AUTHORITY`. FRs: ISOLATE_BROKER_ENVIRONMENTS, SEPARATE_EXECUTION_AUTHORITIES, CLOSE_ADAPTER_RESOURCES. Success: `environment: BrokerEnvironment | None`.
4. `broker.manage-sessions@1` / `ManageSessionsCapability` / `manage_sessions` — ops `OPEN, TRANSITION, RECONNECT, ASSESS_READINESS, CLOSE`. Success: `session: BrokerSessionRef | None`; `state: BrokerSessionState | None`; `readiness: BrokerSessionReadiness | None`. FRs: DEFINE_CONNECTION_STATES, ASSESS_SESSION_READINESS, RECONNECT_SESSIONS.
5. `broker.read-provider-state@1` / `ReadProviderStateCapability` / `read_provider_state` — ops `READ_ACCOUNT, READ_TRADING_STATE, READ_MARKET, PAGE_HISTORY, NORMALIZE_EVENT`. Success: `account: BrokerAccountSnapshot | None`; `trading_state: BrokerTradingState | None`; `market: BrokerMarketState | None`; `page: BrokerHistoryPage | None`. FRs: READ_ACCOUNT_BALANCES, READ_TRADING_STATE, READ_MARKET_STATE, NORMALIZE_PROVIDER_EVENTS, PAGE_PROVIDER_HISTORY, HIDE_PROVIDER_INTERNALS.
6. `broker.transport-orders@1` / `TransportOrdersCapability` / `transport_orders` — ops `VALIDATE_REQUEST, SUBMIT, CANCEL, MODIFY, JOURNAL`. Success: `outcome: BrokerOperationOutcome | None`; `receipt: BrokerOperationReceipt | None`; `correlation: ProviderCorrelation | None`. FRs: VALIDATE_TRANSPORT_REQUEST, CORRELATE_PROVIDER_OPERATIONS, CLASSIFY_TRANSPORT_OUTCOME, VALIDATE_ORDER_POLICIES, JOURNAL_PROVIDER_WRITES, BLOCK_BLIND_RETRIES, RETURN_BROKER_RESULTS.
7. `broker.certify-adapters@1` / `CertifyAdaptersCapability` / `certify_adapters` — ops `RUN_CONFORMANCE, CERTIFY_VERSION, CERTIFY_WRITE`. Success: `certification: BrokerAdapterCertification | None`; `write_certification: BrokerWriteCertification | None`. FRs: TEST_ADAPTER_CONFORMANCE, CERTIFY_BROKER_WRITES, VERSION_ADAPTER_CERTIFICATION.

Cross-owner references: `ProviderRef`, `InstrumentRef` (Catalogue); `SecretRef` IDs (Workspace); trading operation and Risk authorization IDs (Trading/Risk). No subscriptions.
