...
> Cross-domain execution ownership follows [`docs/EXECUTION_PARITY.md`](../../../docs/EXECUTION_PARITY.md): Runtime Risk is the common admission authority for executable `SIM`, `PAPER`, `DEMO`, and `LIVE` Trading paths, while Portfolio is optional evidence rather than a hard prerequisite of core Risk.
...
Runtime Risk is the non-bypassable, deterministic admission authority for executable actions entering the common Trading lifecycle. It applies to historical simulation parity (`SIM`), forward paper (`PAPER`), broker demo (`DEMO`), and live (`LIVE`) according to the selected versioned risk profile. It evaluates current evidence, profiles, limits, operational eligibility, optional allocation budgets, approval policy, reservations, and kill-switch state. It returns bounded decisions and authorization evidence; it never places an order and never owns execution transport.

A route may intentionally use a route-specific risk profile—for example a deterministic simulation profile—but Simulator must not recreate an independent sizing/admission authority when execution parity is requested.

### Owns

- Versioned runtime risk profiles, firm mandates, limits, freshness rules, route applicability, and precedence.
- Immutable account/strategy risk snapshots and optional portfolio-enriched risk snapshots used for current-state decisions.
- Position-sizing recommendations and approved maximum executable size; recommendations alone never imply approval.
- Proposed-trade admission and the canonical risk governor for executable `SIM`, `PAPER`, `DEMO`, and `LIVE` Trading paths.
- Strategy operational eligibility and optional portfolio allocation/budget activation decisions.
...
- Simulation scheduler, matching, intrabar paths, spread/slippage/commission/swap mechanics, or authority-side simulated fills; Simulator owns those execution mechanics.
- Order routing, dispatch, canonical operation/order lifecycle, execution retry, reconciliation, or protective-order management; Trading owns them.
- Broker/provider credentials, provider transport, or SDK mutation; Broker Connectivity owns them.
...
### Portfolio optionality invariant

Portfolio is not a required runtime dependency of core Runtime Risk.

- `portfolio_scope` may be empty for account/strategy actions.
- Portfolio-owned evidence is consumed only when the requested risk profile/action actually requires portfolio/allocation semantics.
- `FEAT-RISK-GOVERN_ALLOCATIONS` is implemented against versioned optional Portfolio capability contracts and deterministic provider fixtures.
- When Portfolio is absent, portfolio review/budget operations return the documented capability-unavailable outcome, while profile/evidence validation, snapshot/sizing, admission, approvals/capacity, kill-switch, and audit remain healthy for scopes that do not require Portfolio.
- Installing/removing Portfolio must not change Risk ownership or make single-strategy/account Trading unavailable.
...
The v1 record shapes already allow this architecture without requiring Portfolio presence: `RiskEvidenceRef.source_owner` may identify `PORTFOLIO`, `ProposedAction.portfolio_scope` may be empty, and allocation records are used only by portfolio-aware operations. Route/profile policy determines which evidence categories are mandatory.
...
4. `risk.govern-admission@1` / `GovernAdmissionCapability` / `govern_admission` — ops `BIND_ACTION, EVALUATE, NO_TRADE, REVALIDATE`. Success: `action: ProposedAction | None`; `decision: RiskDecision | None`; `no_trade: NoTradeDecision | None`. Governor mutates no Trading/Simulator/Broker state; recommendations/reports/scenarios/attestations are never execution authority. FRs: BIND_PROPOSED_ACTION, EVALUATE_RISK_GOVERNOR, RETURN_RISK_DECISION, RETURN_NO_TRADE, PREVENT_EXECUTION_EFFECTS, REVALIDATE_RISK_AUTHORITY.
...
Cross-owner references: `PrincipalRef` (Workspace); `ProviderRef`/`InstrumentRef` (Catalogue); strategy versions (Strategy); optional portfolio versions/projections (Portfolio); evidence sources across owners. No subscriptions.

### Core invariant

```text
No executable SIM/PAPER/DEMO/LIVE Trading mutation
  unless every route-applicable kill-switch scope is known and clear,
  evidence and configuration are current,
  a Risk decision allows the exact action and size,
  required human approval (if any) is valid,
  and required capacity is atomically reserved.
```

The route profile may intentionally make some gates inapplicable—for example human approval for a deterministic historical SIM profile—but a route may not silently skip an applicable business/risk gate merely because it is simulated.
...
| Missing | `risk_kill_switch_state`, `risk_kill_switch_events` | Hierarchical, versioned, tamper-evident; unknown blocks applicable action. |
...
|---|---|---|---|---|
| Missing | `FEAT-RISK-DEFINE_RISK_CONTRACTS` Contracts, Profiles, and Evidence | `profiles_evidence/` | Configure and validate exact risk vocabulary, profiles, mandates, and evidence using definitions from `app/contracts/risk/` | Executable action admission is unavailable. |
| Missing | `FEAT-RISK-CALCULATE_RISK` Snapshot and Sizing | `snapshot_sizing/` | Understand current risk and calculate bounded size without granting approval | Current-state review and Risk-owned parity sizing are unavailable. |
| Missing | `FEAT-RISK-GOVERN_ADMISSION` Admission and Governor | `admission_governor/` | Receive one deterministic allow/warn/approval/reject/block decision for the explicit route profile | Trading cannot dispatch new risk-bearing actions. |
| Missing | `FEAT-RISK-MANAGE_APPROVALS` Approvals and Capacity | `approvals_capacity/` | Bind human approval where required and reserve risk budget exactly once | Approval-required or capacity-consuming actions block. |
| Missing | `FEAT-RISK-CONTROL_KILL_SWITCH` Kill Switch | `kill_switch/` | Block or recover scopes through authorized, audited transitions | Actions requiring a clear state block because it cannot be proven. |
| Missing | `FEAT-RISK-GOVERN_ALLOCATIONS` Eligibility and Allocation | `eligibility_allocation/` | Govern strategy versions and optional portfolio/allocation budgets for execution | Strategy eligibility/reservation behavior and portfolio-aware operations withdraw according to their declared capability requirements; core unrelated Risk features continue. |
...
|---|---|---|---|---|
| Missing | `WF-RISK-001` Review proposed action | Trading submits preflight for SIM/PAPER/DEMO/LIVE | Exact Strategy intent or manual plan, current route-applicable account/market evidence, optional portfolio evidence, route/profile, risk profile | Decision package plus optional bounded authorization reference |
| Missing | `WF-RISK-002` Calculate position size | Strategy/Trading requests recommendation | Method, stop evidence, account/equity, instrument rules, FX, limits, optional portfolio constraints | Requested and normalized size, constraints, gaps; never approval |
| Missing | `WF-RISK-003` Kill-switch transition/check | Authorized command or pre-dispatch check | Applicable scope, current state/version, reason, approval if policy requires clearing approval | New state or block/recovery decision |
| Missing | `WF-RISK-004` Activate strategy/allocation | Promotion or optional portfolio/rebalance request | Exact versions, evidence, policy, route, optional allocation plan | Eligibility decision and, when Portfolio capability exists, allocation/budget decision |
| Missing | `WF-RISK-005` Validate approval and reserve capacity | Admission requires approval/capacity | Decision, token/attestation where required, exact action hash, current state | Atomic validation plus reservation, or no authority |
...
| Missing | `WF-RISK-007` Scenario and report | Operator/researcher requests advisory analysis | Immutable snapshot, versioned scenario, explicit seed if stochastic | Advisory result and separated evidence/assumptions/warnings |
...
| Missing | `FR-RISK-VERSION_RISK_PROFILES` | P0 | Risk profiles shall be immutable, effective-dated, strictly validated, and define thresholds, units, route/mode applicability, freshness, rounding, concurrency, approval, audit, and failure precedence without implicit defaults. | Invalid/incomplete profiles or a route/profile mismatch cannot activate. | Profile/config requirements |
| Missing | `FR-RISK-PIN_RISK_PROVENANCE` | P0 | Every decision shall pin profile/mandate versions, canonical configuration hash, evidence references/hashes, `as_of`, request/workflow/correlation IDs, route/profile identity, and schema version. | Missing or conflicting provenance produces `NEEDS_MORE_EVIDENCE` or `BLOCK`, never approval. | Config hash/audit |
| Missing | `FR-RISK-VALIDATE_SOURCE_EVIDENCE` | P0 | Consumed account, FX, market, news, session, strategy, broker, and optional portfolio evidence shall be validated for owner schema, identity, UTC time, freshness, coverage, provenance, explicit missingness, and exact units without redefining it. | Stale/incompatible/absent required evidence fails closed; absent non-required Portfolio evidence does not block core Risk. | Requirement evidence |
...
| Status | Requirement ID | Pri | Responsibility | Failure / acceptance | Source / confidence |
|---|---|---|---|---|---|
| Missing | `FR-RISK-CALCULATE_RISK_SNAPSHOT` | P0 | A risk snapshot shall compute base-currency equity, daily/total loss, drawdown, gross/net and dimensional exposure, margin/leverage, historical tail risk, volatility/correlation contribution where evidence exists, limit results, assumptions, coverage, and regime from one pinned evidence set. | Independent fixtures reconcile exact decimals; optional portfolio-derived fields remain explicit when absent. | Requirement evidence |
| Missing | `FR-RISK-INCLUDE_PENDING_EXPOSURE` | P0 | Snapshot construction shall include remaining pending-order quantity, current open-position exposure, and reserved exposure from the canonical Trading/authority evidence available for the route, with deterministic duplicate/in-flight treatment. | Open, pending, and reserved exposure cannot be silently omitted or double-counted. | Execution-state semantics |
| Missing | `FR-RISK-CALCULATE_POSITION_SIZE` | P0 | Sizing shall support versioned methods declared by the profile and return requested size, normalized size, instrument/account/optional-portfolio constraints, evidence gaps, fallback disclosure, and calculation trace. The same Risk-owned sizing path is available to simulation-parity execution. | A sizing result cannot claim approval or exceed an applicable hard limit. | Requirement evidence |
...
| Status | Requirement ID | Pri | Responsibility | Failure / acceptance | Source / confidence |
|---|---|---|---|---|---|
| Missing | `FR-RISK-BIND_PROPOSED_ACTION` | P0 | A proposed-action contract shall embed the complete immutable Strategy intent or Trading manual plan unchanged and add exact account scope, optional portfolio scope, valuation, stop, explicit route/profile, evidence, and requested risk profile. | Incompatible version, conflicting fact, invalid scope/size, or missing mandatory route-applicable evidence is rejected. | Requirement evidence |
| Missing | `FR-RISK-EVALUATE_RISK_GOVERNOR` | P0 | The governor shall evaluate kill switch, evidence validity, profile validity, eligibility, session/news/regime constraints, stop validity, size, market/account limits, applicable optional portfolio/allocation budgets, approval policy, and capacity in a fixed documented precedence. | Earlier blocking reasons cannot be masked by later approvals; simulated routes cannot skip applicable business/risk gates. | Canonical governor |
...
| Missing | `FR-RISK-RETURN_NO_TRADE` | P0 | Mandatory-gate rejection with a valid system/evidence state shall return a successful `NO_TRADE` business outcome distinct from transport, calculation, or storage failure. | Interfaces, Trading, Simulator, and analytics can distinguish safe stand-down from failed evaluation. | Feature evidence |
| Missing | `FR-RISK-PREVENT_EXECUTION_EFFECTS` | P0 | The governor shall not mutate Trading, Simulator, or Broker state and shall not treat a recommendation, report, scenario, or human attestation as execution authority. | Capability-boundary tests prove no execution side effect. | Domain boundary |
...
| Status | Requirement ID | Pri | Responsibility | Failure / acceptance | Source / confidence |
|---|---|---|---|---|---|
| Missing | `FR-RISK-BIND_HUMAN_APPROVAL` | P0 | Where the selected route/profile requires human approval, evidence shall bind authenticated principal, exact action/scope/policy, decision/config hashes, issue/expiry times, and trace IDs; it carries no secret and grants no authority by itself. | Unbound, expired, mismatched, or unauthorized required approval is invalid; a profile that explicitly requires no human approval does not invent one. | Requirement evidence |
| Missing | `FR-RISK-SIGN_APPROVAL_TOKENS` | P0 | A required risk approval token shall be signed, scoped, expiring, nonce-bearing, revocable, and bound to decision/config/action hashes and approver identity. | Invalid, expired, revoked, consumed, or mismatched tokens grant no verdict. | Requirement evidence |
...
| Missing | `FR-RISK-RESERVE_RISK_CAPACITY` | P0 | Risk capacity shall be atomically reserved before dispatch against every applicable account, strategy, optional portfolio, symbol, and global budget, then committed, released, or expired through fenced transitions. | Concurrent admissions cannot exceed any applicable hard budget. | Durable capacity reservation |
...
| Status | Requirement ID | Pri | Responsibility | Failure / acceptance | Source / confidence |
|---|---|---|---|---|---|
| Missing | `FR-RISK-DEFINE_KILL_SCOPES` | P0 | Kill-switch scopes shall include global, environment, broker account, portfolio, strategy, and symbol; an active broader applicable scope dominates narrower clear states. | Effective-state fixtures prove deterministic hierarchy and route applicability. | Kill-switch authority |
| Missing | `FR-RISK-CHECK_KILL_SWITCH` | P0 | Every preflight and immediately pre-dispatch check shall read a known current kill-switch version for every scope applicable to the exact action/route profile. | Active, unknown, stale, unavailable, or version-mismatched applicable state blocks. | Fail-closed kill switch |
| Missing | `FR-RISK-AUTHORIZE_KILL_TRANSITIONS` | P0 | Activate/block transitions require authorized principal and reason; clear/recover additionally requires separate matching current approval and verified remediation evidence when policy requires it. | Unauthorized or incomplete required clearance leaves the block active. | Kill-switch command/state |
...
This feature is part of the complete Risk domain but its Portfolio collaboration is optional. The feature must activate without importing or requiring Portfolio; Portfolio-specific operations advertise unavailable when the optional provider is absent, while strategy eligibility and non-portfolio reservation behavior remain usable as declared.

| Status | Requirement ID | Pri | Responsibility | Failure / acceptance | Source / confidence |
|---|---|---|---|---|---|
| Missing | `FR-RISK-ASSESS_STRATEGY_ELIGIBILITY` | P0 | Strategy operational eligibility shall bind an exact strategy/version, runtime profile, route, policy, evidence, approval context, conditions, validity interval, and decision lineage without altering Strategy registration or requiring Portfolio. | A different strategy version/profile/route requires a new decision. | Strategy eligibility |
| Missing | `FR-RISK-REVIEW_PORTFOLIO_ALLOCATION` | P0 | When the optional Portfolio capability is present, allocation review shall consume a self-contained projection of exact portfolio/allocation versions, ordered weights/actions, eligibility, account/market/FX evidence, and runtime scope without importing Portfolio internals. | Incomplete/stale projections are rejected; absent required Portfolio capability returns `CAPABILITY_UNAVAILABLE` without degrading core Risk. | Allocation review |
| Missing | `FR-RISK-AUTHORIZE_ALLOCATION_BUDGET` | P0 | A portfolio allocation decision shall return capped weights, authoritative risk-budget projection, conditions, evidence/policy lineage, and expiry; activation shall bind predecessor and effective time. | Trading receives budget authority only for the activated exact version; operation is unavailable when its optional provider is absent. | Allocation decision/activation |
| Missing | `FR-RISK-VALIDATE_PORTFOLIO_BUDGET` | P0 | Execution-time portfolio budget verdicts shall bind the current allocation decision, portfolio/allocation version, plan ID/hash, budget unit, reasons, and UTC validity; Trading validates and never recalculates budget consumption. | Binding mismatch blocks; non-portfolio actions do not acquire an artificial Portfolio prerequisite. | Requirement evidence |

#### Feature usage examples

The primary domain-logic module `app/services/risk/eligibility_allocation/eligibility_allocation.py` owns the executable `__main__` usage harness, with named scenarios for provider present, provider absent, and every requirement listed above.

### 4.7 `FEAT-RISK-AUDIT_RISK_DECISIONS` Revalidation, Scenarios, and Audit

| Status | Requirement ID | Pri | Responsibility | Failure / acceptance | Source / confidence |
|---|---|---|---|---|---|
| Missing | `FR-RISK-REVALIDATE_RISK_AUTHORITY` | P0 | Before reuse, retry, modification, or delayed dispatch, Risk shall revalidate decision/token/reservation against current time, route-applicable evidence, configuration, kill switch, execution-authority generation where applicable, and in-flight exposure. | Any material change requires refresh or blocks; prior approval is not grandfathered. | Decision reuse revalidation |
...
|---|---|---|---|
| Missing | `risk_profile_id` | None | Required for executable risk-bearing actions using Runtime Risk. |
| Missing | `risk_decision_ttl` | 30 seconds | Profiles may narrow; deterministic SIM may define scheduler-time validity, but reuse after expiry requires reevaluation. |
| Missing | `approval_token_ttl` | 5 minutes | Cannot outlive the underlying decision or attestation; used only when the route/profile requires approval. |
| Missing | `capacity_reservation_ttl` | 30 seconds | Expiry releases capacity unless already committed; time authority follows the execution context. |
| Missing | `evidence_freshness` | No implicit defaults | Each evidence kind and route/profile declares a required policy. |
...
- Deterministic inputs plus deterministic time authority produce byte-identical canonical decisions and audit payload hashes.
- Applicable kill-switch and hard-limit checks are non-bypassable through UI, API, CLI, MCP, automation, Simulator, retry, or direct adapter use.
- Missing/unknown/stale security-critical state fails closed when applicable to the selected route/profile.
- Risk has no broker credential, no matching engine, and no execution transport authority. Route/profile is an input to policy, not permission for Risk to dispatch.
- Physical removal of Portfolio does not prevent activation of base Risk capabilities whose declared requirements do not include Portfolio.
...
- Fault tests cover persistence loss, stale evidence, conflicting evidence, unknown execution-authority state, concurrent token use, reservation expiry, and audit tamper.
- Route-parity tests prove equivalent route-applicable inputs traverse the same Risk decision/gate categories for `SIM`, `PAPER`, `DEMO`, and `LIVE`, with differences only where the explicit profile declares them.
- System tests prove that Trading cannot dispatch without a current decision, reservation where required, and immediately pre-dispatch clear applicable kill-switch state.
- Simulator integration tests prove parity simulation uses Risk-owned admission/sizing and cannot increase or replace approved quantity.
- Physical-removal tests prove Portfolio absence withdraws portfolio-specific review/budget behavior but leaves base Risk and eligible single-strategy/account admission healthy.

## 8. Change Process

Any threshold, calculation, rounding, precedence, freshness, route applicability, approval, token, reservation, hierarchy, or evidence-binding change requires a new profile/contract version and independent fixtures. Risk must never absorb Trading dispatch, Simulator matching, or Broker transport responsibilities.
