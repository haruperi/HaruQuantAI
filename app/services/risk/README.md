...
| Completed | `FEAT-RISK-12` Canonical Risk Governor | `governor/` | Exact declarations and decision contracts: Section 4.12; real durable capacity reservation (`capacity/`) and the manual-order preflight orchestration (`governor/manual_preflight.py`) extend this feature | Section 4.12 functional requirements; `FR-RISK-091`–`095` | `tests/risk/usage/features/12_governor.py` |
...
| Completed | `FEAT-RISK-16` Stop-Loss Validator | `stop_validation/` | `build_stop_validation`/`parse_stop_validation`/`validate_stop_loss` — side, tick validity, invalidation distance, noise/venue distance, projected loss, widening permissions | `FR-RISK-082`..`FR-RISK-084` | `tests/risk/usage/features/16_stop_validation.py` |
| Completed | `FEAT-RISK-17` No-Trade Success State | `no_trade_state/` | `build_no_trade_outcome`/`parse_no_trade_outcome`/`classify_no_trade_outcome` distinguishing safe stand-down from failed gameplay when mandatory gates reject a setup | `FR-RISK-085`..`FR-RISK-087` | `tests/risk/usage/features/17_no_trade_state.py` |
...
| Completed | `FR-RISK-080` | Register the sim, demo, live, and prop-firm defaults idempotently through the immutable policy-version boundary. | `register_default_risk_policies(*, effective_at, request_id, correlation_id)` | Insert four `risk_policy_versions` rows | Standard Risk persistence errors | **Usage:** `tests/risk/usage/features/02_config.py::fr_risk_080()` **Unit:** `tests/risk/unit/test_default_policies.py::test_default_policy_registration_round_trips_idempotently()` |
...
| Completed | `FR-RISK-091` | Implement the `_CapacityGuard` port with a real durable reservation: one active reservation per exact `(account, strategy, symbol)` scope, first-writer-wins on the reservation key, an idempotent replay of the same key, and an explicit `conflict` when a different active reservation already holds the scope. | `build_risk_capacity_guard()`; `app/services/risk/capacity/runtime.py` | Durable write (`risk_capacity_reservations`) | Returns `"conflict"`/`"already_reserved"`; any storage failure propagates and fails the governor closed | **Usage:** `tests/risk/usage/features/12_governor.py::fr_risk_091()`<br>**Unit:** `tests/risk/unit/test_capacity_guard.py` |
| Completed | `FR-RISK-092` | Track one account's inception/peak/day-start equity across manual-preflight calls; seed on first observation, raise the peak monotonically, and reset day-start once per UTC calendar day. Never fabricates a history the account does not have yet. | `app/services/risk/governor/manual_preflight.py::_track_equity` | Durable write (`risk_equity_history`) | Falls back to `(equity, equity, equity)` when persistence is unavailable rather than blocking the review | **Usage:** `tests/risk/usage/features/12_governor.py::fr_risk_092()`<br>**Unit:** `tests/risk/unit/test_manual_preflight.py` |
| Completed | `FR-RISK-093` | Review one human-initiated order through the exact same fixed-precedence `review_trade_risk` gate a strategy-initiated order receives, composing a Strategy-owned `TradeIntent` (the registered Discretionary Manual Order strategy), a real Data-owned account snapshot, real market-context evidence, and a real human `ApprovalAttestation` bound to the current authenticated caller. | `review_manual_order`; `app/services/risk/governor/manual_preflight.py` | Real Data/Strategy reads; durable Risk writes (audit, approval token, capacity) | `RiskDomainError`: validation, evidence, approval, or persistence failure anywhere in the fixed-precedence review | **Usage:** `tests/risk/usage/features/12_governor.py::fr_risk_093()`<br>**Unit:** `tests/risk/unit/test_manual_preflight.py` |
| Completed | `FR-RISK-094` | Never invent account state, market evidence, or approval for a manual-order review. Every fact not obtainable from a real source (spread, session state, calendar state, volatility, liquidity, correlation) is surfaced as an explicit `missing_fields` entry or `"unknown"` state rather than a fabricated value. | `app/services/risk/governor/manual_preflight.py::_build_market_context` | Real Data/Indicators reads, each independently degradable | Missing evidence never blocks the call; it is passed through to Risk's own evidence-freshness gates, which fail closed on what they receive | **Usage:** `tests/risk/usage/features/12_governor.py::fr_risk_094()`<br>**Unit:** `tests/risk/unit/test_manual_preflight.py` |
| Completed | `FR-RISK-095` | Authorize one order cancellation — a single order or every working order — through Risk's real current-state `run_portfolio_risk_governor` gate rather than the single-proposal trade-review gate, issue a real signed approval token bound to a human `ApprovalAttestation`, and scope the resulting `ActionPolicyVerdict` to the exact target order for a single cancellation or with a `max_children` ceiling drawn from the account's own registered `max_pending_orders` limit for a bulk cancellation. | `review_cancel_authorization`; `app/services/risk/governor/manual_preflight.py` | Real Data reads; durable Risk writes (audit, approval token) | `RiskDomainError`: validation, evidence, approval, or persistence failure anywhere in the review | **Usage:** `tests/risk/usage/features/12_governor.py::fr_risk_095()`<br>**Unit:** `tests/risk/unit/test_manual_preflight.py` |
| Completed | `FR-RISK-096` | Historical SIM review shall validate market/account freshness against replay time while issuing decisions, approval tokens, audit timestamps, and expiry against wall-clock time; exact session, dataset revision, and cursor references are mandatory. | `review_manual_order`, `review_trade_risk(..., evidence_now=...)` | Durable Risk decision/audit writes | Missing or cross-route historical evidence fails closed | **Unit:** `tests/risk/unit/test_manual_preflight.py`; `app/services/risk/governor/orchestration.py` |
| Completed | `FR-RISK-097` | The default LIVE personal-account policy shall be constructible, immutable, database-registered, and fail closed on missing calendar evidence, stressed-window evidence, audit persistence, connectivity, flash crash, drawdown, margin, recovery-lock, and continuous-assessment requirements. Thresholds are 3% daily, 6% weekly, 8% total drawdown, 5%/60s flash crash, 30s connectivity, 80% emergency margin, 900s recovery lock, 120s staleness, and a 252-observation stress lookback including the registered COVID and Ukraine windows. | `build_personal_account_risk_config`, `register_default_risk_policies` | Immutable `risk_policy_versions` insert | Invalid or incomplete live policy cannot be constructed or elected | **Unit:** `tests/risk/unit/test_default_policies.py`; `app/services/risk/config/defaults.py` |
...
- **OD-RISK-02 — `PortfolioState` boundary with Portfolio.** Risk defines a `PortfolioState` input contract at `contracts/evidence.py:240`. The authoritative account/equity/drawdown state is owned by Portfolio. Paired with Portfolio `OD-PORT-02`. Until decided, Risk documents its input contract and does not redefine the authoritative ledger.
