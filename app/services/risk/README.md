...
> **Status:** `Partial` — approved Trading Cockpit Phase 0 findings folded in; all 17 work packages (`TC-IMP-RISK-01`..`TC-IMP-RISK-17`) are code-complete with dedicated passing tests (two — `TC-IMP-RISK-07`/`TC-IMP-RISK-10` — as consumer ports with fail-closed fallback per the deferred-integration rule; `TC-IMP-RISK-12` as an in-domain blocking gate leaving the `OD-RISK-01` cross-domain rename unexecuted). Package promotion to `Completed` is blocked solely by a pre-existing, out-of-scope defect: `app/utils/errors/catalog.py` (unrelated in-progress Utils-domain work already present in the working tree before this pass) fails `ERROR_CATALOG_INVALID:CATEGORY_INVALID` validation, which the `UT` conformance gate's full-suite run surfaces as ~36 Risk unit-test failures whenever a test path formats certain `StandardResponse` error codes — verified via `git stash` diff to be present identically without any Risk-domain change. See `### Trading Cockpit Phase 0 reconciliation`.
...
| --- | --- | --- | --- |
| Completed | `TradingPolicyProfile v1` additions | Extends `FEAT-RISK-02`. Adds drawdown, emergency, and assessment rule groups: `RiskConfig.drawdown_caution_threshold`/`drawdown_restricted_threshold`/`drawdown_critical_threshold` (ordered, bounded by `max_drawdown`); `emergency_flash_crash_move_pct`/`emergency_flash_crash_window_seconds`/`emergency_connectivity_loss_seconds`/`emergency_margin_call_utilization_pct`/`emergency_recovery_lock_seconds`; `assessment_recalc_events`/`assessment_max_staleness_seconds` — all mandatory together for the `live` profile, fail closed otherwise (`app/services/risk/config/profiles.py:313`, `:383`). Schema-field groundwork only; the drawdown state machine (`TC-IMP-RISK-08`), emergency governor (`TC-IMP-RISK-13`), and continuous-monitoring recalculation loop (`TC-IMP-RISK-15`) that consume these fields remain separate, not-yet-implemented work packages. | `TC-IMP-RISK-01` |
| Completed | Effective-rule resolver | Extends `FEAT-RISK-06`/`FEAT-RISK-07`. `resolve_effective_rules(sources, directions)` combines named rule sources (scenario/account/venue_instrument/strategy/simulator_default) via strictest-wins per key (min for `upper_bound`, max for `lower_bound`); a present key with no registered direction fails closed (`limits/evaluation.py:757`). | `TC-IMP-RISK-02` |
| Completed | Trade readiness gate | Extends `FEAT-RISK-12`. `evaluate_trade_readiness(...)` composes market context (session/news), the granular kill-switch lock, entry/stop validity, portfolio limits (broker/data freshness, margin, correlation, drawdown), and the blocking stress-loss gate in one fixed-precedence submit-time re-validation (`governor/orchestration.py:876`). Strategy admission (`review_strategy_admission`) is a separate persistence-owning gate the caller composes alongside this one, not re-implemented inside it. | `TC-IMP-RISK-03` |
| Completed | Planned risk and net reward | Extends `FEAT-RISK-04`. `calculate_planned_risk_reward(...)` computes stop-distance × contract-value × quantity risk plus fees/spread/slippage, target-distance reward, and their ratio (`sizing/calculator.py:398`). | `TC-IMP-RISK-04` |
| Completed | Position sizing additions | Extends `FEAT-RISK-04`. `calculate_position_size(..., additional_caps=...)` resolves the strictest of the broker cap and every supplied named cap (margin/symbol/portfolio/liquidity/strategy/stress) before flooring to the venue step (`sizing/calculator.py:262`); a parametrized property test asserts the normalized size never exceeds any supplied cap (`tests/risk/unit/test_calculator.py::test_normalized_size_never_exceeds_any_supplied_cap`). | `TC-IMP-RISK-05` |
| Completed | Stop-loss validator | `FEAT-RISK-16` — see Feature Registry. Side, tick, invalidation-distance, noise/venue-distance, projected-loss, and widening-permission checks over a `StopValidation v1` contract (`stop_validation/validator.py`). | `TC-IMP-RISK-06` |
| Completed (port) | RR / expectancy gate | `evaluate_reward_risk_gate(..., expectancy_provider=...)` applies the configured minimum RR unless an injected `ExpectancyProvider` port returns an eligible override (`limits/evaluation.py:711`). **Provider business logic deferred to Research `TC-IMP-RES-03`/`TC-IMP-RES-04`** (Phase 11); an absent/`None` provider always falls back to the baseline gate — never an inferred approval. | `TC-IMP-RISK-07` |
| Completed | Drawdown engine | Extends `FEAT-RISK-06`. `NORMAL`/`CAUTION`/`RESTRICTED`/`CRITICAL`/`LOCKED` state classifier consuming the `TC-IMP-RISK-01` threshold fields, added as the `drawdown_state` result in `evaluate_portfolio_limits` (`limits/evaluation.py:257`). | `TC-IMP-RISK-08` |
| Completed (port) | Exposure and correlation gates | Extends `FEAT-RISK-06`. Existing symbol/dimension concentration and correlation checks now consume an optional injected `PortfolioViewProvider` port (`limits/evaluation.py:413`), failing closed to Risk's own `PortfolioState`-derived exposure when absent. **Provider business logic deferred to Portfolio `TC-IMP-PORT-09`/`TC-IMP-PORT-17`** (Phase 12). | `TC-IMP-RISK-09` |
| Completed (port) | Margin and leverage gates | `evaluate_portfolio_limits(..., margin_view_provider=...)` resolves margin utilization/effective leverage from an optional injected `MarginViewProvider` port, falling back independently per field to Risk's own static snapshot checks (`limits/evaluation.py:625`). **Provider business logic deferred to Portfolio `TC-IMP-PORT-07`** (Phase 12); the check is never silently skipped. | `TC-IMP-RISK-10` |
| Completed (recheck) | Market restrictions | Extends `FEAT-RISK-07`/`FEAT-RISK-08`. Rechecked against current code (rule 9): `evaluate_market_context`'s `_session_result` already blocks any `session_state` outside the configured `allowed_session_states` allow-list — weekend/overnight-closed/halted/auction states are already covered as ordinary session-state values, since Data supplies `session_state` as free text, not a fixed enum. No dedicated weekend/overnight/venue-state field exists in the Data-owned market-context evidence contract, and no such field is needed; the generic mechanism already satisfies the requirement (`limits/evaluation.py:737`, `FR-RISK-028`). | `TC-IMP-RISK-11` |
| Completed | Stress-loss and gap-risk model | Extends `FEAT-RISK-14`. `evaluate_stress_loss_gate(...)` is a new, separate, blocking check layering nominal/liquidity-adjusted/gap/event/margin-liquidation/portfolio-stress loss ratios (`scenarios/analysis.py:227`), usable by the governor precedence. Per `OD-RISK-01`, Risk's advisory `ScenarioDefinition`/`run_risk_scenario_analysis` are untouched and unrenamed; the cross-domain name-collision decision with Simulator remains open and unexecuted here. | `TC-IMP-RISK-12` |
| Completed | Emergency risk governor | Extends `FEAT-RISK-12`. `evaluate_emergency_state(...)` detects margin-call, drawdown-breach (LOCKED state), and data/connectivity-staleness emergencies from the `TC-IMP-RISK-01` `emergency_*` fields (`governor/orchestration.py:875`). Flash-crash (price-move-in-window) detection is a **documented gap**: no Data-owned evidence contract currently carries a bounded recent price-move measurement, so it is not approximated from an unrelated field. | `TC-IMP-RISK-13` |
| Completed | Account lock and cooldown | Extends `FEAT-RISK-13` + `risk_kill_switch_states`. `KillSwitchState.locked_permissions`/`cooldown_until` and `KillSwitchCommand.locked_permissions`/`cooldown_seconds` are additive fields (`contracts/results.py:356`, `contracts/requests.py:515`); `permits_risk_action(...)` distinguishes a granular lock (only named actions blocked) from a legacy full-block activation (`kill_switch/authority.py`). Cooldown blocks clearance until it elapses, requiring explicit re-arming. Resolves `OD-RISK-03`'s concrete shape (additive, non-breaking; no `risk_kill_switch_states` schema migration needed — the lock/cooldown fields live in the table's existing `payload_json`). | `TC-IMP-RISK-14` |
| Completed | Continuous monitoring | Re-investigated first per the low-confidence flag: `revalidate_risk_decision` (`FR-RISK-042`) answers "is this prior decision still reusable," a distinct question from "did an event just happen that requires recalculation." `requires_risk_recalculation(event, ...)` is a new, thin, pure event/staleness classifier consuming the `TC-IMP-RISK-01` `assessment_recalc_events`/`assessment_max_staleness_seconds` fields (`validity/revalidation.py`); it is a classifier a caller-owned workflow invokes, not an event loop or poller (out of Risk's deterministic-and-reactive scope). | `TC-IMP-RISK-15` |
| Completed | Explainable risk decision | Extends `FEAT-RISK-15` (`RiskDecisionPackage`). `DecisionState` gains `RESIZE`/`RESTRICT` members (`contracts/enums.py:6`). `classify_decision_outcome(decision)` is a purely additive, read-only classifier layered on the authoritative persisted `decision.state` — never mutating or re-deriving governor precedence — reclassifying an approved-but-reduced-size decision as `RESIZE` and a kill-switch-specific block as `RESTRICT` (`reporting/reports.py`); `generate_risk_report` renders the reclassified outcome. | `TC-IMP-RISK-16` |
| Completed | No-trade success state | `FEAT-RISK-17` — see Feature Registry. `classify_no_trade_outcome(...)` distinguishes a safe stand-down (every failed rule is a registered mandatory gate) from failed gameplay (an avoidable execution mistake, e.g. an invalid stop), failing closed to `failed_gameplay` for any unregistered rule identity (`no_trade_state/classifier.py`). | `TC-IMP-RISK-17` |
...
| Completed | `FEAT-RISK-16` Stop-Loss Validator | `stop_validation/` | Trading Cockpit Phase 0 reconciliation (§1); `build_stop_validation`/`parse_stop_validation`/`validate_stop_loss` — side, tick validity, invalidation distance, noise/venue distance, projected loss, widening permissions | `FR-RISK-082`..`FR-RISK-084` | `tests/risk/usage/features/16_stop_validation.py` |
| Completed | `FEAT-RISK-17` No-Trade Success State | `no_trade_state/` | Trading Cockpit Phase 0 reconciliation (§1); `build_no_trade_outcome`/`parse_no_trade_outcome`/`classify_no_trade_outcome` distinguishing safe stand-down from failed gameplay when mandatory gates reject a setup | `FR-RISK-085`..`FR-RISK-087` | `tests/risk/usage/features/17_no_trade_state.py` |
...
| `drawdown_caution_threshold` | `Decimal | None` | None | Live required; fully specified with the other two thresholds or all `None`; `0 < caution < restricted < critical <= max_drawdown`. |
| `drawdown_restricted_threshold` | `Decimal | None` | None | Live required; see `drawdown_caution_threshold`. |
| `drawdown_critical_threshold` | `Decimal | None` | None | Live required; see `drawdown_caution_threshold`. |
...
| `emergency_flash_crash_move_pct` | `Decimal | None` | None | Live required; fully specified with the rest of the emergency group or all `None`; `(0, 1]`. |
| `emergency_flash_crash_window_seconds` | `int | None` | None | Live required; see `emergency_flash_crash_move_pct`; positive. |
| `emergency_connectivity_loss_seconds` | `int | None` | None | Live required; see `emergency_flash_crash_move_pct`; positive. |
| `emergency_margin_call_utilization_pct` | `Decimal | None` | None | Live required; see `emergency_flash_crash_move_pct`; `(0, 1]`. |
| `emergency_recovery_lock_seconds` | `int | None` | None | Live required; see `emergency_flash_crash_move_pct`; positive. |
| `assessment_recalc_events` | `tuple[str, ...]` | empty | Live required non-empty; unique; each member registered in `{fill, cancellation, position_change, valuation_change, policy_change}`. |
| `assessment_max_staleness_seconds` | `int | None` | None | Live required; positive. |
...
| Completed | `FR-RISK-089` | Define the optional emergency rule group (flash-crash move/window, connectivity-loss window, margin-call utilization, recovery-lock duration), mandatory together; live profile requires them. | `RiskConfig` validation (`app/services/risk/config/profiles.py:384`, `_validate_emergency_policy`) | None | `ValidationError`: partially specified or invalid emergency value | **Usage:** `tests/risk/usage/features/02_config.py::fr_risk_089()` **Unit:** `tests/risk/unit/test_profiles.py::test_live_profile_requires_emergency_rule_group()`, `test_new_rule_groups_reject_invalid_values()` |
| Completed | `FR-RISK-090` | Define the optional continuous-assessment rule group naming registered recalculation-trigger events (`fill`/`cancellation`/`position_change`/`valuation_change`/`policy_change`) and a positive staleness bound; live profile requires a non-empty event set and staleness bound. | `RiskConfig` validation (`app/services/risk/config/profiles.py:390`, `_validate_assessment_policy`, `_validate_assessment_events`) | None | `ValidationError`: unregistered/duplicated event or non-positive staleness bound | **Usage:** `tests/risk/usage/features/02_config.py::fr_risk_090()` **Unit:** `tests/risk/unit/test_profiles.py::test_live_profile_requires_assessment_rule_group()`, `test_new_rule_groups_reject_invalid_values()` |
