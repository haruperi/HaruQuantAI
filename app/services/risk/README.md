...
app/services/persistence/risk.py   # only when the domain persists state
tests/services/risk/[feature_1]/
tests/examples/[domain_number]_risk.py
```

Each `app/services/risk/[feature].py` module is one cohesive feature and physical removal unit.
...
`app/contracts/risk.py` module. Feature modules never import sibling implementations. They
...
consolidated in `app/services/persistence/risk.py`. Feature modules consume focused persistence
...
`tests/examples/[domain_number]_risk.py`. Examples are realistic, offline, deterministic,
...
Position sizes, risk policies, stops, targets, trailing exits, break-even, scale-outs.

### Owns

- Position sizing models (fixed fractional, volatility-adjusted, Kelly criterion, fixed contracts).
- Risk policy enforcement, account draw limits, daily loss limits, and exposure ceilings.
- Dynamic trade protection: stop-loss policies, profit targets, trailing exits, break-even triggers, and scale-out logic.

### Does not own

- Strategy entry logic or rule discovery (owned by D-STRATEGY).
- Broker order transmission or execution fills (owned by D-TRADING and D-BROKERS).

### Shared contracts

The domain's public boundary is `app/contracts/risk.py`. A counterparty may be a producer,
...
| --- | --- | --- | --- | --- |
| Missing | `risk.[capability-name]@1` | `[ProtocolName]` | `1` | [Purpose] |
...
| --- | --- | --- | --- | --- | --- |
| Missing | `risk.[feature_partition]` | `FEAT-RISK-[ACTION_OBJECT]` | `sqlite` | `retain` | `[capability]` |
...
| --- | --- | --- | --- | --- | --- |
| `FEAT-RISK-[ACTION_OBJECT]` | [Value] | `app/services/risk/[feature].py` | `risk.[capability]@1` | [Keys or `None`] | Missing |
...
- **Lead owner:** `FEAT-RISK-[ACTION_OBJECT]`
...
### `[feature].py` — `FEAT-RISK-[ACTION_OBJECT]`

> **Feature ID:** `FEAT-RISK-[ACTION_OBJECT]`
> **Status:** `[Missing | Partial | Completed]`
> **Owner module:** `app/services/risk/[feature].py`
...
- **Domain persistence module:** `app/services/persistence/risk.py` or `None`
- **Namespace:** `risk.[feature]` or `None`
...
| Optional | `app/services/persistence/risk.py` | Domain schema, SQL, and transactions required by this feature | [Focused repository/store symbols] |
| Missing | `tests/examples/[domain_number]_risk.py` | Realistic offline primary-purpose example | `example_<NN>_<feature_slug>()` |
...
| Missing | `ARCH-006` | SQL and schema operations live in `app/services/persistence/risk.py`. | Architecture and schema checks |
...
- [ ] One cohesive `app/services/risk/[feature].py` implementation.
- [ ] Public contracts in `app/contracts/risk.py`.
...
4. Update `app/services/persistence/risk.py` only when database operations change.
