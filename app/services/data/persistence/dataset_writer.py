...
def compute_file_hash(file_path: Path) -> str:
    """Return the SHA256 hash of one caller-resolved file.

    Args:
        file_path: Existing file path already admitted by the caller's storage
            boundary.

    Returns:
        Lowercase hexadecimal SHA256 content hash.

    Raises:
        OSError: If the file cannot be read.
    """
    logger.debug("Computing a governed DATA file hash")
    return _compute_sha256(file_path)


def resolve_data_root(request_id: str) -> Path:
    """Return the configured existing DATA root.

    Args:
        request_id: Trace identifier for configuration failures.

    Returns:
        Resolved configured data root.

    Raises:
        DataError: If ``DATA_DIR`` is absent or unusable.
    """
    logger.debug("Resolving the governed DATA root")
    return _resolve_data_dir(request_id)


def resolve_approved_storage_path(relative_path: Path, request_id: str) -> Path:
    """Resolve a relative path under one configured approved storage root.

    Args:
        relative_path: Data-root-relative path without traversal.
        request_id: Trace identifier for configuration or permission failures.

    Returns:
        Resolved path admitted by ``APPROVED_STORAGE_ROOTS``.

    Raises:
        DataError: If the path is absolute, traversing, or outside approved roots.
    """
    logger.debug("Resolving a governed DATA storage path")
    if (
        ...
        raise DataError(
            "PERMISSION_DENIED",
            safe_details={"field": "relative_path"},
            request_id=request_id,
        )
    data_dir = _resolve_data_dir(request_id)
    return _validate_path(
        data_dir / relative_path,
        _resolve_approved_roots(data_dir, request_id),
        request_id,
    )
