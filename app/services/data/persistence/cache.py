...
def cache_clear_request(
    ...
    *,
    ...
    from app.services.data.persistence.contracts import CACHE_CLEAR_MAX_ENTRIES

    trace_id = request.request_id if request is not None else _request_id(request_id)
    _reject_mixed(
        request, (source_id, symbol, data_kind, dry_run, max_entries), trace_id
    )
    ...
    return CacheClearRequest(
        ...
        max_entries=(
            max_entries if max_entries is not None else CACHE_CLEAR_MAX_ENTRIES
        ),
        request_id=trace_id,
    )
...
def clear_data_cache(
    ...
    *,
    ...
    """
    logger.info("Executing public DATA cache clearing")
    resolved = cache_clear_request(
        request,
        source_id=source_id,
        symbol=symbol,
        data_kind=data_kind,
