"""Canonical market evidence projected into detached analytical DataFrames.

The only sanctioned way an analytical DataFrame crosses the Data boundary. Every
projection is a new mutable copy with a UTC timestamp index; no raw provider frame ever
escapes, and the source ``MarketDataset`` remains the authoritative precision, quality,
provenance, and availability evidence.

Genuine missingness is preserved as ``NaN`` rather than filled. A provider-reported
zero stays zero, and absent evidence never becomes a repeated prior quote.
"""
...
def to_ohlcv_dataframe(dataset: MarketDataset) -> pd.DataFrame:
    ...
    Raises:
        DataError: If the dataset is not bars, contains a non-OHLCV record, has
            inconsistent spread units, or cannot be represented safely as float64.
    """
    ...
    spread_units = {
        record.spread_unit for record in records if record.spread_unit is not None
    }
    ...
    try:
        ...
        values["spread"] = np.asarray(
            [
                np.nan if record.spread is None else float(record.spread)
    ...
    required_values_are_finite = all(
        np.isfinite(values[column]).all() for column in _REQUIRED_OHLCV_COLUMNS
    )
    supplied_spreads_are_finite = not np.isinf(values["spread"]).any()
    if not required_values_are_finite or not supplied_spreads_are_finite:
        raise DataError(
            "PRECISION_MISMATCH",
            safe_details={"operation": "ohlcv_dataframe_projection"},
            request_id=dataset.request_id,
