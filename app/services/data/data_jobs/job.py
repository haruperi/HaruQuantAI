...
repeats its package name.

Owns ``_BACKGROUND_TASKS``, the registry of running asyncio loops. It stays here because
this is the only module that starts or stops one; splitting it out would let ``stop``
fail to cancel a loop that ``start`` created.

A run is never reported successful merely because a timer fired. Success requires
committed chunk evidence, which is why the execution path goes through
``backfill.execute_backfill_chunk`` rather than a status flag.
"""
...
# --- Public job execution operations ---
