...
app/services/persistence/data.py   # only when the domain persists state
tests/services/data/[feature_1]/
tests/examples/[domain_number]_data.py
```

Each `app/services/data/[feature].py` module is one cohesive feature and physical removal unit.
...
`app/contracts/data.py` module. Feature modules never import sibling implementations. They
...
consolidated in `app/services/persistence/data.py`. Feature modules consume focused persistence
...
`tests/examples/[domain_number]_data.py`. Examples are realistic, offline, deterministic,
...
What can be traded and what data exists: instrument conventions, sessions, calender, datasets timeframes, imports, providers, data quality, import/transform.

### Owns

- Instrument conventions, symbol specifications, trading sessions, and exchange calendars.
- Historical datasets, bar timeframes, ticks, quotes, and market depth storage.
- Data providers, live/historical market imports, CSV/TSV parsers, and data transformation.
- Data quality validation, gap detection, split/dividend adjustments, and bar alignment.

### Does not own

- Direct broker execution routing (owned by D-BROKERS and D-TRADING).
- Indicator calculation or mathematical analysis (owned by D-INDICATOR and D-ANALYTICS).

### Shared contracts

The domain's public boundary is `app/contracts/data.py`. A counterparty may be a producer,
...
| --- | --- | --- | --- | --- |
| Missing | `data.[capability-name]@1` | `[ProtocolName]` | `1` | [Purpose] |
...
| --- | --- | --- | --- | --- | --- |
| Missing | `data.[feature_partition]` | `FEAT-DATA-[ACTION_OBJECT]` | `sqlite` | `retain` | `[capability]` |
...
| --- | --- | --- | --- | --- | --- |
| `FEAT-DATA-[ACTION_OBJECT]` | [Value] | `app/services/data/[feature].py` | `data.[capability]@1` | [Keys or `None`] | Missing |
...
- **Lead owner:** `FEAT-DATA-[ACTION_OBJECT]`
...
### `[feature].py` — `FEAT-DATA-[ACTION_OBJECT]`

> **Feature ID:** `FEAT-DATA-[ACTION_OBJECT]`
> **Status:** `[Missing | Partial | Completed]`
> **Owner module:** `app/services/data/[feature].py`
...
- **Domain persistence module:** `app/services/persistence/data.py` or `None`
- **Namespace:** `data.[feature]` or `None`
...
| Optional | `app/services/persistence/data.py` | Domain schema, SQL, and transactions required by this feature | [Focused repository/store symbols] |
| Missing | `tests/examples/[domain_number]_data.py` | Realistic offline primary-purpose example | `example_<NN>_<feature_slug>()` |
...
| Missing | `ARCH-006` | SQL and schema operations live in `app/services/persistence/data.py`. | Architecture and schema checks |
...
- [ ] One cohesive `app/services/data/[feature].py` implementation.
- [ ] Public contracts in `app/contracts/data.py`.
...
4. Update `app/services/persistence/data.py` only when database operations change.
