"""Strict configuration for the market-data reference browser feature.
...
Key capabilities:
    ...
    * Carry the reference database path.
"""
...
def from_dict(data: dict[str, Any] | None) -> BrowseReferenceConfig:
    ...
    """
    if not data:
        return BrowseReferenceConfig()
    unknown = set(data) - _ALLOWED_CONFIG_KEYS
    if unknown:
        message = "Unknown browse-reference configuration keys: " + ", ".join(
            sorted(unknown)
        )
...
class BrowseReferenceConfig:
    """Runtime configuration for FEAT-DATA-BROWSE_REFERENCE."""
