...
def _canonical_tick(tick: _TickLike) -> TickRecord:
    ...
    return TickRecord(
        ...
        available_at=max(tick.provider_receipt_timestamp, tick.event_timestamp),
        bid=tick.bid,
        ask=tick.ask,
...
def _unseen_ticks(
    ...
    Args:
        items: The ``items`` argument.
        cursor: The ``cursor`` argument.
        cursor_counts: The ``cursor_counts`` argument.
