...
def _effective_health(
    ...
    Args:
        state: The ``state`` argument.
        heartbeat_at: The ``heartbeat_at`` argument.
        heartbeat_timeout_seconds: The ``heartbeat_timeout_seconds`` argument.
        now: The ``now`` argument.
        last_error: The ``last_error`` argument.
...
def read_feed_status(
    ...
    *,
    ...
    from app.services.data.market_events.state import _ACTIVE_FEEDS
