...
class LocalMarketDataSource(MarketDataSource):
    ...
    def _artifact(self, symbol: str) -> tuple[Path, Literal["csv", "parquet"]]:
        """Resolve one symbol artifact within the configured root."""
        logger.debug("Resolving local artifact for symbol %s", symbol)
        parquet_path = self._raw_root / f"{symbol}.parquet"
        csv_path = self._raw_root / f"{symbol}.csv"
    ...
    def fetch(self, request: SourceReadRequest) -> RawSourceBatch:
        ...
        artifact, data_format = self._artifact(request.provider_symbol)
        ...
        return RawSourceBatch(
            ...
            records=tuple(record.model_dump() for record in dataset.records),
