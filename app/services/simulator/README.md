...
app/services/persistence/simulator.py   # only when the domain persists state
tests/services/simulator/[feature_1]/
tests/examples/[domain_number]_simulator.py
```

Each `app/services/simulator/[feature].py` module is one cohesive feature and physical removal unit.
...
`app/contracts/simulator.py` module. Feature modules never import sibling implementations. They
...
consolidated in `app/services/persistence/simulator.py`. Feature modules consume focused persistence
...
`tests/examples/[domain_number]_simulator.py`. Examples are realistic, offline, deterministic,
...
Backtesting engines, Retester simulation specs, runs, order matching, fills, trades, account state, costs, execution precision.

### Owns

- Backtesting simulation engines, order matching models (tick-by-tick, bar-open/close, intrabar).
- Retester simulation specifications, backtest runs, and execution precision modes.
- Simulated fills, slippage models, commission structures, spread dynamics, and margin costs.
- Simulated account state transitions and chronological trade ledger generation.

### Does not own

- Live market order transmission (owned by D-TRADING and D-BROKERS).
- Performance metric calculations (owned by D-ANALYTICS).

### Shared contracts

The domain's public boundary is `app/contracts/simulator.py`. A counterparty may be a producer,
...
| --- | --- | --- | --- | --- |
| Missing | `simulator.[capability-name]@1` | `[ProtocolName]` | `1` | [Purpose] |
...
| --- | --- | --- | --- | --- | --- |
| Missing | `simulator.[feature_partition]` | `FEAT-SIMULATOR-[ACTION_OBJECT]` | `sqlite` | `retain` | `[capability]` |
...
| --- | --- | --- | --- | --- | --- |
| `FEAT-SIMULATOR-[ACTION_OBJECT]` | [Value] | `app/services/simulator/[feature].py` | `simulator.[capability]@1` | [Keys or `None`] | Missing |
...
- **Lead owner:** `FEAT-SIMULATOR-[ACTION_OBJECT]`
...
### `[feature].py` — `FEAT-SIMULATOR-[ACTION_OBJECT]`

> **Feature ID:** `FEAT-SIMULATOR-[ACTION_OBJECT]`
> **Status:** `[Missing | Partial | Completed]`
> **Owner module:** `app/services/simulator/[feature].py`
...
- **Provides:** `simulator.[capability]@1`
...
- **Domain persistence module:** `app/services/persistence/simulator.py` or `None`
- **Namespace:** `simulator.[feature]` or `None`
...
| Optional | `app/services/persistence/simulator.py` | Domain schema, SQL, and transactions required by this feature | [Focused repository/store symbols] |
| Missing | `tests/examples/[domain_number]_simulator.py` | Realistic offline primary-purpose example | `example_<NN>_<feature_slug>()` |
...
| Missing | `ARCH-006` | SQL and schema operations live in `app/services/persistence/simulator.py`. | Architecture and schema checks |
...
- [ ] One cohesive `app/services/simulator/[feature].py` implementation.
- [ ] Public contracts in `app/contracts/simulator.py`.
...
4. Update `app/services/persistence/simulator.py` only when database operations change.
