...
> **Status:** `Completed` for `FEAT-SIM-01`..`16` and `FEAT-SIM-18`; `FEAT-SIM-17` remains `Pending` (see "Sim⇄live parity programme registration").
...
| Completed | `FEAT-SIM-05` Matching and Simulated State        | `execution/`  | Exact declarations, execution contracts, evidenced rollover/swap: Section 4.5 | Section 4.5 requirements including `FR-SIM-134`, `FR-SIM-135`, `FR-SIM-205`–`FR-SIM-208` | `tests/simulator/usage/features/05_execution.py`  |
...
The remaining `Pending` feature is registered by the approved sim⇄live parity programme
(`docs/dev/sim-live-parity-implementation-plan.md`) and may not be claimed complete
before its owning phase gate passes. The registration of `FEAT-SIM-15` is the separately
...
| Completed | `FR-SIM-210` | Publish stable calculation-model identity and bind it into request-v2 configuration identity. | `get_calculation_model_identity`; `SimulationBacktestRequestV2` | Read-only | Malformed digest rejected | **Usage:** `tests/simulator/usage/features/16_calculations.py:108` **Unit:** `tests/simulator/unit/test_run_request_v2.py` |
...
| Completed | `FR-SIM-214` | Bind calculation-model and conformance-artifact digests into request-v2 execution identity. | `SimulationBacktestRequestV2` | Read-only | Missing or malformed digest rejected | **Usage:** `tests/simulator/usage/features/16_calculations.py:128` **Unit:** `tests/simulator/unit/test_run_request_v2.py` |
...
**Phase 7 bounded blocker resolution:** legacy fill records do not carry the authority evidence and source sequence required by `FR-SIM-180`. The signed ledger and rollover calculator therefore remain explicit root operations instead of inventing those fields. Scheduler, journal, recovery, and reporting consumers must integrate only when a later execution phase supplies verified fields. Post-swap stop-out ordering remains deferred to Phase 16.
...
| Completed | `FR-SIM-196` | Bind every Phase 4c execution-identity field, provider revision interval, initial authority state, certificate target, and terminal-close policy into immutable request V2 configuration and run identity. | `SimulationBacktestRequestV2`, `run_backtest_async(...)` | None | `ValueError` or structured `SIM_*` failure for incomplete, conflicting, or relabelled identity | **Usage:** `tests/simulator/usage/features/07_run.py::fr_sim_196()` **Unit:** `tests/simulator/unit/test_run_request_v2.py::test_fr_sim_196_v2_binds_complete_execution_identity()` |
| Completed | `FR-SIM-231` | Expose the complete frozen V2 request with canonical provider-revision ordering and continuous effective coverage; trace IDs are excluded from config identity. | `SimulationBacktestRequestV2` | None | `ValueError`: missing field, gap, overlap, duplicate, non-canonical order, or unproved backdating | **Usage:** `tests/simulator/usage/features/07_run.py::fr_sim_231()` **Unit:** `tests/simulator/unit/test_run_request_v2.py` |
...
| Completed | `FR-SIM-233` | Bind distinct source/tick lineage hashes, the declared market-evidence class, and point-in-time availability policy without claiming Phase 9 eligibility. | `SimulationBacktestRequestV2` | None | `ValueError`: unsupported evidence class/policy or malformed lineage | **Usage:** `tests/simulator/usage/features/07_run.py::fr_sim_233()` **Unit:** `tests/simulator/unit/test_run_request_v2.py` |
| Completed | `FR-SIM-234` | Bind explicit terminal-close policy and demo/live certificate target; every provider revision must match the target so demo evidence cannot be relabelled live. | `SimulationBacktestRequestV2` | None | `ValueError`: target mismatch or incomplete revision coverage | **Usage:** `tests/simulator/usage/features/07_run.py::fr_sim_234()` **Unit:** `tests/simulator/unit/test_run_request_v2.py::test_fr_sim_234_demo_evidence_cannot_claim_live_target()` |
| Completed | `FR-SIM-235` | Retain V1 and synchronous compatibility while making `run_backtest_async` the V2-native operation; synchronous use inside an active event loop fails closed. | `run_backtest(...)`, `run_backtest_async(...)` | Run lifecycle side effects | `SimulationError[SIM_UNSUPPORTED_OPERATION]` for running-loop sync misuse | **Usage:** `tests/simulator/usage/features/07_run.py::fr_sim_235()` **Unit:** `tests/simulator/unit/test_run_request_v2.py::test_fr_sim_235_async_success_and_running_loop_sync_failure()` |
