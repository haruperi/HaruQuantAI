...
Canonical execution engines require a non-empty Data-returned provider-revision
history at construction. Missing history fails with `SIM_INVALID_CONFIG`; no
session or order-semantics bypass exists.
...
| Completed | Durable historical-practice request, cursor/state digest, recovery generation, and cursor-bound manual intents in `sim_interactive_sessions` and `sim_interactive_intents` | Interactive session create/step/order/restore/rearm operations | `app/services/simulator/migrations/definitions.py`, additive step `004_simulator_interactive_recovery_v1` |
...
| Completed | `FEAT-SIM-02` Simulation-Owned State              | `state/`      | Exact declarations, historical interactive sessions, and state contracts: Section 4.4a | Section 4.4a requirements including `FR-SIM-251`–`FR-SIM-254` and `FR-SIM-262` | `tests/simulator/usage/features/02_state.py`; `tests/simulator/unit/test_live_sessions.py`; `tests/simulator/unit/test_live_session_authority.py` |
...
| Completed | `FEAT-SIM-09` Results and Canonical Artifacts     | `reporting/`  | Exact declarations, result contracts, and immutable `attach_analytics_report_artifact` (exact-bytes idempotent, conflict-fail-closed): Section 4.6     | Section 4.6 functional requirements  | `tests/simulator/usage/features/09_reporting.py`; `tests/simulator/unit/test_artifacts.py`  |
...
| Completed | `FEAT-SIM-19` Canonical Backtest Recipe | `backtest_recipe/` | Registered strategy descriptors, strategy-driven run dependencies, the end-to-end single-asset pipeline (provider bars, tick generation, canonical request, run, Analytics report), bounded background job execution, and the optional completion sink retaining full attributed terminal evidence (`BacktestRunEvidence`, `BACKTEST_EVIDENCE_PERSISTENCE_FAILED`) | `FR-SIM-255`–`FR-SIM-261` | `tests/simulator/unit/test_backtest_recipe.py` |
...
min/max/step; commission is a debit and swap is a signed cash effect per crossed
...
**Trusted canonical runtime.** After request, data, provider, strategy, and
account configuration pass their canonical boundaries, the orchestrator uses
private raw engine and ledger operations for the per-tick loop. Public methods
retain their standard response envelopes. Raw/public parity tests require the
same receipts and state, and completion still materializes and validates the
canonical result, journal, closed trades, and accounting evidence.
...
- Phase 1 commission is configured non-negative cash debit per lot per side.
  Phase 1 swap is a signed cash effect per lot per crossed UTC rollover (negative
  debit, positive credit), including explicit weekday
...
- Build order is `errors -> validation -> timeline -> accounting -> state -> journal -> execution -> reporting -> run`. State precedes Journal because the
  ...
  `AccountLedger(initial_balance, account_currency, symbol_specification, cost_model)`, `EventDrivenExecutionEngine(ledger, journal_writer, execution_profile, engine_version, provider_revisions)`, and `SimTrader(engine)`. Engine construction rejects empty provider-revision history.
...
| Completed | `FR-SIM-008` | The system shall calculate configured Phase 1 commission and signed swap cash effects deterministically and return an itemized fixed-precision mapping; negative swap is a debit and positive swap is a credit. | `calculate_execution_costs(fill: Mapping[str, object], model: Mapping[str, object]) -> Mapping[str, Decimal]` | None | `SimulationError`: `SIM_COMMISSION_CALCULATION_FAILED`, `SIM_SWAP_CALCULATION_FAILED`, or unsupported model code | **Usage:** `tests/simulator/usage/features/04_accounting.py::fr_sim_008()` **Unit:** `tests/simulator/unit/test_accounting.py::test_calculate_execution_costs_is_exact()`, `::test_calculate_execution_costs_preserves_signed_swap_cash_effect()` |
...
| Completed | `FR-SIM-218` | Every canonical v2 decision shall consume only market records whose event and availability timestamps are at or before its scheduler instant. A run-scoped cursor reuses identical validated prefixes only after verifying monotonic timestamp and availability order; invalid ordering uses the canonical filtering fallback. | `build_point_in_time_dataset` | Read-only Data contract construction | Missing visible evidence produces no decision; invalid evidence fails closed | **Usage:** `tests/simulator/usage/features/07_run.py::fr_sim_218()` **Unit:** `tests/simulator/unit/test_point_in_time_evaluation.py` |
| Completed | `FR-SIM-219` | Canonical v2 shall evaluate after each authority tick through the injected shared Trading-cycle composition instead of precomputing whole-run decisions. The registered naive-trend usage composition reuses request-bound Strategy objects and evaluates every eligible H1 instant through its causal bounded-window evaluator without precomputing or skipping decisions. | incremental run loop | Shared Trading-cycle mutation/evidence | Upstream, Strategy progression, or Trading cycle failure | **Usage:** `tests/simulator/usage/features/07_run.py::fr_sim_219()` **Integration:** `tests/simulator/integration/test_incremental_trading_cycle.py` |
| Completed | `FR-SIM-220` | Future market, indicator, Strategy, Risk, account, position, and margin evidence shall be structurally excluded from a decision boundary. Prefix caching changes dataset materialization only; every canonical tick still invokes the shared point-in-time cycle. | `run_point_in_time_evaluation` | Read-only owner ports | Future-evidence rejection | **Usage:** `tests/simulator/usage/features/07_run.py::fr_sim_220()` **Integration:** `tests/simulator/integration/test_no_future_reads.py`, `tests/simulator/integration/test_incremental_trading_cycle.py` |
...
| Completed | `FR-SIM-251` | Interactive SIM shall consume only the immutable historical dataset revision bound to its session; it shall not read live quotes or route through a broker. | `create_live_simulation_session`, `step_live_simulation` | Historical Data read; isolated virtual state mutation | Missing or changed dataset evidence fails closed | **Unit:** `tests/simulator/unit/test_live_sessions.py`; `app/services/simulator/state/live_sessions.py` |
| Completed | `FR-SIM-252` | Interactive SIM shall accept Risk-approved MARKET, LIMIT, STOP, and STOP_LIMIT intents through Trading's direct Simulator port, while pending orders are evaluated only as the historical timeline advances. | `submit_live_simulation_order` | Virtual order/account mutation | Session, route, or order evidence mismatch fails closed | **Unit:** `tests/trading/unit/routing/test_simulation_route_selection.py`; `app/services/simulator/state/live_sessions.py` |
| Completed | `FR-SIM-253` | A durable practice session shall persist its immutable request, replay cursor, canonical state digest, and every manual intent at its acceptance cursor without serializing private engine objects. | `create_live_simulation_session(..., durable=True)`, `step_live_simulation`, `submit_live_simulation_order` | Transactional writes to `sim_interactive_sessions` and `sim_interactive_intents` | Any unconfirmed write fails closed | **Unit:** `tests/simulator/unit/test_state.py`; `app/services/simulator/migrations/definitions.py` |
| Completed | `FR-SIM-254` | Restart recovery shall create a distinct recovery journal, reload the exact dataset revision, replay from the beginning, reapply manual intents at recorded cursors, compare the persisted state digest, and remain exposure-blocked until explicit rearm. | `restore_live_simulation_session`, `rearm_live_simulation_session` | Durable recovery-state writes | Reconstruction mismatch enters integrity failure | **Unit:** `tests/simulator/unit/test_live_sessions.py`; `app/services/simulator/state/live_sessions.py` |
| Completed | `FR-SIM-261` | The background canonical job registry shall enrich completion-sink evidence with the owning `job_id` and `principal_id` so durable downstream catalogues can attribute terminal evidence, while the compact HTTP snapshot itself stays identity-free. | `BacktestJobRegistry.submit` (sink enrichment in `backtest_recipe/jobs.py`) | None (sink side effects remain the sink's own) | Sink refusal surfaces as `BACKTEST_EVIDENCE_PERSISTENCE_FAILED` | **Unit:** `tests/simulator/unit/test_backtest_recipe.py::test_registry_sink_receives_job_and_principal_attribution` |
| Completed | `FR-SIM-262` | Interactive session authority shall expose bounded session listing, forward-only bounded seek, the six manual command types returning receipts with refreshed state, backwards-only viewport reads that can never expose future rows, and advisory finalization that seals a session against further mutation. | `list_live_simulation_sessions`, `seek_live_simulation`, `execute_live_simulation_command`, `read_live_simulation_viewport`, `finalize_live_simulation_session` | Virtual order/account mutation; advisory journal seal | Rewind, oversized bounds, unknown command discriminator, or finalized-session mutation fails closed | **Unit:** `tests/simulator/unit/test_live_session_authority.py` |
