...
#### Ratified v1 public records (18)

Table anchors: `portfolios(name_normalized UNIQUE)`, `portfolio_versions(portfolio_id,version UNIQUE)`, `portfolio_results(manifest_id UNIQUE)`, `correlation_matrices(candidate_set_hash,settings_hash UNIQUE)`, `portfolio_search_artifacts(research_run_id UNIQUE)`.

| # | Record | Exact wire fields | FRs / rules |
|---|---|---|---|
| R1 | `PortfolioRef` | `portfolio_id: Uuid7`; `schema_version: Literal[1] = 1`. | — |
| R2 | `PortfolioVersion` | `portfolio_id: Uuid7`; `version: int >= 1`; `name: str 1..160`; `members: tuple[PortfolioMember, ...] = ()`; `allocation: Allocation`; `constraints: PortfolioConstraintSet | None = None`; `currency_policy: JsonObject`; `capital: Money`; `valid_from: UtcTimestamp`; `valid_to: UtcTimestamp | None = None`; `search_lineage: JsonObject = {}` (selected candidate rank + search artifact refs); `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable version; invalid portfolios rejected before simulation; promotion of automatic-search results preserves lineage. | FR-PORT-VERSION_PORTFOLIOS, VALIDATE_PORTFOLIO_ADMISSION, VERSION_PORTFOLIO_CHANGES. |
| R3 | `PortfolioMember` | `member_id: Uuid7`; `strategy_version_id: Uuid7`; `result_id: Uuid7 | None = None`; `data_manifest_id: Uuid7 | None = None`; `weight: DecimalValue in [0,1] | None = None`; `sizing_rule: JsonObject = {}`; `schema_version: Literal[1] = 1`. Admission validates date overlap, result compatibility, required currencies, duplicate exposure, constituent availability. | FR-PORT-VERSION_PORTFOLIOS, VALIDATE_PORTFOLIO_ADMISSION. |
| R4 | `Allocation` (incomplete → complete) | `allocation_id: Uuid7`; `method: Literal[FIXED_WEIGHT,FIXED_NOTIONAL,VOLATILITY_SCALED,CUSTOM]`; `custom_policy: JsonObject = {}` (§21.4 plugin method identity when `CUSTOM`); `normalization: Literal[NONE,L1_BUDGET,CAPITAL]`; `rounding: Literal[DOWN,UP,HALF_UP,HALF_EVEN,TOWARD_ZERO]`; `rebalance: RebalancePolicy | None = None` where `RebalancePolicy(schedule: nonempty str, trigger: JsonObject, tolerance_bands: DecimalValue in [0,1] | None, turnover_cost_policy: JsonObject, execution_timing: nonempty str, unresolved_order_policy: nonempty str)`; `exposure_limits: tuple[ExposureLimit, ...] = ()` where `ExposureLimit(kind: Literal[GROSS,NET,PER_INSTRUMENT,PER_CURRENCY,PER_MARKET,PER_STRATEGY,CONCURRENT_POSITIONS], limit: DecimalValue, instrument: InstrumentRef | None = None, currency: CurrencyCode | None = None)`; `shared_instrument_policy: Literal[NET,INDEPENDENT,REJECT] = "REJECT"`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Allocation fixtures reconcile requested vs allocated capital exactly. | FR-PORT-APPLY_ALLOCATION_METHODS, SCHEDULE_REBALANCING, ENFORCE_EXPOSURE_LIMITS, RESOLVE_SHARED_INSTRUMENTS, REGISTER_PORTFOLIO_METHODS. |
| R5 | `PortfolioConstraintSet` | `constraint_set_id: Uuid7`; `exposure_limits: tuple[ExposureLimit, ...] = ()` (as in R4); `custom_constraints: JsonObject = {}`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Breaches prevented or resolved deterministically. | FR-PORT-ENFORCE_EXPOSURE_LIMITS, DEFINE_PORTFOLIO_SEARCH. |
| R6 | `CorrelationRequest` | `request_id: Uuid7`; `candidate_set_hash: ContentHash`; `member_ids: nonempty tuple[Uuid7, ...]`; `return_definition: nonempty str`; `frequency: nonempty str`; `alignment_policy: AlignmentPolicy` (Data-owned); `overlap_policy: nonempty str`; `minimum_observations: int >= 1`; `method: nonempty str`; `method_version: int >= 1`; `schema_version: Literal[1] = 1`. | FR-PORT-VERSION_CORRELATION_INPUTS. |
| R7 | `CorrelationMatrix` | `matrix_id: Uuid7`; `request: CorrelationRequest`; `candidate_set_hash: ContentHash`; `settings_hash: ContentHash`; `observation_counts: dict[nonempty str, int >= 0]`; `matrix_artifact_id: Uuid7`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable; changing any constituent or policy produces a different identity. | FR-PORT-COMPUTE_CORRELATION_MATRICES. |
| R8 | `PortfolioSimulationRequest` | `request_id: Uuid7`; `portfolio_version_id: Uuid7`; `conversion_policy: JsonObject` (fail/carry/explicit fallback per missing rate); `from_at/to_at: UtcTimestamp | None`; `idempotency_key: nonempty str`; `schema_version: Literal[1] = 1`. Aggregate simulation merges constituent cash flows/exposures on a canonical event timeline without future information; conversion uses only rates visible at each event timestamp. | FR-PORT-SIMULATE_AGGREGATE_PORTFOLIOS, CONVERT_PORTFOLIO_CURRENCIES. |
| R9 | `PortfolioResult` | `result_id: Uuid7`; `portfolio_version_id: Uuid7`; `manifest_id: Uuid7`; `aggregate_trades_artifact_id: Uuid7`; `cashflow_artifact_id: Uuid7`; `equity_drawdown_artifact_id: Uuid7`; `exposure_timeline_artifact_id: Uuid7`; `constituent_attribution_artifact_id: Uuid7`; `turnover: JsonObject`; `correlation_matrix_id: Uuid7 | None = None`; `constraint_events_artifact_id: Uuid7 | None = None`; `daily_returns_artifact_id: Uuid7`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Aggregate totals reconcile to constituents; `.sqxpf` export preserves identities/policies/metrics/artifacts and reproduces the canonical portfolio hash on round-trip. | FR-PORT-REPORT_PORTFOLIO_RESULTS, EXPORT_PORTFOLIO_RESULTS. |
| R10 | `PortfolioSearchPlan` | `plan_id: Uuid7`; `candidate_set: tuple[Uuid7, ...]`; `objective_vector: nonempty tuple[ObjectiveSpec, ...]` where `ObjectiveSpec(metric: nonempty str, direction: Literal[MINIMIZE,MAXIMIZE], weight: DecimalValue > 0 | None = None)` (multi-objective stays Pareto, never silently scalarized); `constraints: PortfolioConstraintSet`; `search_method: nonempty str`; `method_version: int >= 1`; `budget: ResearchBudget` (Research-owned); `seeds: tuple[nonempty str, ...]`; `tie_breaker: nonempty str`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Identical inputs select the same portfolio. | FR-PORT-DEFINE_PORTFOLIO_SEARCH, REJECT_INFEASIBLE_SEARCHES, OPTIMIZE_PORTFOLIO_OBJECTIVES. |
| R11 | `PortfolioCandidate` | `candidate_id: Uuid7`; `run_id: Uuid7`; `weights: dict[nonempty str, DecimalValue]`; `result_id: Uuid7 | None = None`; `objective_values: dict[nonempty str, DecimalValue] = {}`; `is_feasible: bool = True`; `rank: int >= 1 | None = None`; `pareto_status: Literal[DOMINATED,NON_DOMINATED,UNRANKED] = "UNRANKED"`; `schema_version: Literal[1] = 1`. Checkpoints persist frontier/population, evaluated candidates, cache keys, budget counters, and RNG state; resume equals uninterrupted execution. | FR-PORT-OPTIMIZE_PORTFOLIO_OBJECTIVES, CHECKPOINT_PORTFOLIO_SEARCH. |
| R12 | `PortfolioRiskReport` | `report_id: Uuid7`; `portfolio_version_id: Uuid7`; `result_id: Uuid7`; `daily_expected_return: DecimalValue`; `daily_volatility: DecimalValue`; `var_confidence: DecimalValue in (0,1)`; `var_horizon_days: int >= 1`; `parametric_var: Money`; `expected_shortfall: Money`; `cvar_method: nonempty str`; `sharpe: DecimalValue`; `risk_free_rate: DecimalValue`; `risk_free_version: int >= 1`; `schema_version: Literal[1] = 1`. | FR-PORT-CALCULATE_PORTFOLIO_RISK. |
| R13 | `PortfolioMetricDefinition` | `metric_id: nonempty str`; `version: int >= 1`; `basis: Literal[AGGREGATE_EQUITY,CONSTITUENT_RETURNS,ALLOCATED_CAPITAL,EXPOSURE_ADJUSTED_CAPITAL]`; `formula_ref: nonempty str`; `unit: Literal[MONEY,PERCENT,PIPS,RATIO,COUNT]`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Metric metadata removes basis ambiguity. | FR-PORT-DEFINE_PORTFOLIO_METRICS. |
| R14 | `MarkowitzOptimizationRequest` | `request_id: Uuid7`; `aligned_daily_returns_artifact_id: Uuid7`; `expected_return_vector: JsonObject`; `sample_covariance_artifact_id: Uuid7`; `weight_constraints: JsonObject`; `method_version: int >= 1`; `simulation_count: int >= 1 | None = None`; `frontier_points: int >= 1 | None = None`; `schema_version: Literal[1] = 1`. | FR-PORT-OPTIMIZE_MARKOWITZ_PORTFOLIOS. |
| R15 | `EfficientFrontier` | `frontier_id: Uuid7`; `request_id: Uuid7`; `points: tuple[FrontierPoint, ...] = ()` where `FrontierPoint(expected_return: DecimalValue, volatility: DecimalValue, weights: dict[nonempty str, DecimalValue])`; `maximum_sharpe_weights: dict[nonempty str, DecimalValue] | None = None`; `minimum_risk_weights: dict[nonempty str, DecimalValue] | None = None`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable `PortfolioOptimizationArtifact`. | FR-PORT-OPTIMIZE_MARKOWITZ_PORTFOLIOS. |
| R16 | `PortfolioMergePlan` | `plan_id: Uuid7`; `mode: Literal[SIMULATED_PORTFOLIO,PARALLEL_COMPOUND,FUZZY_ENSEMBLE]`; `source_strategy_version_ids: nonempty tuple[Uuid7, ...]`; `signal_aggregation: JsonObject`; `conflict_resolution: nonempty str`; `capital_sharing: JsonObject`; `order_identity_policy: nonempty str`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. `PARALLEL_COMPOUND`/`FUZZY_ENSEMBLE` are Experimental §21.9 rules. | FR-PORT-MERGE_PORTFOLIO_STRATEGIES, SPLIT_PORTFOLIO_STRATEGIES. |
| R17 | `PortfolioSplitPlan` | `plan_id: Uuid7`; `compound_strategy_version_id: Uuid7`; `output_strategy_ids: nonempty tuple[Uuid7, ...]`; `allocation_snapshot: Allocation`; `lineage: JsonObject`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Split creates independently versioned constituents preserving allocation and lineage. | FR-PORT-SPLIT_PORTFOLIO_STRATEGIES. |
| R18 | `PortfolioMethodDescriptor` | `method_id: nonempty str`; `version: int >= 1`; `plugin_id: plugin stable-ID` (§21.4 research-method plugin); `input_schema: JsonObject`; `output_schema: JsonObject`; `resource_bounds: JsonObject`; `seed_streams: tuple[nonempty str, ...] = ()`; `conformance_vectors: tuple[Uuid7, ...] = ()`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Experimental plugin algorithms only; complete schemas/determinism/conformance required. | FR-PORT-REGISTER_PORTFOLIO_METHODS. |

Cross-owner references used by these records (never copied): `InstrumentRef` (Catalogue); `AlignmentPolicy` (Data); `ResearchBudget` (Research); strategy versions/results (Strategy/Simulator); plugin stable IDs (Plugins). No subscriptions.

#### Ratified v1 capabilities and operation envelopes

All new (universal rule; shared `PortfolioFailure` with `code: Literal[PORTFOLIO_VALIDATION_FAILED,PORTFOLIO_ADMISSION_REJECTED,PORTFOLIO_VERSION_CONFLICT,CORRELATION_POLICY_INVALID,CURRENCY_RATE_MISSING,EXPOSURE_LIMIT_BREACHED,SEARCH_INFEASIBLE,PORTFOLIO_NOT_FOUND,PORTFOLIO_METHOD_UNSUPPORTED,CAPABILITY_UNAVAILABLE]`; no subscriptions):

1. `portfolio.compose-portfolios@1` / `ComposePortfoliosCapability` / `compose_portfolios` — ops `ADD_MEMBER, REMOVE_MEMBER, EDIT_ALLOCATION, EDIT_CONSTRAINTS, VALIDATE, VERSION, PROMOTE`. Success: `version: PortfolioVersion | None`; `findings: tuple[ValidationIssue, ...] = ()`. FRs: VERSION_PORTFOLIOS, VALIDATE_PORTFOLIO_ADMISSION, COMPOSE_PORTFOLIOS_MANUALLY, VERSION_PORTFOLIO_CHANGES.
2. `portfolio.analyze-correlation@1` / `AnalyzeCorrelationCapability` / `analyze_correlation` — ops `REQUEST, COMPUTE`. Success: `matrix: CorrelationMatrix | None`. FRs: VERSION_CORRELATION_INPUTS, COMPUTE_CORRELATION_MATRICES.
3. `portfolio.simulate-portfolios@1` / `SimulatePortfoliosCapability` / `simulate_portfolios` — ops `SIMULATE, CONVERT`. Success: `result: PortfolioResult | None`. FRs: SIMULATE_AGGREGATE_PORTFOLIOS, CONVERT_PORTFOLIO_CURRENCIES.
4. `portfolio.search-portfolios@1` / `SearchPortfoliosCapability` / `search_portfolios` — ops `PLAN, SEARCH, CHECKPOINT, RESUME`. Success: `plan: PortfolioSearchPlan | None`; `candidates: tuple[PortfolioCandidate, ...] = ()`. FRs: DEFINE_PORTFOLIO_SEARCH, REJECT_INFEASIBLE_SEARCHES, OPTIMIZE_PORTFOLIO_OBJECTIVES, CHECKPOINT_PORTFOLIO_SEARCH.
5. `portfolio.analyze-portfolio-risk@1` / `AnalyzePortfolioRiskCapability` / `analyze_portfolio_risk` — ops `REPORT, DEFINE_METRIC`. Success: `report: PortfolioRiskReport | None`; `metric: PortfolioMetricDefinition | None`. FRs: CALCULATE_PORTFOLIO_RISK, DEFINE_PORTFOLIO_METRICS, REPORT_PORTFOLIO_RESULTS, EXPORT_PORTFOLIO_RESULTS.
6. `portfolio.optimize-markowitz@1` / `OptimizeMarkowitzCapability` / `optimize_markowitz` — ops `OPTIMIZE`. Success: `frontier: EfficientFrontier | None`. FRs: OPTIMIZE_MARKOWITZ_PORTFOLIOS.
7. `portfolio.merge-portfolios@1` / `MergePortfoliosCapability` / `merge_portfolios` — ops `PLAN_MERGE, PLAN_SPLIT, EXECUTE`. Success: `merge_plan: PortfolioMergePlan | None`; `split_plan: PortfolioSplitPlan | None`. FRs: MERGE_PORTFOLIO_STRATEGIES, SPLIT_PORTFOLIO_STRATEGIES.
8. `portfolio.extend-portfolio-methods@1` / `ExtendPortfolioMethodsCapability` / `extend_portfolio_methods` — ops `REGISTER_METHOD` (Experimental gating). Success: `method: PortfolioMethodDescriptor | None`. FRs: REGISTER_PORTFOLIO_METHODS.
