...
from app.contracts.interfaces.operate_settings import (
    OperateDiagnosticsRequest,
    OperateDiagnosticsResult,
)
...
class SettingsGateway:
    """Translate operations without becoming the settings or diagnostics owner."""

    def __init__(
        self,
        provider: AdministerSettingsCapability,
        config: OperateSettingsConfig,
        diagnostics_provider: BuildDiagnosticsCapability | None = None,
    ) -> None:
    ...
    def config(self) -> OperateSettingsConfig:
        """Return active gateway configuration."""
        return self._config
    ...
    async def administer_settings(
        ...
        """Translate one settings request to the Workspace owner."""
        if self._closed:
            return _closed_failure()
        result = await self._provider.administer_settings(
    ...
    async def diagnostics(
        self,
        request: OperateDiagnosticsRequest,
    ) -> OperateDiagnosticsResult:
        """Translate one bounded diagnostics request."""
        if self._closed:
            return _closed_failure()
        if self._diagnostics is None:
            return _diagnostic_failure("Workspace diagnostic owner is unavailable.")
        if request.operation == "SNAPSHOT":
            return self._diagnostics.snapshot(request.providers)
        if request.operation == "COMPARE_BENCHMARK":
            if request.target is None or request.measurement is None:
                return _diagnostic_failure(
                    "target and measurement are required for benchmark comparison"
                )
            return self._diagnostics.compare_benchmark(
                request.target,
                request.measurement,
            )
        if request.operation == "EXPORT":
            if request.workspace_path is None:
                return _diagnostic_failure("workspace_path is required for export")
            return self._diagnostics.build_diagnostic_bundle(
    ...
    def close(self) -> None:
        """Dispose this translation generation."""
        self._closed = True
