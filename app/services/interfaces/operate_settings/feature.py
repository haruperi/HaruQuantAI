...
class OperateSettingsFeature:
    """Mount the translation gateway against exact Workspace owners."""
    ...
    def gateway(self) -> SettingsGateway | None:
        """Return the active gateway when mounted."""
        return self._gateway

    async def mount(self, context: FeatureContext, config: object) -> None:
        """Resolve owners, validate config, and publish the interface capability."""
        ...
        gateway = SettingsGateway(
            context.require(ADMINISTER_SETTINGS_CAPABILITY),
            parsed,
            context.require(BUILD_DIAGNOSTICS_CAPABILITY),
        )
