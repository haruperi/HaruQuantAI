...
| Completed | `memory_budget_mb` | `int` | 256 | No | Heavy workflows | Deterministic admission uses deep frame bytes and documented stage allocation multipliers; it is not OS-level process containment. |
...
|---|---|---|---|---|---|---|
| Completed | `FR-RES-001` | The system shall define bounded row, duration, artifact-size, and deterministic memory-admission budgets without claiming OS-level containment. | `ResearchResourceLimits(max_rows: int, max_duration_seconds: float, max_artifact_bytes: int, memory_budget_mb: int = 256)` | None | invalid/non-positive limit; estimated stage allocation exceeds budget | **Usage:** `01_contracts.py::fr_res_001`<br>**Unit:** `test_contract_configurations.py::test_resource_limits_reject_non_positive`, `test_data_validation.py::test_memory_admission_fails_before_over_budget_work` |
...
| Status | Requirement ID | Responsibility | Class / Function / Method | Side Effects | Raises | Usage / Test |
|---|---|---|---|---|---|---|
| Completed | `FR-RES-097` | Mask and render an approved artifact, enforce allowed root/overwrite/encoding/size/atomic policy, write via temporary replacement, emit a redacted audit event, persist the traceable manifest row through Data, and return `ArtifactReference`; metadata failure compensates the file write before failing. | `write_research_artifact(artifact: ResearchReport \| ResearchProfileSnapshot, destination: Path, *, config: ArtifactWriteConfig, auth: AuthContext, limits: ResearchResourceLimits) -> ArtifactReference` | File and transactional metadata write; event publication | invalid path, traversal, conflict, permission, serialization, size, atomicity, audit or metadata failure | **Usage:** `tests/research/usage/features/12_artifacts.py::fr_res_097`<br>**Unit:** `test_persistence.py`<br>**Integration:** `test_artifact_persistence.py` |
...
| Completed | `FR-RES-105` | Keep Research migration definitions in `app/services/research/migrations/` and execute the complete manifest through Data's ledger, checksum, lock, and transaction authority. | `build_research_migration_request` plus Data `run_domain_migrations` | Transactional schema migration | invalid request; ledger, lock, or checksum failure | **Usage:** `tests/research/usage/features/12_artifacts.py::fr_res_105`<br>**Integration:** `test_artifact_migrations.py::test_research_migration_runs_idempotently_through_data` |
| Completed | `FR-RES-106` | Declare `research_artifacts` as `STRICT`, carry `request_id` and `correlation_id`, and index `audit_event_id` and `sha256`; every artifact write inserts its traceable manifest row. | Schema definition and private persistence create operation | Transactional metadata write | checksum mismatch, insert conflict, or unconfirmed Data response | **Usage:** `tests/research/usage/features/12_artifacts.py::fr_res_106`<br>**Integration:** `test_artifact_migrations.py::test_research_migration_checksum_mismatch_fails_closed`, `test_artifact_persistence.py` |
...
`tests/research/usage/features/12_artifacts.py` contains `fr_res_097()`,
`fr_res_098()`, `fr_res_105()`, and `fr_res_106()`.
...
- [X] Research artifact ownership/migrations match the top-level data ownership table — evidence: `app/services/research/migrations/definitions.py:27`.
