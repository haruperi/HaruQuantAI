...
| Completed | `FR-RES-105` | Research migration definitions shall reside in `app/services/research/migrations/`, keeping schema evolution outside the private persistence layer. | `build_research_migration_request` | None | `ConfigurationError`: invalid request id | **Unit:** existing artifact persistence tests |
| Completed | `FR-RES-106` | `research_artifacts` shall be declared `STRICT`, carry `request_id` and `correlation_id`, and index `audit_event_id` in addition to `sha256`, so every written artifact is traceable to the audit event that authorised it. | Schema definition only | None | None | **Unit:** existing artifact persistence tests |
