"""Exit and Management Plan private model and transport."""
...
class _ManagementPlan(BaseModel):
    ...
    def _validate(self) -> _ManagementPlan:
...
def build_exit_plan(
    *,
    ...
    """Build a validated JSON-safe exit and management plan v1 mapping."""
    logger.info("Building exit and management plan")
    model = _ManagementPlan(
...
def parse_exit_plan(value: Mapping[str, object]) -> dict[str, Any]:
    """Parse a strict exit and management plan v1 mapping."""
    return dict(
        to_json_safe(
            _ManagementPlan.model_validate(dict(value)).model_dump(mode="json")
        )
    )
