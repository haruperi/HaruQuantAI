...
│   ├── naive_ma_trend_incremental.py   # Causal bounded-window MA signal parity
...
| Completed | `naive_ma_trend_incremental.py` | Preserve the same Naive MA Trend signals from one causal bounded bar window for canonical Simulation.             | `NaiveMATrendIncrementalEvaluator` | `signals/_mechanics.py`                                  |
...
| Status    | Requirement ID | Responsibility                                                                                                                                                                                        | Class / Function / Method           | Side Effects                                           | Raises                                                                                    | Usage / Test                                                                                                                                         |
| --------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Completed | `FR-STR-040` | Preserve Naive MA Trend long/short crossover entries and exits with the configured slow/trend filter. The batch evaluator consumes supplied SMA-seeded recursive EMA results; the registered incremental evaluator advances the exact same causal EMA state once per strictly progressing bar window and must retain signal parity. | `NaiveMATrendEvaluator`, `NaiveMATrendIncrementalEvaluator` | None | None; boundary returns config, indicator, incomplete-window, or progression error | **Usage:** `tests/strategy/usage/features/10_strategy_library.py`<br>**Unit:** `tests/strategy/unit/test_naive_ma_trend_evaluator.py` |
...
- Batch evaluator indicator values are supplied through official `IndicatorResult` contracts. The registered Naive MA Trend incremental mode is the sole exception: it maintains only current/prior SMA-seeded recursive EMA state from exact point-in-time bars and is parity-tested against the batch evaluator.
- Canonical Simulation may bind the registered Naive MA Trend evaluator once and consume its private compact active-name projection. Compact and public signal projections are parity-tested at every decision window. Other strategies retain the full public signal path unless their richer multi-action semantics receive equivalent dedicated parity evidence.
...
| Completed | `FR-STR-083` | Register one immutable Discretionary Manual Order strategy version per Trading-reachable route environment (`DEMO`, `LIVE`), idempotently, through the standard `register_strategy_version` registry gate — no bypass of registration, lifecycle-approval, or module-root checks. | `app/services/strategy/discretionary/registration.py`; `tests/strategy/usage/features/20_discretionary.py` |
