...
#### Ratified v1 public records (26)

All 26 records (25 absent + `StrategyVersion` incomplete) and 13 capabilities resolved; nothing removed. All records wire-native. Shared AST type literal: `AstType = Literal[BOOLEAN,INTEGER,DECIMAL,PRICE,QUANTITY,PERCENTAGE,DURATION,TIMEFRAME,INSTRUMENT,ENUM,SCALAR_SERIES,EVENT,ACTION]` (FR-STRAT-DEFINE_AST_TYPES). `strategies(name_normalized UNIQUE)`; versioned tables immutable with `content_hash`.

| # | Record | Exact wire fields | FRs / rules |
|---|---|---|---|
| R1 | `StrategyRef` | `strategy_id: Uuid7`; `schema_version: Literal[1] = 1`. | — |
| R2 | `StrategyVersion` (incomplete → complete) | `strategy_version_id: Uuid7`; `strategy_id: Uuid7`; `version: int >= 1`; `architecture: StrategyArchitecture`; `ast: StrategyAst`; `ast_hash: ContentHash`; `charts: nonempty tuple[ChartDefinition, ...]`; `direction: DirectionPolicy`; `parent_version_id: Uuid7 | None = None`; `creation_method: nonempty str`; `dependency_artifact_ids: tuple[Uuid7, ...] = ()`; `created_at: UtcTimestamp`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Uniqueness `(strategy_id,version)`; committing a draft creates the immutable version; results from the parent stay linked; serialize→deserialize→normalize retains AST hash. | FR-STRAT-VERSION_STRATEGY_DRAFTS, NORMALIZE_STRATEGY_AST, REPRESENT_TYPED_AST. |
| R3 | `StrategyAst` | `nodes: nonempty tuple[StrategyNode, ...]`; `expression_nodes: tuple[ExpressionNode, ...] = ()`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Commutatively reordered AND operands hash equally; ordered action sequences do not. | FR-STRAT-NORMALIZE_STRATEGY_AST. |
| R4 | `StrategyNode` | `node_id: Uuid7`; `kind: nonempty str`; `block_id: nonempty str`; `block_version: int >= 1`; `bindings: tuple[NodeBinding, ...] = ()` where `NodeBinding(port_name: nonempty str, source_node_id: Uuid7 | None = None, constant: JsonValue = None)`; `parameters: dict[str, JsonValue] = {}`; `children: tuple[Uuid7, ...] = ()` (ordered only where semantic); `schema_version: Literal[1] = 1`. | FR-STRAT-DEFINE_AST_NODES. |
| R5 | `ExpressionNode` | `node_id: Uuid7`; `operator: nonempty str`; `operands: nonempty tuple[Uuid7, ...]`; `result_type: AstType`; `schema_version: Literal[1] = 1`. Invalid series→boolean or price→duration connections are rejected. | FR-STRAT-DEFINE_AST_TYPES. |
| R6 | `BlockDefinition` | `block_id: nonempty str`; `version: int >= 1`; `category: nonempty str`; `input_types: tuple[AstType, ...] = ()`; `output_types: tuple[AstType, ...] = ()`; `parameter_schema: JsonObject`; `data_capabilities: tuple[CapabilityIdentifier, ...] = ()`; `supported_events: tuple[nonempty str, ...] = ()`; `target_capabilities: tuple[CapabilityIdentifier, ...] = ()`; `status: Literal[ACTIVE,DEPRECATED,RETIRED]`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Uniqueness `(stable_id,version)`; removing/changing a referenced block version is diagnosable. | FR-STRAT-DESCRIBE_BLOCKS, CATALOG_REFERENCE_BLOCKS. |
| R7 | `ParameterDefinition` | `name: nonempty str`; `value_type: AstType`; `domain: Literal[FIXED,DISCRETE,RANGE_STEP]`; `fixed_value: JsonValue = None`; `discrete_values: tuple[JsonValue, ...] = ()`; `range_min: DecimalValue | None = None`; `range_max: DecimalValue | None = None`; `step: DecimalValue > 0 | None = None`; `default_value: JsonValue = None`; `is_search_eligible: bool = False`; `mutation_behavior: str = ""`; `optimization_visible: bool = True`; `schema_version: Literal[1] = 1`. Integer 5–15 step 5 enumerates exactly 5, 10, 15; invalid/empty domains fail before search queueing. | FR-STRAT-DEFINE_PARAMETER_DOMAINS, DEFINE_SEARCH_PARAMETERS. |
| R8 | `ChartDefinition` | `ordinal: int >= 0`; `instrument: InstrumentRef`; `broker: BrokerRef | None = None`; `timeframe: Timeframe`; `role: Literal[PRIMARY,ADDITIONAL]`; `warmup_bars: int >= 0 = 0`; `schema_version: Literal[1] = 1`. Exactly one PRIMARY; missing chart data blocks execution and identifies every unresolved requirement. | FR-STRAT-CATALOG_BUILTIN_BLOCKS. |
| R9 | `DirectionPolicy` | `direction: Literal[LONG,SHORT,BOTH]`; `symmetry: Literal[INDEPENDENT,DERIVED]`; `opposite_map_version_id: Uuid7 | None = None` (required when DERIVED); `schema_version: Literal[1] = 1`. Derived short logic stored visibly and detachable without mutating long logic. | FR-STRAT-CONFIGURE_TRADE_DIRECTIONS, MAP_OPPOSITE_BLOCKS. |
| R10 | `VisibilityPolicy` | `chart_ordinal: int >= 0`; `shift: int >= 0 = 0`; `look_ahead: Literal[PROHIBITED] = "PROHIBITED"`; `schema_version: Literal[1] = 1`. Shift 0 exposes only the value observable at the current event; higher-timeframe future closes cannot influence lower timeframes. | FR-STRAT-DEFINE_SERIES_SHIFTS. |
| R11 | `StrategyValidationReport` | `report_id: Uuid7`; `strategy_version_id: Uuid7`; `findings: tuple[ValidationIssue, ...] = ()` (structure, types, block versions, parameter domains, charts, instruments, sessions, data precision, order lifecycle, sizing, exits, selected code target); `is_valid: bool`; `schema_version: Literal[1] = 1`. Response contains all findings. | FR-STRAT-VALIDATE_STRATEGIES. |
| R12 | `StrategyTemplate` | `template_id: Uuid7`; `name: nonempty str`; `base_version_id: Uuid7`; `placeholders: tuple[TemplatePlaceholder, ...] = ()` where `TemplatePlaceholder(name: nonempty str, is_required: bool, allowed_domains: tuple[JsonValue, ...] = ())`; `subtree_constraints: tuple[TemplateSubtreeConstraint, ...] = ()` where `TemplateSubtreeConstraint(placeholder: nonempty str, cardinality_min: int >= 0, cardinality_max: int >= 0, compatible_block_ids: tuple[nonempty str, ...] = (), complexity_limit: int >= 1)`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Instantiation with missing required or out-of-domain values fails without creating a version; materialization is type-valid or a structured rejection. | FR-STRAT-DEFINE_STRATEGY_TEMPLATES, CONSTRAIN_TEMPLATE_GRAMMAR. |
| R13 | `StrategyExchangePackage` | `package_id: Uuid7`; `container_schema_version: Literal[1]`; `strategy_id: Uuid7`; `strategy_version_id: Uuid7`; `ast_hash: ContentHash`; `created_at: UtcTimestamp`; `dependencies: tuple[PackageDependency, ...] = ()` where `PackageDependency(role: nonempty str, relative_path: nonempty str, media_type: nonempty str, size_bytes: int >= 0, sha256: ContentHash)`; `settings_hash: ContentHash | None = None`; `results_hash: ContentHash | None = None`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. `.sqxs` deterministic ZIP64 rules per §22.3; export→import reproduces AST/content hash; unknown required role rejects import; unknown optional entries preserved as namespaced attachments. | FR-STRAT-EXCHANGE_NATIVE_STRATEGIES. |
| R14 | `AtmExitDefinition` | `atm_id: Uuid7`; `stages: nonempty tuple[AtmStage, ...]` where `AtmStage(stage_name: nonempty str, size_percent: DecimalValue in (0,100], parameters: JsonObject)`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Sizing/protection/residual-position/collision/state semantics per §§18.6–18.7. | FR-STRAT-MODEL_ATM_EXITS. |
| R15 | `PartialExitDefinition` | `exit_id: Uuid7`; `kind: Literal[TARGET,STOP,TRAILING,BREAKEVEN,BARS,RULE,EOD,FRIDAY]`; `parameters: JsonObject`; `residual_policy: Literal[KEEP_RESIDUAL,CLOSE_ALL]`; `schema_version: Literal[1] = 1`. §23 exit fixtures pass before enablement. | FR-STRAT-MODEL_ATM_EXITS. |
| R16 | `PluginNodeRef` | `plugin_id: plugin stable-ID`; `api_version: semver str`; `schema_ref: nonempty str`; `capabilities: tuple[CapabilityIdentifier, ...] = ()`; `migration_hook: nonempty str | None = None`; `schema_version: Literal[1] = 1`. Missing plugin/migration yields a diagnosable unavailable strategy, never silent node loss. | FR-STRAT-IDENTIFY_PLUGIN_NODES. |
| R17 | `StrategyArchitecture` | `architecture_id: Uuid7`; `version: int >= 1`; `style: Literal[CLASSIC_RULES,SIGNAL_GATED,FUZZY_VOTING,CUSTOM]`; `custom_template_version_id: Uuid7 | None = None` (required for CUSTOM); `schema_version: Literal[1] = 1`. Signal-gated evaluation prevents conflicting entry/exit signals. | FR-STRAT-DEFINE_STRATEGY_ARCHITECTURES. |
| R18 | `RandomGroupVersion` | `group_id: Uuid7`; `version: int >= 1`; `group_type: Literal[CONDITION,VALUE,ACTION]`; `block_templates: nonempty tuple[RandomBlockTemplate, ...]` where `RandomBlockTemplate(block_id: nonempty str, block_version: int >= 1, weight: DecimalValue >= 0)`; `fixed_parameters: tuple[nonempty str, ...] = ()`; `applicability: tuple[CapabilityIdentifier, ...] = ()`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable. | FR-STRAT-DEFINE_RANDOM_GROUPS. |
| R19 | `OppositeMapVersion` | `map_id: Uuid7`; `version: int >= 1`; `source_block_id: nonempty str`; `source_relation: nonempty str`; `action: Literal[MAP,PRESERVE,REJECT]`; `target_block_id: nonempty str | None = None`; `target_relation: nonempty str | None = None`; `parameter_transform: JsonObject = {}`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable. | FR-STRAT-MAP_OPPOSITE_BLOCKS. |
| R20 | `IndicatorDefinition` | `indicator_id: nonempty str`; `version: int >= 1`; `source: Literal[BUILTIN,EXTERNAL]`; `output_lines: nonempty tuple[IndicatorOutputLine, ...]` where `IndicatorOutputLine(line_name: nonempty str, value_type: AstType)`; `parameters: tuple[ParameterDefinition, ...] = ()`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. | FR-STRAT-DEFINE_EXTERNAL_INDICATORS (builtin side). |
| R21 | `ExternalIndicatorDefinitionVersion` | `definition_id: Uuid7`; `version: int >= 1`; `value_kind: Literal[NUMBER,PRICE,PRICE_RANGE,SIGNAL]`; `output_lines: nonempty ordered tuple[IndicatorOutputLine, ...]`; `parameter_schema: JsonObject`; `source_platform: nonempty str`; `source_version: nonempty str`; `chart_binding_defaults: JsonObject = {}`; `warmup_bars: int >= 0 = 0`; `shift_semantics: nonempty str`; `target_fragments: tuple[TargetFragment, ...] = ()` where `TargetFragment(target_id: nonempty str, artifact_id: Uuid7)`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable; fragments isolated from canonical semantics; shift-0/look-ahead sentinels per §15.4. | FR-STRAT-DEFINE_EXTERNAL_INDICATORS, ISOLATE_INDICATOR_FRAGMENTS. |
| R22 | `CodeTargetDescriptor` | `target_id: nonempty str`; `version: int >= 1`; `supported_ast_capabilities: tuple[CapabilityIdentifier, ...] = ()`; `engine_profile_id: Uuid7 | None = None`; `emitter_version: nonempty str`; `formatter: nonempty str`; `compiler_adapter: nonempty str | None = None`; `packaging_rules: JsonObject`; `unsupported_semantics: tuple[nonempty str, ...] = ()`; `compatibility_record_id: Uuid7 | None = None`; `schema_version: Literal[1] = 1`. Advertised only from a versioned compatibility record backed by compiler/runtime versions, capability tests, and §23.13 parity fixtures; `PSEUDOCODE` is a deterministic human-readable built-in target. | FR-STRAT-REGISTER_CODE_TARGETS, DESCRIBE_EMITTER_CAPABILITIES, GENERATE_PSEUDOCODE, ADVERTISE_COMPATIBLE_TARGETS. |
| R23 | `CodegenRequest` | `request_id: Uuid7`; `strategy_version_id: Uuid7`; `target_id: nonempty str`; `target_version: int >= 1`; `settings_hash: ContentHash`; `schema_version: Literal[1] = 1`. | FR-STRAT-GENERATE_CODE_DETERMINISTICALLY. |
| R24 | `CodegenResult` | `result_id: Uuid7`; `request_id: Uuid7`; `source_artifact_id: Uuid7`; `source_hash: ContentHash`; `compile_status: Literal[PENDING,PASSED,FAILED,SKIPPED] = "PENDING"`; `diagnostics: tuple[CompilerDiagnostic, ...] = ()` where `CompilerDiagnostic(severity: Literal[ERROR,WARNING,INFO], code: nonempty str, file: nonempty str, line: int >= 1 | None = None, column: int >= 1 | None = None, message: nonempty str, node_id: Uuid7 | None = None)`; `manifest_id: Uuid7`; `parity_report_id: Uuid7 | None = None`; `schema_version: Literal[1] = 1`. `PASSED` only with real compiler success and existing output artifact; repeated emission has identical normalized source hash. | FR-STRAT-GENERATE_CODE_DETERMINISTICALLY, PARSE_COMPILER_DIAGNOSTICS, VERIFY_MQL5_COMPILE. |
| R25 | `CodeManifest` | `manifest_id: Uuid7`; `strategy_version_id: Uuid7`; `generator_version: nonempty str`; `engine_profile_id: Uuid7 | None = None`; `settings_hash: ContentHash`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Source traceable to exactly one strategy version and generator. | FR-STRAT-EMBED_CODE_MANIFEST. |
| R26 | `DeploymentPackage` | `package_id: Uuid7`; `codegen_result_id: Uuid7`; `target_id: nonempty str`; `target_version: int >= 1`; `engine_profile_id: Uuid7 | None = None`; `strategy_source_artifact_id: Uuid7`; `strategy_binary_artifact_id: Uuid7 | None = None`; `dependency_artifact_ids: tuple[Uuid7, ...] = ()`; `installation_manifest: JsonObject`; `checksums: dict[nonempty str, ContentHash]`; `validation_result_id: Uuid7 | None = None`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable; uniqueness `(codegen_run_id,package_hash)`. | FR-STRAT-PACKAGE_TARGET_CODE, MAP_ORDER_IDENTITIES, STORE_CODE_ARTIFACTS. |

Cross-owner references: `InstrumentRef`, `BrokerRef` (Catalogue); plugin stable IDs (Plugins); artifact IDs (Workspace artifact store); `Timeframe` (common). No subscriptions.

#### Ratified v1 capabilities and operation envelopes

All new (universal rule; shared `StrategyFailure` with `code: Literal[STRATEGY_VALIDATION_FAILED,STRATEGY_AST_INVALID,STRATEGY_BLOCK_UNAVAILABLE,STRATEGY_TEMPLATE_INVALID,STRATEGY_IMPORT_INVALID,STRATEGY_PLUGIN_UNAVAILABLE,STRATEGY_TARGET_UNSUPPORTED,CODEGEN_FAILED,COMPILE_FAILED,CAPABILITY_UNAVAILABLE]`; no subscriptions — no FR requires live/stream/replay):

1. `strategy.define-ast@1` / `DefineAstCapability` / `define_ast` — ops `NORMALIZE, VALIDATE`. Success: `ast: StrategyAst | None`; `report: StrategyValidationReport | None`. FRs: REPRESENT_TYPED_AST, DEFINE_AST_NODES, DEFINE_AST_TYPES, NORMALIZE_STRATEGY_AST, VALIDATE_STRATEGIES.
2. `strategy.catalog-blocks@1` / `CatalogBlocksCapability` / `catalog_blocks` — ops `CATALOG, DESCRIBE, FILTER_COMPATIBLE` (schema-aware insertion-point filtering). Success: `blocks: tuple[BlockDefinition, ...] = ()`; `parameters: tuple[ParameterDefinition, ...] = ()`. FRs: DESCRIBE_BLOCKS, SUPPORT_STRATEGY_NODES, DEFINE_PARAMETER_DOMAINS, CATALOG_REFERENCE_BLOCKS, FILTER_COMPATIBLE_BLOCKS.
3. `strategy.configure-charts@1` / `ConfigureChartsCapability` / `configure_charts` — ops `CONFIGURE_CHARTS, CONFIGURE_DIRECTION, DECLARE_SHIFT`. Success: `charts: tuple[ChartDefinition, ...] = ()`; `direction: DirectionPolicy | None`; `visibility: VisibilityPolicy | None`. FRs: CATALOG_BUILTIN_BLOCKS (charts), CONFIGURE_TRADE_DIRECTIONS, DEFINE_SERIES_SHIFTS.
4. `strategy.version-strategies@1` / `VersionStrategiesCapability` / `version_strategies` — ops `CREATE_DRAFT, COMMIT_VERSION, SNAPSHOT_DRAFT` (backtest binding commits the exact draft). Success: `strategy_version: StrategyVersion | None`; `strategy_version_id: Uuid7 | None = None`. FRs: VERSION_STRATEGY_DRAFTS, SNAPSHOT_BACKTEST_DRAFT.
5. `strategy.edit-templates@1` / `EditTemplatesCapability` / `edit_templates` — ops `DEFINE_TEMPLATE, INSTANTIATE` (visual-edit operations are client-side; commands flow through version-strategies). Success: `template: StrategyTemplate | None`; `instantiated_version_id: Uuid7 | None = None`. FRs: DEFINE_STRATEGY_TEMPLATES, CONSTRAIN_TEMPLATE_GRAMMAR, EDIT_STRATEGIES_VISUALLY (command side).
6. `strategy.exchange-strategies@1` / `ExchangeStrategiesCapability` / `exchange_strategies` — ops `EXPORT, IMPORT, IMPORT_LEGACY` (legacy `.sqx` only through an isolated importer plugin; unmapped source fields preserved in a namespaced attachment; never silently invents semantics). Success: `package: StrategyExchangePackage | None`; `imported_version_id: Uuid7 | None = None`. FRs: EXCHANGE_NATIVE_STRATEGIES, ISOLATE_LEGACY_IMPORTS, IMPORT_LEGACY_STRATEGIES.
7. `strategy.define-architectures@1` / `DefineArchitecturesCapability` / `define_architectures` — ops `DEFINE_ARCHITECTURE, DEFINE_RANDOM_GROUP, MAP_OPPOSITE`. Success: `architecture: StrategyArchitecture | None`; `random_group: RandomGroupVersion | None`; `opposite_map: OppositeMapVersion | None`. FRs: DEFINE_STRATEGY_ARCHITECTURES, DEFINE_RANDOM_GROUPS, MAP_OPPOSITE_BLOCKS.
8. `strategy.define-indicators@1` / `DefineIndicatorsCapability` / `define_indicators` — ops `DEFINE_INDICATOR, DEFINE_EXTERNAL`. Success: `indicator: IndicatorDefinition | None`; `external_definition: ExternalIndicatorDefinitionVersion | None`. FRs: DEFINE_EXTERNAL_INDICATORS.
9. `strategy.model-atm-exits@1` / `ModelAtmExitsCapability` / `model_atm_exits` — ops `DEFINE_ATM, DEFINE_PARTIAL_EXIT`. Success: `atm: AtmExitDefinition | None`; `partial_exit: PartialExitDefinition | None`. FRs: MODEL_ATM_EXITS.
10. `strategy.extend-plugin-nodes@1` / `ExtendPluginNodesCapability` / `extend_plugin_nodes` — ops `REGISTER_PLUGIN_NODE` (Volume-Profile/TPO nodes are Experimental and gated per §2). Success: `plugin_node: PluginNodeRef | None`. FRs: IDENTIFY_PLUGIN_NODES, CALCULATE_VOLUME_PROFILES.
11. `strategy.generate-code@1` / `GenerateCodeCapability` / `generate_code` — ops `REGISTER_TARGET, GENERATE` (deterministic; typed-value lowering; shared canonical AST/engine-profile semantics; no target fragment persisted in canonical AST). Success: `target: CodeTargetDescriptor | None`; `request: CodegenRequest | None`; `result: CodegenResult | None`; `manifest: CodeManifest | None`. FRs: REGISTER_CODE_TARGETS, GENERATE_CODE_DETERMINISTICALLY, EMBED_CODE_MANIFEST, LOWER_TYPED_VALUES, DESCRIBE_EMITTER_CAPABILITIES, SHARE_TARGET_SEMANTICS, GENERATE_PSEUDOCODE, ADVERTISE_COMPATIBLE_TARGETS.
12. `strategy.generate-mql5@1` / `GenerateMql5Capability` / `generate_mql5` — ops `GENERATE, COMPILE, VERIFY, COMPARE, PACKAGE` (MetaEditor invoked in an isolated worker with timeout/captured output; exit-success without expected artifact is not `PASSED`; order-identity/Magic-Number mapping in lowering). Success: `result: CodegenResult | None`; `package: DeploymentPackage | None`. FRs: GENERATE_MQL5_TARGET, INVOKE_METAEDITOR, PARSE_COMPILER_DIAGNOSTICS, VERIFY_MQL5_COMPILE, COMPARE_MQL5_RESULTS, STORE_CODE_ARTIFACTS, PACKAGE_TARGET_CODE, MAP_ORDER_IDENTITIES.
13. `strategy.generate-targets@1` / `GenerateTargetsCapability` / `generate_targets` — ops `IMPLEMENT_TARGET, VALIDATE_TARGET` (MQL4, EasyLanguage/MultiCharts, JForex). Success: `target: CodeTargetDescriptor | None`; `result: CodegenResult | None`. FRs: IMPLEMENT_CODE_TARGETS, ISOLATE_INDICATOR_FRAGMENTS (target side).
