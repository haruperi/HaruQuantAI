...
2. **`plugins.sandbox-permissions@1` / `SandboxPermissionsCapability`** — `async def sandbox_permissions(request)`. Operations `Literal[GRANT,INSPECT]`: GRANT requires `plugin_id`,`workspace_id`,`version`; INSPECT requires `plugin_id`,`workspace_id`. Success: `permission_set: PluginPermissionSet | None`. FRs: FR-PLUG-ISOLATE_PLUGIN_EXECUTION, FR-PLUG-RESTRICT_PLUGIN_SECRETS. Event union empty.
