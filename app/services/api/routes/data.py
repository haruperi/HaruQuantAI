...
from app.services.api.routes.market_overlays import build_technical_overlay
...
def _build_directory_gateway_response(
    directory_response: object,
    *,
    request_id: str,
    route: str,
    operation: str,
    success_message: str,
    failure_message: str,
    row_augment: Callable[[dict[str, Any]], dict[str, Any]] | None = None,
) -> object:
    """Normalize a Data directory-shaped standard response into the envelope.

    Shared by ``/markets`` (full-catalog) and ``/quotes`` (explicit-symbol)
    reads, which both return the same ``MarketDirectory`` shape from Data.

    Args:
        directory_response: Data's standard response carrying a
            ``MarketDirectory``-shaped payload.
        request_id: Canonical operation request identifier.
        route: Canonical route path for the envelope metadata.
        operation: Canonical operation id for the envelope metadata.
        success_message: Message used on a successful response.
        failure_message: Message used on a failed response.
        row_augment: Optional per-row transform (e.g. adding API-composed
            technical overlays) applied after converting the Data-owned
            payload to a plain dict; the Data contract itself is unchanged.

    Returns:
        Gateway-envelope response.
    """
    response_status = str(getattr(directory_response, "status", "success"))
    data_payload = getattr(directory_response, "data", None)
    if row_augment is not None and response_status == "success" and data_payload:
        dumped = data_payload.model_dump(mode="json")
        dumped["rows"] = [row_augment(row) for row in dumped["rows"]]
        data_payload = dumped
    ...
    if response_status != "success" and data_error is not None:
        ...
        gateway_error = {
            ...
            "message": str(getattr(data_error, "message", failure_message)),
...
def _list_markets(
    ...
    return _build_directory_gateway_response(
...
def _get_quotes(
    ...
    Unlike ``/markets``, this never walks the source's full symbol catalog —
    it enriches exactly the caller-named symbols (e.g. one watchlist's
    contents), so cost scales with the list size, not the broker's universe.

    Args:
        context: Validated authentication context.
        symbols: Comma-separated broker-native symbols to quote.
        source_id: Explicit source override; resolved to the configured
            runtime broker when omitted.
        include_technicals: When true, composes each row with the API-owned
            Volatility/ADR/Range overlay (Data's D1 bars plus Indicators'
            formulas). Opt-in because it costs a historical fetch and two
            indicator calculations per symbol, well above a plain quote.

    Returns:
        Gateway-envelope categorized quote response.
    ...
    resolved_source_id = resolve_runtime_source_id(source_id, request_id=request_id)
    quote_request = build_symbols_quote_request(
        source_id=resolved_source_id,
    ...
    def _augment(row: dict[str, Any]) -> dict[str, Any]:
        """Merge one row's composed technical overlay.

        Returns:
            The row with volatility/adr/range_percent_of_adr merged in.
        """
        overlay = build_technical_overlay(
            resolved_source_id, str(row["symbol"]), request_id=request_id
        )
        last = row.get("last")
        open_price = overlay.open if overlay.open is not None else row.get("open")
        high = overlay.high if overlay.high is not None else row.get("high")
        low = overlay.low if overlay.low is not None else row.get("low")
        change = (
            float(last) - float(open_price)
            if last is not None and open_price is not None and float(open_price) != 0
            else None
        )
        change_percent = (
            change / float(open_price) * 100.0
            if change is not None and open_price is not None
            else None
        )
        change_pips = (
            change / overlay.pip_size
            if change is not None
            and overlay.pip_size is not None
            and overlay.pip_size > 0
            else None
        )
        return {
            **row,
            ...
            "volatility": overlay.volatility_percent,
            "adr": overlay.adr_pips,
            "range_percent_of_adr": overlay.range_percent_of_adr,
        }

    return _build_directory_gateway_response(
        get_symbols_quotes(quote_request),
        request_id=request_id,
        route="/api/v1/data/quotes",
        operation="api.data.quotes",
        success_message="Quotes retrieved",
        failure_message="Quotes unavailable",
        row_augment=_augment if include_technicals else None,
    )
