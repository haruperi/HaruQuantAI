"""Authenticated account-owned watchlist HTTP boundary.

Every read and write delegates to the Identity-owned watchlist functions,
...
def _list_watchlists(
    ...
    return list(
        list_watchlists(
            context.principal_id, source_id=source_id, request_id=request_id
        )
    )
    ...
    try:
        ...
        if body.name is not None:
            current = rename_watchlist(
                watchlist_id, context.principal_id, body.name, request_id=request_id
            )
