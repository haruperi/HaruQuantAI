...
- Safe documentation browsing and, only when approved for the environment, audited
  documentation writes within a configured root.
...
- Another domain's tables, artifacts, migrations, credentials, SDK objects, or internal
  imports.
...
| `TradeIntent`, `StrategyRegistrationRequest`, `StrategyParameterUpdateRequest` | `v1` | Strategy | Strategy views and explicitly approved registration/update commands. |
| `RiskDecision`, `ActionPolicyVerdict`, `KillSwitchCommand`, `KillSwitchState`, `ApprovalAttestation` | `v1` | Risk | Risk views, Risk-owned action permission, governed operator commands, canonical safety state/hierarchy, and approval attestation. |
...
| `TradeRecord`, `ExecutionReceipt`, `OrderIntent` | `v1` | Trading | Live/paper status and governed execution outcomes. |
...
|---|---|---|---|
| Missing | User, session, and settings state | UI/API identity/settings contracts | `app/services/api/state/migrations.py` |
| Missing | HTTP-idempotency records | UI/API replay/conflict checks only | `app/services/api/state/migrations.py` |
| Missing | Frontend client state | Browser-local, non-authoritative UI state only | None |
...
│   ├── strategies.py             # Strategy catalogue, versions, imports, SQX
...
│   ├── documentation.py          # Safe documentation tree/read/write/delete
...
| `CAP-UI-006` health/readiness | `health/`; `FR-API-018`, `FR-API-019` |
...
| `CAP-UI-010` strategy/SQX | `routes/strategies.py`; `FR-API-025` |
...
| `CAP-UI-019` documentation | `routes/documentation.py`; `FR-API-033` |
...
| `UIAPI-FR-202`–`207` | `FR-API-024`, `FR-API-033` |
...
|---|---|---|---|---|---|---|---|---|---|
| Partial | `WF-API-001` | Internal | Gateway startup and readiness | None | Process configuration | Canonical app with truthful readiness | `FR-API-014`, `FR-API-015`, `FR-API-033`, `FR-API-034` | Required failure blocks startup/readiness; approved optional failure is reported degraded | `tests/api/integration/test_startup.py::test_required_failure_blocks_readiness()` |
...
| Partial | `WF-API-003` | Cross-domain | Authentication and settings | None | Credentials or session | Validated `AuthContext` and UI/API-owned settings response | `FR-API-008`–`FR-API-013`, `FR-API-021`, `FR-API-022` | No fallback identity; unavailable session/idempotency storage fails governed writes closed | `tests/api/integration/test_auth_settings.py::test_login_settings_logout()` |
| Partial | `WF-API-004` | Cross-domain | Market data and dataset preparation | `SYS-WF-001`, `SYS-WF-002` | Authenticated source/range request | Bounded Data result or provider error | `FR-API-023` | Requested provider failure is surfaced; no provider or user fallback | `tests/api/integration/test_data_boundary.py::test_prepare_dataset_delegates_to_data()` |
| Partial | `WF-API-005` | Cross-domain | Strategy catalogue, approved optimization adoption, and SQX lifecycle (SQX = StrategyQuant X strategy-file import/list/score; every update reaches Strategy only as a `StrategyRegistrationRequest` / `StrategyParameterUpdateRequest` — UI/API never registers strategy state itself) | `SYS-WF-003`, `SYS-WF-004` | Authenticated strategy command/query or explicitly approved Optimization selection | Strategy-owned record, artifact, or compensating failure | `FR-API-025` | Missing approval blocks optimization adoption; partial artifact failure is reconciled; gateway never writes strategy state | `tests/api/integration/test_strategy_boundary.py::test_strategy_lifecycle()` |
| Partial | `WF-API-006` | Cross-domain | Synchronous backtest run and result review | `SYS-WF-001` | Exact `SimulationBacktestRequestV1` plus `AuthContext` | Completed `SimulationResult`, derived `PerformanceReport`, or structured error | `FR-API-026`, `FR-API-018`–`FR-API-020` | Incomplete runs are not published; no session/queue/log-stream state is implied | `tests/api/integration/test_backtest_boundary.py::test_synchronous_backtest_run_and_review()` |
...
| Partial | `WF-API-009` | Cross-domain | Synchronous Optimization and scenario run | `SYS-WF-003` | Bounded optimization request | Terminal in-memory `OptimizationResult` or structured error | `FR-API-030` | No create/detail/cancel/progress/WebSocket or repository-backed job lifecycle is exposed | `tests/api/integration/test_optimization_boundary.py::test_synchronous_run_and_result()` |
| Partial | `WF-API-010` | Cross-domain | Risk decision support | `SYS-WF-002`, `SYS-WF-005` | Authorized risk request | Typed Risk-owned evaluation | `FR-API-028` | Missing evidence or stale state fails closed; gateway performs no calculation | `tests/api/integration/test_risk_boundary.py::test_risk_delegation()` |
| Partial | `WF-API-011` | Cross-domain | Core Edge Lab research | `SYS-WF-004` | Dataset/research request | Registered `ResearchReport v1` or structured error | `FR-API-031` | Leakage/provider failures block publication; internal profiles, snapshots, and unsupported endpoints are absent | `tests/api/integration/test_research_boundary.py::test_core_edge_workflow()` |
| Partial | `WF-API-012` | Cross-domain | Live/paper session and governed broker action | `SYS-WF-002`, `SYS-WF-005` | Authenticated Trading command | Trading-owned status, receipt, or rejection | `FR-API-029` | Closed live flags, Risk, approval, reconciliation, idempotency, audit, or kill-switch gate causes no broker mutation | `tests/api/integration/test_trading_boundary.py::test_live_mutation_cannot_bypass_gates()` |
| Partial | `WF-API-013` | Cross-domain | Operator approval, kill switch, and event review | `SYS-WF-005` | Validated operator principal | `ApprovalAttestation`, audited Risk/Trading command result, or protected event | `FR-API-034` | Underprivileged caller is rejected; no public sample stream | `tests/api/integration/test_operator_boundary.py::test_operator_kill_switch()` |
| Partial | `WF-API-014` | Internal | Documentation management | None | Authorized safe relative path | Tree/content or audited write/delete | `FR-API-031` | Traversal, symlink escape, bad content, or disallowed environment is rejected before I/O | `tests/api/integration/test_documentation.py::test_safe_document_lifecycle()` |
...
**Purpose:** Hash and verify UI/API identity credentials, authenticate users, enforce
sessions and permissions, and construct the Utils-owned `AuthContext`. UI/API owns
durable state ownership and browser/service authentication transport.
...
| Partial | `sessions.py` | Authenticate credentials and manage a single active UI/API-owned session | `authenticate_user`, `create_session`, `validate_session`, `revoke_session` | **Standard library:** `datetime`<br>**Required third-party:** `pydantic>=2.13.4`<br>**Local:** UI/API state store; Utils auth/context API |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `redaction.py` | Publish only allowlisted, secret-safe request telemetry | `SecretRedactionMiddleware` | **Standard library:** None<br>**Required third-party:** FastAPI/Starlette; exact compatible constraints belong in `pyproject.toml`<br>**Local:** Utils redaction/logger APIs |
| Partial | `context.py` | Attach request/trace, route intent, actor, and session context | `RequestContextMiddleware` | **Standard library:** None<br>**Required third-party:** FastAPI/Starlette; exact compatible constraints belong in `pyproject.toml`<br>**Local:** `contracts`, `identity` public APIs |

| Status | Requirement ID | Responsibility | Class / Function / Method | Side Effects | Raises | Usage / Test |
|---|---|---|---|---|---|---|
| Partial | `FR-API-016` | Redact secrets before any log/trace/metric emission and log only allowlisted method, route, identifiers, status, duration, and error code. | `SecretRedactionMiddleware` | Log publication | `TelemetryError`: safe telemetry cannot be emitted where required | **Usage:** `tests/api/usage/test_usage_middleware.py::test_usage_redacted_request()`<br>**Unit:** `tests/api/unit/test_redaction.py::test_tokens_never_logged()` |
| Partial | `FR-API-017` | Create/validate request and correlation IDs, classify registered route intent, authenticate where required, and attach canonical context. | `RequestContextMiddleware` | Local state mutation | `AuthenticationError`: protected request lacks valid authority; `ValidationError`: invalid identifiers | **Usage:** `tests/api/usage/test_usage_middleware.py::test_usage_request_context()`<br>**Unit:** `tests/api/unit/test_context.py::test_unknown_route_has_bounded_metadata()` |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `probes.py` | Report minimal public liveness and protected dependency readiness | `get_liveness`, `get_readiness` | **Standard library:** `collections.abc`<br>**Required third-party:** None<br>**Local:** approved dependency health APIs; `contracts` |

| Status | Requirement ID | Responsibility | Class / Function / Method | Side Effects | Raises | Usage / Test |
|---|---|---|---|---|---|---|
| Partial | `FR-API-018` | Return HTTP 200 with coarse service status only when the process accepts requests; expose no private dependency data. | `get_liveness() -> ApiResponse[Liveness]` | None | None | **Usage:** `tests/api/usage/test_usage_health.py::test_usage_liveness()`<br>**Unit:** `tests/api/unit/test_health.py::test_liveness_contains_no_private_data()` |
...
**Configuration and Limits Manifest:** None. Required/optional classification is owned
by composition and the configured dependency set.
...
### 4.5 `streams/` — Ordered event delivery
...
### 4.6 `routes/` — Thin HTTP and streaming boundaries
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `auth.py` | Authentication HTTP boundary | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts` |
| Partial | `settings.py` | Settings boundary | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`; approved settings state-owner API |
| Partial | `data.py` | Market symbols and prepared datasets | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`; Data public API |
| Partial | `strategies.py` | Strategies, versions, imports, and SQX | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`; Strategy and approved Analytics public APIs |
| Partial | `backtests.py` | One bounded synchronous backtest request returning its terminal result/report | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`; Simulation and Analytics public APIs |
...
| Partial | `risk.py` | Risk decision support | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`; Risk public API |
| Partial | `trading.py` | Live/paper Trading lifecycle, reads, mutations, and events | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`, `streams`; Trading and Risk public APIs |
| Partial | `optimization.py` | Synchronous Optimization runs and scenarios | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`; Optimization public API |
| Partial | `research.py` | Initial core Edge Lab research | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`; Research, Data, and Analytics public APIs |
| Partial | `dashboards.py` | Read-only operational/analytics snapshots | `router` | **Standard library:** None<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`; Data, Trading, Analytics, and Utils public APIs |
| Partial | `documentation.py` | Safe docs tree/read and controlled write/delete | `router` | **Standard library:** `pathlib`<br>**Required third-party:** FastAPI and the manifest-declared Pydantic constraint; exact compatible FastAPI constraint belongs in `pyproject.toml`<br>**Local:** `identity`, `contracts`, Utils audit/redaction |
| Partial | `operator.py` | Approvals, kill-switch commands, protected owner events, and bounded audit views | `router` | Same; Risk, Trading, Data audit query, streams |
...
| Status | Requirement ID | Responsibility | Class / Function / Method | Side Effects | Raises | Usage / Test |
|---|---|---|---|---|---|---|
| Partial | `FR-API-022` | Expose typed registration, login, and logout without fallback identities. | `auth.router: APIRouter` | Persistence write | Standard 400/401/403/422/429/500/503 envelopes | **Usage:** `tests/api/usage/test_usage_auth_routes.py::test_usage_auth_routes()`<br>**Unit:** `tests/api/unit/test_auth_routes.py::test_auth_contracts()` |
| Partial | `FR-API-023` | Expose authenticated settings read/update through one canonical path. | `settings.router: APIRouter` | Read-only; persistence write | Standard 401/403/404/409/422/503 envelopes | **Usage:** `tests/api/usage/test_usage_settings_routes.py::test_usage_settings_routes()`<br>**Unit:** `tests/api/unit/test_settings_routes.py::test_settings_contracts()` |
| Partial | `FR-API-024` | Expose authenticated symbol discovery and bounded delegated dataset preparation. | `data.router: APIRouter` | Read-only; external API call | Standard 401/403/413/422/502/503 envelopes | **Usage:** `tests/api/usage/test_usage_data_routes.py::test_usage_data_routes()`<br>**Unit:** `tests/api/unit/test_data_routes.py::test_data_contracts()` |
| Partial | `FR-API-025` | Expose strategy catalogue/version/import/export, explicitly approved Optimization-result adoption, and SQX import/list/score through Strategy-owned commands and state. | `strategies.router: APIRouter` | Read-only; owner-domain persistence write; artifact read/write | Standard 401/403/404/409/413/415/422/503 envelopes | **Usage:** `tests/api/usage/test_usage_strategy_routes.py::test_usage_strategy_routes()`<br>**Unit:** `tests/api/unit/test_strategy_routes.py::test_strategy_contracts()` |
| Partial | `FR-API-026` | Expose one exact synchronous `SimulationBacktestRequestV1` run returning a terminal `SimulationResult`/report or structured error through Simulation/Analytics; no queued/query/session/log-stream lifecycle. | `backtests.router: APIRouter` | Read-only external-domain calls | Standard 401/403/409/422/503 envelopes | **Usage:** `tests/api/usage/test_usage_backtest_routes.py::test_usage_synchronous_backtest_routes()`<br>**Unit:** `tests/api/unit/test_backtest_routes.py::test_backtest_contract_matches_adr_0014()` |
...
| `strategies.py` | `GET /api/strategies/templates/{name}`; CRUD `/api/strategies`; version list/code/rollback; export/import; explicit approved registration/parameter-update commands; `POST /api/sqx/import`; `POST /api/sqx/calculate-scores`; `GET /api/sqx/strategies` | Authenticated and command permission; Strategy, Analytics for approved scoring | Reads plus Strategy-owned writes/artifacts; every mutation/import idempotent with compensation |
...
| `documentation.py` | `GET /api/docs/files`; `GET /api/docs/content`; `POST /api/docs/save`; `DELETE /api/docs/delete` | Authenticated developer/operator; UI/API boundary | Local read/write/delete; writes environment-gated, audited, idempotent, Markdown-only |
| `operator.py` | Protected readiness; `POST /api/operator/approvals`; `POST /api/operator/kill-switch`; `GET /api/operator/audit-events`; `GET /api/operator/events/stream` | Validated human operator; UI/API owns attestation, Risk owns token/policy/state, Data owns audit query/page, Trading enforces | Attestation and governed command submission; bounded audit reads delegate to Data; UI/API never issues Risk token/verdict or reads audit tables; public stream forbidden |
...
| Status | Setting / Limit | Type | Default | Required | Used by | Description |
|---|---|---|---|---|---|---|
| Missing | `DOCS_ROOT` | `Path` | None | For docs routes | `documentation.router` | Missing/unsafe root blocks docs I/O. |
| Missing | `DOCS_WRITE_ENABLED` | `bool` | `false` | Yes | `documentation.router` | False blocks all write/delete before filesystem mutation. |
| Missing | `STRATEGY_IMPORT_MAX_BYTES` | `int` | None | Yes before strategy/SQX import activation | `strategies.router` | Must be explicitly configured from measured evidence; oversized input is rejected before parsing. |
| Missing | `STRATEGY_IMPORT_CONTENT_TYPES` | `frozenset[str]` | None | For strategy/SQX imports | `strategies.router` | Unsupported types are rejected and partial artifacts cleaned. |
...
applies; raw exceptions never cross the boundary; 204 never has a body; imports define
type/size/cleanup; partial mutation failure rolls back, compensates, or returns an
...
### 4.7 `composition/` — Canonical application lifecycle
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Partial | `lifecycle.py` | Initialize required dependencies, report optional degradation, and close owned resources | `lifespan` | **Standard library:** `contextlib`<br>**Required third-party:** FastAPI; exact compatible constraint belongs in `pyproject.toml`<br>**Local:** `health`; approved dependency lifecycle APIs |
...
### 4.8 `ui/clients/` — Typed frontend transport
...
### 4.9 `ui/context/` — Session, governed, page, and stream context
...
### 4.10 `ui/components/` — Approved workflow presentation
...
| Missing | `strategies.tsx` | Strategy/SQX workflow presentation | `StrategyWorkspace` | **Standard library:** None<br>**Required third-party:** React version pinned in the frontend manifest before implementation<br>**Local:** strategy client/types |
...
| Missing | `documentation.tsx` | Documentation navigation/render/edit presentation | `DocumentationWorkspace` | **Standard library:** None<br>**Required third-party:** React and Markdown renderer versions pinned in the frontend manifest before implementation<br>**Local:** documentation client |
...
| Missing | `FR-API-048` | Render strategy list/create/edit/version and SQX workflows using typed clients only. | `StrategyWorkspace(props: StrategyWorkspaceProps) => JSX.Element` | External API call through client | `ApiClientError` | **Usage:** `tests/api/usage/test_usage_frontend_components.tsx::testUsageStrategies()`<br>**Unit:** `ui/components/strategies.test.tsx::usesTypedClient()` |
...
| Missing | `FR-API-052` | Render documentation navigation, Markdown, table of contents, and environment-authorized editing. | `DocumentationWorkspace(props: DocumentationWorkspaceProps) => JSX.Element` | External API call through client | `ApiClientError` | **Usage:** `tests/api/usage/test_usage_frontend_components.tsx::testUsageDocumentation()`<br>**Unit:** `ui/components/documentation.test.tsx::editHiddenWhenDisallowed()` |
...
### 4.11 `ui/app/` — Protected workflow pages
...
| Partial | `NFR-API-005` | Security | Logs, errors, traces, telemetry, examples, and screenshots contain no tokens, credentials, passwords, CSRF values, raw secrets, or private broker data. | Redaction tests |
...
| Missing | `NFR-API-010` | Observability | Boundary actions carry request/correlation IDs and emit redacted audit/telemetry with route, intent, actor when available, status, duration, and error code. | Trace inspection tests |
...
- **E2E:** login/logout/session recovery, protected pages, stale blocking, governed-write
  rejection/success with mocked owners, stream recovery, and documentation policy.
