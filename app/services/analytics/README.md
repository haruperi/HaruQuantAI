...
> **Domain ID:** `ANLT`
> **Status:** `Completed` — all active contracts, adapters, metric kernels, report operations, allocation evidence, dashboard projection, package exports, and non-excluded workflows satisfy the Section 7 validation gate.
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Completed | `errors.py` | Define Analytics exceptions and bounded error conversion. | `AnalyticsError`, `AnalyticsValidationError`, `to_analytics_error_payload` | **Standard library:** `collections.abc`<br>**Required third-party:** None<br>**Local:** None |
| Completed | `models.py` | Define immutable canonical input, evidence, report, runtime-configuration, scorecard, and dashboard contracts. | `ClosedTrade`, `TradingResult`, `MetricEvidence`, `SectionEvidence`, `AnalyticsWarning`, `QualityFlag`, `Lineage`, `ReproducibilityHashes`, `PerformanceReport`, `PortfolioPerformanceReport`, `PortfolioAllocationEvidence`, `DashboardPayload`, `RiskFreeRateEvidence`, `StatisticalValidationConfig`, `AnalyticsRunConfig`, `StrategyQualityEvidence` | **Standard library:** `dataclasses`, `datetime`, `decimal`, `enum`, `typing`<br>**Required third-party:** `pydantic`<br>**Local:** `errors.py → AnalyticsValidationError` |
...
| Completed | `returns.py` | Monetary PnL, equity, return, and period evidence from the ledger and its derived curves. | `calculate_return_evidence` | **Standard library:** `collections.abc`, `datetime`, `decimal`, `statistics`<br>**Required third-party:** `numpy`, `pandas`<br>**Local:** `contracts.models → TradingResult, SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
| Completed | `drawdowns.py` | Core closed-trade drawdown depth, duration, and recovery evidence. | `calculate_drawdown_evidence` | **Standard library:** `collections.abc`, `decimal`<br>**Required third-party:** `numpy`<br>**Local:** `contracts.models → SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
| Completed | `risk.py` | Approved annualized volatility, historical VaR, and conditional VaR evidence from the daily resample. | `calculate_risk_evidence` | **Standard library:** `collections.abc`, `decimal`, `statistics`<br>**Required third-party:** `numpy`<br>**Local:** `contracts.models → SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
| Completed | `ratios.py` | Approved core Sharpe, Sortino, Calmar, profit-factor, payoff, and expectancy ratios. | `calculate_ratio_evidence` | **Standard library:** `collections.abc`, `decimal`<br>**Required third-party:** `numpy`<br>**Local:** `contracts.models → SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
...
| Completed | `distributions.py` | Canonical moments, percentiles, tails, histogram/outlier evidence required by reports. | `calculate_distribution_evidence` | **Standard library:** `collections.abc`, `statistics`<br>**Required third-party:** `numpy`<br>**Local:** `contracts.models → SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
| Completed | `statistics.py` | Real bounded seeded bootstrap, permutation, multiple-comparison, and sample evidence. | `run_statistical_validation` | **Standard library:** `collections.abc`, `random`, `statistics`<br>**Required third-party:** `numpy`<br>**Local:** `contracts.models → SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
...
| Completed | `groups.py` | Compose approved metric groups in deterministic order with source context. | `calculate_grouped_evidence` | **Standard library:** `collections.abc`<br>**Required third-party:** None<br>**Local:** all metric files above; `contracts.models → TradingResult, SectionEvidence` |
...
| Completed | `allocation.py` | Project validated component evidence into the registered `PortfolioAllocationEvidence v1` cross-domain contract after the Simulation-owned `FR-SIM-033` fixture exists. This is the final reports implementation file. | `build_portfolio_allocation_evidence` | **Standard library:** `collections.abc`, `datetime`, `decimal`, `hashlib`, `math`<br>**Required third-party:** None<br>**Local:** `contracts.models → PerformanceReport, PortfolioPerformanceReport, PortfolioAllocationEvidence`; `contracts.errors → AnalyticsValidationError`; `portfolio.py → build_portfolio_performance_report` |
...
- [X] Every workflow has status `Completed` or `Excluded`, and every `Completed` workflow has a passing integration test. Evidence: `tests/analytics/integration/test_build_performance_report.py:9`.
...
- [X] Every emitted warning and quality-flag code exists in the Evidence Catalog. Evidence: `app/services/analytics/contracts/catalogs.py:241`.
...
- [X] Every package-wide requirement has status `Completed`. Evidence: `app/services/analytics/__init__.py:8`.
...
- [X] Analytics persists no state and mutates no live/external system. Evidence: `app/services/analytics/reports/builder.py:63`.
- [X] Every dependency is documented and directly declared when imported. Evidence: `app/services/analytics/metrics/benchmarks.py:7`.
- [X] Every public symbol has exactly one functional requirement, usage example, and unit test. Evidence: `tests/analytics/usage/test_usage_contracts.py:1`.
...
- [X] All targeted tests and quality checks pass with at least 80% coverage. Evidence: `tests/analytics/unit/test_catalogs.py:18`.
