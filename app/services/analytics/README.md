...
| Completed | `models.py` | Define immutable canonical input, evidence, report, runtime-configuration, scorecard, and dashboard contracts. | `ClosedTrade`, `TradingResult`, `MetricEvidence`, `SectionEvidence`, `AnalyticsWarning`, `QualityFlag`, `Lineage`, `ReproducibilityHashes`, `PerformanceReport`, `PortfolioPerformanceReport`, `PortfolioAllocationEvidence`, `DashboardPayload`, `RiskFreeRateEvidence`, `StatisticalValidationConfig`, `AnalyticsRunConfig`, `ANALYTICS_SCHEMA_VERSION` | **Standard library:** `collections.abc`, `datetime`, `decimal`, `math`, `types`, `typing`<br>**Required third-party:** `pydantic`<br>**Local:** `errors.py → AnalyticsValidationError`; `catalogs.py → EVIDENCE_CATALOG`; `app.utils → logger` |
