...
#### Ratified v1 public records (22)

Table anchors: `databanks(project_id,name_normalized UNIQUE)`, `databank_items(databank_id,strategy_version_id,result_id UNIQUE)`, `databank_decisions(databank_id,sequence UNIQUE)`, `analysis_artifacts(result_id,type,settings_hash UNIQUE)`, `benchmark_comparisons(result_id,settings_hash UNIQUE)`, `operational_journal_artifacts(source_set_hash,policy_hash UNIQUE)`, `qualification_profile_versions(profile_id,version UNIQUE)`; metric tables live in the Simulator group (`metric_definitions(stable_id,version UNIQUE)`, `metric_values(result_id,segment,direction,definition_id UNIQUE)`).

| # | Record | Exact wire fields | FRs / rules |
|---|---|---|---|
| R1 | `DatabankRef` | `databank_id: Uuid7`; `schema_version: Literal[1] = 1`. | FR-ANA-CREATE_DATABANK. |
| R2 | `DatabankVersion` | `databank_id: Uuid7`; `version: int >= 1`; `name: str 1..160`; `project_id: Uuid7`; `capacity: int >= 1 | None = None`; `view_id: Uuid7 | None = None`; `membership_policy: MembershipPolicy`; `row_version: int >= 1 = 1`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`, where `MembershipPolicy(capacity: int >= 1 | None, duplicate_scope: Literal[STRATEGY,RUN,RESULT_FINGERPRINT], rank_policy: nonempty str, replacement_tiebreaker: Literal[OLDEST,LOWEST_RANK,REJECT], correlation_policy: JsonObject = {})`. Duplicate names rejected; rename preserves membership; concurrent rename/delete yields one success and one version conflict. | FR-ANA-CREATE_DATABANK, VERSION_DATABANK_MUTATIONS, DEFINE_MEMBERSHIP_POLICY. |
| R3 | `DatabankItem` | `item_id: Uuid7`; `databank_id: Uuid7`; `strategy_version_id: Uuid7`; `result_id: Uuid7 | None = None`; `display_name: str = ""`; `accepted_at: UtcTimestamp`; `source: nonempty str`; `rank: DecimalValue | None = None`; `schema_version: Literal[1] = 1`. Insertion transactional and idempotent (one membership row per repeat); copy/move/remove/rename never mutate strategy versions; referenced strategies/results/artifacts/lineage are never deleted by eviction or removal. | FR-ANA-LINK_STRATEGY_RESULT, MODIFY_DATABANK_ITEMS, PRESERVE_REFERENCED_ARTIFACTS. |
| R4 | `DatabankDecision` | `decision_id: Uuid7`; `databank_id: Uuid7`; `sequence: int >= 0`; `candidate: Uuid7` (strategy-version or result identity); `stage: nonempty str`; `policy_version: int >= 1`; `observed: JsonObject`; `outcome: Literal[ACCEPTED,REJECTED,REPLACED,DUPLICATE]`; `replaced_item_id: Uuid7 | None = None`; `decided_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. One transactional decision per admission; crash/retry yields one membership outcome and one decision record. | FR-ANA-ADMIT_DATABANK_ITEMS, MATCH_RESULT_FINGERPRINTS. |
| R5 | `ResultQuery` | `query_id: Uuid7`; `databank_id: Uuid7 | None = None`; `columns: tuple[nonempty str, ...] = ()`; `sort: tuple[SortSpec, ...] = ()` where `SortSpec(column: nonempty str, ascending: bool = True)`; `filters: tuple[FilterSpec, ...] = ()` where `FilterSpec(column: nonempty str, operator: nonempty str, value: JsonValue)` (allowlisted fields/functions, bounded complexity); `formulas: tuple[FormulaColumn, ...] = ()` where `FormulaColumn(name: nonempty str, expression: nonempty str, unit: nonempty str)`; `grouping: tuple[nonempty str, ...] = ()`; `cursor: str | None = None`; `page_size: int 1..500 = 100`; `schema_version: Literal[1] = 1`. | FR-ANA-QUERY_RESULTS_TABLE, EVALUATE_FORMULAS_SAFELY. |
| R6 | `SavedResultView` | `view_id: Uuid7`; `version: int >= 1`; `name: nonempty str`; `query: ResultQuery`; `pinned_segments: tuple[Literal[FULL,IS,VALIDATION,OOS,NO_TRADE], ...] = ()`; `display_units: JsonObject = {}`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Reopening reproduces the query even after default column changes. | FR-ANA-VERSION_SAVED_VIEWS. |
| R7 | `ResultPage` | `page_id: Uuid7`; `query_id: Uuid7`; `rows: tuple[JsonObject, ...] = ()` (typed cells; bounded 500 rows/16 MiB per §15.7); `next_cursor: str | None = None`; `total_count: int >= 0 | None = None`; `schema_version: Literal[1] = 1`. Server-side plans, deterministic downsampling, bounded points; exported full-resolution data is not downsampled. | FR-ANA-QUERY_RESULTS_TABLE, BOUND_RESULT_QUERIES. |
| R8 | `MetricDefinition` | `metric_id: nonempty str` (stable §9 catalogue ID); `version: int >= 1`; `formula_ref: nonempty str`; `input_series: tuple[nonempty str, ...]`; `unit: Literal[MONEY,PERCENT,PIPS,RATIO,COUNT]`; `scope: Literal[RESULT,SEGMENT,DIRECTION]`; `rounding: Literal[DOWN,UP,HALF_UP,HALF_EVEN,TOWARD_ZERO]`; `annualization: nonempty str`; `minimum_sample: int >= 0 = 0`; `null_policy: nonempty str`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Phase 1 implements exactly the §9 catalogue; no unspecified metric aliases another formula; changing a formula version never alters committed values. | FR-ANA-CALCULATE_METRICS, CATALOG_METRICS. |
| R9 | `MetricValue` | `result_id: Uuid7`; `segment: Literal[FULL,IS,VALIDATION,OOS,NO_TRADE]`; `direction: Literal[LONG,SHORT,BOTH]`; `metric_id: nonempty str`; `metric_version: int >= 1`; `value: DecimalValue | None = None`; `null_reason: nonempty str | None = None`; `schema_version: Literal[1] = 1`. Exactly one of `value`/`null_reason`; unique composite key per table anchor. | FR-ANA-CALCULATE_METRICS. |
| R10 | `ResultComparison` | `comparison_id: Uuid7`; `baseline_result_id: Uuid7`; `compared_result_id: Uuid7`; `aligned_metrics: tuple[AlignedMetricDelta, ...] = ()` where `AlignedMetricDelta(metric_id, metric_version, baseline_value: DecimalValue | None, compared_value: DecimalValue | None, delta: DecimalValue | None, null_reason: nonempty str | None = None)`; `incompatibility_reason: nonempty str | None = None`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Segments, currencies, metric versions, and sampling align first; incompatible comparisons return a structured reason. | FR-ANA-ALIGN_RESULT_COMPARISONS. |
| R11 | `ChartSpec` | `chart_id: Uuid7`; `kind: Literal[EQUITY,BALANCE,DRAWDOWN,VOLATILITY,BENCHMARK,VOLUME,CUSTOM]`; `series_refs: tuple[Uuid7, ...] = ()`; `downsampling: Literal[EXTREMA_PRESERVING,FULL]`; `bounded_points: int >= 1`; `schema_version: Literal[1] = 1`. Downsampled curves retain global and bucket min/max without loading full artifacts into API memory. | FR-ANA-DOWNSAMPLE_EQUITY_SERIES, BOUND_RESULT_QUERIES. |
| R12 | `TradeAnalysis` | `analysis_id: Uuid7`; `result_id: Uuid7`; `event_basis: nonempty str`; `timezone: IANA name`; `dimensions: tuple[Literal[YEAR,ENTRY_HOUR,EXIT_HOUR,DAY_OF_WEEK,DAY_OF_MONTH], ...]`; `segment: Literal[FULL,IS,VALIDATION,OOS,NO_TRADE]`; `direction: Literal[LONG,SHORT,BOTH]`; `metric_ids: tuple[nonempty str, ...]`; `empty_bucket_policy: nonempty str`; `bucket_artifact_id: Uuid7`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Immutable artifact per table anchor. | FR-ANA-ANALYZE_TRADE_TIMING, RECONSTRUCT_CHART_TRADES. |
| R13 | `BenchmarkComparison` | `comparison_id: Uuid7`; `result_id: Uuid7`; `benchmark_data_version_id: Uuid7`; `holding_method: Literal[BUY_AND_HOLD]`; `eligible_from/eligible_to: UtcTimestamp`; `initial_capital: Money`; `normalization: Literal[NONE,ABSOLUTE_DRAWDOWN,PERCENT_DRAWDOWN,MONEY_MANAGEMENT,EXPOSURE]`; `normalization_version: int >= 1`; `aligned_equity_artifact_id: Uuid7`; `metric_ids: tuple[nonempty str, ...] = ()`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Normalization recomputes only the benchmark initial-capital/allocation input. | FR-ANA-COMPARE_BENCHMARK_EQUITY, NORMALIZE_BENCHMARK. |
| R14 | `ResultExchangePackage` | `package_id: Uuid7`; `result_id: Uuid7`; `strategy_ref: StrategyRef`; `strategy_content_hash: ContentHash`; `settings_artifact_id: Uuid7`; `orders_artifact_id: Uuid7`; `fills_artifact_id: Uuid7`; `trades_artifact_id: Uuid7`; `equity_artifact_id: Uuid7`; `metrics_artifact_id: Uuid7`; `journal_index_artifact_id: Uuid7`; `selected_only: bool = True`; `checksums: dict[nonempty str, ContentHash]`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. `.sqxr` container per §22.3; tampering with one member fails verification naming that member; external importers normalize through versioned adapters preserving unmapped fields as namespaced attachments. | FR-ANA-PACKAGE_RESULT_ARTIFACTS, IMPORT_EXTERNAL_RESULTS, EXPORT_RESULT_ROWS. |
| R15 | `BulkSelectionToken` | `token_id: Uuid7`; `databank_id: Uuid7`; `item_ids: nonempty tuple[Uuid7, ...]`; `pinned_query_hash: ContentHash`; `dry_run_impact: JsonObject = {}`; `created_at: UtcTimestamp`; `expires_at: UtcTimestamp`; `schema_version: Literal[1] = 1`. Pinned snapshot: membership changes during execution do not alter the target set. | FR-ANA-PIN_BULK_SELECTION. |
| R16 | `BulkDatabankCommand` | `command_id: Uuid7`; `selection_token_id: Uuid7`; `action: Literal[COPY,MOVE,EXPORT,DELETE,TAG,NOTE,RETEST,CUSTOM]`; `target_databank_id: Uuid7 | None = None`; `conflict_policy: Literal[REJECT,KEEP_EXISTING,CREATE_NEW_VERSION]`; `idempotency_key: nonempty str`; `impact_counts: JsonObject = {}`; `schema_version: Literal[1] = 1`. Retrying an interrupted bulk transfer is idempotent; conflicts, deduplication, and rejected memberships are recorded. | FR-ANA-TRANSFER_DATABANK_ITEMS. |
| R17 | `AnalysisPanelDescriptor` | `panel_id: nonempty str`; `plugin_id: plugin stable-ID | None = None`; `supported_schemas: tuple[nonempty str, ...]`; `required_permissions: tuple[nonempty str, ...] = ()`; `frontend_bundle_hash: ContentHash`; `compatibility_range: nonempty str`; `schema_version: Literal[1] = 1`. Incompatible panels are disabled with diagnostics while core results remain available. | FR-ANA-DECLARE_RESULT_PANELS. |
| R18 | `SimilarityQuery` | `query_id: Uuid7`; `databank_id: Uuid7`; `reference_result_id: Uuid7`; `tolerances: Tolerances(number_of_trades: DecimalValue in [0,1], net_profit: DecimalValue in [0,1], drawdown: DecimalValue in [0,1])`; `rule_version: int >= 1`; `schema_version: Literal[1] = 1`. Relative-tolerance fingerprint matching independent of semantic AST duplicates. | FR-ANA-MATCH_RESULT_FINGERPRINTS. |
| R19 | `SimilarityMatch` | `query_id: Uuid7`; `candidate_result_id: Uuid7`; `is_similar: bool`; `deviations: JsonObject`; `schema_version: Literal[1] = 1`. | FR-ANA-MATCH_RESULT_FINGERPRINTS. |
| R20 | `OperationalJournalArtifact` | `journal_id: Uuid7`; `source_set_hash: ContentHash`; `policy_hash: ContentHash`; `intent_refs: tuple[Uuid7, ...] = ()`; `risk_decision_refs: tuple[Uuid7, ...] = ()`; `trading_operation_refs: tuple[Uuid7, ...] = ()`; `annotation_refs: tuple[Uuid7, ...] = ()`; `time_basis: nonempty str`; `output_artifact_id: Uuid7`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Joins immutable Strategy intent/manual plan, Risk decisions, Trading operations, annotations, and outcomes using stable identities. | FR-ANA-BUILD_OPERATIONAL_JOURNAL. |
| R21 | `QualificationProfileVersion` | `profile_id: Uuid7`; `version: int >= 1`; `evidence_kinds: tuple[Literal[TRAINING,REPLAY,LIVE], ...]`; `competencies: JsonObject`; `thresholds: JsonObject`; `validity_days: int >= 1`; `reviewer_approval_required: bool = True`; `content_hash: ContentHash`; `schema_version: Literal[1] = 1`. Versioned policy. | FR-ANA-QUALIFY_OPERATORS. |
| R22 | `OperatorQualification` | `qualification_id: Uuid7`; `principal_id: Uuid7`; `profile_id: Uuid7`; `profile_version: int >= 1`; `outcome: Literal[QUALIFIED,CONDITIONAL,NOT_QUALIFIED,INSUFFICIENT_EVIDENCE]`; `evidence_refs: tuple[Uuid7, ...] = ()`; `decided_at: UtcTimestamp`; `expires_at: UtcTimestamp | None = None`; `reviewer_principal_id: Uuid7 | None = None`; `schema_version: Literal[1] = 1`. Sparse/biased samples surface as `INSUFFICIENT_EVIDENCE`, never as defensible conclusions. | FR-ANA-QUALIFY_OPERATORS, MEASURE_PLAN_ADHERENCE, SUMMARIZE_BEHAVIOR, ANALYZE_EMERGENCY_RESPONSE, EXPORT_OPERATIONAL_ANALYTICS. |

Cross-owner references: `StrategyRef`, `StrategyVersion` IDs (Strategy); `RunManifest`, `SimulationResult` IDs (Simulator); `BulkRequestToken` pairs from Interfaces; `PrincipalRef` IDs (Workspace). No subscriptions.

#### Ratified v1 capabilities and operation envelopes

All new (universal rule; shared `AnalyticsFailure` with `code: Literal[ANALYTICS_VALIDATION_FAILED,DATABANK_NOT_FOUND,DATABANK_NAME_CONFLICT,DATABANK_VERSION_CONFLICT,DATABANK_CAPACITY_EXCEEDED,RESULT_NOT_FOUND,QUERY_INVALID,QUERY_TOO_EXPENSIVE,FORMULA_INVALID,METRIC_NOT_FOUND,COMPARISON_INCOMPATIBLE,PANEL_INCOMPATIBLE,CAPABILITY_UNAVAILABLE]`; no subscriptions):

1. `analytics.databank-membership@1` / `DatabankMembershipCapability` / `databank_membership` — ops `CREATE, RENAME, DELETE, ADMIT, MODIFY_ITEMS, DEFINE_POLICY`. Success: `databank: DatabankVersion | None`; `item: DatabankItem | None`; `decision: DatabankDecision | None`. FRs: CREATE_DATABANK, LINK_STRATEGY_RESULT, MODIFY_DATABANK_ITEMS, VERSION_DATABANK_MUTATIONS, DEFINE_MEMBERSHIP_POLICY, ADMIT_DATABANK_ITEMS.
2. `analytics.query-results@1` / `QueryResultsCapability` / `query_results` — ops `QUERY, SAVE_VIEW, EVALUATE_FORMULA, BOUND_SERIES`. Success: `page: ResultPage | None`; `view: SavedResultView | None`; `chart: ChartSpec | None`. FRs: QUERY_RESULTS_TABLE, VERSION_SAVED_VIEWS, EVALUATE_FORMULAS_SAFELY, DEFINE_CORRELATION_POLICY, BOUND_RESULT_QUERIES.
3. `analytics.interpret-results@1` / `InterpretResultsCapability` / `interpret_results` — ops `OVERVIEW, LIST_TRADES, METRICS, COMPARE, SHOW_MANIFEST`. Success: `overview: JsonObject` — typed as `metric_values: tuple[MetricValue, ...] = ()`; `comparison: ResultComparison | None`; `manifest: RunManifest | None` (Simulator-owned); `scope` fields `segment/direction/pl_unit` applied consistently. FRs: APPLY_RESULT_SCOPE, SHOW_RESULT_OVERVIEW, LIST_RESULT_TRADES, CALCULATE_METRICS, CATALOG_METRICS, ALIGN_RESULT_COMPARISONS, SHOW_RUN_MANIFEST.
4. `analytics.analyze-trades@1` / `AnalyzeTradesCapability` / `analyze_trades` — ops `TIMING_ANALYSIS, RECONSTRUCT_CHART, COMPARE_BENCHMARK`. Success: `analysis: TradeAnalysis | None`; `benchmark: BenchmarkComparison | None`. FRs: DOWNSAMPLE_EQUITY_SERIES, ANALYZE_TRADE_TIMING, RECONSTRUCT_CHART_TRADES, COMPARE_BENCHMARK_EQUITY, NORMALIZE_BENCHMARK.
5. `analytics.exchange-results@1` / `ExchangeResultsCapability` / `exchange_results` — ops `EXPORT_ROWS, PACKAGE, IMPORT_EXTERNAL`. Success: `package: ResultExchangePackage | None`; `artifact_id: Uuid7 | None = None`. FRs: EXPORT_RESULT_ROWS, PACKAGE_RESULT_ARTIFACTS, IMPORT_EXTERNAL_RESULTS.
6. `analytics.bulk-databank@1` / `BulkDatabankCapability` / `bulk_databank` — ops `PIN_SELECTION, DRY_RUN, EXECUTE`. Success: `selection: BulkSelectionToken | None`; `command: BulkDatabankCommand | None`. FRs: PIN_BULK_SELECTION, TRANSFER_DATABANK_ITEMS, PRESERVE_REFERENCED_ARTIFACTS.
7. `analytics.match-results@1` / `MatchResultsCapability` / `match_results` — ops `QUERY_SIMILARITY`. Success: `matches: tuple[SimilarityMatch, ...] = ()`. FRs: MATCH_RESULT_FINGERPRINTS.
8. `analytics.custom-panels@1` / `CustomPanelsCapability` / `custom_panels` — ops `RUN_CUSTOM_ANALYSIS, DECLARE_PANEL`. Success: `panel: AnalysisPanelDescriptor | None`; `output_artifact_id: Uuid7 | None = None`. FRs: RUN_CUSTOM_ANALYSIS, DECLARE_RESULT_PANELS.
9. `analytics.qualify-operations@1` / `QualifyOperationsCapability` / `qualify_operations` — ops `BUILD_JOURNAL, MEASURE_ADHERENCE, SUMMARIZE_BEHAVIOR, ANALYZE_EMERGENCY, QUALIFY, EXPORT`. Success: `journal: OperationalJournalArtifact | None`; `qualification: OperatorQualification | None`; `profile: QualificationProfileVersion | None`; `export_artifact_id: Uuid7 | None = None`. FRs: BUILD_OPERATIONAL_JOURNAL through EXPORT_OPERATIONAL_ANALYTICS (actor-scoped redaction, source IDs/hashes, policy versions, caveats preserved).
