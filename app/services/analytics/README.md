...
| Completed | `evidence.py` | Build ordered report evidence and enforce report-specific finite JSON safety. Validated Analytics-owned reports may exceed the untrusted item ceiling while retaining type, finite-value, depth, and cycle validation. It imports Utils-owned redaction and serialization primitives and must not wrap, export, redefine, or shadow the name of any of them. | `build_warning`, `build_quality_flag`, `to_report_json_safe` | **Standard library:** `collections.abc`, `json`, `typing`**Required third-party:** `numpy`, `pandas`**Local:** `catalogs.py → EVIDENCE_CATALOG`; `models.py → AnalyticsWarning, QualityFlag`; `errors.py → AnalyticsValidationError`; `app.utils → redact_mapping_value, canonical_json, to_json_safe` |
...
| Completed | `FR-ANLT-025` | The system shall normalize report-specific pandas and NumPy values into Utils-supported types, delegate trusted Analytics-owned report conversion to Utils canonical serialization with `max_items=None`, and translate serialization failure into `AnalyticsValidationError`. It shall retain finite-value, type, cycle, and depth validation, keep untrusted warning/metadata bounds unchanged, and shall not define a symbol named `to_json_safe`. | `to_report_json_safe(value: object) -> object` | None | `AnalyticsValidationError`: value is non-finite, unsupported, cyclic, or unserializable | **Usage:** `tests/analytics/usage/features/01_contracts.py::fr_anlt_025()`**Unit:** `tests/analytics/unit/test_evidence.py` |
...
| Status    | File                 | Responsibility                                                                                                                                                                                            | Key exports                                                                        | Dependencies                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Completed | `hashes.py`        | Compute deterministic SHA-256 evidence hashes from validated Analytics-owned structures without applying the untrusted item ceiling. | `compute_reproducibility_hashes` | **Standard library:** `collections.abc`, `hashlib`**Required third-party:** None**Local:** `app.utils → canonical_json`; `contracts.errors → AnalyticsValidationError`; `contracts.evidence → to_report_json_safe`; `contracts.models → TradingResult, PerformanceReport, ReproducibilityHashes` |
| Completed | `serialization.py` | Serialize a validated report to canonical JSON without the untrusted item ceiling, or to minimal approved human-readable text, while enforcing the configured response-byte bound. | `serialize_report` | **Standard library:** `enum`**Required third-party:** None**Local:** `app.utils → canonical_json`; `contracts.evidence → to_report_json_safe`; `contracts.models → PerformanceReport` |
...
| Status    | Requirement ID  | Responsibility                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Class / Function / Method                                                                                                                                                                                                                                                                                                                                                         | Side Effects                                                     | Raises                                                                                                                                                              | Usage / Test                                                                                                                                                                                              |
| --------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Completed | `FR-ANLT-039` | The system shall compute deterministic SHA-256 input, config, ledger, equity, optional benchmark, and report hashes from canonical JSON while excluding documented nondeterministic fields. Complete validated reports are trusted structures and may exceed the untrusted serialization item ceiling without relaxing other serialization safety checks. | `compute_reproducibility_hashes(result: TradingResult, report: PerformanceReport | None = None) -> ReproducibilityHashes` | None | `AnalyticsValidationError`: required evidence cannot be serialized/hashed | **Unit:** `tests/analytics/unit/test_evidence.py`, `tests/analytics/unit/test_hashes.py` |
