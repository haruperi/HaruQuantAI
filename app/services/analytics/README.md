...
> **Domain ID:** `ANLT`
> **Status:** `Missing` — specification completed 2026-07-19; two scoped owner decisions remain in §6, neither blocking the core build
...
| Blocked | `WF-ANLT-013` | Cross-domain | Build portfolio allocation evidence | Component reports and/or `PortfolioSimulationResult` plus FX evidence | `PortfolioAllocationEvidence v1` | `FR-ANLT-041 → FR-ANLT-047 → FR-ANLT-048` |
...
**Status:** `Blocked` by `D-ANLT-004` (§6). `PortfolioSimulationResult v1` has no
field-level schema in `app/services/simulator/README.md` or `docs/PROJECT.md`, so the
`PortfolioSimulationResult` input leg cannot be specified. The component
`PerformanceReport` leg is fully specified and may be implemented first if the owner
narrows the initial scope. `FR-ANLT-047` and `FR-ANLT-048` define the contract and
the projector.
...
| Missing | `models.py` | Define immutable canonical input, evidence, report, scorecard, and dashboard contracts. | `ClosedTrade`, `TradingResult`, `MetricEvidence`, `SectionEvidence`, `AnalyticsWarning`, `QualityFlag`, `Lineage`, `ReproducibilityHashes`, `PerformanceReport`, `PortfolioPerformanceReport`, `PortfolioAllocationEvidence`, `DashboardPayload`, `StrategyQualityEvidence` | **Standard library:** `dataclasses`, `datetime`, `decimal`, `enum`<br>**Required third-party:** `pydantic`<br>**Local:** `errors.py → AnalyticsValidationError` |
...
| Missing | `evidence.py` | Build ordered report evidence and enforce report-specific finite JSON safety. It imports Utils-owned redaction and serialization primitives and must not wrap, export, redefine, or shadow the name of any of them. | `build_warning`, `build_quality_flag`, `to_report_json_safe` | **Standard library:** `collections.abc`, `datetime`, `decimal`, `enum`<br>**Required third-party:** `numpy`, `pandas`<br>**Local:** `catalogs.py → EVIDENCE_CATALOG`; `models.py → AnalyticsWarning, QualityFlag`; `errors.py → AnalyticsValidationError`; `app.utils → redact_mapping_value, canonical_json, to_json_safe` |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Missing | `results.py` | Map an approved closed-trade ledger from Trading or Simulation into the canonical Analytics input, and derive its equity curves. | `adapt_trading_result`, `build_closed_trade_equity_curve` | **Standard library:** `collections.abc`, `datetime`, `decimal`<br>**Required third-party:** None<br>**Local:** `contracts.catalogs → validate_contract_version`; `contracts.errors → AnalyticsValidationError`; `contracts.models → ClosedTrade, TradingResult, Lineage` |
...
| Status | Setting / Limit | Type | Default | Required | Used by | Description |
|---|---|---|---|---|---|---|
| Missing | `MAX_TRADES` | `int` | None | Yes before public activation | `adapt_trading_result()` | Deployment must supply a measured positive bound; source ledgers larger than the ceiling are rejected before allocation/calculation. |
...
| Status | Requirement ID | Responsibility | Class / Function / Method | Side Effects | Raises | Usage / Test |
|---|---|---|---|---|---|---|
| Missing | `FR-ANLT-027` | The system shall deterministically map an approved versioned closed-trade ledger from Trading or Simulation to `TradingResult`, preserving IDs, phase, UTC window, account currency, initial balance, strategy, symbols, timeframe, every `ClosedTrade` field, quality metadata, and bounded source metadata without silent field loss. Open, pending, and placeholder rows are rejected, not silently filtered. Benchmark and FX evidence are caller-supplied and validated, never sourced. | `adapt_trading_result(source: Mapping[str, object], *, source_contract: str, initial_balance: Decimal, account_currency: str, benchmark: Mapping[str, object] \| None = None, fx_evidence: Mapping[str, object] \| None = None) -> TradingResult` | None | `AnalyticsValidationError`: mapping, version, or required field is missing, conflicting, oversized, or incompatible; `initial_balance` is absent or non-positive; `account_currency` is absent | **Usage:** `tests/analytics/usage/test_usage_adapters.py::test_usage_results_adapt_trading_result()`<br>**Unit:** `tests/analytics/unit/test_results_adapter.py::test_adapt_trading_result_preserves_required_lineage()` |
| Missing | `FR-ANLT-050` | The system shall derive one deterministic closed-trade equity curve by ordering the ledger on `(exit_time, ticket)` and accumulating `initial_balance + cumsum(net_trade_pnl)` in `Decimal`, where `net_trade_pnl = profit + commission + swap`, and shall additionally emit a UTC calendar-daily resample of that curve carrying the last equity value of each day. The curve is labelled `curve_basis="closed_trade"`. Time-based metrics consume the daily resample; per-trade statistics consume the trade-indexed curve. Mark-to-market equity is never synthesized. | `build_closed_trade_equity_curve(trades: Sequence[ClosedTrade], *, initial_balance: Decimal) -> tuple[tuple[Mapping[str, object], ...], tuple[Mapping[str, object], ...]]` | None | `AnalyticsValidationError`: ledger is empty, timestamps are non-UTC or unorderable, or `initial_balance` is non-positive | **Usage:** `tests/analytics/usage/test_usage_adapters.py::test_usage_results_build_equity_curve()`<br>**Unit:** `tests/analytics/unit/test_results_adapter.py::test_equity_curve_ordering_is_deterministic_and_daily_resampled()` |
...
| Missing | `returns.py` | Monetary PnL, equity, return, and period evidence from the ledger and its derived curves. | `calculate_return_evidence` | **Standard library:** `collections.abc`, `datetime`, `decimal`, `statistics`<br>**Required third-party:** `numpy`, `pandas`<br>**Local:** `contracts.models → TradingResult, SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
| Missing | `drawdowns.py` | Core closed-trade drawdown depth, duration, and recovery evidence. | `calculate_drawdown_evidence` | **Standard library:** `collections.abc`, `decimal`<br>**Required third-party:** `numpy`<br>**Local:** `contracts.models → SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
| Missing | `risk.py` | Approved annualized volatility, historical VaR, and conditional VaR evidence from the daily resample. | `calculate_risk_evidence` | **Standard library:** `collections.abc`, `decimal`, `statistics`<br>**Required third-party:** `numpy`<br>**Local:** `contracts.models → SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
...
| Missing | `cost_efficiency.py` | Ledger commission/swap attribution, MAE/MFE, intra-trade excursion, duration, and efficiency evidence. | `calculate_cost_efficiency_evidence` | **Standard library:** `collections.abc`, `datetime`, `decimal`<br>**Required third-party:** None<br>**Local:** `contracts.models → TradingResult, SectionEvidence`; `contracts.catalogs → METRIC_DEFINITION_CATALOG` |
...
| Missing | `FR-ANLT-031` | The system shall calculate only approved annualized volatility, historical VaR, and conditional VaR evidence from the daily return resample, with cataloged sign, confidence, sample, and units. Expected shortfall is not calculated separately: it is mathematically identical to conditional VaR and the catalog permits one implementation. | `calculate_risk_evidence(daily_returns: Sequence[float], *, confidence: float = 0.95) -> SectionEvidence` | None | `AnalyticsValidationError`: returns or risk policy is invalid | **Usage:** `tests/analytics/usage/test_usage_metrics.py::test_usage_risk_calculate_risk_evidence()`<br>**Unit:** `tests/analytics/unit/test_risk.py::test_risk_evidence_uses_cataloged_tail_convention()` |
...
| Missing | `FR-ANLT-037` | The system shall calculate ledger cost drag, duration, MAE/MFE, `max_intratrade_excursion`, and selected efficiency evidence with documented sign conventions and no source mutation. Because `profit` is gross, `commission` and `swap` are real deductions: `gross_pnl_before_costs` is `sum(profit)`, `total_cost_drag` is their signed sum, and `net_pnl` is their combination. | `calculate_cost_efficiency_evidence(result: TradingResult) -> SectionEvidence` | None | `AnalyticsValidationError`: cost sign, timestamps, or required fields are invalid | **Usage:** `tests/analytics/usage/test_usage_metrics.py::test_usage_cost_efficiency_calculate_evidence()`<br>**Unit:** `tests/analytics/unit/test_cost_efficiency.py::test_cost_evidence_preserves_rebates_and_signs()` |
...
| Missing | `serialization.py` | Serialize a report to canonical JSON or minimal approved human-readable text. | `serialize_report` | **Standard library:** `enum`<br>**Required third-party:** None<br>**Local:** `app.utils → canonical_json`; `contracts.evidence → to_report_json_safe`; `contracts.models → PerformanceReport` |
...
| Blocked | `allocation.py` | Project validated component evidence into the registered `PortfolioAllocationEvidence v1` cross-domain contract. Blocked by `D-ANLT-004` (§6) for the `PortfolioSimulationResult` input leg only; the component `PerformanceReport` leg is fully specified. | `build_portfolio_allocation_evidence` | **Standard library:** `collections.abc`, `datetime`, `decimal`<br>**Required third-party:** None<br>**Local:** `contracts.models → PerformanceReport, PortfolioPerformanceReport, PortfolioAllocationEvidence`; `contracts.errors → AnalyticsValidationError`; `contracts.evidence → build_quality_flag`; `portfolio.py → build_portfolio_performance_report` |
| Missing | `builder.py` | Orchestrate canonical report sections, criticality, warnings, lineage, validation, and hashes. | `build_performance_report` | **Standard library:** `datetime`<br>**Required third-party:** None<br>**Local:** `adapters.results → adapt_trading_result`; `metrics.groups → calculate_grouped_evidence`; `contracts.models → PerformanceReport`; `contracts.evidence → to_report_json_safe`; `hashes.py → compute_reproducibility_hashes` |
...
| Missing | `FR-ANLT-048` | The system shall project validated component `PerformanceReport` evidence, and where supplied `PortfolioSimulationResult` evidence, into `PortfolioAllocationEvidence v1` after exact component/source schema, measurement-window, base-currency, fresh Data-owned `FXConversionEvidence`, and finite-value validation. It shall emit no partial cross-domain evidence, recommend no weight, approve no portfolio, set no risk budget, and infer no missing value. | `build_portfolio_allocation_evidence(reports: Sequence[PerformanceReport], *, base_currency: str, fx_evidence: Mapping[str, object] \| None, portfolio_simulation_result: Mapping[str, object] \| None = None) -> PortfolioAllocationEvidence` | None | `AnalyticsValidationError`: source schema, measurement window, base currency, FX evidence, or limit is invalid | **Usage:** `tests/analytics/usage/test_usage_reports.py::test_usage_allocation_build_evidence()`<br>**Unit:** `tests/analytics/unit/test_allocation.py::test_allocation_evidence_blocks_without_fx()` |
...
| Missing | `payloads.py` | Project approved report sections into one versioned dashboard contract. | `build_dashboard_payload` | **Standard library:** `collections.abc`<br>**Required third-party:** None<br>**Local:** `contracts.models → PerformanceReport, DashboardPayload`; `contracts.evidence → to_report_json_safe`; `truncation.py → truncate_series` |
...
Five decisions opened by the 2026-07-19 readiness audit are now resolved and have
been written into the specification: the canonical closed-trade ledger input and its
derived equity curve (`FR-ANLT-049`, `FR-ANLT-050`), the benchmark and FX entry
points (`FR-ANLT-027`), the per-fill key contract, the Metric Definition Catalog, and
the Evidence Catalog. Two remain.

| Decision ID | Blocks | Question | Options | Recommendation |
|---|---|---|---|---|
| `D-ANLT-004` | `FR-ANLT-048`, `WF-ANLT-013` — portfolio leg only | `PortfolioSimulationResult v1` is registered in `docs/PROJECT.md` §5 and named as a `WF-ANLT-013` input, but has no field-level schema in `app/services/simulator/README.md` or anywhere else — only prose. The component `PerformanceReport` leg of `FR-ANLT-048` is fully specified and unaffected. | **A.** Simulation publishes the field list as an `FR-SIM-*` row. **B.** Narrow `WF-ANLT-013` and `FR-ANLT-048` to component `PerformanceReport` inputs for the initial build and add the `PortfolioSimulationResult` leg later. | **B** unblocks the initial build immediately and **A** remains required before the second leg ships. The two are compatible; choosing **B** does not waive **A**. |
| `D-ANLT-007` | `FR-ANLT-014`, `FR-ANLT-044`, `WF-ANLT-004` | `scorecards/` is `Excluded` because `STRATEGY_QUALITY_THRESHOLDS` and `ALLOWED_RECOMMENDATION_LANGUAGE` have no owner-approved value. | **A.** Keep the exclusion for the initial build. **B.** Supply both settings now and build the feature. | **A.** Scorecards are promotion-adjacent; Analytics must not invent a threshold that influences a governance-adjacent judgement. |
...
- **Unit:** Verify every `FR-ANLT-*`, golden formulas, invalid inputs,
  ...
  documented errors.
- **Integration:** Verify every workflow whose status is not `Excluded` or
  `Blocked`, including Trading/Simulation contract fixtures without live side
  effects. `WF-ANLT-004` is `Excluded` and `WF-ANLT-013` is `Blocked`; neither
  requires integration evidence until its §6 decision resolves.
...
- **Upstream fixture parity:** Where a producer domain is not yet implemented,
  ...
  against `FR-SIM-024` in `app/services/simulator/README.md`. No fixture may be built
  for a contract with no documented field list; see `D-ANLT-004`.
...
- [ ] Every requirement table has status `Completed`, `Excluded`, or `Removed`.
- [ ] Every workflow has status `Completed` or `Excluded`, and every `Completed`
      workflow has a passing integration test.
- [ ] No requirement, file, or workflow remains `Blocked`.
- [ ] Every metric emitted by any report, dashboard, warning, or quality flag has a
      complete Metric Definition Catalog row and a passing golden fixture.
- [ ] Every emitted warning and quality-flag code exists in the Evidence Catalog.
- [ ] `net_pnl` equals `sum(profit + commission + swap)`, and every classification,
      aggregation, equity, and ratio metric uses `net_trade_pnl` rather than raw
      `profit`. Only `gross_pnl_before_costs` and `trade_efficiency` consume raw
      `profit`.
- [ ] Drawdown output is labelled `curve_basis="closed_trade"` and
      `max_intratrade_excursion` is reported alongside it.
...
- [ ] Analytics defines no symbol colliding with a Utils-owned public primitive.
...
| `P-ANLT-005` | `app/services/analytics/scorecards/` | 7 | `scorecards` module seam only; its `FR-ANLT-*` behavior is `Excluded` from the initial build (§4.5, `D-ANLT-007`) |
