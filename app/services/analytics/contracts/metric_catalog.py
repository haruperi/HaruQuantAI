"""Metric definition catalog, tool catalog, configurations, and validations.

This module owns the canonical list of approved metrics, formulas, units,
tolerances, and tools. It performs no I/O, network calls, database mutations,
broker calls, or trading side effects.
"""
...
# Decimal precision mandate & tolerance policy (ANL-NFR-086)
...
# Metric definition catalog (ANL-NFR-076, ANL-NFR-082)
METRIC_DEFINITION_CATALOG: Mapping[str, MetricDefinition] = {
    # ── Return metrics ───────────────────────────────────────────────────────
    ...
    "total_return_usd": MetricDefinition(
        ...
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when equity curve has fewer than two points",
    ),
    "cagr": MetricDefinition(
        name="cagr",
        formula="((final_value / initial_value) ** (1.0 / years) - 1.0) * 100",
        units="percent",
    ...
    "annualized_return": MetricDefinition(
        ...
        formula="((geometric_mean + 1) ** periods_per_year - 1) * 100",
        units="percent",
    ...
    "compound_monthly_growth_rate": MetricDefinition(
        ...
        formula="((final_value / initial_value) ** (1.0 / months) - 1.0) * 100",
        units="percent",
        ...
        minimum_sample_size=1,
        undefined_behavior=(
            "return None when initial_value, final_value, or months is non-positive"
        ),
    ),
    "buy_and_hold_cagr": MetricDefinition(
        name="buy_and_hold_cagr",
        formula="CAGR(price_values[0], price_values[-1], years)",
        units="percent",
    ...
    "buy_and_hold_return": MetricDefinition(
        ...
        formula="((price_values[-1] - price_values[0]) / price_values[0]) * 100",
        units="percent",
    ...
    "breakeven_trades": MetricDefinition(
        ...
        formula="count(t for t in closed_trades if abs(pnl(t)) <= epsilon)",
        units="count",
        required_inputs=("trades",),
    ...
    "short_trades": MetricDefinition(
        name="short_trades",
        formula="count(t for t in closed_trades if direction(t) in (short,sell))",
        units="count",
        required_inputs=("trades",),
        return_scale="count",
        minimum_sample_size=0,
    ...
    "gross_profit": MetricDefinition(
        name="gross_profit",
        formula="sum(pnl(t) for t in closed_trades if pnl(t) > 0)",
        units="currency",
        required_inputs=("trades",),
        return_scale="currency",
        minimum_sample_size=1,
    ...
    "avg_win_loss": MetricDefinition(
        ...
        return_scale="ratio",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when avg_loss is zero or no losing trades exist",
    ),
    ...
    "avg_time_in_trade": MetricDefinition(
        ...
        return_scale="hours",
        minimum_sample_size=1,
        undefined_behavior="return 0.0 when no closed trades have parseable timestamps",
    ),
    ...
    "r_multiple_proxy_profit_loss": MetricDefinition(
        ...
        minimum_sample_size=1,
        undefined_behavior=(
            "emit DEGRADED_CONFIDENCE warning; mark affected "
            "metric as confidence=degraded"
        ),
    ...
    "sqn": MetricDefinition(
        ...
        return_scale="scalar",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when fewer than 2 closed trades or zero std",
        role="scorecard_input",
    ...
    "kelly_criterion": MetricDefinition(
        ...
        return_scale="fraction",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when payoff_ratio <= 0 or no closed trades",
    ),
    # ── Drawdown metrics ─────────────────────────────────────────────────────
    ...
    "max_strategy_drawdown": MetricDefinition(
        ...
        return_scale="currency",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when no drawdown exists in the equity curve",
    ),
    "max_strategy_drawdown_percent": MetricDefinition(
        ...
        return_scale="percent",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when no drawdown exists or peak is non-positive",
        role="scorecard_input",
    ),
    ...
    "recovery_factor": MetricDefinition(
        ...
        return_scale="ratio",
        minimum_sample_size=1,
        undefined_behavior="return 0.0 when max_drawdown is zero or non-positive",
    ),
    # ── Risk metrics ─────────────────────────────────────────────────────────
    ...
    "volatility": MetricDefinition(
        ...
        sample_convention="sample",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when fewer than 2 returns or zero variance",
    ),
    ...
    "downside_volatility": MetricDefinition(
        ...
        sample_convention="sample",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when fewer than 2 below-target returns",
    ),
    "value_at_risk": MetricDefinition(
        name="value_at_risk",
        formula="abs(sorted_returns[int(n * (1 - confidence))]) * 100",
        units="percent",
        required_inputs=("returns",),
    ...
    "conditional_var": MetricDefinition(
        ...
        formula="mean(abs(r) for r in sorted_returns[:tail_idx]) * 100",
        units="percent",
        required_inputs=("returns",),
        optional_inputs=("confidence",),
        return_scale="percent",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when returns list is empty or tail is empty",
    ),
    ...
    "tracking_error": MetricDefinition(
        ...
        sample_convention="sample",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when fewer than 2 aligned returns",
    ),
    ...
    "return_skewness": MetricDefinition(
        ...
        undefined_behavior="return 0.0 when fewer than 3 returns or zero variance",
    ),
    "return_kurtosis": MetricDefinition(
        ...
        undefined_behavior="return 0.0 when fewer than 4 returns or zero variance",
    ),
    # ── Efficiency metrics ───────────────────────────────────────────────────
    "capital_efficiency": MetricDefinition(
        ...
        return_scale="ratio",
        minimum_sample_size=1,
        undefined_behavior="return 0.0 when nominal_capital_deployed is non-positive",
    ),
    ...
    "percent_time_in_market": MetricDefinition(
        ...
        formula="time_in_market_duration(trades) / period_duration_hours",
        units="fraction",
        required_inputs=("trades", "period_duration_hours"),
        return_scale="fraction",
        minimum_sample_size=1,
        undefined_behavior="return 0.0 when period_duration_hours is non-positive",
    ),
    "time_in_market_duration": MetricDefinition(
        name="time_in_market_duration",
        formula="sum of merged overlapping open/close trade intervals in hours",
        units="hours",
        required_inputs=("trades",),
        return_scale="hours",
        minimum_sample_size=1,
    ...
    "r_signal_to_noise": MetricDefinition(
        ...
        sample_convention="sample",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when R-multiple std is zero or fewer than 2",
    ),
    "rolling_expectancy_stability": MetricDefinition(
        ...
        undefined_behavior="return 0.0 when fewer trades than window size",
    ),
    "win_after_win_probability": MetricDefinition(
        name="win_after_win_probability",
        formula="count(win then win pairs) / count(win as first in pair)",
        units="fraction",
        required_inputs=("trades",),
        return_scale="fraction",
        minimum_sample_size=2,
    ...
    "runs_test_zscore": MetricDefinition(
        ...
        formula="(observed_runs - expected_runs) / sqrt(variance_runs)",
        units="dimensionless",
        required_inputs=("trades",),
        return_scale="scalar",
        minimum_sample_size=2,
        undefined_behavior="return 0.0 when only wins or only losses; variance zero",
    ),
    # ── Size metrics ─────────────────────────────────────────────────────────
    ...
    "max_net_size_held": MetricDefinition(
        ...
        formula="max abs value of running net (long - short) position size",
        units="contracts",
        required_inputs=("trades",),
        return_scale="contracts",
        minimum_sample_size=1,
    ...
    "open_position_pnl": MetricDefinition(
        ...
        formula="sum(unrealized_pnl(t) for t in trades if t.is_open)",
        units="currency",
        required_inputs=("trades",),
        return_scale="currency",
        minimum_sample_size=0,
...
# Official Analytics Tool Catalog mapping (ANL-NFR-081)
OFFICIAL_ANALYTICS_TOOL_CATALOG: Mapping[str, ToolDefinition] = {
...
def validate_metric_catalog(
    ...
    if not isinstance(active, dict) and not isinstance(active, Mapping):
        ...
        raise ValidationError(msg)
    if not active:
        msg = "metric catalog must be non-empty."
        raise ValidationError(msg)

    seen: set[str] = set()
    for key, definition in active.items():
        if key in seen:
            msg = f"duplicate metric definition: {key}"
            raise ValidationError(msg)
        seen.add(key)
        if not isinstance(definition, MetricDefinition):
            msg = f"metric definition for {key!r} is invalid."
            raise ValidationError(msg)
        if key != definition.name:
            msg = f"metric catalog key mismatch for {key!r}."
            raise ValidationError(msg)
        if not definition.formula.strip():
            msg = f"metric {key!r} must define a formula."
            raise ValidationError(msg)
        if not definition.required_inputs:
            msg = f"metric {key!r} must define required inputs."
            raise ValidationError(msg)
        if definition.minimum_sample_size < 0:
            msg = f"metric {key!r} must not use a negative minimum sample size."
            raise ValidationError(msg)

    return {
        "status": "valid",
        "metric_count": len(active),
        "metrics": sorted(active.keys()),
    }
