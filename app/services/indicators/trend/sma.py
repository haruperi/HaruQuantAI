...
def _build_config(
    ...
    """Build or validate the immutable SMA configuration.
    ...
    if (
        ...
        raise IndicatorError(
            ...
            "supplied config disagrees with sma wrapper arguments",
            {"indicator_id": "sma"},
        )
    return config
...
def sma(
    ...
    *,
    ...
    """Calculate SMA over the inclusive current rolling window.
    ...
    Returns:
        A deterministic SMA ``IndicatorResult``.

    Raises:
        IndicatorError: On validation or atomic calculation failure.
    """
    logger.info(
        "Calculating sma for %s (period=%d, source=%s)", data.symbol, period, source
    )
    resolved_config = _build_config(period, source, config)
    _unwrap_indicator_response(validate_indicator("sma", data, resolved_config))
