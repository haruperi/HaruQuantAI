"""Garman-Klass range volatility calculator.

Implements spec ``IND-VOL-06``:

``sigma_GK = sqrt(A/n * sum_{i=1..n} [0.5*(ln(H_i/L_i))^2 -
(2*ln2 - 1)*(ln(C_i/O_i))^2])``

A negative windowed variance from finite-precision arithmetic is clamped
to zero only within a declared tiny tolerance; a variance more negative
than that tolerance is never silently square-rooted and instead marks the
affected row unavailable with reason ``negative_variance``.
"""
...
_TWO_LN2_MINUS_1 = 2.0 * np.log(2.0) - 1.0


def _build_config(
    period: int, annualization_factor: float, config: IndicatorConfig | None
) -> IndicatorConfig:
    """Build or validate the immutable Garman-Klass volatility configuration.
    ...
    if (
        ...
        raise IndicatorError(
            ...
            "supplied config disagrees with garman_klass_volatility wrapper arguments",
            {"indicator_id": "garman_klass_volatility"},
        )
    return config
...
def garman_klass_volatility(
    ...
    *,
    ...
    """Calculate Garman-Klass OHLC-range annualized volatility.
    ...
    Returns:
        A deterministic Garman-Klass volatility ``IndicatorResult``.

    Raises:
        IndicatorError: On validation or atomic calculation failure.
    """
    logger.info(
        "Calculating garman_klass_volatility for %s (period=%d, A=%s)",
        data.symbol,
        period,
    ...
    _unwrap_indicator_response(
        validate_indicator("garman_klass_volatility", data, resolved_config)
    )
    ...
    per_bar = 0.5 * np.square(np.log(high / low)) - _TWO_LN2_MINUS_1 * np.square(
        np.log(close / open_)
    )
    ...
    output_column = f"garman_klass_volatility_{period}"
...
__all__ = ["garman_klass_volatility"]
