"""One reusable adapter conformance suite (``FEAT-BRK-10``).
...
class _ConnectLikeAdapter(Protocol):
    ...
    async def is_connected(self) -> StandardResponse[bool]:
        """Return the adapter-local connection state.

        Returns:
            Canonical response containing the connection state.
        """
        ...

    async def supports(self, capability: object) -> StandardResponse[bool]:
        """Return whether one capability is available.

        Args:
            capability: Canonical capability identity.

        Returns:
            Canonical response containing the availability decision.
        """
        ...
