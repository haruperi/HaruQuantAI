"""FR 8: cTrader Trade Execution and Calculations."""
...
def place_order(request: dict[str, Any]) -> dict[str, Any]:
    """Submit order to cTrader OpenAPI.
...
def modify_order(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if not is_connected():
        msg = "cTrader is not connected. Call connect() first."
        raise RuntimeError(msg)
    return {"status": "MODIFIED", "orderId": request.get("orderId")}
...
def modify_position(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if not is_connected():
        msg = "cTrader is not connected. Call connect() first."
        raise RuntimeError(msg)
    return {"status": "MODIFIED", "positionId": request.get("positionId")}
...
def close_position(
    ...
    if not is_connected():
        msg = "cTrader is not connected. Call connect() first."
        raise RuntimeError(msg)
    return {"status": "CLOSED", "positionId": position_id}
