"""Strict feature configuration for the cTrader provider."""
...
class CTraderConfig:
    """Trusted, process-local cTrader feature configuration."""
    ...
    @classmethod
    def from_dict(cls, values: dict[str, object]) -> "CTraderConfig":
        """Validate raw process-local provider configuration."""
        unknown = set(values) - _ALLOWED_KEYS
        if unknown:
            raise ValueError(f"unknown config keys: {sorted(unknown)}")
        parsed: dict[str, str] = {}
        for key in ("profile_id", "profile_version_id", "account_ref", "environment"):
            value = values.get(key)
            if not isinstance(value, str) or not value.strip():
                raise ValueError(f"{key} must be a non-empty string")
            parsed[key] = value
        if parsed["environment"] not in {"LIVE", "DEMO"}:
            raise ValueError("cTrader environment must be LIVE or DEMO")
        ...
        if credentials["account_id"].get_secret_value() != parsed["account_ref"]:
            raise ValueError("account_ref must match the resolved cTrader account_id")
