...
| MT5 | Completed; connection verified | Full transport/mapping/adapter unit tests, including latest/ranged ticks, latest/ranged historical bars, and provider-derived spread, plus prior real configured demo-account connection evidence (`connect`, `disconnect`, `is_connected`, `get_account_info`) | Broader credential-gated provider-response evidence remains outside ordinary regression. |
