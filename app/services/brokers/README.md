...
| **Missing** | Useful V1 behavior exists but canonical migration, validation, or tests remain. |
| **Missing** | Final behavior is implemented and verified. |
...
| Missing | `WF-BRK-002` | Internal | Connect and authenticate | Caller-owned adapter and immutable config | Verified session, capability report, and lifecycle events | `FR-BRK-048 → FR-BRK-052 → FR-BRK-073` |
...
| Status | File | Responsibility | Key exports | Dependencies |
|---|---|---|---|---|
| Missing | `transport.py` | Own one MT5 terminal/account session and isolate blocking calls from the event loop. | None | **Standard library:** `asyncio, threading`<br>**Required third-party:** `MetaTrader5>=5.0.5735`<br>**Local:** `contracts → BrokerConnectionConfig, BrokerErrorCode`; `app.utils → logging/redaction` |
...
| Missing | `transport.py` | Perform bounded Binance REST/WebSocket calls and respect provider weight headers. | None | **Standard library:** `asyncio`<br>**Required third-party:** `python-binance>=1.0.37`<br>**Local:** `contracts → BrokerConnectionConfig, BrokerErrorCode`; `app.utils → logging/redaction` |
...
| Missing | `NFR-BRK-013` | Dependencies | Provider library versions shall match `pyproject.toml`; directly imported transitive packages must be pinned before implementation. | Dependency manifest audit |
