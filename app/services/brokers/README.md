...
6. **Canonical Timeframes**: Provider conversion maps are keyed by the Data-owned `ENUM_TIMEFRAMES`; provider strings or integer constants are accepted only by each provider's explicit boundary resolver. Partial providers raise `ValueError` for unsupported canonical periods instead of silently substituting M1, while documented provider-native strings such as Binance `D3` remain boundary-only extensions.
