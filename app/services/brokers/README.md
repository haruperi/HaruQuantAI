...
- The tracked production package has 16 top-level implementation packages:
  `_shared`, `binance`, `canonical_contracts`, `capabilities`, `conformance`,
  `ctrader`, `dukascopy`, `events`,
...
| `environment_guards/` | SPLIT + DELETE | Retired by Child 1.11: Workspace/Composition owns admission/configuration, Trading/Risk owns mutation authority, and provider features own endpoint verification. |
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------ | -------------------------------- |
| Single-target mutations (`check_order`, `place_order`, `modify_order`, `cancel_order`, `modify_position`, `close_position`)                                                                                                                                                                         | `CAP-BRK-010`; `FR-BRK-033`–`038`, `FR-BRK-091`–`097`                 | MT5, cTrader                                     | `FEAT-BRK-07`                  |
...
Each registered feature owns exactly one production folder and exactly one numbered standalone usage program. Provider facade classes compose private focused files; unreleased writes remain unreachable through public release policy. The registry holds **five** completed features, `FEAT-BRK-02` through `FEAT-BRK-06`. `FEAT-BRK-00` moved to Catalogue in Task 1.02; `FEAT-BRK-01` was retired in Task 1.10; `FEAT-BRK-07`, `FEAT-BRK-09`, and `FEAT-BRK-18` were retired in Task 1.12 with market checkpoints moved to Data, trading reconciliation to Trading, provider events mapped to `app/contracts/broker` and published via Kernel EventBus, and provider specifications folded into MetaTrader provider truth; `FEAT-BRK-08` was split and retired in Task 1.11; `FEAT-BRK-10` adapter conformance test kit was relocated to test infrastructure under `tests/brokers/conformance/` in Task 1.14; `FEAT-BRK-17` simulation broker channel was retired in Task 1.13 with simulation order behavior moved to `simulator.simulate-orders@1` and SIM/DEMO/LIVE route selection kept in Trading; IDs `FEAT-BRK-11` through `FEAT-BRK-16` are retired from current-state registration after their behavior moved into provider channels. `canonical_contracts/` and `_shared/` are documented non-feature support and own no independent feature behavior.
...
| Completed | `FR-BRK-173` Every MT5 observation-owned mapping timestamp shall use an injected aware-UTC clock, captured once per provider payload, while live callers retain the current UTC default. | Private MT5 mapping clock | `app/services/brokers/metatrader/mapping.py`; `tests/brokers/unit/test_mt5_mapping_clock.py`; `tests/brokers/usage/features/02_metatrader.py` |

#### Retired simulation channel requirements (moved to Simulator & Trading)

| Status | Requirement | Former public contract | Disposition / New Owner |
| --- | --- | --- | --- |
| Retired | `FR-BRK-167` Brokers shall register stable `sim` and `simulation` identities. | `get_broker_id`, `get_broker_environment` | Stable canonical enums retained; simulation route handled by Trading |
| Retired | `FR-BRK-168` The factory shall admit only the exact simulation identity/environment pair with an injected authority. | `create_simulation_broker_adapter` | Retired; Simulator provides direct execution authority |
| Retired | `FR-BRK-169` Brokers shall publish an exhaustive, fail-closed simulation capability intersection. | `create_simulation_broker_adapter` | Retired; Simulator owns simulation capabilities |
| Retired | `FR-BRK-170` The simulation channel shall mirror connect, disconnect, reconnect, ping/status, ordered events, and run finalization while blocking session-required behavior when disconnected. | canonical lifecycle functions; `finalize_simulation_broker_session` | Retired; Simulator owns simulated lifecycle |
| Retired | `FR-BRK-171` The simulation channel shall open no socket, require no credentials, and import no Simulation symbol. | `create_simulation_broker_adapter` | Retired; Brokers owns only real external provider channels |
| Retired | `FR-BRK-172` Brokers shall own a structural authority protocol with canonical adapter signatures and run finalization, while owning no matching or accounting. | `SimulationAuthorityPort` (private contract) | Retired; moved to `simulator.simulate-orders@1` |
| Retired | `FR-BRK-174` The simulation adapter shall bind admitted reads only through its injected Brokers-owned structural authority port. | `create_simulation_broker_adapter` | Retired; moved to Simulator |
| Retired | `FR-BRK-175` Each authority read shall bind source sequence, observation, receipt, availability, and injected simulated-clock evidence in aware UTC. | `build_simulation_read_envelope` | Retired; moved to Simulator |
| Retired | `FR-BRK-176` Simulation shall project authoritative symbols, current specifications, quotes, spreads, ticks, and bars through canonical Broker operations without recomputation. | Canonical Broker read functions | Retired; moved to Simulator / Data |
| Retired | `FR-BRK-177` Simulation shall reject naive, reversed, or future availability evidence before exposing a read. | Canonical Broker read functions | Retired; moved to Simulator |
| Retired | `FR-BRK-178` Simulation account, balance, and permission reads shall project authority-owned ledger values exactly and perform no accounting. | Canonical Broker account functions | Retired; moved to Simulator |
| Retired | `FR-BRK-179` Simulation position, open-order, single-record, and order-history reads shall project authority-owned state exactly. | Canonical Broker position/order functions | Retired; moved to Simulator |
| Retired | `FR-BRK-180` Trading-session reads require an injected revision identity and never infer dated exceptions from weekly definitions; deal and transaction reads remain unavailable until their Phase 17b evidence is admitted. | Canonical session/release boundary | Retired; moved to Simulator |
| Retired | `FR-BRK-181` Simulation reads shall fail closed on stale, duplicate, missing, gapped, or out-of-order delivery without sorting or external IO. | Canonical delivery evidence | Retired; moved to Simulator |
| Retired | `FR-BRK-182` Simulation mutations shall require the exact sim/simulation route and reject target-environment or request-echo tampering before acceptance. | Canonical mutation functions | Retired; Trading routes directly to Simulator |
| Retired | `FR-BRK-183` The authority shall return request-bound MT5-shaped mutation payloads mapped through the live mapping path. | `build_simulation_mutation_envelope` | Retired; moved to Simulator |
| Retired | `FR-BRK-184` Simulation shall classify only verified provider retcodes and fail closed on malformed or unknown values. | Canonical mutation functions | Retired; moved to Simulator |
| Retired | `FR-BRK-185` Simulation shall not create spontaneous timeout or unknown-outcome ambiguity before explicit Phase-20 fault injection. | Canonical mutation functions | Retired; moved to Simulator |
| Retired | `FR-BRK-186` Simulation shall delegate check/place/modify/cancel order exactly once with immutable request identity and idempotency guards. | Canonical order mutation functions | Retired; moved to `simulator.simulate-orders@1` |
| Retired | `FR-BRK-187` Simulation shall delegate modify/reduce/close position and require authority-owned projected position state where applicable. | Canonical position mutation functions | Retired; moved to `simulator.simulate-orders@1` |
| Retired | `FR-BRK-188` The adapter shall own no matching, accounting, fill, or position-state business logic. | Simulation authority port | Retired; moved to Simulator |
| Retired | `FR-BRK-189` Immutable v2 fill/time policy and specification checksum shall cross the simulation port without inference. | Canonical order mutation functions | Retired; moved to Simulator |
| Retired | `FR-BRK-194` Simulation shall return only explicitly bounded, ordered deal-history pages and preserve authority-owned order/position linkage, entry/reason, fee, timestamp, sequence, and pagination evidence without recomputation. | `list_broker_deal_history` | Retired; moved to Simulator |
| Retired | `FR-BRK-195` Simulation shall return one exact authority-owned deal or canonical `BROKER_DEAL_NOT_FOUND`, without inventing identity or linkage. | `get_broker_deal` | Retired; moved to Simulator |
| Retired | `FR-BRK-196` Simulation shall return explicitly bounded account-transaction pages with authority-owned transaction types, signed amounts, timestamps, sequence, and pagination unchanged. | `list_broker_account_transactions` | Retired; moved to Simulator |

#### Calculation conformance evidence requirements

| Status | Requirement | Public contract | Evidence |
| --- | --- | --- | --- |
| Completed | `FR-BRK-190` MT5 order checks shall preserve projected balance, equity, profit, margin, free margin, and margin level as exact finite Decimals. | `check_broker_order`; opaque `order_check` value | `app/services/brokers/canonical_contracts/models.py`; `app/services/brokers/metatrader/mapping.py`; `tests/brokers/integration/test_mt5_calculation_evidence.py`; `tests/brokers/usage/features/10_conformance.py::fr_brk_190()` |
| Completed | `FR-BRK-191` Projected calculation evidence shall bind environment, redacted account digest, provider-specification checksum, terminal build, and aware-UTC observation time. | `check_broker_order`; opaque `order_check` value | `app/services/brokers/metatrader/commands.py`; `tests/brokers/integration/test_mt5_calculation_evidence.py`; `tests/brokers/usage/features/10_conformance.py::fr_brk_191()` |
| Completed | `FR-BRK-192` Brokers shall provide a bounded immutable JSON-safe sanitized calculation fixture with canonical checksum verification. | `build_broker_calculation_fixture`, `dump_broker_calculation_fixture`, `parse_broker_calculation_fixture` | `app/services/brokers/conformance/fixtures.py`; `tests/brokers/unit/conformance/test_calculation_fixture_schema.py`; `tests/brokers/usage/features/10_conformance.py::fr_brk_192()` |
| Completed | `FR-BRK-193` Provider fixture collection shall require separate invocation on exact dev/demo, digest the raw account identity, and remain unreachable from the default conformance suite. | `collect_broker_calculation_fixture` | `app/services/brokers/conformance/fixtures.py`; `tests/brokers/unit/conformance/test_calculation_fixture_schema.py`; `tests/brokers/integration/test_mt5_calculation_evidence.py`; `tests/brokers/usage/features/10_conformance.py::fr_brk_193()` |
...
- Usage examples live under `tests/brokers/usage/`: exactly one standalone
  numbered program for each registered feature, `FEAT-BRK-02` through
  `FEAT-BRK-06`, and `FEAT-BRK-10`. Programs use a main guard and bounded secret-safe inputs. Each
...
| `BrokerOrderCheck`                  | Non-final validation plus optional complete projected balance/equity/profit/margin/free-margin/margin-level Decimals bound to environment, account digest, provider-specification checksum, terminal build, and observation time. |
...
| --- | --- | --- | --- |
| Completed | `FR-BRK-159` | Typed current snapshot covering execution/order/filling/expiration/GTC modes, stops/freeze, directional volume limit, calculation mode, margin/swap evidence, instrument scalars, and account permission evidence; raw MT5 rollover sentinel `7` is preserved as `UNSPECIFIED`, never a guessed weekday. | **Usage:** `tests/brokers/usage/features/18_specifications.py::fr_brokers_159()` **Unit:** `tests/brokers/unit/test_provider_specifications.py` |
...
| Completed | `FR-BRK-159` | Publish one multi-symbol Depth-of-Market read per configured interval over the same authenticated EA connection, admitted only for the currently acknowledged revision and tracked with its own independent sequence and gap detection from quote snapshots. A book subscriber's first read is bounded to 5 seconds, matching the existing hello/idle/symbol-acknowledgment bounds: a producer that never publishes a book frame fails the subscriber with `TimeoutError` rather than holding it open indefinitely. | `stream_metatrader_book_snapshots`; `metatrader/snapshot_gateway.py` | `tests/brokers/unit/test_metatrader_snapshot_gateway.py` |
| Completed | `FR-BRK-160` | Treat a symbol whose EA-side book subscription failed as explicitly unavailable in the `errors` array; never synthesize price levels for it. A subscribed symbol reporting zero current resting levels remains a genuine empty book, not an error. | `metatrader/snapshot_protocol.py`; `integrations/mt5/TickBridge.mq5` | `tests/brokers/unit/test_metatrader_snapshot_protocol.py` |
| Completed | `FR-BRK-161` | Reuse the existing bounded multi-consumer symbol-demand lifecycle (`acquire_metatrader_snapshot_symbols`/`release_metatrader_snapshot_symbols`) for Depth-of-Market reads; book publication requires no separate subscription channel. | `metatrader/snapshot_gateway.py` | `tests/brokers/unit/test_metatrader_snapshot_gateway.py` |
...
# Run homogeneous full-domain usage program directly:
python tests/brokers/usage/features/features.py
```
...
The six standalone programs under `tests/brokers/usage/features/` provide one bounded execution example for each current Brokers feature (`FEAT-BRK-02` through `FEAT-BRK-06`, and `FEAT-BRK-10`).
