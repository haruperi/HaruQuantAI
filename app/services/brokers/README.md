...
| Completed | `FR-BRK-174` The simulation adapter shall bind admitted reads only through its injected Brokers-owned structural authority port. | `create_simulation_broker_adapter` | `app/services/brokers/simulation/contracts.py`; `tests/brokers/unit/simulation/test_simulation_reads.py:86`; `tests/brokers/usage/features/17_simulation.py::fr_brk_174()` |
| Completed | `FR-BRK-175` Each authority read shall bind source sequence, observation, receipt, availability, and injected simulated-clock evidence in aware UTC. | `build_simulation_read_envelope` | `app/services/brokers/simulation/contracts.py`; `tests/brokers/unit/simulation/test_simulation_read_time.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_175()` |
| Completed | `FR-BRK-176` Simulation shall project authoritative symbols, current specifications, quotes, spreads, ticks, and bars through canonical Broker operations without recomputation. | Canonical Broker read functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_reads.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_176()` |
| Completed | `FR-BRK-177` Simulation shall reject naive, reversed, or future availability evidence before exposing a read. | Canonical Broker read functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_read_time.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_177()` |
| Completed | `FR-BRK-178` Simulation account, balance, and permission reads shall project authority-owned ledger values exactly and perform no accounting. | Canonical Broker account functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_reads.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_178()` |
| Completed | `FR-BRK-179` Simulation position, open-order, single-record, and order-history reads shall project authority-owned state exactly. | Canonical Broker position/order functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_reads.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_179()` |
| Completed | `FR-BRK-180` Trading-session reads require an injected revision identity and never infer dated exceptions from weekly definitions; deal and transaction reads remain unavailable until their Phase 17b evidence is admitted. | Canonical session/release boundary | `app/services/brokers/capabilities/matrix.py`; `tests/brokers/integration/test_simulation_read_conformance.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_180()` |
| Completed | `FR-BRK-181` Simulation reads shall fail closed on stale, duplicate, missing, gapped, or out-of-order delivery without sorting or external IO. | Canonical delivery evidence | `app/services/brokers/simulation/adapter.py`; `tests/brokers/integration/test_simulation_delivery_gaps.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_181()` |
| Completed | `FR-BRK-182` Simulation mutations shall require the exact sim/simulation route and reject target-environment or request-echo tampering before acceptance. | Canonical mutation functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_order_mutations.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_182()` |
| Completed | `FR-BRK-183` The authority shall return request-bound MT5-shaped mutation payloads mapped through the live mapping path. | `build_simulation_mutation_envelope` | `app/services/brokers/simulation/contracts.py`; `tests/brokers/integration/test_simulation_mutation_conformance.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_183()` |
| Completed | `FR-BRK-184` Simulation shall classify only verified provider retcodes and fail closed on malformed or unknown values. | Canonical mutation functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_retcode_mapping.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_184()` |
| Completed | `FR-BRK-185` Simulation shall not create spontaneous timeout or unknown-outcome ambiguity before explicit Phase-20 fault injection. | Canonical mutation functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_order_mutations.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_185()` |
| Completed | `FR-BRK-186` Simulation shall delegate check/place/modify/cancel order exactly once with immutable request identity and idempotency guards. | Canonical order mutation functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_order_mutations.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_186()` |
| Completed | `FR-BRK-187` Simulation shall delegate modify/reduce/close position and require authority-owned projected position state where applicable. | Canonical position mutation functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_position_mutations.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_187()` |
| Completed | `FR-BRK-188` The adapter shall own no matching, accounting, fill, or position-state business logic. | Simulation authority port | `app/services/brokers/simulation/adapter.py`; `tests/brokers/integration/test_simulation_mutation_conformance.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_188()` |
| Completed | `FR-BRK-189` Immutable v2 fill/time policy and specification checksum shall cross the simulation port without inference. | Canonical order mutation functions | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_order_mutations.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_189()` |
| Completed | `FR-BRK-194` Simulation shall return only explicitly bounded, ordered deal-history pages and preserve authority-owned order/position linkage, entry/reason, fee, timestamp, sequence, and pagination evidence without recomputation. | `list_broker_deal_history` | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_deals.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_194()` |
| Completed | `FR-BRK-195` Simulation shall return one exact authority-owned deal or canonical `BROKER_DEAL_NOT_FOUND`, without inventing identity or linkage. | `get_broker_deal` | `app/services/brokers/simulation/adapter.py`; `tests/brokers/integration/test_simulation_deal_conformance.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_195()` |
| Completed | `FR-BRK-196` Simulation shall return explicitly bounded account-transaction pages with authority-owned transaction types, signed amounts, timestamps, sequence, and pagination unchanged. | `list_broker_account_transactions` | `app/services/brokers/simulation/adapter.py`; `tests/brokers/unit/simulation/test_simulation_transactions.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_196()` |
