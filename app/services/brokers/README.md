...
app/services/persistence/brokers.py   # only when the domain persists state
tests/services/brokers/[feature_1]/
tests/examples/[domain_number]_brokers.py
```

Each `app/services/brokers/[feature].py` module is one cohesive feature and physical removal unit.
...
`app/contracts/brokers.py` module. Feature modules never import sibling implementations. They
...
consolidated in `app/services/persistence/brokers.py`. Feature modules consume focused persistence
...
`tests/examples/[domain_number]_brokers.py`. Examples are realistic, offline, deterministic,
...
Real broker profile: Expose explicitly actual external providers, source adapters, capability and availability reporting. No business logic here just actual connection.

### Owns

- Real broker profile management and explicit external provider connections.
- Broker source adapters, protocols, and network gateway communication.
- Broker capability, permissions, session state, and availability reporting.

### Does not own

- Trading order generation or portfolio decision logic (owned by D-TRADING and D-PORTFOLIO).
- Simulated fill algorithms or backtesting matching engines (owned by D-SIMULATOR).

### Shared contracts

The domain's public boundary is `app/contracts/brokers.py`. A counterparty may be a producer,
...
| --- | --- | --- | --- | --- |
| Missing | `brokers.[capability-name]@1` | `[ProtocolName]` | `1` | [Purpose] |
...
| --- | --- | --- | --- | --- | --- |
| Missing | `brokers.[feature_partition]` | `FEAT-BROKERS-[ACTION_OBJECT]` | `sqlite` | `retain` | `[capability]` |
...
| --- | --- | --- | --- | --- | --- |
| `FEAT-BROKERS-[ACTION_OBJECT]` | [Value] | `app/services/brokers/[feature].py` | `brokers.[capability]@1` | [Keys or `None`] | Missing |
...
- **Lead owner:** `FEAT-BROKERS-[ACTION_OBJECT]`
...
### `[feature].py` — `FEAT-BROKERS-[ACTION_OBJECT]`

> **Feature ID:** `FEAT-BROKERS-[ACTION_OBJECT]`
> **Status:** `[Missing | Partial | Completed]`
> **Owner module:** `app/services/brokers/[feature].py`
...
- **Domain persistence module:** `app/services/persistence/brokers.py` or `None`
- **Namespace:** `brokers.[feature]` or `None`
...
| Optional | `app/services/persistence/brokers.py` | Domain schema, SQL, and transactions required by this feature | [Focused repository/store symbols] |
| Missing | `tests/examples/[domain_number]_brokers.py` | Realistic offline primary-purpose example | `example_<NN>_<feature_slug>()` |
...
| Missing | `ARCH-006` | SQL and schema operations live in `app/services/persistence/brokers.py`. | Architecture and schema checks |
...
- [ ] One cohesive `app/services/brokers/[feature].py` implementation.
- [ ] Public contracts in `app/contracts/brokers.py`.
...
4. Update `app/services/persistence/brokers.py` only when database operations change.
