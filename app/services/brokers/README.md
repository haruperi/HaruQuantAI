...
| Completed | `FEAT-BRK-17` Simulation Broker Channel | `simulation/` | Exact `sim`/`simulation` factory, socket-free authority injection, canonical lifecycle/finalization, clock-safe authoritative reads, provider-shaped mutations, and capability intersection | `FR-BRK-167`–`FR-BRK-172`, `FR-BRK-174`–`189` | `tests/brokers/usage/features/17_simulation.py` |
...
| Completed | `FR-BRK-180` Deal and transaction reads remain unsupported; trading-session reads require an injected revision identity and never infer dated exceptions from weekly definitions. | Canonical unsupported/session boundary | `app/services/brokers/capabilities/matrix.py`; `tests/brokers/integration/test_simulation_read_conformance.py`; `tests/brokers/usage/features/17_simulation.py::fr_brk_180()` |
