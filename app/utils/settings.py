...
This module exports the Pydantic-based ``Settings`` class and its global
singleton instance ``settings``.

Note on import-time side effects: ``settings = Settings()`` at module scope
reads environment variables and the optional ``.env`` file at import time.
This is intentional — most application code requires the singleton
immediately. Tests that need custom settings should construct a fresh
``Settings(...)`` instance rather than relying on the singleton.
