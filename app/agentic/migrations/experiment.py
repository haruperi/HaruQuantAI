"""Agentic-owned experimentation schema definitions executed by Data.
...
rather than extended by a follow-on migration; see ``FR-AGT-005``.

The migration identifier is renumbered ``002_agentic_experiment_v1`` ->
``005_agentic_experiment_v1``: two Agentic migrations previously claimed
sequence ``002``, leaving intended apply order ambiguous.
"""
...
_DOMAIN = "agentic"
_MIGRATION_ID = "005_agentic_experiment_v1"
...
def get_experiment_migration_statements() -> tuple[str, ...]:
    """Return the ordered Agentic experimentation statements.
...
def build_experiment_migration_request(request_id: str) -> object:
    """Return the deterministic Agentic-owned experimentation migration.
    ...
    """
    logger.debug("Building the Agentic experimentation migration request")
