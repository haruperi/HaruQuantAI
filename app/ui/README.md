...
| Completed | `FEAT-UI-31` Simulation Workbench | `src/features/simulation-workbench/`, `src/app/workstation/simulator/` | `SimulationWorkbench`, `SimulationHome`, `SimulationStatusBadge`, `SimulationRunBuilder` (eight ordered stages), `CanonicalRunMonitor`, `BatchRunMonitor`, `InteractiveSimulationWorkspace`, `SimulationSessionHeader`, `MarketViewport`, `ManualCommandPanel`, `SessionStatePanels`, `WhatIfPanel`, `SimulationRecoveryPanel`, `SimulationFinalizeDialog`, `SimulationPlaybackWorkspace`, `ScenarioPanel`, `ChecklistPanel`, `MissionPanel`, `PortfolioSimulationPanel`, `simulation-store`, `simulation-selectors`; `/workstation/simulator`, `/workstation/simulator/new`, and the `runs`, `practice`, `batch`, `history`, and `replay` catch-all segments | `FR-UI-265`–`FR-UI-270` | `src/features/simulation-workbench/SimulationWorkbench.test.tsx`; `SimulationRunBuilder.test.tsx`; `CanonicalRunMonitor.test.tsx`; `BatchRunMonitor.test.tsx`; `SimulationHome.test.tsx`; `InteractiveSimulationWorkspace.test.tsx`; `ManualCommandPanel.test.tsx`; `SimulationRecoveryPanel.test.tsx`; `SimulationPlaybackWorkspace.test.tsx`; `ScenarioPanel.test.tsx`; `PortfolioSimulationPanel.test.tsx`; `src/app/workstation/simulator/routes.test.tsx`; `src/clients/simulationWorkbench.test.ts`; `e2e/workbench-journeys.spec.ts` |
