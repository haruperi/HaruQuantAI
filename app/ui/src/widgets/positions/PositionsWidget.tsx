...
import {
  selectTradingActivityDisabled,
  useWorkspaceStore,
} from "../workspaces";

export interface PositionsWidgetProps {
  /** Initial active tab. */
  readonly initialTab?: "positions" | "orders";
}

export const PositionsWidget: React.FC<PositionsWidgetProps> = ({
  initialTab = "positions",
}) => {
  const [activeTab, setActiveTab] = useState<"positions" | "orders">(initialTab);
      ...
      {activeTab === "positions" && (
        ...
        >
          ...
          <div
            style={{
              padding: "6px 10px",
              background: "var(--cme-navy-header)",
              borderBottom: "1px solid var(--cme-navy-border)",
              display: "flex",
              alignItems: "center",
          ...
          >
            ...
            <button
              ...
              style={{ color: "var(--cme-warning-yellow)" }}
            >
            ...
            <table className="cme-table">
              ...
              <tbody>
                {positions.length === 0 ? (
                  ...
                  positions.map((p, idx) => (
                    <tr key={idx}>
                      <td style={{ fontWeight: 700, color: "var(--cme-blue-cyan)" }}>
                        {p.symbol}
                      </td>
                      <td>{p.contract}</td>
                      <td>{p.month}</td>
                      ...
                      <td
                        style={{
                          fontWeight: 600,
                          color: p.position.includes("Long")
                            ? "var(--cme-buy-green)"
                            : "var(--cme-sell-red)",
                        }}
                      >
