...
interface BatchRunMonitorProps {
  ...
  onCompareAnalytics?: (runIds: string[]) => void;
  className?: string;
}
...
export function BatchRunMonitor({
  ...
  const handleOpenAnalytics = useCallback(
    (targetRunId: string) => {
      if (onOpenAnalytics) {
        onOpenAnalytics(targetRunId);
        return;
      }
      useAnalyticsWorkbenchStore.getState().setSelection([targetRunId]);
      useWorkspaceStore
        .getState()
        .addWidgetToWorkspace(
          "analytics",
          `Analytics: ${targetRunId}`,
          undefined,
          undefined,
          targetRunId,
        );
  ...
  const handleCompareAnalytics = useCallback(
    (targetRunIds: string[]) => {
      if (onCompareAnalytics) {
        onCompareAnalytics(targetRunIds);
        return;
      }
      useAnalyticsWorkbenchStore.getState().setSelection(targetRunIds);
      useWorkspaceStore
        .getState()
        .addWidgetToWorkspace(
          "analytics",
          `Analytics Comparison (${targetRunIds.length})`,
          undefined,
          undefined,
          targetRunIds[0],
        );
        ...
        <div className="mt-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {successfulRunIds.length >= 2 ? (
              <button
                type="button"
                onClick={() => handleCompareAnalytics(successfulRunIds)}
                className="px-3 py-1.5 rounded bg-slate-700 text-white cursor-pointer hover:bg-slate-600"
              >
            ...
            {batch.items.map((item) => (
              <li key={item.item_id} className="flex items-center gap-2 py-1">
                ...
                {item.status === "completed" && item.run_id ? (
                  <button
                    type="button"
                    onClick={() => handleOpenAnalytics(item.run_id!)}
                    className="text-teal-300 text-xs underline cursor-pointer bg-transparent border-0 p-0"
                  >
                    Open Analytics
                  </button>
                ) : null}
