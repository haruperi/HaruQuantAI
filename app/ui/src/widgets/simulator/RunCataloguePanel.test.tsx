...
describe("RunCataloguePanel", () => {
  ...
  it("renders recorded runs with an Analytics hand-off button", async () => {
    listRuns.mockResolvedValue({ status: "success", data: { runs: [ENTRY] } });
    const onOpen = vi.fn();

    render(<RunCataloguePanel onOpenAnalytics={onOpen} />);
    ...
    const btn = screen.getByRole("button", { name: "Open in Analytics" });
    expect(btn).toBeInTheDocument();
    fireEvent.click(btn);
    expect(onOpen).toHaveBeenCalledWith("canonical-1");
  });
