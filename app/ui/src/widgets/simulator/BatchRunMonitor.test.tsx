...
describe("BatchRunMonitor", () => {
  ...
  it("shows per-item partial failure without failing the whole batch", async () => {
    const onOpen = vi.fn();
    render(<BatchRunMonitor batchId="batch-abc" onOpenAnalytics={onOpen} />);
    await screen.findByText(/completed:/i);

    expect(screen.getByText("market data gap")).toBeInTheDocument();
    expect(screen.getByText(/Failed: 1/)).toBeInTheDocument();
    const btn = screen.getByRole("button", { name: /open analytics/i });
    expect(btn).toBeInTheDocument();
    fireEvent.click(btn);
    expect(onOpen).toHaveBeenCalledWith("canonical-2");
  });
  ...
  it("compares successful runs by identity and infers no portfolio", async () => {
    ...
    render(<BatchRunMonitor batchId="batch-abc" onCompareAnalytics={onCompare} />);
    await screen.findByText(/completed:/i);
    ...
    expect(onCompare).toHaveBeenCalledWith(["canonical-1", "canonical-2"]);
