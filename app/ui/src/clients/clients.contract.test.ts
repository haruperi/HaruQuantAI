...
const EXPECTED: ReadonlyArray<{
  ...
  {
    id: "api.optimization.parameter_sweep",
    method: "POST",
    path: "/api/v1/optimization/parameter-sweep",
    permission: "optimization:run",
  },
  {
    id: "api.optimization.walk_forward",
    method: "POST",
    path: "/api/v1/optimization/walk-forward",
    permission: "optimization:run",
  },
  {
    id: "api.optimization.walk_forward_matrix",
    method: "POST",
    path: "/api/v1/optimization/walk-forward-matrix",
    permission: "optimization:run",
  },
  {
    id: "api.optimization.robustness",
    method: "POST",
    path: "/api/v1/optimization/robustness",
    permission: "optimization:run",
  },
  {
...
describe("clients match the backend route catalog", () => {
  ...
  it("does not declare an unknown route id", () => {
    ...
    for (const contract of ROUTE_CONTRACTS) {
      expect(
        knownIds.has(contract.id),
        `unexpected route id ${contract.id}`,
      ).toBe(true);
    }
  });
  ...
  it("marks expectancy transition as governed and idempotent", () => {
    const transition =
      ROUTE_CONTRACTS_BY_ID["api.research.transition_expectancy"];
    expect(transition?.governed).toBe(true);
    expect(transition?.idempotencyRequired).toBe(true);
    expect(transition?.sideEffect).toBe("governed_write");
  });

  it("marks Research job creation as idempotency-required", () => {
    for (const id of [
      ...
      expect(
        contract?.idempotencyRequired,
        `${id} should require idempotency`,
      ).toBe(true);
    }
  });

  it("marks Research evidence creation as governed and idempotent", () => {
    for (const id of [
      "api.research.create_expectancy",
      "api.research.create_stress_scenario",
    ]) {
      const contract = ROUTE_CONTRACTS_BY_ID[id];
      expect(contract?.governed).toBe(true);
      expect(contract?.idempotencyRequired).toBe(true);
      expect(contract?.permission).toBe("research:govern");
    }
  });
  ...
  it("marks the three trading mutation routes as governed + idempotent", () => {
    for (const id of [
      ...
      expect(contract?.sideEffect, `${id} should be governed_write`).toBe(
        "governed_write",
      );
    }
  });
